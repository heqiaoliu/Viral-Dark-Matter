function [hiddenneurons, countation, ANNname, datastring] = GetCountationNameAndData(totalcount)
    ...
    if totalcount/131 >= 1
        derpcount = totalcount - 130;
        if derpcount/10 <= 1
            ANNname = 'ANN_LambdadosRatio_TAIL_United';
            datastring = 'LambdadosRatio_tail_pct';
            countation = derpcount;
        end
    end
    
    if totalcount/121 >= 1
        derpcount = totalcount - 120;
        if derpcount/10 <= 1
    ...
    if totalcount/61 >= 1
        ...
        if derpcount/10 <= 1
            ANNname = 'ANN_LambdadosRatio_MCP_United';
            datastring = 'LambdadosRatio_mcp_pct';
            countation = derpcount;
        end
    end
