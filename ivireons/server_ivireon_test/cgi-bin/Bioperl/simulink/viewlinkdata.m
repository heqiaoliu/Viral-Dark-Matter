...
switch (Action)
 case 'Create'
  ...
  if ~usejava('MWT')
    DAStudio.error('Simulink:tools:viewLinkDataRequiresJava');
  end
  ...
  if (ud_idx < 1)
    ...
    % Is it read only
    USERDATA(idx).READONLY = readOnly;
    if readOnly
        USERDATA(idx).GUI.OK_BUTTON.setEnabled(0);
        USERDATA(idx).GUI.BLOCK_LIST_DELETE.setEnabled(0);
    end
  else
  ...
  i_PopulateViewer(USERDATA(idx).GUI, USERDATA(idx).DATA, get_param(arg, 'Name'), readOnly);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function i_PopulateViewer(gui, data, name, readOnly)
...
titleStr = ['Link changes: ' name];
if (readOnly)
    titleStr = [titleStr, ' [Read Only]'];
end
