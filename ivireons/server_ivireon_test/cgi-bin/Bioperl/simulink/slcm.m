...
case 'Create'
  ...
  %
  USERDATA = LocalCreate(USERDATA);
      
case 'Ok'
  % 
  % Callback for OK button
  %
  USERDATA = LocalOk(USERDATA);
  
case 'Cancel'
  % 
  % Callback for Cancel button
  %
  USERDATA = LocalCancel(USERDATA);
...
case 'UpdateAllCMBlocksInThisModel'
  ...
  %
  if isempty(gcbf)  
    LocalUpdateDiagram(gcbh,bdroot(gcbh))
  end
...
case 'UpdateAllCMBlocksInThisModelInit'
  ...
  if isempty(gcbf)
    LocalUpdateDiagram(block,blockdiagram);
  end
  set_param(blockdiagram, 'dirty', dirtyFlag);
...
case 'ManuallyUpdateAllCMBlocksInThisModel'
  ...
  LocalUpdateDiagram(blockID,bdroot(blockID));
...
otherwise
  ...
  %
  error('Simulink:slcm:UnhandledOption', 'Unknown option: %s', action);
  
end

% ====================================================
function USERDATA = LocalCreate(USERDATA)
% Create the gui for the Configuration Information Block
  ...
  %
  % Define the choices for text alignment.
  % Get the current value.
  % Find which number the current value is.
  %
  ...
  %
  % Must do explicit translation since HG does not translate for us.
  % Also saves the English strings in the popup's UserData.
  %
  l10n_textHorzAlignString = cellfun(@xlate, textHorzAlignString, 'UniformOutput',false);
  ...
  %
  labelList = {'Model properties:', 'Configuration manager properties:'};
  ...
  %
  % Get the name of the current Configuration Manager.
  %
  ...
  %
  % If no CM is selected or it's otherwise invalid, disable the
  % transfer button.
  %
  if isempty(CMFields)
  ...
  USERDATA(end+1).BlockHandle = gcbh;
  USERDATA(end).FigureHandle =  toplevel;
  % 
...
% The output "newString" is a cell array of strings.
  ...
  if iscell(oldString)
    ...
    newString = cellstr(strvcat(oldString, addedString)); %#ok (strvcat slow but useful)
  end
...
% ====================================================
function USERDATA = LocalCancel(USERDATA)
% Callback for the "Cancel" button
  ...
  if ishandle(ud)
    idx  = LocalFindObjectHandle(USERDATA, gcbh,'Block');
    if ~isempty(idx)
      USERDATA(idx) = [];
      delete(ud);
    ...
    if ~isempty(gcbf)
      idx = LocalFindObjectHandle(USERDATA, gcbf,'Figure');
      if ~isempty(idx)
        blkH = USERDATA(idx).BlockHandle;
        if(ishandle(blkH))
          ud  = get_param(blkH, 'UserData');  
          if ishandle(ud)
...
% ====================================================
function value = getValueFromTagName(blockdiagram, tagToSearch)
...
% Callback for the "Apply" button
  ...
  allData      = get(gcbf, 'UserData');
  blockdiagram = bdroot(allData.gcbh);
  ...
  %
  % Selects the English string (stored in UserData)
  ...
  %
  LocalUpdateBlock(allData.gcbh, blockdiagram, horzAlign{value});
  
  % 
  % Update SourceBlockDiagram
  %
  set_param(allData.gcbh, 'SourceBlockDiagram', get_param(blockdiagram,'Name'));
...
% ====================================================
function USERDATA = LocalOk(USERDATA)
% Callback for OK button
...
% information mask is updated
  ...
  if length(displayStringWithTags) >= minimOptionLength + 3
    ...
    for i = 1 : nModelFields
      ...
      insertString = getValueFromTagName(blockdiagram, tagToSearch);
    ...
    if ~strcmpi(CM, 'none') 
      ...
      if isempty(SaveTempField)
        setSameValueToAllFields(block, CM, 'CheckBlockInOutToUpdate');
        SaveTempField = get_param(block, 'SaveTempField');
      end
...
% Common function for loading / copying a block
  ...
  % block diagram.
  %
  sourceBlockDiagram = get_param(block, 'SourceBlockDiagram');
  destBlockDiagram   = get_param(blockdiagram, 'Name');
  ...
  if ~strcmpi(destBlockDiagram, sourceBlockDiagram)
    ...
    setSameValueToAllFields(block, CM, 'CheckInOutToUpdate');
...
% Callback for block load function
  ...
  if CMParentIsLinkedSubsystem(gcbh)
    ...
    %
    set_param(gcbh, 'SourceBlockDiagram', get_param(blockdiagram,'Name'));

    LoadOrCopyBlock(gcbh,blockdiagram);
...
% y = {'CMTag1', '', 'CMTag2, '',  ..... }
  ...
  TagMaxNumber  = str2double(get_param(block, 'TagMaxNumber'));

  y = cell(1, 2 * TagMaxNumber);
  for i = 1 : TagMaxNumber
    y{2 * i - 1} = sprintf('CMTag%d',i);
    y{2 * i}     = '';
  end
  ...
  if ~(strcmpi(CM, 'none') || isempty(CM))
    ...
    for i = 1 : min(TagMaxNumber, size(CMFields, 1))
      if isempty(CMFields{i, 3})
        SaveTempField = [SaveTempField SameValue '#|#']; %#ok (grows in a loop)
        ...
        %
        SaveTempField = [SaveTempField 'x' '#|#']; %#ok (grows in a loop)
...
% ====================================================
function LocalUpdateDiagram(block,blockdiagram)
% Callback for "UpdateDiagram".  Both inputs are numeric handles.

  %fprintf(1,'slcm LocalUpdatDiagram: %s\n',getfullname(block));
  if CMParentIsLinkedSubsystem(block)
    sourcebd=get_param(gcbh, 'SourceBlockDiagram');
    LocalUpdateBlock(block, sourcebd, 'defaultLeftAlignment');
    return
  end
  
  bdname = get_param(blockdiagram, 'Name');
  if ~strcmp(bdname, get_param(block, 'SourceBlockDiagram')),
    set_param(block, 'SourceBlockDiagram', bdname);
  end
  ...
  updateInstalledCM(gcbf, block, oldCM, newCM, 0, true);
...
function updateInstalledCM(fromWhere, block, ...
                           ...
                           link_already_checked)
% if oldCM and newCM are different, update tags

  if (nargin<6 || ~link_already_checked) && CMParentIsLinkedSubsystem(block)
    return
  end
  ...
  if ~strcmpi(oldCM, newCM)
    ...
    if strcmpi(newCM, initialBlockCM)
      ...
      %
      initialSaveTempField = get_param(block, ...
                                      'InitialSaveTempField');
      InitialBlockCM = get_param(block, 'InitialBlockCM');
                                    
      set_param(block, 'SaveTempField', initialSaveTempField, ...
  ...
  if updateList == 1
    if strcmpi(newCM, 'none')
      set(allData.transferButton(2), 'Enable', 'off');
      set(allData.list(2).list,      'Enable', 'off');
      set(allData.list(2).text,      'Enable', 'off');
    else
      set(allData.transferButton(2), 'Enable', 'on');
      set(allData.list(2).list,      'Enable', 'on');
      set(allData.list(2).text,      'Enable', 'on');
    end
...
% SaveTempField based on tag values ("_____")
  ...
  for i = 1 : min(size(CMFields, 1), TagMaxNumber)
    CMTag = get_param(block, sprintf('CMTag%d',i));
    ...
    if (CMTag(1) ~= 'x') % do something only if not a command
      ...
      saveTempField = [saveTempField CMTagValue '#|#']; %#ok (grows in a loop)
...
% ====================================================
function idx = LocalFindObjectHandle(UD, H,obj)
% Finds the index of the handle
idx = [];
if isempty(UD)
  return;
end
...
% such as reseting CMTag#, for instance
  ...
  % 
  % is the CMBlock part of a subsystem that is a link?
  %
...
% Set/reset the visibility of all HG elements
  ...
  if onoff(notbool(visibility))
    ...
    allData.activeObjects = allObjects(allObjects ~= gcbf);
  end
