...
otherwise,
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  errordlg(sprintf('Invalid action command: %s',Action),[Dialog_name ' Error'],'modal'); 
...
function varargout = LocalGetSubsystemLayer(sysHandle, LookUnderMasks_str, FollowLinks_str,...
 ...
 if(strcmp(get_param(sysHandle,'Type'),'block'))
   if(strcmp(get_param(sysHandle,'BlockType'),'ModelReference'))
     ...
     i_UpdateBlockForVariants(sysHandle);
 ...
 varargout{1} = find_system(sysHandle,...
  ...
  if(ShowModelRefs)
    ...
    if(~isempty(mRefBlocks))
      for k = 1:length(mRefBlocks)
        % Update block for variants
        i_UpdateBlockForVariants(mRefBlocks(k));
  ...
  if nargout > 1,
    ...
    % Append 'StateflowChart' to the stateflow block
    appendSFFlag = strcmp(get_param(varargout{1},'Type'),'block') & ...
                   strcmp(get_param(varargout{1},'BlockType'),'SubSystem') & ...
                   strcmp(get_param(varargout{1},'MaskType'),'Stateflow');
    ...
    if(pMRBlkHandle < 0)
      ...
      % Maintain the current parent block
...
%   - returns the encoded path for SubSystem nodes or Modelreference nodes
%==================================================================  
function [encodedBlockFullPath] = LocalGetEncodedBlockFullPath(encodedBlockFullPath,rest, isSysHandleModelBlock)
    ...
    appendSFFlag = strcmp(get_param(encodedBlockFullPath,'Type'),'block') & ...
                   strcmp(get_param(encodedBlockFullPath,'BlockType'),'SubSystem') & ...
                   strcmp(get_param(encodedBlockFullPath,'MaskType'),'Stateflow');
    ...
    if(okTogo)
      for k=1:length(encodedBlockFullPath)      
        ...
        if(strcmp(get_param(encodedBlockFullPath{k},'BlockType'),'ModelReference'))          
          % Update block for variants
          i_UpdateBlockForVariants(encodedBlockFullPath(k));
        ...
        if(isSysHandleModelBlock)
          % Idealy we should use slprivate('decpath') to get the rest
          % as follows:
