...
%   $Revision: 1.65.4.28 $ $Date: 2010/04/21 21:59:19 $
...
%-------------------------------------------------------------------------
function [SystemData, origSysObject, origSysVisible] = LocalGetPrintSystemList(Sys,SysOption,LookUnderMask,ExpandLibLinks)

sfPrintJob = 0;

origSys = Sys;
origSysObject = get_param(origSys,'Object');
origSysVisible = strcmpi(get_param(origSys,'Open'),'On');
...
switch SysOption,
    case {'CurrentSystem','CurrentSystemAndAbove'},
        ...
        if stateflow_inside_l(Sys),
            ...
            if sfPrintJob,
                % Get the original object being viewed so that we can
                % bring it to the foreground after the printing is done
            origSysObject = idToHandle(sfroot, sf('get',portal,'.viewObject'));
                origViewObj =  sf('get',portal,'.viewObject');
                origSysVisible = (sf('get',origViewObj, '.visible') == 1);
        ...
        try
            if sfPrintJob
                Sys = sf('Private', 'sf_hier_print', Sys, 'getSystems', directive);
            end
        catch me
            disp(me.getReport)
        end;
...
if BD,BDSys=Systems(1);Systems(1)=[];end  %#ok - mlint
...
if BD,MaskFlag=[false;MaskFlag];end
%-------------------------------------------------------------------------
function LocalHelp
ttlStr = 'Print Dialog';
...
[SystemData, origSysObj, origSysVisible] =LocalGetPrintSystemList(Data.Sys,varargin{2:4});
...
%-------------------------------------------------------------------------
function CloseDlg=LocalPrintFromDlg(Data,varargin)  %#ok - mlint
...
[SystemData, origSysObj, origSysVisible] = LocalGetPrintSystemList(Data.Sys,Data.CurrentOpt, ...
...
if ~isempty(device)
    if ( findstr(device,'jpeg') == 1 )
        % need to strip off quality from 'jpeg' devices
        device = 'jpeg';
    end

    [ options, devices, extensions, classes ] = printtables; %#ok - mlint
    devIndex = strmatch( device, devices, 'exact' );
    if ( isempty(devIndex) )
        % print allows unique partial matches, we should too.
        devIndex = strmatch( device, devices );
        
        if length( devIndex ) > 1
            error( 'MATLAB:Print:deviceOpt', 'Device option ''%s''is not unique',in_device )
        elseif isempty(devIndex)
            error('MATLAB:Print:illegalDeviceOpt', 'Illegal device option, ''%s'', specified.',in_device);
        end
    end
...
%-------------------------------------------------------------------------
function [isSFBasedPJ, portal] = LocalIsSFBasedPrintJob
...
if(~isempty(portal))
    portal = portal(1);
    isSFBasedPJ  = ( sf('get', portal, '.sfBasedPrintJob') == 1);
end
%-------------------------------------------------------------------------
...
if PrintData.IncludePrintFrame
    renderframe('initialize',PrintData.PrintFrame,NumSys);
end

device=strrep(deblank(device),' ',''',''');
device(findstr(device,1)) = ' ';

[isSFBasedPrintJob sfOldPortal] = LocalIsSFBasedPrintJob;
chartIDToBringToFG = -1;

if(isSFBasedPrintJob)
    chartIDToBringToFG = sf('get',sfOldPortal,'.chart');
end
...
if(isSFBasedPrintJob)
    sf('set',chartIDToBringToFG,'.visible',1);
else
    set_param(initSys,'Open', initSysVisibility);
end
...
% reset persistent data if we are printing with frames and new pipeline
renderframe('reset');
%-------------------------------------------------------------------------
...
ImBtnPos=[Offset+2 sum(SimOptPos(3,[2 4]))+Offset size(X(:,:,1))];  %#ok - mlint
...
SimOptString={'Tiled print all systems',...
    ...
    'Print Sample Time Legend'};
SimOptCall={'simprintdlg TiledPrint foo bar'
    ''
    ''
    ''
    ''};
...
for BtnLp=1:size(ImBtnTags,1),
    CData=LocalInd2RGB(X(:,:,BtnLp),Map);  %#ok - mlint
