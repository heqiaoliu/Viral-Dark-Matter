...
% Creates a node for the StateflowDataLogsNode object (ts) to be inserted in the
...
nodeName = h.getTstoolNodeName(ts,'tsguis.stateflowDataLogsNode',varargin{:});
if isempty(nodeName)
    node = [];
    return;
end

% Create a @stateflowDataLogsNode
node = tsguis.stateflowDataLogsNode(nodeName,ts);
