...
otherwise,
   DAStudio.error('Simulink:blocks:InvalidGUIAction');
end


function out = i_IsLegend(h),
  if strcmpi(get(h,'Tag'),'Legend') | ...
        strcmpi(get(ancestor(h,'Axes'),'Tag'),'Legend'),
    out = true;
  else,
    out = false;
  end
...
case 'normal',
    ...
    if abs(XLim(1) - XLim(2)) > 1e-10*(abs(XLim(1)) + abs(XLim(2))),
        ...
        DAStudio.warning('Simulink:blocks:MaxZoomLimit4Axis');
    end
    ...
    if resizeHiLites
        callScope(fig,'ResizeHiLites',fig);
    end
...
%       are located at the current point (i.e., zoom origin).
%
...
otherwise,
    DAStudio.error('Simulink:blocks:InvalidZoomMode');
end
...
eval('zoomUserStruct = scopeUserData.zoomUserStruct;',...
     'DAStudio.error(''Simulink:blocks:zoomUserStructDataMissing'')');
...
eval('currentAxisList = scopeUserData.scopeAxes;',...
     'DAStudio.error(''Simulink:blocks:scopeAxesDataMissing'')');

% Function =====================================================================
% Calls appropriate Scope callback {simscope, simscopesv}
% This function requires the first argument to be the figure handle to the scope 
% window.  It uses the figure handle to access the Scope UserData to determine 
% which type of scope to call.  All other arguments are passed blindly to the
% invoked Scope callback.

function callScope(fig, varargin)

scopeUserData = getScopeUserData(fig);
if (i_IsModelBased(scopeUserData.block))
    simscopesv(varargin{:});
else
    simscope(varargin{:});
end
...
% Function =====================================================================
% Returns true if block is a model-based scope
