...
%Find all blocks which reference NAME
XRefChildH=[
    find_system(SystemsH,'SearchDepth',1,'SourceBlock',Name)
...
if isempty(XRefChildH),
  ...
  return
    ...
    XRefChildH = unique(XRefChildH);
end % if

%Find all parent systems to referring blocks
...
for i=1:length(XRefChildH)
    parentPage = find(SystemsH == XRefH(i));
    for j=1:length(parentPage)
        XRefPage(end+1) = parentPage(j);
        blkName = get(XRefChildH(i),'Name');
        blkName(blkName == char(10)) = ' '; %remove carriage returns
        XRefStr{end+1}  = blkName;
    end
end

[XRefPage,sortIdx] = sort(XRefPage);
XRefStr = XRefStr(sortIdx);
