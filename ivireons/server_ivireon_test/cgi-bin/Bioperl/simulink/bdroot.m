...
%   name of the top-level system containing the specified object.
%   
%   Example:
%   
%       bdroot(gcb)
...
switch class(origsys),
  ...
  case 'double',
    for lp=length(origsys):-1:1,
      root_sys(lp,1)=get_param(LocalGetRootSys(origsys(lp)),'Handle'); %#ok<AGROW>
    end
  case 'cell'
    for lp=length(origsys):-1:1,
      root_sys{lp,1}=LocalGetRootSys(origsys{lp}); %#ok<AGROW>
    end
  otherwise,
    DAStudio.error('Simulink:utility:invalidArgObjType','BDROOT',class(origsys));
end % switch
