...
% 
tabStrings = {i_GetGeneralString, i_GetDataHistoryString};
...
str2 = lclMessage('TryRightClicking');
...
ext = get(textExtent, 'Extent');
set(textExtent, 'String', lclMessage('None'));
...
set(textExtent, 'String', lclMessage('SampleTime'));
ext = get(textExtent, 'Extent');
set(textExtent, 'String', lclMessage('Decimation'));
ext = max(ext, get(textExtent, 'Extent'));
...
% ...Floating scope.
%
cxCur = cxCur + editWidth + (generalGeom.colDelta / 4);
...
set(textExtent, 'String', lclMessage('SaveDataToWorkspace'));
ext = get(textExtent, 'Extent');
geom.saveToWorkspaceWidth = ext(3) + sysOffsets.checkbox(3);

set(textExtent, 'String', lclMessage('VariableName'));
ext = get(textExtent, 'Extent');
...
ext = get(textExtent, 'Extent');
set(textExtent, 'String', lclMessage('Structure'));
ext = max(ext, get(textExtent, 'Extent'));
set(textExtent, 'String', lclMessage('Array'));
ext = max(ext, get(textExtent, 'Extent'));
...
children.saveToWorkspaceCheckbox = uicontrol(...
    'Parent',             dialogFig,...
    'Style',              'checkbox',...
    'String',             lclMessage('SaveDataToWorkspace'),...
	'tag', 'SaveDataToWorkspace', ...
    'Position',           dataCtrlPos.saveToWorkspaceCheckbox);

children.variableNameLabel = uicontrol( ...
    'Parent',             dialogFig, ...
    'Style',              'text', ...
    'String',             lclMessage('VariableName'), ...
...
children.variableNameEdit = uicontrol( ...
	...
	'tag', 'VariableNameValue', ...
    'BackgroundColor',    'w');

children.formatLabel = uicontrol( ...
    'Parent',             dialogFig, ...
    'Style',              'text', ...
    'String',             lclMessage('Format'), ...
    'Position',           dataCtrlPos.formatLabel);

string = { ...
    lclMessage('StructureWithTime'),  ...
    lclMessage('Structure'), ...
    lclMessage('Array')};
...
otherwise,
    DAStudio.error('Simulink:blocks:InvalidPage', Action);
end
...
if ~strcmp(lower(userstr), 'auto'),
    ...
    resolvedTRange = simscope('SimTimeSpan', block, block_diagram, simStatus);
end
...
if ~compiled,
    ...
    propStrs = {'Decimation', 'SampleTime'};
