...
%   $Revision: 1.9.4.4 $ $Date: 2008/06/20 08:48:36 $
...
for i=1:nargin,
  if ~ischar(varargin{i}),
    DAStudio.error('Simulink:util:AllArgsMustBeStrs');
  end
end
...
fprintf(1,'\n\n');
fprintf(1,[ DAStudio.message('Simulink:util:MdlPrecCheckingCompleted') '\n']);
fprintf(1,[ DAStudio.message('Simulink:util:NumFilesWithLogicalOpPrecAmbiguity', num_found) '\n']);
fprintf(1,[ DAStudio.message('Simulink:util:FilesFoundWere') '\n']);
...
for i=1:size(w.path,2),
  ...
  for j=1:size(mlist,1),
    if checkFilePrecedence(fullfile(char(w.path(i)),mlist{j})) ~= 0,
      ...
      files(end+1) = {fullfile(char(w.path(i)),mlist{j})}; %#ok
    end
  end
end
