...
% $Revision: 1.1.8.2 $  $Date: 2010/03/22 04:41:33 $
...
if (strcmp(action, 'start'))
   if (nargin == 0)
      noargmsg = sprintf(['To use POLYTOOL with your own data, type:\n' ...
                     '   polytool(x, y, n)\n\n' ...
      ...
      polytool(x,y);
      uiwait(warndlg(noargmsg, 'Polynomial Fitting','modal'));
      return
   end
...
set(poly_fig,'Name',['Prediction Plot of ',model,' Model'],'Units','Normalized');
poly_axes = axes;
...
set(poly_fig,'WindowButtonMotionFcn','polytool(''motion'',0)',...
...
reference_line(1) = plot([newx newx],yrange,'k--');
reference_line(2) = plot(xrange,[newy newy],'k:');
...
set(fitline(1),'XData',x,'YData',y,'Color','b','Marker','+','tag','data');
set(fitline(2),'XData',xfit,'YData',yfit,'Color','g','Linestyle','-','tag','fit');
set(fitline(3),'XData',xfit,'YData',ylo,'Color','r','Linestyle','--','tag','lowerBound');
set(fitline(4),'XData',xfit,'YData',yhi,'Color','r','Linestyle','--','tag','uppperBound');
