...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:15:31 $
...
if ischar(data)
    ...
    [eid,errmsg,dist,cens,freq,alpha,ntrials,options, ...
          pdf,logpdf,nloglf,otherArgs] = internal.stats.getargs(pnames, dflts, varargin{:});
    if ~isempty(eid)
...
elseif ischar(dist)
    ...
    error('stats:mle:InvalidDistName', 'The value of the ''distribution'' argument must be a distribution name.');
end
