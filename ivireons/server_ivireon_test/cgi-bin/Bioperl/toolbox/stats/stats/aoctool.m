...
%   $Revision: 1.1.8.2 $  $Date: 2010/04/24 18:31:15 $
...
if (strcmp(action, 'start'))
   noargmsg=[];
   if (nargs == 0)
      noargmsg = sprintf(['To use AOCTOOL with your own data, type:\n' ...
                     '    aoctool(x, y, g)\n\n' ...
...
switch action
 ...
 case 'start',
  ...
  if (dodisp)
     ...
     if ~isempty(noargmsg)
         warndlg(noargmsg, 'Analysis of Covariance','modal');
     end
 ...
 case 'edittext',
  if isnan(newx) 
     ...
     set(x_field,'String',num2str(newx));
     warndlg('The X value must be a number. Resetting to previous value.', 'Analysis of Covariance','modal');
     return;
  end
