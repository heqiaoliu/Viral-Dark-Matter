...
%   and each each iteration of the loop performs one invocation of
...
%   'argmax' is specified the SMARTFOR, the reduction variable value
...
if nSlicedOutputs > maxSlicedOutputs 
    error('stats:smartFor:Internal', ...
        'Loopbody produces more sliced outputs than we can handle.');
end
        ...
        for iter = 1:niter
            ...
            if iter==1
                ...
                for i=1:nSlicedOutputs
                    if iscell(slice{i})
                        varargout{i} = cell(numel(slice{i}),niter);
                    else
                        varargout{i} = zeros(numel(slice{i}),niter);
                    end
                end
