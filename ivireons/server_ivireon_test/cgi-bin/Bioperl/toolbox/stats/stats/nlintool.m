...
%   $Revision: 1.1.8.2 $  $Date: 2010/03/22 04:41:29 $
...
if (nargin == 0)
   ...
   warndlg(msg, 'Nonlinear Fitting','modal');
   return
end
...
set(nlin_fig,'UserData',ud,'HandleVisibility','callback', ...
			 ...
			 'WindowButtonMotionFcn','nlintool(''motion'',0)');

if ~isempty(wmsg)
   warndlg(wmsg, 'Nonlinear Fitting','modal');
end
...
case 'edittext',
   ...
   if isnan(cx)
      ...
      warndlg('Please type only numbers in the editable text fields.', 'Nonlinear Fitting','modal');
      return
   end  
   xl = get(nlin_axes(flag),'Xlim');
   if cx < xl(1) || cx > xl(2)
      % Create Bad Settings Warning Dialog.
      warndlg('This number is outside the range of the data for this variable.', 'Nonlinear Fitting','modal');
      set(x_field(flag),'String',num2str(double(xsettings(1,flag))));
      return
   end
