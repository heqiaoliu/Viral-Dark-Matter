...
%   $Revision: 1.1.6.2 $ $Date: 2010/04/24 18:31:23 $
...
[errid,errmsg,glyph,features,stdize,obslabs,varlabs,centers,...
       radius,grid,page,plotArgs] = internal.stats.getargs(pnames, dflts, varargin{:});
if ~isempty(errid)
...
if nargout > 0
    ...
    if dfltObslabs
        obsText = ['Observation: ' num2str(clickObsNum)];
    else
        obsText = ['Observation: ' obslabs{clickObsNum}];
    end
