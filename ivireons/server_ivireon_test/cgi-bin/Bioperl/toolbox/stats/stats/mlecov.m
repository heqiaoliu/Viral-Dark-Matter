...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:15:32 $
...
[eid,errmsg,pdf,cdf,logpdf,logsf,nloglf,cens,freq,options] = ...
                                 internal.stats.getargs(pnames, defaults, varargin{:});
if ~isempty(eid)
...
if ~isempty(nloglf)
    ...
    try
        ...
        throw(addCause(MException('stats:mlecov:NloglfError', ...
                                  geterrtext('nloglf',nloglf)),...
                       ME));
    end
...
try
    ...
    throwAsCaller(addCause(MException('stats:mlecov:LogpdfError', ...
                                      geterrtext('logpdf',logpdf)),...
                           ME));
end
...
if ~isempty(logsf)
    try
        ...
        throwAsCaller(addCause(MException('stats:mlecov:LogsfError', ...
                                          geterrtext('logsf',logsf)),...
                               ME));
    end
...
try
    ...
    throwAsCaller(addCause(MException('stats:mlecov:PdfError', ...
                                      geterrtext('pdf',pdf)),...
                           ME));
end
...
if ~isempty(cdf)
    try
        ...
        throwAsCaller(addCause(MException('stats:mlecov:CdfError', ...
                                          geterrtext('cdf',cdf)),...
                               ME));
    end
...
%==========================================================================
function txt = geterrtext(ftype,fhandle)
txt = sprintf('Error evaluating the user-supplied %s function ''%s''.',...
              ftype,func2str(fhandle));
