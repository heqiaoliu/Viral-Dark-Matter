...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:20:04 $
...
if ~isempty(Tree.surrvar)
    ...
    if ~isempty(Tree.hidesurrvar)
        Tree.hidevarassoc = Tree.hidevarassoc(tokeep,:);
        Tree.hidesurrvar = Tree.hidesurrvar(tokeep,:);
        Tree.hidevarassoc(isleaf,:) = {[]};
        Tree.hidesurrvar(isleaf,:) = {[]};
    end
end
