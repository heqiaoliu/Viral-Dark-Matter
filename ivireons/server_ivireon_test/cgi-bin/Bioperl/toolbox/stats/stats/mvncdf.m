...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:15:40 $
...
if isempty(Sigma)
    ...
    if sz(1) ~= sz(2)
        error('stats:mvncdf:BadCovariance',...
              'SIGMA must be a vector or a square matrix.');
    elseif ~isequal(sz, [d d])
        error('stats:mvncdf:InputSizeMismatch',...
          ['SIGMA must be a square matrix with size equal to the '...
          'number of columns in X, or a row vector with length equal to '...
          'the number of columns in X.']);
    else
        if ~sigmaIsDiag
            [T,err] = cholcov(Sigma,0); %#ok<ASGLU>
            if err ~= 0
                error('stats:mvncdf:BadCovariance',...
            ...
            if any(Sigma<=0)
                error('stats:mvncdf:BadDiagSigma', ...
