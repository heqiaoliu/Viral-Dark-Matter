...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:17:54 $

if nargin < 2
    error('stats:trimmean:TooFewInputs', 'TRIMMEAN requires two input arguments.');
elseif ~isscalar(percent) || percent >= 100 || percent < 0
    error('stats:trimmean:InvalidPercent', ...
          'PERCENT must be a scalar between 0 and 100.');
end
...
elseif ischar(flag) && isequal(lower(flag),'floor')
    ...
    error('stats:trimmean:BadFlag',...
          'FLAG must be ''round'', ''floor'', or ''weighted''.')
end
