...
%   $Revision: 1.1.8.4.2.1 $  $Date: 2010/07/06 14:43:09 $

if nargin < 2
    error('stats:plsregress:TooFewInputs', ...
...
ny = size(Y,1);
if ny ~= n
    error('stats:plsregress:SizeMismatch', ...
...
elseif ~isscalar(ncomp) || ~isnumeric(ncomp) || (ncomp~=round(ncomp)) || (ncomp<=0)
    error('stats:plsregress:BadNcomp', ...
          'NCOMP must be a positive integer value.');
elseif ncomp > maxncomp
    error('stats:plsregress:MaxComponents', ...
          'NCOMP must be less than or equal to %d for these data.',maxncomp);
end
...
if ncomp > maxncomp
    warning('stats:plsregress:MaxComponentsCV', ...
        'Cannot cross-validate more than %d components.',maxncomp);
    ncomp = maxncomp;
end
