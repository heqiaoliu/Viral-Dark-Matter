...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:16:58 $

pnames = {   'scaling'  'reflection'};
dflts =  {       true         'best'};
[eid,errmsg,doScaling,doReflection] = internal.stats.getargs(pnames, dflts, varargin{:});
if ~isempty(eid)
    error(sprintf('stats:procrustes:%s',eid),errmsg);
end

if ~isscalar(doScaling) || ~(islogical(doScaling) || isnumeric(doScaling))
    error('stats:procrustes:BadScaling',...
          'The ''Scaling'' parameter must be true or false.');
end
if isequal(doReflection,'best')
    doReflection = [];
elseif ~isscalar(doReflection) || ~(islogical(doReflection) || isnumeric(doReflection))
    error('stats:procrustes:BadReflection',...
          'The ''Reflection'' parameter must be true, false, or ''best''.');
end
