...
%   $Revision: 1.1.8.3.2.1 $  $Date: 2010/07/14 23:42:52 $
...
if ~iscell(bootfun) % default syntax
    ...
    [eid,errmsg,type,alpha,stderrfun,nbootstd,weights,bootstrpOptions] = ...
...
if isempty(stderrfun) % studentized with bootstrap error
    ...
    if size(stat)~=size(out)
        error('stats:bootci:BadStderrSize',...
            'Size of STDERRFUN output does not match size of BOOTFUN output.');
    end
end;
