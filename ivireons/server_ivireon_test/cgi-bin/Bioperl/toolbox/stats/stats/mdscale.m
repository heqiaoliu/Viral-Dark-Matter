...
%   $Revision: 1.1.6.1 $ $Date: 2010/03/16 00:15:28 $
...
elseif ischar(criterion)
    ...
    if length(i) > 1
        error('stats:mdscale:AmbiguousCriterion', ...
              'Ambiguous ''criterion'' parameter value:  %s.', criterion);
    elseif isempty(i)
        error('stats:mdscale:UnknownCriterion', ...
              'Unknown ''criterion'' parameter value:  %s.', criterion);
    end
    switch i
    case 1 % 'stress'
        stressFun = @stressCrit;
...
for rep = 1:nreps
    ...
    if metric
        if strain
            [Y,stress,iter] = MDS(Y0,A,weights,strainFun,metric,weighted,options);
        else
...
while true
    ...
    if stress < options.TolFun
        ...
        if verb > 2
            fprintf('Iterations terminated:  Criterion less than OPTIONS.TolFun.\n');
        end
        break;
    elseif normGrad < options.TolFun*stress
        if verb > 2
            fprintf('Iterations terminated:  Relative norm of gradient less than OPTIONS.TolFun.\n');
        end
        break;
    elseif (oldStress-stress) < options.TolFun*stress
        if verb > 2
            fprintf('Iterations terminated:  Relative change in criterion less than OPTIONS.TolFun.\n');
        end
        break;
    elseif stepLen < options.TolX * norm(Y(:))
        if verb > 2
            fprintf('Iterations terminated:  Norm of change in configuration less than OPTIONS.TolX.\n');
        end
        break;
...
if verb > 1
    fprintf('%d iterations, Final stress criterion = %g\n',iter,stress);
end
