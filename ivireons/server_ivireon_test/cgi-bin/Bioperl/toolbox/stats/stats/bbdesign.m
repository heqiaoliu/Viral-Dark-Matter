...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:12:13 $

okargs = {'center' 'blocksize'};
defaults = {[] Inf};
[eid,emsg,ncenter,blocksize] = internal.stats.getargs(okargs,defaults,varargin{:});
if ~isempty(eid)
...
if blocksize ~= Inf
   if ~isnumeric(blocksize) | blocksize<1 | blocksize~=floor(blocksize)
      error('stats:bbdesign:BadBlockSize',...
            'Value of ''blocksize'' parameter must be a positive integer.');
   end
end
