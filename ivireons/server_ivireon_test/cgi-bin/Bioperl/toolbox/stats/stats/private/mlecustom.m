...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:29:49 $
...
[eid,errmsg,pdf,cdf,logpdf,logsf,nloglf,start,cens,freq,alpha,userOpts,...
    lb,ub,optimFun,optimOpts] = internal.stats.getargs(pnames, defaults, varargin{:});
if ~isempty(eid)
...
try
    ...
    throwAsCaller(addCause(MException(errID, ...
        'Error while evaluating the user-supplied %s function ''%s''.',...
        type,func2str(fun)),...
                           ME));
end
