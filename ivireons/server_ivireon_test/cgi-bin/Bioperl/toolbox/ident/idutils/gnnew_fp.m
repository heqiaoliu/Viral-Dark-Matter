function [V, Vt, e, psi] = gnnew_fp(z,par,algorithm,oeflag)
%GNNEW_FP Computes The Gauss-Newton search direction for polynomial models
%when using frequency domain data

%   Copyright 1986-2008 The MathWorks, Inc.
%   $Revision: 1.5.4.5 $  $Date: 2008/04/28 03:21:35 $
...
if nargout==2
    [V, Vt] = gnx_f(z,par,struc,algorithm,oeflag);
else
    [V, Vt, psi, e, Nobs] = gnx_f(z,par,struc,algorithm,oeflag); 
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [V,lamtrue,R,Re,nobs] = gnx_f(ze,par,nn,algorithm,oeflag)
...
index = setdiff(1:length(par),nn.fixparind); %algorithm.estindex;
stablim = algorithm.Advanced.Zstability;
stablimc = algorithm.Advanced.Sstability;
stab = 0;
if ischar(algorithm.Focus) && any(strcmpi(algorithm.Focus,{'Stability','Simulation'}));
    stab = 1;
end

nu = nn.nu; na = nn.na; nb = nn.nb; %nc = nn.nc;  nd = nn.nd; 
nf = nn.nf; nk = nn.nk;
Ts = nn.T;
T = Ts;
%n = sum(nb)+sum(nf);
...
% deal with Ub0==0 (r.s., sep 28, 2007)
Ind = Ub0~=0;
Db = zeros(size(OM(indtb1,:)))';
Db(Ind,:) = -(OM(indtb1,Ind).').*((Ub0(Ind))./((fb0*OM(1:length(fb0),Ind)).')*ones(1,nbb0)); 
if realflag
