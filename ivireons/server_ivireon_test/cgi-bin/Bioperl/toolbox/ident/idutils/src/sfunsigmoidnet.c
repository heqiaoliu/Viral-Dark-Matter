/*  File    : sfunsigmoidnet.c
 *  Abstract:
 *  Evaluate sigmoidnet nonlinearity.
 * 
 *  Written by: Rajiv Singh
 *  Copyright 2007 The MathWorks, Inc.
 *  $Revision: 1.1.8.3 $ $Date: 2008/12/04 22:34:26 $
 */

#define S_FUNCTION_NAME sfunsigmoidnet
...
static void mdlStart(SimStruct *S)
{
   ...
   if (Par == NULL){
    ...
    Par->Dilation           =   mxGetData(PAR_DILATION(S));
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
   ...
   evaluate_sigmoidnet(y, Reg, 1, DimInp, Par);
...
static void mdlJacobian(SimStruct *S)
{
    ...
    TypeStr         = mxCreateString("sigmoidnet");
    fnamesPar       = mxCalloc(9, sizeof(*fnamesPar));
    ...
    fnamesPar[5]  = "Dilation";
    fnamesPar[6]  = "Translation";
    fnamesPar[7]  = "OutputCoef";
    fnamesPar[8]  = "OutputOffset";
    
    ParStruct     = mxCreateStructMatrix(1,1,9,fnamesPar);
    ...
    mxSetFieldByNumber(ParStruct, 0, 5,  mxDuplicateArray(PAR_DILATION(S)));
    mxSetFieldByNumber(ParStruct, 0, 6,  mxDuplicateArray(PAR_TRANSLATION(S)));
    mxSetFieldByNumber(ParStruct, 0, 7,  mxDuplicateArray(PAR_OUTPUTCOEF(S)));
    mxSetFieldByNumber(ParStruct, 0, 8,  mxDuplicateArray(PAR_OUTPUTOFFSET(S)));
