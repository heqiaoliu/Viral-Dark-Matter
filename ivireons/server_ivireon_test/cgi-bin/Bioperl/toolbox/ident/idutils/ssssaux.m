...
%   $Revision: 1.7.4.8 $ $Date: 2008/10/02 18:52:02 $
...
if strcmp(type,'kric')
    ...
    a1=a-b*invr*nn';
    [n,ndum]=size(a);
    [v,d] = eig([a1+b*invr*b'/a1'*q  -b*invr*b'/a1'; -a1'\q  pinv(a1)']);
    
    d = diag(d);
    [e,index] = sort(abs(d));    % sort on magnitude of eigenvalues
    out2 = 0;
    if ~((e(n) < 1) && (e(n+1)>1))
        ctrlMsgUtils.warning('Ident:utility:EigenvalueSortFailure')
        out2 = 1;
    else
        e = d(index(1:n));
    end
    ...
    if min(svd(chi))<10*eps
        ctrlMsgUtils.warning('Ident:utility:IllconditionedEstimation')
        out2 = 2;
    end
    lambda = v((n+1):(2*n),index(1:n));
    s = real(lambda/chi)';
    k = (a'*s*b+nn)*pinv(b'*s*b+r);
    %   k = (r+b'*s*b)\b'*s*a + r\nn';
