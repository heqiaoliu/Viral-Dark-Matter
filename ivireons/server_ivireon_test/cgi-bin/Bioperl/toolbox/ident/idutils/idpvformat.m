function DispStr = idpvformat(varargin)
%IDPVFORMAT  Formats property names and values for display.
%
%   VALSTR = IDPVFORMAT(VALUES) formats the property value 
%   information.
%
%   DISPSTR = IDPVFORMAT(PROPSTR, VALSTR) returns the character array to be
%   displayed by GET(SYS). Both PROPSTR and VALSTR are cell arrays of
%   strings.

% Copyright 2005-2006 The MathWorks, Inc.
% $Revision: 1.1.8.1 $ $Date: 2006/12/27 20:57:28 $
...
if (nargin == 1)
    ...
    for i = 1:Nprops
        ...
        if (isempty(val_str) || (length(val_str) > too_big_constant))
            ...
            if isa(val, 'idnlfun')
                val_str = sprintf('%dx1 ', size(val, 1));
            else
                val_str = sprintf('%dx', size(val));
            end
