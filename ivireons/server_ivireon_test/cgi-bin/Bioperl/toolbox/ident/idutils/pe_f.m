...
%	$Revision: 1.13.4.8 $  $Date: 2009/12/07 20:42:39 $
...
if ~(isa(m,'idss') || isa(m,'idgrey') || isa(m,'idproc'))
    m = idss(m);%error('Model must be an idss object.')
end
...
killK = 0;
if ~isempty(init)&&strcmp(init(1),'o') % This is for a special call from compare
    init = 'e';
    killK = 1;
end
if isa(data,'iddata')
...
elseif ischar(init)
    ...
    if ~any(strcmp(init,{'m','z','e','d'}))
        ctrlMsgUtils.error('Ident:analysis:peInvalidIniFreqData')
    end
else
    ctrlMsgUtils.error('Ident:analysis:peNumericalX0WithFreqData')
...
if ~isa(data,'iddata')
    ...
    if Ts>0
        MInd = find(cellfun(@(x)x~=Ts,Tsdata));
        if ~isempty(MInd)
            ctrlMsgUtils.warning('Ident:analysis:peDataModelTsMismatch',...
                sprintf('%g',Ts),sprintf('%g',Tsdata{MInd(1)}))
        end
    end
    ...
    for kexp = 1:nexp
        ...
        if Tsdata{kexp}==0
            %BL-Continuous time system %%LL modification
            farg{kexp} = 1i*freq{kexp}*picorr;
        elseif Ts>0
            farg{kexp} = exp(1i*freq{kexp}*Ts*picorr);
        else % cont time model
            farg{kexp} = exp(1i*freq{kexp}*Tsdata{kexp}*picorr);
            sample = 1;
        end
    end
end
...
if Ts==0 && ~sample, %Continuous time system
    if init=='e'&& ~residcall && ~initwasa
        ctrlMsgUtils.warning('Ident:analysis:X0estCTModel')
    end
    init = 'z';
