...
%   $Revision: 1.14.4.5 $ $Date: 2008/10/31 06:14:07 $
...
for ku = 1:nu
    ...
    if T>0
        ...
        switch modsa(1)
            case 'z'
                Kp = zeros(size(F,1),0);
                [A1,B1,C1,D1] = Local_idsample(F,G,Cp,Dp,Kp,T,'zoh',1,TD);
            ...
            case 'f'
                ...
                %Cc = Cp;Dd=Dp;
                [A1,B1,C1,D1] = Local_idsample(F,G,Cp,Dp,Ktemp,T,'foh',1,TD);
...
switch dist
    case 'ARMA1'
        ...
        %else
        if T>0
            [a,b] = Local_idsample(a,b,[],[],zeros(size(b)),T,'zoh');
    ...
    case 'ARMA2'
        try
            dpol = fstab([1 pard(1) pard(2)],0);
            cpol = fstab([1 pard(3) pard(4)],0);
...
%--------------------------------------------------------------------------
function  varargout = Local_idsample(varargin)
% sample model with error checking

try
    [varargout{1:nargout}] = idsample(varargin{:});
catch E
    if strcmp(E.identifier,'MATLAB:nomem')
        ctrlMsgUtils.error('Ident:estimation:idprocTooManyParameters2')
    else
        throw(E)
    end
end
