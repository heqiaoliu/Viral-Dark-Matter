...
%
% Frequency domain subspace based estimation algorithm; called by n4sid_f.
%
...
% $Revision: 1.3.4.7 $  $Date: 2009/04/21 03:22:47 $
...
switch lower(mode.x0)
    ...
    case {'noestim'}
        estx0 = 0;
    otherwise
        %erro('Not a valid fsub_estim mode.x0 choice');
end
...
switch lower(mode.alg)
    ...
    case {'wproj'}
        wproj = 1;
        %case {'iv'}
        %erro('Not a valid fsub_estim algorithm choice');
    otherwise
        %erro('Not a valid fsub_estim mode.alg choice');
end
...
%         stab = 1;
%     otherwise
%         %err('Not a valid fsub_estim mode.stab choice');
% end
...
if check_inpidf(u,w,n)
    ...
    if strcmpi(mode.x0,'estim')
        ctrlMsgUtils.warning('Ident:estimation:fsubEstimchek1')
    end
    estx0 = 0;
    u = [];
    for kexp = 1:nexp,
...
% end
if ~isreal(w)
    ctrlMsgUtils.error('Ident:estimation:fsubEstimchek2')
end
...
[dum,m] = size(u);
if dum~=N
    ctrlMsgUtils.error('Ident:estimation:fsubEstimchek5')
end

[dum,p] = size(y);
if dum~=N
    ctrlMsgUtils.error('Ident:estimation:fsubEstimchek6')
end
if q<n+1
    ctrlMsgUtils.error('Ident:estimation:fsubEstimchek7')
end

% Check number of data points (is two samples conservative if
% only complex data)
if 2*N-2 < n + q*m
    %erro('Too few frequency points (2*N-2 >= n + q*m).');
    ctrlMsgUtils.error('Ident:estimation:tooFewSamples')
end

if length(nk)~=m
    ctrlMsgUtils.error('Ident:estimation:fsubEstimchek8')
end
if ~all((nk==0) | (nk==1)),
    ctrlMsgUtils.error('Ident:estimation:fsubEstimchek9')
end
...
if nargin<8, % Must calculate QR and SVD
    ...
    if q*p ~= size(K,2) || q*p ~= size(E,1)
        ctrlMsgUtils.error('Ident:estimation:fsubEstimchek10')
    end
end
EE = K*E(:,1:n);
...
if strcmpi(mode.stab,'stable')
    a = stab(a); % To do or not do?
end
...
%dcolidx = find(nk==0);
if nestD,
    d(:,nk==0) = reshape(DB(1:p*nestD),p,nestD);
    b(:) = DB(p*nestD+1:end);
else
    % b(:) = DB(p*m+1:end);
    b(:) = DB;
...
%       mypinv(A,[],setrank) calculates the pseudo-inverse explicitly
