...
%VERTCAT  Vertical concatenation of IDFRD models.
%
...
%   $Revision: 1.8.4.8 $  $Date: 2009/03/09 19:13:32 $
...
for kj = 2:length(varargin)
    sysj = varargin{kj};
    if ~isa(sysj,'idfrd')
        ctrlMsgUtils.error('Ident:combination:concatClassType','IDFRD')
    end
    ...
    if nameflag
        ctrlMsgUtils.error('Ident:dataprocess:idfrdVertcatYName')
    end
    ...
    if nu~=Nu
        ctrlMsgUtils.error('Ident:combination:vertcatNu','IDFRD')
    end
    try
    ...
    if ~all(strcmp(sys1.InputName,sysj.InputName)) ||...
        ...
        ctrlMsgUtils.warning('Ident:dataprocess:idfrdVertcatCheck3')
    end
    if sys1.Ts~=sysj.Ts
