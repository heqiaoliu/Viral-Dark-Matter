...
%IDFRD/FCAT Concatenation of IDFRDs along the frequency dimension.
%   MC = FCAT(M1,M2,...Mn)
%
...
%   $Revision: 1.1.4.6 $  $Date: 2009/03/09 19:13:29 $
...
for kd = 1:length(varargin)
    id = varargin{kd};
    if ~isa(id,'idfrd')
        ctrlMsgUtils.error('Ident:dataprocess:fcatCheck1')
    end

    [ny1,nu1,dum,ns1] = size(id);
    if ny1~=ny || nu1~=nu || ns1~=ns
        ctrlMsgUtils.error('Ident:dataprocess:fcatCheck2')
    end

    if ~strcmp(cat(2,inpn{:}),cat(2,id.InputName{:})) ||...
            ~strcmp(cat(2,outn{:}),cat(2,id.OutputName{:}))
        ctrlMsgUtils.warning('Ident:dataprocess:fcatCheck3')
    end

    if ~strcmp(cat(2,inpu{:}),cat(2,id.InputUnit{:})) ||...
            ~strcmp(cat(2,outu{:}),cat(2,id.OutputUnit{:}))
        ctrlMsgUtils.warning('Ident:dataprocess:fcatCheck4')
    end

    if ~strcmp(un,id.Units)
        ctrlMsgUtils.error('Ident:dataprocess:fcatCheck5')
    end
    if ts~=id.Ts
        ctrlMsgUtils.error('Ident:dataprocess:fcatCheck6')
    end
