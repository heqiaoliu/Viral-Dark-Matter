...
%   $Revision: 1.6.4.9 $  $Date: 2009/01/20 15:32:22 $
...
uni = get(f,'unit');
d = pvset(d,'Tstart',uni);
if strcmp(uni,'Hz')
    TimeU = '';
else
    TimeU = uni(findstr(uni,'/')+1:end);
end
d = pvset(d,'TimeUnit',TimeU);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function d = reminf(d)
% Removes entries that correspond to infinite y-values in IDDATA d
y=pvget(d,'OutputData');
u = pvget(d,'InputData');
fre = pvget(d,'SamplingInstants');
if any(any(~isfinite(cat(1,y{:}))))
    for kexp = 1:length(y)
    ...
    ctrlMsgUtils.warning('Ident:dataprocess:infFreRespRemoved')
end
