...
%
%   Moreover, any IDMODEL property can be defined, and also
%
%   DisturbanceModel: Describes a possible additive noise model.
%        'None': Means no noise model is used.
%        'ARMA1': A first order ARMA model is used.
...
%
%   See also IDPOLY, IDSS, IDARX, IDMODEL, PEM, IDMODEL/D2C.

%   Copyright 1986-2008 The MathWorks, Inc.
%   $Revision: 1.6.4.8 $  $Date: 2008/12/04 22:33:45 $

ni = nargin;

if ni && (isa(varargin{1},'ss') || isa(varargin{1},'zpk') || isa(varargin{1},'tf') ||...
        (isa(varargin{1},'idmodel') && ~isa(varargin{1},'idproc')))
    ctrlMsgUtils.error('Ident:transformation:idproc2idmodel',upper(class(varargin{1})))
end

% Quick exit for idgrey objects
if ni && isa(varargin{1},'idproc')
    if ni~=1
        ctrlMsgUtils.error('Ident:general:useSetForProp','IDPROC');
    end
    sys = varargin{1};
    return
end
...
% Dissect input list
PVstart = find(cellfun('isclass',varargin(2:end),'char'), 1)+1;
%{
if isempty(PVstart)
    DoubleInputs = ni;
else
...
for ku = 1:nu
    ...
    if ~ischar(type) || lower(type(1))~='p'
        ctrlMsgUtils.error('Ident:idmodel:idprocCheck1')
    end
    Type = '';
    Type(1) = 'P';
    ...
    elseif any(findstr(type,'3'))
        ...
        ctrlMsgUtils.error('Ident:idmodel:idprocCheck2')
    end
    ...
    if any(findstr(lower(type),'z'))
        if eval(Type(2))==0
            ctrlMsgUtils.error('Ident:idmodel:idprocCheck3')
        end
        Type(nr) = 'Z';
        nr = nr+1;
        %npar = npar +1;
    ...
    if any(findstr(lower(type),'u'))
        if eval(Type(2))<2
            ctrlMsgUtils.error('Ident:idmodel:idprocCheck4')
        end
        Type(nr) = 'U';
        %nr = nr+1;
    end
