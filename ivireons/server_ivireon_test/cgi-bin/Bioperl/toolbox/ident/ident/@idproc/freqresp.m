...
%   $Revision: 1.5.4.1 $ $Date: 2008/10/02 18:49:11 $
...
for ku = 1:nu
    ...
    np = 0;
    if strcmpi(Kp.status{ku},'estimate')
        dgdp = [dgdp,dgdk.'];
        np = np+1;
    end
    if strcmpi(Tp1.status{ku},'estimate')
        dgdp = [dgdp,dgdtp1.'];
        np = np+1;
    end
    if strcmpi(Tp2.status{ku},'estimate')
        dgdp = [dgdp,dgdtp2.'];
        np = np+1;
    end
    
    if strcmpi(Tw.status{ku},'estimate')
        dgdp = [dgdp,dgdtw.'];
        np = np+1;
    end
    if strcmpi(Zeta.status{ku},'estimate')
        dgdp = [dgdp,dgdzeta.'];
        np = np+1;
    end
    if strcmpi(Tp3.status{ku},'estimate')
        dgdp = [dgdp,dgdtp3.'];
        np = np+1;
    end
    if strcmpi(Td.status{ku},'estimate')
        dgdp = [dgdp,dgdtd.'];
        np = np+1;
    end
    if strcmpi(Tz.status{ku},'estimate')
        dgdp = [dgdp,dgdtz.'];
        np = np+1;
    end
