...
%IDPROC/INIVAL Sets initial values for process models
%
...
%   $Revision: 1.14.4.12 $  $Date: 2009/10/16 04:55:54 $
    ...
    if any(type=='I')
        for kexp = 1:Nexp
            if ~fd
                ...
                y{kexp} = y{kexp};
                if Ts{kexp}>0
                    rat = 1-exp(-1i*samp{kexp}*Ts{kexp});
                else
                    rat = 1i*samp{kexp};
                end
...
% parameters ar fixed w.r.t IDPROC structure; not applicable for generic
% (idpoly structure) initialization
m_al = pvget(m,'Algorithm');
m_al.FixedParameter = [];

try % This is just due to possible problems in very special structures with inoe='b';
    if ncd>0
        mstart = bj(z,[nb ncd ncd nf nk],'cov','none','ini',inoe,...
            'alg',m_al); %was 'e'
    else
        mstart = oe(z,[nb nf nk],'cov','none','ini',inoe,'alg',m_al);
    end
catch
    inoe = 'z';
    if ncd>0
...
for ku = 1:nu
    ...
    if eval(type(2))>0
        ...
        try
            ...
            ctrlMsgUtils.error('Ident:estimation:idprocInitFailure')
        end
        ...
        elseif eval(type(2))==2
            if any(type=='U')
                ...
                if isreal(rr(1))
                    ...
                    ctrlMsgUtils.warning('Ident:estimation:underdampedIDPROC')
                    
                    was = warning('off'); [lw,lwid] = lastwarn;
                    mstart1 = oe(z(:,:,ku),[1 1 nk(ku)],'cov','none');
                    f = pvget(mstart1,'f');
    ...
    if any(type=='D') && ~tdflag(ku)
        if eval(type(2))==0
            ...
            if zflag
                [~,Tz(ku)] = findtau(Td(ku),m0,Tsm,zt,zflag);
            end
            if any(type=='I') && fd
                Td(ku)=Td(ku)-Tsm; %%WHY
            end
