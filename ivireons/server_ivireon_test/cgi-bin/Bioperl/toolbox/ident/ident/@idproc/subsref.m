...
%   $Revision: 1.10.4.9 $ $Date: 2009/03/09 19:13:51 $
...
switch Struct(1).type
    ...
    otherwise
        ctrlMsgUtils.error('Ident:general:unSupportedSubsrefType', Struct(1).type,'IDPROC')
end
...
% Now find the parameter indices that have been selected
pnr = [];
for ku=uind
...
if ~isempty(x0)
    ...
    [a,b,c,d,k,x0dum,xnrr,levnr] = procmod(sys.idgrey.par,0,sys.idgrey.file);%%%WAS 1
    xnr = [];
    for ku = uind
        if ku==1
            xnr=[xnr; (1:xnrr(ku))'];
