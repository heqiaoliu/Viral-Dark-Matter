...
%       taken together.
%
%   See also IDMODEL/BODE, IDMODEL/NYQUIST, ETFE, IDFRD.

%   L. Ljung 10-1-86, 29-3-92 (mv-case),11-11-94 (complex data case)
%   Copyright 1986-2009 The MathWorks, Inc.
%   $Revision: 1.29.4.10 $ $Date: 2009/12/07 20:42:22 $
...
T = T{1};
if isempty(T)
    ctrlMsgUtils.error('Ident:iddata:wrongDataTs','spa')
end
dw = 0;
...
if dom=='t'
    Mdef = max(min(30,floor(mean(Ncaps)/10)),2);
else
    Mdef = max(min(30,floor(mean(2*Ncaps)/10)),2);
end
if isempty(M), M = Mdef; end       %Mod 89-10-31
if any(M<0), M = Mdef*ones(1,length(M)); end
M = M(:)';
if any(M-M(1)~=0)
    ctrlMsgUtils.warning('Ident:estimation:spacheck1')
end
...
dw = 0; %disable fft based computation (r.s., 08/2009)
...
if M>=min(Ncaps)
    M = min(Ncaps)-1;
    ctrlMsgUtils.warning('Ident:estimation:spacheck2',M)
end

[nrw,rcw] = size(W); if nrw>rcw, W = W.'; end
if ~isreal(M) || floor(M)~=M || M<2
    ctrlMsgUtils.error('Ident:estimation:spaIncorrectLags')
end
...
% *** Form the Hann lag window ***
pd = pi/(M-1);
window = [1 0.5*ones(1,M-1)+0.5*cos(pd*(1:M-1))];
if dw~=1 || nz>2 || spec_case || (nz==2 && ny==2)
    window(1) = window(1)/2;
end
...
if nz<3 && ny==1 && ~spec_case
    ...
    if nz==2
        ...
        if dw~=1
            rtem = R(4,:).*window;
            FIU = 2*real(polyval(rtem(M:-1:1),exp(-1i*W*T)));
            rtem = R(2,:).*window; rtemp=R(3,:).*window;
            FIYU = polyval(rtem(M:-1:1),exp(-1i*W*T)) + ...
                polyval(rtemp(M:-1:1),exp(1i*W*T));
        end
    ...
    if dw~=1
        FIY = 2*real(polyval(rtem(M:-1:1),exp(-1i*W*T)));
    end
    if nz==2
        FFV = (FIYU.*conj(FIYU))./FIU;
    else
        FFV = zeros(1,n);
    end
    ...
    if ny==1 && ~inhib && nz>2 && advflag,
        ctrlMsgUtils.warning('Ident:estimation:spacheck3',nu)
    end
    FI=zeros(nz,n);
    for k=1:nz^2
        rtem=R(k,:).*window;
        FI(k,:) = polyval(rtem(M:-1:1),exp(1i*W*T));
    end
    outputs = 1:ny;
    inputs = ny+1:nz;
    ZSPE = zeros(nz^2,n);
    for k=1:n
        ...
        zspect(:,:,k) = FIZ;
        if nu>0,
            x_ = FIZ(inputs,inputs);
            PHIUINV = pinv(x_); % numerically well conditioned inversion (r.s., Apr 20, 2006)
            GCk = FIZ(outputs,inputs)*PHIUINV;
...
if any(nargout == [0 1])
    ...
    GH=idfrd(response,W',T,'CovarianceData',covdata,...
        ...
        'InputUnit',InputUnit,'OutputUnit',OutputUnit,'Units',Units);
    delete(was)
