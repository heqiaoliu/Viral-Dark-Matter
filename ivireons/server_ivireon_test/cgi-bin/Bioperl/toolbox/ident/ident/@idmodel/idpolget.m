...
%   $Revision: 1.21.4.17 $ $Date: 2009/12/05 02:03:01 $
    ...
    if ~isempty(idp)
        ...
        if norm(lam)>0
            try
                ...
                try
                    L = chol(lam+1e4*eps*eye(size(lam)));
                catch
                    L = sqrt(norm(lam))*eye(size(lam));
                    ctrlMsgUtils.warning('Ident:general:illCondCovar');
                end
            end
...
% if strcmp(sys.CovarianceMatrix,'None') % This needs to be modified
%     idp = [];
%     return
% end
idu = sys.Utility;
...
if ~isempty(idp)
    if noises == 'd' % then we must extract the right B make it C
        % and delete the correpsonding cov-cross, unless it is an all-measured model
        if norm(sys.NoiseVariance)>0
            
            [Ny,Nu] = size(sys);
            for ky = 1:Ny
                pol = idp{ky};
                pol = pvset(pol,'BFFormat',-1);
                ...
                % If C not monic ...
                if C(1)~=1
                    ctrlMsgUtils.warning('Ident:analysis:noiseComputeFailure')
                    C = 1;
                end
                ...
                nbn = pvget(pol,'nb'); %nbcorr = nb(1:Nu) - nbn;
                na = pvget(pol,'na');
                covnr = 1:na;
                for ku=1:Nu
                    covnr = [covnr,na+Nb(ku)+1:na+Nb(ku)+nbn(ku)];
                end
                covnr = [covnr, na+Nb(Nu+1)+2:nend];
                if length(covnr)~=length(pvget(pol,'ParameterVector'))
                    ctrlMsgUtils.warning('Ident:analysis:incorrectCovDim')
                    cov  = [];
                else
                    try
                        cov = cov(covnr,covnr);
...
T=pvget(sys,'Ts');
noP=0;
if isa(sys,'idss') && (isempty(pvget(sys,'CovarianceMatrix'))...
        ||ischar(pvget(sys,'CovarianceMatrix')))
    % här problem om
...
if isempty(syspar),syspar=0;end
dt=nuderst(syspar);
...
if T>0 && (isa(sysa,'idarx') || (isa(sysa,'idss') &&...
        any(strcmpi(pvget(sysa,'SSParameterization'),{'Structured','Canonical'}))))
    test=1;
else
    test=0;
end
...
for iy=nny
    ...
    if strcmp(noises,'d')
        ...
        Bpol=Bpol(1:nu,:);
        lambb=lambda(iy,iy);
        if T>0,ic=find(abs(Cpol)>eps, 1, 'last' );Cpol=Cpol(1:ic);end
    else
        Cpol=1;
        lambb=0;
    end
    if T>0,ia=find(abs(Apol)>eps, 1, 'last' );Apol=Apol(1:ia);end
...
A=A+1e-10*randn(s,size(A)); [VA,DA]=eig(A); LA=diag(DA); VAI=inv(VA);
