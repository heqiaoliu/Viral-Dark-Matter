...
%   band instead: IMPULSE(M,'sd',3,'fill'). The confidence region is
%   rendered using a lighter shade of the line color.
%
...
%   $Revision: 1.19.4.16 $  $Date: 2009/11/09 16:23:35 $
...
if length(kpf)>1
    ctrlMsgUtils.error('Ident:analysis:impulseCheck1');
end
if ~isempty(kpf)
    if kpf == length(varargin)
        ctrlMsgUtils.error('Ident:analysis:impulseCheck2')
    end
    NA = varargin{kpf+1};
    if ~isa(NA,'double') || ~isscalar(NA)
        ctrlMsgUtils.error('Ident:analysis:impulseCheck2')
    end
...
if length(kpf)>1
    ctrlMsgUtils.error('Ident:analysis:impulseCheck3');
end
if ~isempty(kpf)
    if kpf == length(varargin)
        ctrlMsgUtils.error('Ident:analysis:impulseCheck4')
    end
    sd = varargin{kpf+1};
    if ~isa(sd,'double') || ~isscalar(sd) || sd<0
        ctrlMsgUtils.error('Ident:analysis:impulseCheck5')
    end
...
[sys,sysname,PlotStyle,T,Tdata,Tsdemand] = sysirdec(NA,0,varargin{:});
if isempty(sys)
    if nargout
        you = NaN;
        tou = NaN;
    ...
    ctrlMsgUtils.warning('Ident:analysis:emptyModelResponse','Impulse')
    return
end
if length(T)==1
    T =[-T/4 T];
...
if ~isa(T,'double') || any(~isfinite(T)) || ndims(T)~=2
    ctrlMsgUtils.error('Ident:analysis:stepTvalue','impulse')
end
...
if nargout
    if length(sys)>1
        ctrlMsgUtils.error('Ident:analysis:RequiresSingleModelWithOutputArgs','impulse')
    end
...
if isaimp(sys1)
    ...
    if ~sys1.Ts % continuous time model
        if sd >0
            ctrlMsgUtils.warning('Ident:analysis:SDForCTModel')
        end
        sys1 = idss(sys1);
        B = pvget(sys1,'B');
        sys1 = c2d(sys1,Ts);
