...
%   $Revision: 1.24.4.12 $  $Date: 2009/12/05 02:03:17 $
...
if size(th,'Nu')>1 && ((isa(th,'idpoly') && sum(pvget(th,'nf'))>0) || isa(th,'idproc'))
    [z,p,K,dz,dp,dK] = zpkdatauax(th,vec,nargout);
    return
end
...
if ~isreal(th)
    ...
    for ky = 1:ny
        for ku = 1:nu
            ...
            knr = find(abs(num{ky,ku})>eps, 1 );
            K(ky,ku) = num{ky,ku}(knr);
        end
    end
    if strcmp(vec(1),'v');
        z = z{1,1}; p = p{1,1};
    ...
    if nargout>3
        ctrlMsgUtils.warning('Ident:analysis:noCovForComplexModel')
    end
    dp=[];dz=[];dK=[];
    return
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  [z,p,K,dz,dp,dK]=zpkdatauax(th,vec,na)
nu = size(th,'nu');
for ku = 1:nu
    if na < 4
        [z1,p1,K1] = zpkdata(th(ku),vec);
        z(:,ku)=z1;
        p(:,ku)=p1;
        K(:,ku)=K1;
        dz={};dp={};dK={};
    else
        [z1,p1,K1,dz1,dp1,dK1]=zpkdata(th(ku),vec);
        z(:,ku)=z1;
        p(:,ku)=p1;
        K(:,ku)=K1;
        dz(:,ku)=dz1;
