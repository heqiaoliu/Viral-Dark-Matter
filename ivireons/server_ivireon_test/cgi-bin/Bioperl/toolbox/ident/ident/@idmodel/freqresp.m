...
%FREQRESP Frequency response of IDMODEL models.
%
%   H = FREQRESP(M,W) computes the frequency response H of the
%   IDMODEL model M at the frequencies specified by the vector W.
%   These frequencies should be real and in angular frequency
%   (rad/s or rad/M.TimeUnit). If W is omitted a default choice is made.
%
...
%   $Revision: 1.18.4.15 $  $Date: 2009/12/05 02:02:57 $
...
[a,b,c,d,k] = ssdata(sys); [~,nu] = size(b); ny = size(c,1);
if sys.Ts>0 && max(w>pi/sys.Ts+eps*1e4)
    w = w(w<=pi/sys.Ts+eps*1e4);
    if isempty(w)
...
%{
try
    Ncap = sys.EstimationInfo.DataLength;
...
for ku=nnu
    ...
    if delays(ku)~=0
        ff = ff.*(exp(-1i*w*delays(ku))*ones(1,length(nny)));
    end
...
if nargout>2
    ...
    if ~timeseries
        ...
        for ky = nny
            thbb = thbbmod{ky};
            [~,~,covtemp] = freqresp(thbb(1,1:nu),w);
            covff(ky,:,:,:,:) = covtemp;
        end
    else % if time series
...
try
    ...
    g0 = mimofr(a,b,[],[],w(:));
    g0 = reshape(g0,[length(a) numel(w)]);
end

g = c * g0 + diag(d) * ones(no,nw);
g(any(isinf(g0))) = Inf;
