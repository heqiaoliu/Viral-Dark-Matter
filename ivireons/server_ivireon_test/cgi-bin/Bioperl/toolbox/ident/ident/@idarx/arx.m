...
%   errors and LAMBDA is Mi.NoiseVariance. Note that Algorithm properties
%   are ignored since the estimation process is non-iterative.
%   In order to minimize the criterion with arbitrary weighting (W) (that
%   is minimize norm of E'*W*E) set the NoiseVariance property of the
%   initial model to inv(W). Note that W must be a positive definite matrix
%   of size equal to number of model outputs. W can be used to specify
%   relative importance of outputs in multi-output models (or reliability
%   of corresponding data). For such a specification, set W to a diagonal
%   matrix with larger entries for more significant (more reliable) output
%   channel.
%
...
%   $Revision: 1.20.4.15 $  $Date: 2009/12/22 18:53:42 $
...
%isDet = strcmpi(nn.Algorithm.Criterion,'Det');
...
if p==1
    ...
    T = pvget(nn,'Ts');
    data = iddata(ze{1}(:,1:nyd),ze{1}(:,nyd+1:end),T,'Name',datan);
    for kk = 2:length(ze)
        newdata = iddata(ze{kk}(:,1:nyd),ze{kk}(:,nyd+1:end),T);
...
if (ischar(foc) && strcmpi(foc,'Simulation')) || ~ischar(foc) %% The focus
    ...
    if isa(foc,'idmodel') || isa(foc,'lti') || iscell(foc)
        ts = pvget(data,'Ts');ts=ts{1};[~,den1]=tfdata(m0);
