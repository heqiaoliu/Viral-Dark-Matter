...
%   TH = ivx(DATA,MODEL,X)
%
%   DATA: The output-input data DATA = [Y U] with the outputs Y
...
%
%   With TH = ivx(DATA,MODEL,X,MAXSIZE,T) some additional
%   options can be achieved. See IDPROPS ALGORITHM for details.

%   L. Ljung 10-3-90
%   Copyright 1986-2008 The MathWorks, Inc.
%   $Revision: 1.12.4.9 $ $Date: 2009/01/20 15:32:19 $
...
if p && ~strcmp(pvget(nn,'CovarianceMatrix'),'None')
    ...
    try
        ...
        ctrlMsgUtils.warning('Ident:estimation:illConditionedCovar')
        pvar = [];
    end
...
if p==1,
    ...
    idm = pvset(idm,'ParameterVector',pvget(m,'ParameterVector'),...
        'MaxSize',maxsize,'CovarianceMatrix',pvar,...
