...
%   $Revision: 1.20.4.7 $ $Date: 2009/03/09 19:13:19 $
...
switch Struct(1).type
        ...
        try
            if StrucL==1,
                ...
                if ~(isempty(pvget(sys,'PName')) && isempty(pvget(sys,'FixedParameter')))
                    if fixflag
                        ctrlMsgUtils.warning('Ident:idmodel:idarxSubsref1')
    ...
    otherwise
        ctrlMsgUtils.error('Ident:general:unSupportedSubsrefType',Struct(1).type,'IDARX')
end
