function datf = fft(dat,n,varargin)
%IDDATA/FFT Compute FFT of time domain IDDATA signals.
%
%   DATF = FFT(DAT)
%   converts the time domain IDDATA signal DAT to the frequency domain data
%   DATF using FFT and sorting the data wrt frequencies.
%
...
%   $Revision: 1.8.4.10 $ $Date: 2009/04/21 03:22:08 $
...
for kexp = 1:length(y)
    ...
    Y1 = fft(y{kexp},N,1)/sqrt(min(N)); % think if it is N or Ndat!
    U1 = fft(u{kexp},N,1)/sqrt(min(N));
