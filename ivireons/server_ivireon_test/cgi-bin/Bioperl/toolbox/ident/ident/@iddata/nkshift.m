...
%   $Revision: 1.9.4.5 $ $Date: 2008/10/02 18:46:57 $
...
if iscell(nk)
    if ~length(nk)==Ne && ~length(nk)==1
        ctrlMsgUtils.error('Ident:iddata:multiExpIncorrectNK')
    end
    if Ne>1 && length(nk)==1
        nk = repmat(nk,1,Ne);
    end
    try
        ...
        ctrlMsgUtils.error('Ident:iddata:incorrectNK1')
    end
else
    if min(size(nk)) == 1
        nk = nk(:)';
...
nunk = size(nk,2);
if nunk~=nu
    ctrlMsgUtils.error('Ident:iddata:incorrectNK2')
end
if nu == 0
    ctrlMsgUtils.error('Ident:dataprocess:nkshiftCheck1')
end

if strcmpi(pvget(dat2,'Domain'),'frequency')
