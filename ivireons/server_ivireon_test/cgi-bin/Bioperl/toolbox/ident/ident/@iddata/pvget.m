...
%       $Revision: 1.7.4.2 $  $Date: 2007/11/09 20:12:28 $
...
if nargin==2,
    ...
    if strcmp(Property,'SamplingInstants')
        ...
        for kk=1:length(Ncap) 
            if isempty(Value{kk})
                if strcmpi(sys.Domain,'frequency') % It is not suitable
                    ...
                    %Value{kk}= [tsta:Tss{kk}:Ncap*Tss{kk}]';% [tsta+[0:Ncap(kk)-1]*Tss{kk}]';
                end
            end             
        end
