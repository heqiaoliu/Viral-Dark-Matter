...
%FREQRESP Frequency response of IDPOLY models.
%
...
%   $Revision: 1.18.4.12 $ $Date: 2009/12/05 02:03:25 $

T = pvget(sys.idmodel,'Ts');
sys = pvset(sys,'BFFormat',1); % force double format for B/F
...
w = w(:);
if T>0 && any(w>pi/T+eps*1e4*max(w)),
    w = w(w<=pi/T+eps*1e4);
    if isempty(w)
...
if ~timeseries
    ...
    if isP && nargout>2,
        ...
        if ~isempty(Ncap)
            ...
            P1 = P(ll,ll);
            [~,~,~,varamp] = ffsdcal(a,c,d,na,nc,nd,1,H,OM,P1,T);
            var(1,1,:) = varamp;
