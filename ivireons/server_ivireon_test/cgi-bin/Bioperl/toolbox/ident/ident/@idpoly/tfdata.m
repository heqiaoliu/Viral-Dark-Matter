...
%   $Revision: 1.12.2.3 $  $Date: 2009/12/05 02:03:47 $
...
if nu>0
    ...
    for iu = 1:nu
        ...
        if Ts==0
            nume = b(iu,end-th.nb(iu)+1:end);
            [nume,deno] = localStripZeros(nume,deno,true);
        else
            % discrete in q^-1
            nume = b(iu,1:th.nb(iu)+th.nk(iu));
            [nume,deno] = localStripZeros(nume,deno,false);
        end
        
        ld = length(deno); ln = length(nume); mn = max(ld,ln);
        
        % equalize num and den sizes
        if Ts>0
            den{1,iu} = [deno,zeros(1,mn-ld)];
            num{1,iu} = [nume,zeros(1,mn-ln)];
        else
            den{1,iu} = [zeros(1,mn-ld),deno];
            num{1,iu} = [zeros(1,mn-ln),nume];
        end
    end
else
    deno = conv(a,d);
...
if nu>0
    for iu = 1:nu
        ...
        ld = length(deno); ln = length(nume); mn = max(ld,ln);
        if Ts>0
            dden{1,iu} = [deno,zeros(1,mn-ld)]; 
            dnum{1,iu} = [nume,zeros(1,mn-ln)];
        else
            dden{1,iu} = [zeros(1,mn-ld),deno];
            dnum{1,iu} = [zeros(1,mn-ln),nume];
        end
    end
else
...
%--------------------------------------------------------------------------
function [num,den] = localStripZeros(nume,deno,isCT)
% Strip leading (if isCT) zeros or trailing zeros (if ~isCT)

if isCT
    num = nume(find(nume,1,'first'):end);
    den = deno(find(deno,1,'first'):end);  
else
    num = nume(1:find(nume,1,'last'));
    den = deno(1:find(deno,1,'last'));  
end
