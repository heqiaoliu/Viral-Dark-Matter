...
%   $Revision: 1.13.4.9 $ $Date: 2009/12/05 02:03:44 $

th = pvset(th,'BFFormat',1); % force double format
th0 = th;
nkorig = th.nk;
nkflag = 0;
if any(nkorig>1) && pvget(th,'Ts')>0
    nknew = nkorig>=1;
    th = pvset(th,'nk',nknew);
    nkflag = 1;
end
...
if T > 0
    ...
    fp(nu+1,1:nf1)=zeros(1,nf1);
    fp(nu+1,nf1-th.nd:nf1)=dp;
end
nf=th.nf;
nb=th.nb;
nf(nu+1)=th.nd;
...
for ku=1:nu+1
    ...
    bpp(ku,1:length(btemp)) = btemp;
    nbpp(ku) = length(btemp);
end
[~,maxindb] = size(bpp); nap = length(ap); nab = max(maxindb,nap);
bp = zeros(nu+1,nab);
if T<=0
    if maxindb>nap
        ctrlMsgUtils.error('Ident:analysis:ssdataImproperModel')
    end
    for ku = 1:nu+1
...
if bp(nu+1,1)~=1 && pvget(th,'NoiseVariance')>0  %%LLsom
    ctrlMsgUtils.warning('Ident:analysis:ssdataProperNoiseModel');
end
...
x0 = zeros(nx,1);
if nkflag
    ms = struct('a',a,'b',b,'c',c,'d',d,'k',k,'x0',x0);
    ms = addnk(ms,nkorig,'par');
    a = ms.a;
    b = ms.b;
    c = ms.c;
    d = ms.d;
...
if nargout>6
    cov = pvget(th0.idmodel,'CovarianceMatrix');
    if isempty(cov)
        dA = []; dB = []; dC = [];
        dD = []; dK = []; dX0 = [];
    else
        pp = pvget(th0.idmodel,'ParameterVector');
        th = parset(th0,pp+1i*sqrt(diag(cov)));
