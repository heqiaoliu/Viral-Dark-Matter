...
%   $Revision: 1.17.4.5 $ $Date: 2009/12/22 18:53:48 $
...
nu = size(m,'nu');
BFC = pvget(m,'BFFormat');
m = pvset(m,'BFFormat',1); % use double format 
if conf && isreal(m)
...
case 'est'
    DN = estim.DataName;
    if ~isempty(DN)
        str = str2mat(sprintf('Estimated using %s on data set %s',...
...
if nu>1 && BFC==1
    txt = str2mat(txt,' ','(model configured to operate in backward compatibility mode)');
end
