...
%   $Revision: 1.11.4.7 $ $Date: 2009/12/05 02:03:21 $
...
for kder = nkder
    ...
    if armax
        ...
        if kder==0
            ei = eig(A);
            if any(abs(ei)<eps),
                ctrlMsgUtils.warning('Ident:transformation:poleAtOrigin')
            end
            
            if any(abs(ei-1))<eps
                ctrlMsgUtils.warning('Ident:transformation:poleAtUnitCircle')
            end
            if any((ei==real(ei)) & (real(ei)<0))
                ctrlMsgUtils.warning('Ident:transformation:poleAtNegRealAxis')
            end
            warning('off'), [lw,lwid] = lastwarn; %#ok<WNOFF>
        end
        %nx = size(A,1);
        nu = size(B,2);
        %nz = 1+nu;
        [Ac,Bc,~,Dc,Kc,noisenorm] = idsample(A,B,C,D,K,T,inters,0);
        ff=poly(Ac);
        thb = [];
        ...
        for k = list
            ...
            [Ac,Bcc,~,D] = idsample(A,B,C,D,zeros(n-1,0),T,inters,0);
