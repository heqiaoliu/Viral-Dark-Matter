...
%   $Revision: 1.21.4.12 $ $Date: 2009/12/05 02:03:46 $
...
switch Struct(1).type
    ...
    otherwise
        ctrlMsgUtils.error('Ident:general:unSupportedSubsrefType',Struct(1).type,'IDPOLY')
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~(isempty(covm) || ischar(covm))
    %nnb = sys.nb;
    if flagall% & nc+1 ~= nnb(end)
        %sys = pvset(sys,'nb',[nnb(1:end-1) nc+1]);
        nnb = [nb(uind) nc+1];
    else
        nnb = nb(uind);

    end
    sys = pvset(sys,'nb',nnb);%%
    ...
    if length(parind)~=length(pvget(sys.idmodel,'ParameterVector'))
        ctrlMsgUtils.warning('Ident:idmodel:idpolySubsrefCheck1')
        covv = [];
    else
