...
%	$Revision: 1.10.4.4 $  $Date: 2008/10/02 18:48:55 $

% *** Set up the relevant orders ***

alg = pvget(M,'Algorithm');
Zstab = alg.Advanced.Threshold.Zstability;
if ~iscell(z)
    z = {z};
end
Ne = length(z);
...
if nc+nd>0
    ...
    if nc>0
        ...
        c  = fstab([1 t(nd+1:nd+nc).'],1,Zstab);
        tc = c(2:length(c)).';
        if nd>0
