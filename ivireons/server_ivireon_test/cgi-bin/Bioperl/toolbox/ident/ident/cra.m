...
%
%   The data Z must have 2 signals. If using an IDDATA object, Z should
%   represent single-input, single-output data.
%
...
%   $Revision: 1.14.4.8 $  $Date: 2009/10/16 04:54:40 $
...
if isa(z,'iddata')
    ...
    T = pvget(z,'Ts');
    if length(T)>1
        ctrlMsgUtils.warning('Ident:estimation:craMultiExpData')
    end
    T = T{1};
...
if dom=='f'
    ctrlMsgUtils.error('Ident:general:FreqData','cra')
end
[Ncap,nz]=size(z);redisp=0;
if nz==4, if z((Ncap+1)/2,1)==0, redisp=1;end,end
if nz>2 && ~redisp,
    ctrlMsgUtils.error('Ident:estimation:craTooManySignals');
end
if redisp
    r=z;
    n=0;
...
if ~redisp
    ...
    if n>=Ncap
        n=floor(Ncap/2);
        ctrlMsgUtils.warning('Ident:estimation:craFilterOrderChange',int2str(n));
    end
    
    if M>=Ncap
        M=floor(Ncap/2);
        ctrlMsgUtils.warning('Ident:estimation:craLagChange',int2str(M));
    end
    if nz==1
        ctrlMsgUtils.error('Ident:estimation:craSingleSignal');
    end
    ...
    %M=M-1;
    if M<=0 || round(M)~=M
        ctrlMsgUtils.error('Ident:estimation:negativeLags')
    end
...
if fp>1 || redisp
    ...
    plot(r(:,1),r(:,4))
    if n>0
        title('Correlation from u to y (prewh)')
    else
        title('Correlation from u to y')
    end
