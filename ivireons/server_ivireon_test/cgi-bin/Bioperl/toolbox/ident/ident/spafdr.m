...
%   See also ETFE, SPA, BODE, NYQUIST, FFPLOT, IDFRD. 

%   L. Ljung
%   Copyright 1986-2009 The MathWorks, Inc.
%   $Revision: 1.10.4.9 $ $Date: 2009/10/16 04:54:53 $
...
if ~isa(data,'iddata')
    ctrlMsgUtils.error('Ident:estimation:spafdrcheck1')
end
nam = pvget(data,'Name');
if isempty(nam)
    nam = inputname(1);
...
if (length(freqs)==length(w) && all(sort(freqs(:))==sort(w(:)))) &&...
...
if iscell(w)
    if length(w)<2
        ctrlMsgUtils.error('Ident:estimation:spafdrcheck2')
    end
    if length(w)==2
        NP = 50;
    else
    ...
    if w{1}<=0
        ctrlMsgUtils.error('Ident:estimation:spafdrcheck3')
    end
...
if length(res)~=length(w)
    ctrlMsgUtils.error('Ident:estimation:spafdrcheck4')
end
...
for k = 1:length(w)
    ...
    if nu>0
        if rank(ruu)==nu
            ...
            miss = 1;
            ruin = pinv(diag(diag(ruu))); %changed inv to pinv (r.s., Aug 07, 06)
        end
        gg = ryu*ruin;
...
resp = respsmooth;
if any(isnan(spe))
    ctrlMsgUtils.warning('Ident:estimation:spafdrcheck5')
end

if miss
    ctrlMsgUtils.warning('Ident:estimation:spafdrcheck6')
end
...
est.DataInterSample = pvget(dat,'InterSample');
if isa(dat,'iddata');
    if strcmpi(pvget(dat,'Domain'),'time')
        TimeUnit = pvget(dat,'TimeUnit');
