...
%   and then uses System Identification Toolbox to identify a linear
%   model.
%
...
%
%   See also SIMIDENT, SFUNTMPL, SLIDENT.

%   Copyright 1986-2010 The MathWorks, Inc.
%   $Revision: 1.1.8.2 $ $Date: 2010/03/22 03:48:43 $
...
switch flag,
    ...
    otherwise
        ctrlMsgUtils.error('Ident:simulink:sfunid1')
end
...
if HowOften > npts
    ctrlMsgUtils.error('Ident:simulink:sfunid2')
end
...
if (div == round(div))
    ...
    th = feval(method, [y u], order, 'ts', ts);
    if ~isempty(mn)
        temp = evalin('base',mn);
        temp = [temp;{t,th}];
    ...
    if iscell(yest)
        yest = yest{1}; %predict now returns cell array
    end
    ...
    set(ud.SysInputTitle,'String',...
        ['Actual Output (Red Line) vs. The ',str,' Model Output (Blue Line)']);
    
    % plot the error.
    err = yest-y;
    set(ud.SysErrorAxis,'Visible','on',...
        'Xlim',[min(tvec) max(tvec)],...
        'Ylim',[min(err) max(err)+eps]);
...
switch action
    ...
    otherwise,
        ctrlMsgUtils.error('Ident:simulink:sfunid3')
...
ud.SysInputLine1 = plot(0,0,'m'); hold on;
ud.SysInputLine2 = plot(0,0,'m','color','b');
...
%=============================================================================
% LocalCopyLoadBlock
% This is the System ID block's CopyFcn and LoadFcn.  Initialize the block's
