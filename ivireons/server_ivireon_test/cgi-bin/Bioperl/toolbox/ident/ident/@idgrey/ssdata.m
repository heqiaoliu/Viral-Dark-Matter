...
%   $Revision: 1.8.4.7 $ $Date: 2008/10/31 06:11:15 $
...
if strcmp(cd,'c') && Ts>0
    ctrlMsgUtils.error('Ident:idmodel:idgreySSDataCheck1')
elseif strcmp(cd,'d') && Ts==0
    ctrlMsgUtils.error('Ident:idmodel:idgreySSDataCheck2')
else
    try
        [A,B,C,D,K,X0] = feval(mf,th,Ts,arg);
    catch E
        ctrlMsgUtils.error('Ident:idmodel:idgreySSDataCheck3',mf,mat2str(th),E.message)
    end
end
