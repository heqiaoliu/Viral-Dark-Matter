...
%   $Revision: 1.6.4.3 $  $Date: 2009/03/23 16:37:46 $
...
if length(dkx)>3
   nk=dkx(4:length(dkx));
   if length(nk)~=nu
       ctrlMsgUtils.error('Ident:idmodel:canform1')
   end
   if any(nk~=fix(nk)) || any(nk<0)
      ctrlMsgUtils.error('Ident:idmodel:canform2')
   end
...
if nu==0 && dkx(2)==0
   ctrlMsgUtils.error('Ident:idmodel:canform3')
end

if any(pseudobsind<0),
  ctrlMsgUtils.error('Ident:idmodel:canform4')
end
...
d=zeros(ny,nu);
if dkx(1)
   d(:,nk==0) = NaN*ones(ny,length(find(nk==0)));
end
if dkx(3)
   x0 = NaN*ones(nx,1);
else
