...
%   $Revision: 1.23.4.9 $ $Date: 2009/04/21 03:22:35 $
...
for i=1:2:nargin-1,
    ...
    switch Property
        ...
        case 'X0s'
            ...
            nabcdkx(6)=1;
        case 'StateName'
            stn = idnamchk(Value,'StateName','IDSS');
            sys.StateName = stn;
            ut = pvget(sys,'Utility');
            if isfield(ut,'Pmodel') && isa(ut.Pmodel,'idss')
        ...
        case 'SSParameterization'
            ...
            try
                ...
                ctrlMsgUtils.error('Ident:idmodel:idssSSParVal')
            end
            oldsspar = sys.SSParameterization;
            if ~strcmpi(oldsspar,Value)
        ...
        case 'InitialState' % some value tests
            ...
            try
                ...
                ctrlMsgUtils.error('Ident:idmodel:idssIncorrectIni')
            end
            sys.InitialState = Value;
            is = Value;
            inn(1)=1;
        case 'DisturbanceModel'
            PossVal = {'Estimate';'None';'Fixed';'Zero'};
            Ind = [];
            if ischar(Value)
                Ind = strncmpi(Value,PossVal,length(Value));
            end
            if ~any(Ind)
                ctrlMsgUtils.error('Ident:idmodel:idssIncorrectDist')
            else
                Value = PossVal{Ind};
            end
            
            if strcmpi(Value(1),'z')
                Value  = 'None';
            end
        ...
        case 'CanonicalIndices'
            ...
            elseif ~all(fix(Value)==Value) || any(Value<0)
                ctrlMsgUtils.error('Ident:general:nonnegIntRowPropVal','CanonicalIndices')
            else
                Value = Value(:).';
            end
...
if inn(3)
    if length(nk)~=size(sys.Bs,2)
        ctrlMsgUtils.error('Ident:idmodel:nkLength')
    end
end
...
if strcmp(Est.Status(1:3),'Est')
    ...
    % Special treatment of setting canonical from just estimated free model
...
if any(abcdkx) % touched the matrices
    ...
    if parflag
        ctrlMsgUtils.error('Ident:idmodel:idssSetCheck1')
    end
...
if any(strcmp(SSParameterization,{'Canonical','Free'}))
    ...
    if strcmp(SSParameterization,'Canonical')  && ~isempty(sys)
    ...
    if parflag
        if length(ParameterVector)~=length(pars)
            ctrlMsgUtils.error('Ident:idmodel:idssSetCheck3')
        else
...
elseif strcmp(SSParameterization,'Structured')
    if ~isempty(nk) && inn(3)
        ctrlMsgUtils.error('Ident:idmodel:strucIdssNkSet')
    end
    if ~isempty(nm)
        switch nm
...
Ts = pvget(sys,'Ts');
if any(nk>1) && Ts==0
    ctrlMsgUtils.error('Ident:idmodel:CTIdssNkSet')
end

indp = pvget(sys,'InputDelay');

if ~all(indp==0) && any(nk>1) && Ts>0
    ctrlMsgUtils.warning('Ident:idmodel:nkAndDelaySet')
end
