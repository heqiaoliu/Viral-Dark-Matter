...
%   data points in Z. The analysis of V for selecting optimal orders is
%   performed by SELSTRUC.
%
%   ZE : the output-input data as an IDDATA object.
...
%   of the corresponding ARX model.
%
...
%
%   See also AUXVAR, IVSTRUC, SELSTRUC, STRUC and ARX.

%   L. Ljung 4-12-87, 9-26-1993
%   Copyright 1986-2008 The MathWorks, Inc.
%   $Revision: 1.19.4.7 $  $Date: 2009/10/16 04:54:38 $
...
if isa(z,'iddata')
    dom = pvget(z,'Domain');
    if lower(dom(1))=='f'
        ts = pvget(z,'Ts'); tsv = pvget(zv,'Ts');
        if any(cellfun(@(x)x==0,ts)) || any(cellfun(@(x)x==0,tsv))
            ctrlMsgUtils.error('Ident:analysis:arxstrucCTdata')
        end
    ...
    if ~isempty(Name), z.Name = Name; end
    
    if ny>1
        ctrlMsgUtils.error('Ident:analysis:arxstrucMultiOutput')
    end
    nz = 1+nu;
...
if isa(zv,'iddata') % check domain here too
    ...
    if fd~=fdv
        ctrlMsgUtils.error('Ident:analysis:arxstrucDataDomainsMismatch')
    end
    if fd && init=='e' % then insert an extra input for the initial conditions
    ...
    if ~isempty(errflag.message), error(errflag), end
    if ~isempty(Name), zv.Name = Name; end
...
[nm,nl] = size(nn);
if nu>1 && nm==1,
    ctrlMsgUtils.error('Ident:analysis:arxstrucTooFewOrderRows')
end
...
if nl~=1+2*(nz-1)
    ctrlMsgUtils.error('Ident:analysis:arxstrucIncorrectOrders')
end
if nzv~=nz
    ctrlMsgUtils.error('Ident:analysis:arxstrucDataDimMismatch')
end
if nz>1,
...
for kexp = 1:max(Ne,Nev)
    if fd
        ...
        if vz
            if Tsdatv{kexp}>0,
                OMv=exp(-1i*(0:nmax)'*wv'*Tsdatv{kexp});
                indav = 2:nma+1;
                YYv = Yv;
            else
...
for j = 1:nm
    ...
    if estparno>0
        ...
        %TH=(RR\FF);
        TH = pinv(RR)*FF; %change made by r.s. (Apr 04, 2006)
