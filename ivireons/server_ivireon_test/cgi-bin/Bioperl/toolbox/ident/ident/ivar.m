...
%   $Revision: 1.10.4.3 $  $Date: 2008/10/02 18:46:04 $
...
if isa(data,'frd') || isa(data,'idfrd') || (isa(data,'iddata') ...
    ...
    ctrlMsgUtils.error('Ident:estimation:estUsingFrequencyData','ivar')
end
...
if ny>1
    ctrlMsgUtils.error('Ident:estimation:multiOutputNotAllowed','ivar')
end
if nu>0
    ctrlMsgUtils.error('Ident:estimation:IODataNotAllowed','ivar')
end
...
if ~isscalar(na) || na<=0
    ctrlMsgUtils.error('Ident:estimation:ivarInvalidOrder')
end
