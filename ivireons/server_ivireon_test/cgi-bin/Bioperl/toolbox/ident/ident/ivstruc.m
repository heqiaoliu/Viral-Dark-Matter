...
%   $Revision: 1.11.4.4 $  $Date: 2008/10/02 18:46:05 $
...
if isa(z,'iddata')
    if strcmp(pvget(z,'Domain'),'Frequency')
        ctrlMsgUtils.error('Ident:estimation:ivstruc1')
    end
    ...
    if ny>1
        ctrlMsgUtils.error('Ident:analysis:ivstrucForMultiOutputData')
    end
    %nz = 1+nu;
else
...
if any(any(nb==zeros(snbr,snbc))')
    ctrlMsgUtils.error('Ident:analysis:ivstrucWithNonPosNB')
end % Mod 911227
