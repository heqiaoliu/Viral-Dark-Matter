...
%SOINITIALIZE: single object initialization for DEADZONE estimators.
%
%  [nlobj,ei,nv,covmat] = soinitialize(nlobj, yvec, regmat, algo)
%
%  yvec and regmat must be vectors.

% Copyright 2005-2008 The MathWorks, Inc.
% $Revision: 1.1.8.6 $ $Date: 2008/10/02 18:52:50 $
...
if isempty(interval) || all(~isinf(interval)) % Two sides
    ...
    elseif length(indnoninf)==1 % Single side case
        if indnoninf==1 % Left linear
            interval(1) = xmin + 0.7*(xmax-xmin);
        else  % Right linear
            interval(2) = xmin + 0.3*(xmax-xmin);
        end
