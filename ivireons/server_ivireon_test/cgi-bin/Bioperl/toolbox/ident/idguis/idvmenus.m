...
%   $Revision: 1.11.4.4 $  $Date: 2009/10/16 04:56:17 $
...
elseif strcmp(arg,'options')
    ...
    elseif any(k==[5,6]) % This is transient response and residual plot
        if k==5  % Transient response
            ...
            h1=uimenu(hm,'Label',label,'callback',...
                [s1,'idopttog(''check'');',s3],'separator','on','tag','step');
            [label,acc]=menulabel('Im&pulse response');
            h2=uimenu(hm,'Label',label,'callback',...
                [s1,'idopttog(''check'');',s3],...
                'userdata',[h1 k 3 2],'tag','impulse');
            set(h1,'userdata',[h2 k  3 1]);
            if eval(usd(3))==1,
                chn=h1;
                stepon = 'on';
        ...
        if k==5,
            ...
            [label,acc]=menulabel('Time &span (time units)');
        else
        ...
        h1=uimenu(hm,'Label',label,'separator','on');
        if k == 5
            [label,acc] = menulabel('&default');
            hh(1) = uimenu(h1,'Label',label,'userdata',[k optno 0]);
            [label,acc]=menulabel('&1');
            hh(2)=uimenu(h1,'Label',label,'userdata',[k optno 1]);
        end
        [label,acc]=menulabel('&5');
        hh(3)=uimenu(h1,'Label',label,'userdata',[k optno 5]);
        [label,acc]=menulabel('&10');
        hh(4)=uimenu(h1,'Label',label,'userdata',[k optno 10]);
        [label,acc]=menulabel('&20');
        hh(5)=uimenu(h1,'Label',label,'userdata',[k optno 20]);
        [label,acc]=menulabel('&40');
        hh(6)=uimenu(h1,'Label',label,'userdata',[k optno 40]);
        [label,acc]=menulabel('1&00');
        hh(7)=uimenu(h1,'Label',label,'userdata',[k optno 100]);
        if k==6
            hh = hh(3:7);
        end
        ...
        if k==5
            ...
            stepsize = str2mat(opt(4,:),opt(5,:));
            str = ['(',stepsize(1,:),' -> ',stepsize(2,:),')'];
            [label,acc]=menulabel(['Step Size ',str]);
            h1 = uimenu(hm,'Label',label,'separator','on','tag','stepsize',...
                'enable',stepon);
            [label,acc] = menulabel('0 -> 1');
            uimenu(h1,'Label',label,'callback','iduisstep(''def'');iduiclpw(5,1);');
            [label,acc] = menulabel('&Other');
            uimenu(h1,'Label',label,'callback','iduisstep(''set'');iduiclpw(5,1);')
        end
    elseif k==3 % This is Model output
        ...
        set(chn,'checked','on')
        [label,acc]=menulabel('Set data span for &fit...');
