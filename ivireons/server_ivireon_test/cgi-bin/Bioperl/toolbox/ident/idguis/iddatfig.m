...
%   $Revision: 1.18.4.7.2.1 $ $Date: 2010/07/01 20:43:06 $
   ...
   if any(Figno==[1,14])   % This is the time plot case
      ...
      for k=datas
         ...
         if doplot
            ...
            if isempty(ky) || (isempty(ku)&&nu>0) || (nu==0 && ~isequal(kudes{1},0))
         ...
         if doplot
            ...
            xusd1(2*kn+1,1) = line(xval,yval,'color',color,'userdata',kstr);
            set(xax(1),'vis','on'),%vy=axis;
            set(xax(1),'userdata',xusd1); 
            set(xax(1),'xlimmode','auto'); set(xax(1),'ylimmode','auto');
            plha=xusd1(2*kn+1,1);
            if Figno==1,
            ...
            if ~isempty(ku)
               ...
               xusd2(2*kn+1,1)=line(xs,ys,'color',color,'userdata',kstr);
         ...
         if doplot
            ...
            if dom=='t'
               ...
               if ~isfft
                  ...
                  %(periodogram option for time domain data)
                  ggwy = (0:dl-1)/dl*2*pi/tSamp; kmax = floor(dl/2);
                  ggwy = ggwy(1:kmax+1)'; ggwu = ggwy;
                  yf = fft(data(:,ky),dl);
                  ggay = abs(yf(1:kmax+1)).^2*tSamp/dl;
                  if nu>0,
                     uf = fft(data(:,ny+ku),dl);
                     ggau = abs(uf(1:kmax+1)).^2*tSamp/dl;
                  end
               end  % if isfft
            else %dom=='f'
               ggay = abs(data(:,ky)).^2*tSamp;
               ggau = abs(data(:,ny+ku)).^2*tSamp;
            ...
            if k==-1
               [~,~,linh] = idnextw('data');color=get(linh,'color'); % color needed
            else
               color = get(klin,'color');
            end
            newbox1 = 0; newbox2 = 0;
            if k==0,
               ...
               if ~isempty(hb),if ishandle(hb(1)), newbox1 = 0; end, end
            ...
            xusd1(2*kn+1,1)=line(ggwy,ggay,'color',color,'userdata',kstr);
            if newbox1
               hb(1)=line('color','g','linestyle','--','vis','off');
            end
            ...
            if ~isempty(ku)
               ...
               xusd2(2*kn+1,1) = line(ggwu,ggau,'color',color,'userdata',kstr);
               if newbox2
                  hb(2) = line('color','g','linestyle','--','vis','off');
               end
               set(xax(2),'vis','on','userdata',xusd2)
               %haxe=[xax(1) xax(2)];
            ...
            if k>0,
               usd = [idnonzer(get(khax,'userdata')); handpl(:)];
               set(khax,'userdata',usd);
            end
   ...
   elseif Figno==40   % This is the Bode Case
      ...
      for k=datas
         ...
         if doplot
            ...
            % labels added by Rajiv (12/01/05)
            if hz
               xlabel('Frequency (Hz)')
            else
