function gtws=idparest(arg,onoff,varargin)
...
%   setpop  Sets the correct pop-up menu for the various  supported
%              model structures. (This depends on the inputs and outputs
%          in the estimation data.)

%   L. Ljung 4-4-94
%   Copyright 1986-2009 The MathWorks, Inc.
%   $Revision: 1.22.4.14 $ $Date: 2009/12/22 18:53:56 $
...
if strcmp(arg,'open')
    ...
    if ~idfigflag(FigName,0)
        iduistat('Opening the Linear Parametric Models dialog ...')
        if ~isfield(XID,'mse')
            XID.mse = [0 0 0 0];
        end
        ...
        XID.iter(2) = uicontrol(f,'pos',pos(6,:),'style','check','string',...
            'Display','callback','drawnow',...
            'tooltip','Check to give full details of iterations in Command Window.');
        ...
        uicontrol(f,'pos',pos(1,:),'style','frame');
        uicontrol(f,'pos',pos(8,:)+[0 0 8 0],'style','text',...
        ...
        XID.parest(18) = uicontrol(f,'pos',pos(3,:),'style','pop',...
            'backgroundcolor','white',...
            'string',{'Prediction','Simulation','Stability','Filter...'},'callback',...
            [s1,'idparest(''filter'');',s3],...
            'tooltip',['Prediction/Simulation: Optimize the fit for prediction or simulation.',...
            ' Stability: Guarantee stability. Filter: Choose a frequency range.']);
        uicontrol(f,'pos',pos(6,:)+[0 0 2 0],'style','text',...
        ...
        FigXY = max([sumbpos(1) sumbpos(2)-FigH-35],[0 50]); % -35 is for the header
    ...
    [flag,hnr]=idfigflag(idlaytab('figname',21),1);
    if flag
...
elseif strcmp(arg,'continue')
    ...
    mod = pvset(mod,'OptimMessenger',XID.OptimMessenger);
    
    displayVal = idGetDisplayVal(XID.parest(1),Xsum);
    try
        modc = pem(edat,mod,'Display',displayVal);
    catch E
...
elseif strcmp(arg,'mstype')
    ...
    if dom=='f'
        ...
        if Mtype==1 && Ts==0 && ny>1
            err = 1;
            newMtype = 5;
            text = 'continuous time, multi-output data.';
        end
    end
    if ny>1
    ...
    if err
        mse=0;%oldMtype=1;
        ll1 = iduiwok(21);
        if idIsValidHandle(ll1) && strcmpi(get(ll1,'vis'),'on')
            mse = 1;
        end
        if mse
    ...
    if (Mtype==1 && ny==1 && get(XID.arx(1,2),'value')) ||... %%%
        ...
        % Disable ARX Order Selection button for CT frequency domain data
        if (Mtype==1 && dom=='f' && Ts==0)
        ...
        set(XID.parest(5),'string','Iteration Options...',...  %%%
            'callback',['iduiopt(''dlg_ioopt'',''ioopt'',1,1:7);'],'tooltip',...
...
elseif strcmp(arg,'setfocus')
    if strcmpi(get(XID.parest(20),'style'),'text')
    ...
    oldstr = get(XID.parest(18),'string');

    val = strmatch(oldstr{val},str);
...
elseif strcmp(arg,'ordedit')
    ...
    mse=0;
    [fl,fi]=idfigflag(idlaytab('figname',21),1);
...
elseif strcmp(arg,'ord_sel')    
    ...
    Ts = pvget(edat,'Ts'); Ts = Ts{1};
    if Ts==0  
        % should only disable ARX order editor, not N4SID
        errordlg('Order selection is not supported for continuous-time frequency domain data.',...
            'Error Dialog','modal')
