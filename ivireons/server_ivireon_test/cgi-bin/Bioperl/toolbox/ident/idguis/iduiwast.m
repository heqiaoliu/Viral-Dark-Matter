...
%   $Revision: 1.9.4.10 $ $Date: 2008/12/04 22:33:57 $
...
if strcmp(arg,'show')
    ...
    if newfigure
        ...
        uicontrol(wb,'pos',[0,Figpos(4)-butwh(2)-bb,Figpos(3),butwh(2)],...
            'style','text','string','Icons can be dragged back to data/model boards.');
        ...
        for ka=1:4*no_col
            ...
            hl=line('vis','off','tag','modelline0');
        end
...
elseif strcmp(arg,'throw')
    ...
    if strcmp(tag1,get(get(XID.hw(3,1),'zlabel'),'userdata'))|...
        ...
        errordlg(sprintf(['The data you want to throw away is',...
    ...
    lineth0 = findobj(ax1,'type','line');
    if ~isempty(lineth0) && ishandle(lineth0)
        maybeModel = get(lineth0,'userdata');
        if isa(maybeModel,'idnlarx') || isa(maybeModel,'idnlhw')
            % broadcast model addition event for the benefit of nonlinear plots and estimation GUI
            if isa(maybeModel,'idnlarx')
                evtype = 'nlarxRemoved';
            else
                evtype = 'nlhwRemoved';
            end
            ...
            ed.Info = get(maybeModel,'Name');
            messenger.send('identguichange',ed);
            % todo (may be): disable checkbox if no model left?
        end
    end
    ...
    hnr=findobj(allchild(0),'flat','tag','sitb34','vis','on');
    if ~isempty(hnr)
        iduiwast('show',ax2);
    end
...
elseif strcmp(arg,'kill')
    ...
    [flag,fig]=idfigflag(idlaytab('figname',34));
