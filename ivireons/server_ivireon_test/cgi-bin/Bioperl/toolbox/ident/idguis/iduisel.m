...
%   $Revision: 1.23.4.9 $ $Date: 2008/12/29 02:07:50 $
...
if strcmp(flag,'open_portions')
    ...
    for kh=1:2
        axes(xax(kh));cla
        for kb=1:2
            hbox(kb,kh)=line('linestyle','--','color',col(kb),'vis','off');
        end
    end
    ...
    iduistat('Mark time span using mouse (draw rectangle) or keyboard.',0,14)
    set(Xsum,'UserData',XID);
...
elseif strcmp(flag,'revert')
    ...
    iduistat('Mark time span for new data set.',0,14);
...
elseif strcmp(flag,'detrend')
    [dat,dat_info,dat_no]=iduigetd('e');
    dat = detrend(dat,xdat);
    dat_no = pvget(dat,'Name');
    dat_n=[dat_no,'d'];
    dat = iduiinfo('add',dat,[' ',dat_n,' = detrend(',dat_no,',',int2str(xdat),')']);
