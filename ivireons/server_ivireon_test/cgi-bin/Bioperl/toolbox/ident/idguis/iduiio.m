...
%   $Revision: 1.25.4.12 $ $Date: 2008/05/19 23:03:46 $
...
if strcmp(arg,'open')
    ...
    [fl,fi]=idfigflag(FigName,1);oldpos=[50 200];
    if fl
    ...
    if ~idfigflag(FigName,0)
        ...
        kbas=4;
        XID.ss(1,7)=uicontrol(f,'pos',[pos(5+kbas,1),pos(6+kbas,2),pos(4+kbas,3:4)],...%pos(3+kbas,:)+[butw 0 butw 0],...
            'style','edit','vis','off','tooltip',...
            'Model horizon(s) (N4Horizon). Leave empty for default',...
            'backgroundcolor','w','string','');
        XID.ss(1,9)=uicontrol(f,'pos',[pos(4+kbas,1),pos(6+kbas,2),pos(4+kbas,3:4) ],...
            'style','text','vis','off','string',...
            'N4Horizon','Horizontalalignment','right');
        XID.ss(3,7)=uicontrol(f,'pos',[pos(3+kbas,1),pos(6+kbas,2),pos(5+kbas,3:4)],...
            'style','pop','vis','on','string',...
            'Auto|MOESP|CVA',...
            'backgroundcolor','white',...
            'Tooltip','The weighting used (N4Weight)');
        XID.ss(1,10) = uicontrol(f,'pos',[pos(6+kbas,1),pos(3+kbas,2:4)]+[0 0 5 0],...
            'style','text','string','N4SID options:');
        XID.ss(2,7)=uicontrol(f,'pos',pos(6+kbas,:),...
            'style','text','vis','off','string',...
            'N4Weight','Horizontalalignment','right');
        ...
        if nu>0
            ...
            XID.ss(1,8)=uicontrol(f,'pos',[pos(5+kbas,1),nkpos(2) pos(5+kbas,3:4)],...
            ...
            XID.ss(2,8)=uicontrol(f,'pos',[pos(4+kbas,1),nkpos(2) pos(4+kbas,3:4)],...%+[3*butw 0 0 0],...
                'style','text','vis','off','string',...
        ...
        set(XID.io(1,6),'style','popup',...
            ...
            'callback',s5);
        iduiadj(f);%set(get(f,'children'),'units','norm');set(get(f,'children'),'units','normal')
...
elseif strcmp(arg,'estimate') % Covers I/O and 'By name'
    ...
    if any(Mtype==[2 3 4 5])
        ...
        try
            % estimate
            displayVal = idGetDisplayVal(XID.parest(1),Xsum);
            %XID.OptimMessenger.Stop = false;
            LASTM = pem(eDat,[na nb nc nd nf nk],argin{:},...
                'OptimMessenger',XID.OptimMessenger,'Display',displayVal);
            warmess = lastwarn;
        catch E
            XID.BusyAction.Busy = false;
            errordlg(idlasterr(E),'Error','Modal');
    ...
    if Mtype==2&~usepem  %ARMAX
        if nu==0
            ordstr2 = [int2str(na),' ',int2str(nc)];
            %ordstr2 = ordstr;
        else
            %ordstr = [int2str(na(1)),int2str(nb(1)),int2str(nc),int2str(nk(1))];
    ...
    elseif Mtype==6  % Model by name
        ...
        if ~isa(nn,'idmodel')%nro<3 | nco<7
            errordlg(str2mat(['The entry in the Initial model field is ', ...
                'not a model of IDMODEL type.'],'',['Change the model',...
        ...
        try
            displayVal = idGetDisplayVal(XID.parest(1),Xsum);
            %XID.OptimMessenger.Stop = false;
            oldmod = pvset(oldmod,'OptimMessenger',XID.OptimMessenger);

            LASTM = pem(eDat,oldmod,argin{:},'Display',displayVal); %6
            warmess = lastwarn;
        catch E
            XID.BusyAction.Busy = false;
...
elseif strcmp(arg,'close')
    set(XID.ss(1,7),'string','')
    set(XID.ss(3,7),'value',1);
...
elseif strcmp(arg,'nkupdate')
    ...
    try
        ...
        errordlg('The Delay from u cannot be evaluated. Please check entry.','Error Dialog','modal')
    end
