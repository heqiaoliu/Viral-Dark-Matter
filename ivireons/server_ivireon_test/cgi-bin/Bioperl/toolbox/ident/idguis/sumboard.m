...
%   $Revision: 1.26.4.18 $  $Date: 2009/10/16 04:56:18 $
...
CBWH = [2.1*mStdButtonWidth mStdButtonHeight];
PUWH = [2.1*mStdButtonWidth CBWH(2)];

% Make axes have appealing "golden" ratio
AxisW = (CBWH(1)-Voff)/2;
AWH = AxisW*[1 2/(1+sqrt(5))];

% Define figure width and height
FigW = 3*CBWH(1) + PUWH(1) + Voff + 9*mFrameToText;
...
fig = figure('Unit','pix','Position',[10 scrsize(4)-FigH-60 FigW FigH], ...
...
[label,acc]=menulabel('System Identification Tool &GUI Help ^G');
uimenu(h,'Label',label,'Accel',acc,'callback',...
    'iduihelp(''idmw.hlp'',''Help: System Identification Tool'');')
[label,acc]=menulabel('S&ystem Identification Process ^Y');
uimenu(h,'Label',label,'Accel',acc,'callback',...
    'iduihelp(''sitbgui.hlp'',''Help: The Big Picture'');')
[label,acc]=menulabel(['System Identification',' Toolbox &Help ^H']);
uimenu(h,'Label',label,'Accel',acc,'callback','doc ident')

[label,acc]=menulabel('I&nteractive demo of the GUI');
uimenu(h,'Label',label,'Accel',acc,'separator','on','callback','iduidemo(0);')
[label,acc]=menulabel('Product &demos ^D');
...
[label,acc]=menulabel('&About System Identification Toolbox');
...
H1 = 2*mFrameToText;                % Left corner of Data Views checkbox
...
V5 = V4 + CBWH(2) + Voff;
V6 = V5 + CBWH(2) + Voff;
CheckBox(1) = uicontrol(fig,'Style','check','String','Time plot', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H1 V6 CBWH],...
    'Tooltip',['Plot data. Select sets by single',...
    ' clicks on the data icons.']);
CheckBox(2) = uicontrol(fig,'Style','check','String','Data spectra', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H1 V5 CBWH],...
    'Tooltip',['Plot data spectra. Select sets by single',...
    ' clicks on the data icons.']);
CheckBox(9) = uicontrol(fig,'Style','check','String','Frequency function', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
...
CheckBox(3) = uicontrol(fig,'Style','check','String','Model output', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H4 V6 CBWH],...
    'Tooltip',['Compare Model and Measured Output. Select models',...
    ' by clicking the model icons.']);
CheckBox(4) = uicontrol(fig,'Style','check','String','Model resids', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H4 V5 CBWH],...
...
gap = 25;
CheckBox(5) = uicontrol(fig,'Style','check','String','Transient resp', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H6-gap V6 CBWH],...
    'Tooltip',['Model impulse and step response. Select models',...
    ' by clicking the model icons.']);
CheckBox(6) = uicontrol(fig,'Style','check','String','Frequency resp', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H6-gap V5 CBWH],...
    'Tooltip',['Model Bode plots. Select models',...
    ' by clicking the model icons.']);
CheckBox(7) = uicontrol(fig,'Style','check','String','Zeros and poles', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H6-gap V4 CBWH],...
    'Tooltip',['Zeros and poles of the models. Select models',...
    ' by clicking the model icons.']);
CheckBox(8) = uicontrol(fig,'Style','check','String','Noise spectrum', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H6-gap V3 CBWH],...
    'Tooltip',['Spectrum of additive noise at outputs. Select models',...
    ' by clicking the model icons.']);

H8 = 2*(H6-gap)-H4; %for new column of nonlinear plot checkboxes
CBWH1 = CBWH;
CBWH1(1) = CBWH(1)+10;
CheckBox(10) = uicontrol(fig,'Style','check','String','Nonlinear ARX', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H8 V6 CBWH1],'tag','idnlarx',......
    'Tooltip',['Plot of nonlinear ARX models. Select models',...
    ' by clicking on the IDNLARX model icons.'],'enable','off',...
    'callback',@(es,ed)localNlarxPlot(es,ed));

CheckBox(11) = uicontrol(fig,'Style','check','String','Hamm-Wiener', ...
    'HorizontalAlignment','left','BackgroundColor',DefUIBgC, ...
    'Position',[H8 V5 CBWH1],'tag','idnlhw',...
    'Tooltip',['Plot of Hammerstein-Wiener models. Select models',...
    ' by clicking on the IDNLHW model icons.'],'enable','off',...
    'callback',@(es,ed)localNlhwPlot(es,ed));
...
Axis(26) = axes('Position',...
    [H4 V5-SpecialAWH(2)-H0 SpecialAWH],'tag','selva','color',axescolor,'parent',fig);
Text(4)=get(Axis(26),'xlabel');
set(Text(4),'string','Validation Data','color','k')

text('pos',[0.5 0],'units','norm','tag','name',...
    'verticalalignment','bottom');
line('color',textcolor,'vis','off','tag','selline');
XID.hw(4,1)=Axis(26);
...
if ~iscstbinstalled
    ...
    Axis(32) = axes('Position',pos32,'Color',DefUIBgC, ...
        ...
        'DefaultTextColor','k','tag','ltivi','parent',fig);
    text(0.5,0.5,{'To','LTI Viewer'});
    axlist = Axis(32);
end
Axis(27) = axes('Position',pos27,'Color',DefUIBgC, ...
    'XColor','k','YColor','k','Zcolor','k', ...
    'DefaultTextColor','k','tag','expor','parent',fig);
...
Axis(28) = axes('Position',[TrashLeft TrashBottom TrashWH], ...
    ...
    'zcolor','k',...
    'Xlim',[0 50],'Ylim',[0 100],'tag','waste','parent',fig);
...
for i=4:-1:1,
    Axis(6*(i-1)+1) = axes('Position',[H2 V2 AWH],'NextPlot','add',...
        'tag',['data ',int2str(kd)],'parent',fig);
    text('pos',[0.5 0],'units','norm','tag','name',...
        'verticalalignment','bottom');
    line('color',map(kd,:),'vis','off','tag','dataline0');
    kd=kd-1;
    ...
    for kpos=6:-1:3
        Axis(6*(i-1)+kpos) = axes('Position',[HPOS(kpos+1) V2 AWH],...
            'NextPlot','add','tag',['model',int2str(km)],'parent',fig);
        text('pos',[0.5 0],'units','norm','tag','name',...
            'verticalalignment','bottom');
        line('color',map(km,:),'vis','off','tag','modelline0');
        km=km-1;
    end
    V2 = V2 + AWH(2) + Voff;
end
...
Axis(30) = axes('position',apos,'vis','off','parent',fig);
a1 = iddrawarrow(Axis(30),-90);
apos = [H2-Voff/2-CBWH(2)/2 V2-Voff+H0 asize];
Axis(31) = axes('position',apos,'vis','off','parent',fig);
...
Popup(1) = uicontrol(fig,'Style','popup', ...
    ...
    'Callback',[s2,'XIDh=findobj(allchild(0),''''tag'''',''''sitbdatapop'''');',...
...
XID.pop = popusd;
Popup(3) = uicontrol(fig,'Style','popup','String',Pop3Str, ...
    'backgroundcolor','white',...
...
% {
Pop4Str = {' Estimate -->','Linear parametric models...','Process models...', ...
    'Nonlinear models...','Spectral models...','Correlation models...','Quick start'};
%}
Popup(4) = uicontrol(fig,'Style','popup','String',Pop4Str, ...
    'backgroundcolor','white',...
...
Axis(25) = axes('Position',[H3+(CBWH(1)-AWH(1))/2 VCDaxes AWH], ...
    'NextPlot','add','tag','seles','color',axescolor,'parent',fig);
text('pos',[0.5 0],'units','norm','tag','name',...
    'verticalalignment','bottom','color',textcolor);
line('vis','off','tag','selline');
...
set(XID.plotw(5,2),'callback',[s2,'iduipw(5);',s4],...
    'UserData',str2mat(int2str(pd(8,5)),num2str(pd(9,5)),int2str(pd(7,5)),int2str(0),int2str(1)));
...
set([Text; CheckBox; Axis; Popup;],'Unit','norm')
set(0,'Unit','Pixel')
XID.posd=[];XID.posm=[];
for ka=0:3
...
FigW=0.85*FigW;
FigH=0.85*FigH;
set(fig,'Position',[10 scrsize(4)-FigH-72 FigW FigH]);
if length(XID.layout)>15
    if XID.layout(16,3)
        try
            set(fig,'pos',XID.layout(16,1:4))
        end
    end
end
%fpos0 = get(fig,'pos');
iduiadj1(fig);
% pixel to char ratio may result in bad postition for figure
fpos = get(fig,'pos');
if ((fpos(1)+10)<scrsize(1)) || (fpos(1)+fpos(3)+10)>scrsize(3) || ...
        (fpos(2)+fpos(4)+72)>scrsize(4)
    % reset the position only so that GUI is placed relatively well w.r.t
    % the screen
    set(fig,'Position',[10, scrsize(4)-fpos(4)-72 fpos(3:4)]);
end

set(XID.sumb(1),'vis','on')
set(fig,'HandleVisibility','Callback');

% Store messenger object and install a listener for it
% This is to allow linear GUI windows to show iteration display information
XID.OptimMessenger = nlutilspack.optimmessenger;
XID.OptimListener = handle.listener(XID.OptimMessenger,...
    'optiminfo',{@idmodelOptimListener,fig});
XID.BusyAction = nlutilspack.busyactionindicator;
...
function iduiadj1(f)
ch = get(f,'children');
ch1=ch(strcmp(get(ch,'type'),'uicontrol'));
ch2=ch(strcmp(get(ch,'type'),'axes'));
ch=[ch1;ch2];
set(ch,'units','pix');
...
z = R*xy + 0.5;
ahan = patch(z(1,:),z(2,:),[0.5 0.5 0.5],'parent',a);

%------------------------------------------------------------------------
function localNlarxPlot(es,~)
% callback for idnlarx plot checkbox clicked

f = plotpack.getNlarxPlotGUIInstance;
if isempty(f) || ~ishandle(f)
    if get(es,'Value')==1
        msgbox('No active Nonlinear ARX models found in model board(s).',...
            'Nonlinear ARX Plot Warning','modal')
        set(es,'value',0);
    end
    return;
end
...
%------------------------------------------------------------------------
function localNlhwPlot(es,~)
% callback for idnlhw plot checkbox clicked

f = plotpack.getNlhwPlotGUIInstance;
if isempty(f) || ~ishandle(f)
    if get(es,'Value')==1
        msgbox('No active Hammerstein-Wiener models found in model board(s).',...
            'Hammerstein-Wiener Plot Warning','modal')
        set(es,'value',0);
    end
    return;
end
