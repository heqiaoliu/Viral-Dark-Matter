...
% $Revision: 1.1.8.3 $ $Date: 2007/12/14 14:45:59 $
...
xx = [];
if strcmpi(h.ImportSource,'file') % file and pathname supplied
    if ~isempty(h.filename)
        try
            xx = whos('-file',h.filename);
        catch E
            errordlg(idlasterr(E),'Invalid file','modal')
            xx = [];
        end
    end
else
...
for k=1:length(xx)
    if (isempty(h.typesallowed) || any(strcmpi(xx(k).class,h.typesallowed))) && length(xx(k).size)<=2 && ...
        ...
        entry = VariableIcon.getIcon(xx(k).name, xx(k).class);
...
for k=1:length(strucarray)
    members{k} = strucarray(k).(fieldname);
end
