function Out = idaxlimdlg(message,OptionFlags,PromptString,AxesHandles,XYZstring,DefLim,ApplyFcn)
%IDAXLIMDLG Axes limits dialog box.
%   FIG = IDAXLIMDLG(DlgName,OptionFlags,PromptString, ...
...
%   idaxlimdlg(DlgName,OptionFlags,PromptString,AxesHandles,XYZstring,DefLim)
%
%   Notes:
...
%   $Revision: 1.1.10.5 $  $Date: 2008/10/02 18:50:14 $
...
switch lower(message)
    case 'editcallback'
        ...
        elseif any(~isfinite(lim)) || ~isreal(lim)
            error_str = 'Axes limits must be real and finite numbers.';
        end
        if isempty(error_str),
            ...
            errordlg(error_str);
            set(Edit,'String',mat2str(get(Edit,'UserData'),6))
        end
        return
    ...
    case 'applycallback'
        ...
        for i=1:size(ud,1),
            ...
            if isempty(ax) || ~all(BooVec),
                error_str = str2mat('IDAXLIMDLG: Empty or invalid axes handles.', ...
            ...
            if IsAutoChecked,
                ...
                set(ud(i,2),'String',mat2str(lim,6))
            else
...
DlgName = message;
% Error checking
% idaxlimdlg(message,OptionFlags,PromptString,AxesHandles,XYZstring,DefLim,ApplyFcn)
if ni<2,
    OptionFlags = [0 0];
elseif size(OptionFlags,2)~=2,
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidOptionFlags')
end
if ni<3,
...
elseif isempty(PromptString) || ~ischar(PromptString),
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidStingMatrix')
end
...
elseif isempty(AxesHandles) || ischar(AxesHandles) || (size(AxesHandles,1)~=1),
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidAxesHandlesInput')
elseif sum(isnan(AxesHandles))~=rows-1,
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidInput')
end
if ni<5,
    XYZstring = ['x'; 'y';'z'];
    XYZstring = XYZstring(rem((1:rows)-1,3)+1,:);
elseif any(size(XYZstring)~=[rows 1]),
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidXYZstringInput')
end
if ni<6,
    DefLim = [];
...
elseif any(size(DefLim)~=[rows 2]),
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidDefLimMatrix')
end
if ni<7,
    ApplyFcn = '';
elseif (size(ApplyFcn,1)~=1) || ~ischar(ApplyFcn),
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidApplyFcnString')
end
OptionFlagsRows = size(OptionFlags,1);
if OptionFlagsRows==1,
    OptionFlags = OptionFlags(ones(1,rows),:);
elseif OptionFlagsRows~=rows,
    ctrlMsgUtils.error('Ident:idguis:axlimInvalidOptionFlagsInput')
end

% Get idlayoutscript parameters
idlayoutscript
...
FigPos = [0 0 FigWH];
FigPos    = idgetnicedialoglocation(FigPos, get(0,'DefaultFigureUnits'));
...
for i=1:rows,
    ...
    ud(i,2) = uicontrol(fig,'Style','edit','String',EditStr,'BackgroundColor','white', ...
        ...
        'UserData',DefLim(i,:),'Callback','idaxlimdlg(''EditCallback'')','HandleVisibility','callback');
    UIPos = UIPos -[0 Voff 0 0];
end

% Make the pushbuttons
Hspace = (FigWH(1)-2*BWH(1))/3;
ApplyFcn = [ApplyFcn 'idaxlimdlg(''ApplyCallback'');'];
