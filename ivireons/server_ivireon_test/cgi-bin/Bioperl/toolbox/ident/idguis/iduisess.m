...
%   $Revision: 1.15.4.6.2.1 $ $Date: 2010/07/01 20:43:08 $
...
elseif strcmp(arg,'load')
   Model = []; Data = [];% To avoid possible confusion with an MATLAB file/model if such exists
   gcm=gcbo;
   ...
   if ischar(file_name)
      err=0;
      eval(['load(''' fullfile(path_name,file_name) ''',''-mat'')'],'err=1;')
      if err
         errordlg(['The file ',fullfile(path_name,file_name),' cannot be found' ...
      ...
      cursbs=findobj(allchild(0),'tag','sitb30');%Current summary boards
      ...
      if ~isempty(nono)
         ...
         for kk=lbn:-1:1
            if newlist(kk)~=Board.nr(kk)
               ...
               if Board.nr(kk)~=1
                  ...
                  for ki=indxm'
                     [~,knrim] = min(sum(abs((XID.posm-ones(16,1)*modp(ki,2:5))')));
                     modp(ki,2:5)=XID.posm1(knrim,:);
                  end
                  for ki=indxd'
                     [~,knrid] = min(sum(abs((XID.posd-ones(8,1)*datp(ki,2:5))')));
         ...
         set(XID.sumb(1),'name',['System Identification Tool - ',sessname])
      ...
      for kbcou=1:length(Board.nr)
         ...
         if kbn>1
            create=0;
            try
               if strcmp(get(XID.sumb(kbn),'tag'),'sitb30')&...
   ...
   if newsess,
      ...
      kma = strmatch(file_name,XID.sessions,'exact');
      if ~isempty(kma)
         id = false(size(XID.sessions,1),1);
         id(kma(strmatch(path_name,XID.sessions(kma+1,:),'exact'))) = true;
         id(find(id)+1) = true;
         XID.sessions(id,:)=[];
      end
...
elseif strcmp(arg,'save')
   ...
   sumb=[XID.sumb(1);findobj(allchild(0),'flat','tag','sitb30')];
   Model = {};
   Data = {};
   km = 0; kd = 0;kb = 0;
   
   Board.nr = [];
   % Aug 18, 2008; sessions saved before R2009a will use sidversion=1,
   % unless explicitly overridden in the file. In R2008b, idutils.ver
   % returns 3, but sidversion is set to a fixed value of 1.
   sidversion = idutils.ver;
   for ksb=sumb(:)'
      kb = kb+1;
...
elseif strcmp(arg,'save_as')
   ...
   set(XID.sumb(1),'name',['System Identification Tool - ',sessname])
   extras=findobj(allchild(0),'flat','tag','sitb30');
   for ksb=extras(:)'
      kwinname=get(ksb,'name');
      kpar1=find(kwinname=='(');kpar2=find(kwinname==')');
      set(ksb,'name',['System Identification Tool - ',sessname,' ',kwinname(kpar1:kpar2)])
   end
   
   kma = strmatch(filen,XID.sessions,'exact');
   id = [];
   if ~isempty(kma)
      id = strmatch(pathn,XID.sessions(kma+1,:),'exact');
   end
   if isempty(id)
