...
%   $Revision: 1.15.4.10 $  $Date: 2007/12/14 14:45:27 $
...
if strcmpi(arg,'open')
    ...
    if ~idfigflag(figname,0)
        iduistat('Opening Spectral Analysis dialog box ...')
        idlayoutscript
        FigW = iduilay2(3);
...
elseif strcmpi(arg,'estimate')
    ...
    try
        ...
        try
            ...
            try
                if strcmpi(ress(1:3),'def')
                    res = [];
                elseif strcmpi(ress(1:3),'max')
                    res = 'max';
                end
            catch
