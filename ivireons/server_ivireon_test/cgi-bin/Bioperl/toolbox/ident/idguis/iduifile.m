...
%   $Revision: 1.29.4.21 $  $Date: 2009/03/09 19:14:12 $
...
elseif strcmp(arg,'insert')
    ...
    if strcmp(datatype(1:4),'data') % then we must first create the object
        ...
        if stype=='fd'|stype=='fr' % pick frequencies
            ...
            if isempty(fren)|strcmp(deblank(fren),'[]')
                errordlg(['For frequency functions and frequency domain data,'...
        ...
        if stype=='fr'
            ...
            fdimu = size(size(u)); fdimu = fdimu(2);
            if ap,fdimy = size(size(y)); fdimy = fdimy(2);else fdimy=2;end

            if ~any(fdimu==[2 3])|~any(fdimy==[2 3])
            ...
            if ap
                ert = 0;
                try
                    ert = ~all(size(u)==size(y));
                catch
                    ert =1;
                end
                if ert%~all(size(u)==size(y))
        ...
        if stype == 'td'|stype == 'fd'
            fdimu=size(size(u));fdimy=size(size(y));
            if fdimu(2)~=2|fdimy(2)~=2
                errordlg('The input and output should the vectors or 2D matrices','Error','modal');
                return
            end
    ...
    if isa(data,'iddata') && isempty(data.Ts)
        errordlg('Data with non-uniform sampling cannot be imported. The property "Ts" of the imported data object must be non-empty.','Data Import','modal')
        return
    end
    ...
    if dom=='t'
        if iduifile('test1',str2,'startt');return,end
        data = pvset(data,'Tstart',evalin('base',str2));
    end
    ...
    if get(XID.hload(1,6),'userdata')
        data=pvset(data,'Ts',evalin('base',str1));
    end
    ...
    if N==0
        errordlg(['Empty data set',...
            ' (after frequencies above the Nyquist frequency have been removed). Check sampling interval.'],...
            'Error Dialog','modal')
...
elseif strcmp(arg,'insert_model')
    ...
    if ~isa(mod,'idmodel') && ~isa(mod,'idnlarx') &&~isa(mod,'idnlgrey')&& ~isa(mod,'idnlhw') %grey box not allowed yet
        if isa(mod,'lti')
            mod = idss(mod);
    ...
    if isa(mod,'idnlarx') || isa(mod,'idnlhw')
        if ~nlutilspack.isValidModelName(pvget(mod,'Name'),class(mod))
            msg = sprintf('A model of type %s with name ''%s'' already exists in the model board. \nChoose a unique name for model being imported.',class(mod),pvget(mod,'Name'));
            errordlg(msg,'Non-unique Model Name','modal');
            return;
        end
    end
