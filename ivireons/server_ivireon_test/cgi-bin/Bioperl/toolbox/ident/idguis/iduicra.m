...
%   $Revision: 1.20.4.12 $ $Date: 2009/10/16 04:56:12 $
...
elseif strcmp(arg,'tspan')
    ...
    Tsp = eval(deblank(OPt(2,:)));
    if Tsp == 0, Tsp = 40*TSamp;end
    Tspan = ['[',num2str(-5*TSamp,3),' ',num2str(Tsp,3),']'];
...
elseif strcmp(arg,'estimate')
    ...
    if nu==0
        errordlg(['Correlation Analysis cannot be applied to',...
            ' time series to estimate the impulse response.'],'Impulse Response','modal');
    else
        NA=eval(usd(1,:));
        if isempty(NA) || strcmpi(NA(1),'d')
            NA = 10;
        end
        ...
        try
            ...
            if strcmp(E.identifier,'Ident:general:CTData')
                errordlg('Correlation model cannot be computed for continuous-time frequency domain data.',...
                    'Error Dialog','modal');
            else
        ...
        if length(pvget(IR,'B'))<2
            warndlg('Input not sufficiently exciting to estimate impulse response',...
