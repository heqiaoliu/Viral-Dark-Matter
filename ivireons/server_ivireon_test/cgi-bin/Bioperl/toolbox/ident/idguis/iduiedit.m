...
%   $Revision: 1.22.4.10 $ $Date: 2008/10/31 06:12:09 $
...
elseif strcmp(arg,'close_all')
    ...
    if isempty(XID.laypath)
        ...
        if strcmpi(click,'yes')
            ...
            if strcmpi(click,'continue')
                midprefs(1,1);idlaytab('save_file');
            end
        elseif strcmpi(click,'cancel')
            return
        end
    end
...
if strcmp(arg,'pres')
    ...
    set(h(6),'string',cols,...
        'userdata',[hand1]);
    if isa(dat,'idnlarx') || isa(dat,'idnlhw')
        istr = display(dat);
        Ind = findstr(istr,char(10));
        Ind = [0,Ind];
        infostr = cell(length(Ind));
        for ki = 1:length(Ind)-1
            infostr{ki} = istr(Ind(ki)+1:Ind(ki+1));
        end
        infostr{length(Ind)} = istr(Ind(end)+1:end);
    elseif ~isaimp(dat)%isa(dat,'iddata')|isa(dat,'idpoly')
        if isa(dat,'idproc')
            dashfactor = XID.dash;
    ...
    elseif isa(dat,'idarx') && isaimp(dat) % Impulse response case
        ny = size(dat,'ny');
        nu = size(dat,'nu');
...
elseif strcmp(arg,'update_name')
    ...
    % handle nonlinear models, which must have unique names
    nonlinmodelnamechange = false;
    if isa(dat,'idnlarx') || isa(dat,'idnlhw')
        if ~nlutilspack.isValidModelName(newnam,class(dat))
            errordlg(sprintf('An %s model must have a unique and valid MATLAB variable name.',upper(class(dat))),...
                'Model Name Change','modal');
            set(h(5),'string',oldnam);
            return
        else
            nonlinmodelnamechange = true;
        end
    end
    ...
    set(objhand,'userdata',[]); %r.s.: avoid AbortSet issue
    set(objhand,'userdata',dat);
    if nonlinmodelnamechange
    ...
    if ismodel
        ...
        if any(ishandle(plots))
            fittext=findobj(plots(ishandle(plots)),'flat','tag','fits');
            if ~isempty(fittext)
...
elseif strcmp(arg,'update_color');
    ...
    Model = get(obj,'userdata');
    % update color in nonlinear plots
    if isa(Model,'idnlarx') || isa(Model,'idnlhw')
        % broadcast name change event
        if isa(Model,'idnlarx')
            evtype = 'nlarxColorChanged';
        else
            evtype = 'nlhwColorChanged';
        end
        messenger = nlutilspack.getMessengerInstance('OldSITBGUI');
        ed = nlutilspack.idguievent(messenger,evtype);
        ed.NewValue = col;
...
%--------------------------------------------------------------------------
function deleteOptimObj(XID)
% delete optim messenger and listener

delete(XID.OptimListener)
XID.OptimListener = [];
delete(XID.OptimMessenger)
delete(XID.BusyAction)
