...
%   $Revision: 1.20.4.6 $ $Date: 2009/07/09 20:52:12 $
...
elseif strcmp(flag,'filter')
    if logical(get(XID.filt(4,1),'value')-1), BP = 0; else BP = 1; end
    [dat,~,dat_n] = iduigetd('e',1,false);
    if isa(dat,'iddata')
        ...
        % idfrd data
        freqs = eval(['[',get(XID.filt(3,1),'string'),']'],'[]');
        if ~isa(freqs,'double') || length(freqs)~=2
            errordlg('Frequency range must be specified using a 2 element double vector.','Error Dialog','modal');
            return
        end
        freqs = sort(freqs);
        if BP
            zf = fselect(dat,freqs(1),freqs(2));
            filtstr = sprintf('fselect(%s,%f,%f);',dat_n,freqs(1),freqs(2));
        else
            freqv = dat.Frequency;
            zf = fselect(dat, find(freqv<freqs(1) | freqv>freqs(2)));
            filtstr = sprintf('fselect(%s, find(%s.Frequency<%f | %s.Frequency>%f));',dat_n,dat_n,freqs(1),dat_n,freqs(2));
        end
    end
    
    set(XID.filt(1,1),'userdata',zf);
    set(XID.filt(5,1),'userdata',filtstr);
    
    %{
    % filter specification from estimation windows now only allow a range
    % specification (see iduifoc).
    try
    ...
    %}
    if isa(dat,'iddata') && dom=='t'
        % filter option is available explicitly only for time domain data
        iddatfig(-1,15);
        XID = get(Xsum,'UserData');
        callcase = get(XID.filt(7,1),'enable');
        if strcmpi(callcase,'on')
            iduistat('Filtered data spectra shown. Press Insert to accept.',0,15)
        else
            iduistat('Filtered spectra shown. This filter will be used for Estimation Focus.',0,15)
        end
    end
