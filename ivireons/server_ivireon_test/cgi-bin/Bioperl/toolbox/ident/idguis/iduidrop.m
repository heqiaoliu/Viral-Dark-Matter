...
%   $Revision: 1.29.4.12 $ $Date: 2008/12/29 02:07:48 $
...
elseif strcmp(tag2(1:5),'modst')% This ia a drop on a model structure
    ...
    if strcmp(tag1(1:5),'data ')|nodrop
        errordlg('You can only drop a linear model here.','Error Dialog','modal');
        return
    elseif strcmp(tag1(1:5),'model')
        ...
        if ~isa(mod,'idmodel')%if strcmp(type,'spa')|strcmp(type,'cra')
            msg = sprintf('%s\n\n%s\n',...
                'You cannot drop a nonlinear, spectral or a correlation model here.',...
                'For nonlinear models, choose the Nonlinear Models... estimation option',...
                'and use the ''Initial model'' popup in the resulting window.');
            errordlg(msg,'Invalid Initial Model','modal');
            return
        end
        if isa(mod,'idproc')
            idprocest('open');
...
elseif strcmp(tag2(1:5),'ltivi')
    ...
    if ismodel
        ...
        if isa(dat,'idnlhw') || isa(dat,'idnlarx')
            %dat = getLinearModel(dat);
            %iduistat(['Launching LTI Viewer for linear part of model ',dat.Name]);
            %elseif isa(dat,'idnlarx')
            iduistat(['Launching LTI Viewer for linear approximation of model ',dat.Name,'.']);
            was = warning('off'); [lw,lwid] = lastwarn;
            dat = linapp(dat,iduigetd('e'));
            warning(was), lastwarn(lw,lwid)
        elseif ~isa(dat,'idmodel')&&~isa(dat,'idfrd')
            iduistat('Cannot use LTI Viewer for CRA or IDNLGREY models.'),
            return
        ...
        elseif isa(dat,'idfrd') && (nu == 0)
            ...
            return
        elseif isa(dat,'idmodel')
            dat = pvset(dat,'OptimMessenger',[]);
        end
    ...
    if ~iscstbinstalled
        iduistat('No LTI Viewer available. This tool requires Control System Toolbox.')
        return
    end
    ...
    if isempty(kk)
        ...
        set(fig,'Name','System Identification Tool: LTI Viewer','Tag','sitb37');
...
elseif strcmp(tag2(1:5),'expor')
    ...
    nr = findstr(' ',dat_n);
    if ~isempty(nr), dat_n(nr)='';end

    dat= iduiinfo('add',dat,[' Export   ',dat_n]);


    statusex =[];
    try
        %evalin('base',[dat_n,';']);
        wvarnames = evalin('base','who');
        if any(strcmp(wvarnames,dat_n))
            exvar = 1;
        else
            exvar = 0;
        end
    ...
    if isempty(statusex)
        try
            ...
            errordlg(['The name ',dat_n,' is not allowed as a Workspace variable name.'...
                ' Rename the variable (double-click on the icon) before you export it.'],'Error for Export','modal');
            statusex = ' not';
        end
    end
...
elseif strcmp(tag2,'merge')
    ...
    if par1~=par2
        ...
        usd = get(hl1,'userdata');
        if isa(usd,'idnlarx') || isa(usd,'idnlhw')
            %check uniqueness of name
            Model_name = nlutilspack.generateUniqueModelName(class(usd),get(usd,'Name'));
            if ~strcmp(Model_name,get(usd,'name'))
                set(usd,'Name',Model_name);
                set(hstr2,'string',Model_name);
                msgbox('Model name changed to ensure uniqueness of names in model board.',...
                    'Nonlinear Model Name Change','warn','replace')
            end
                      
        end
        
        % set axes colors depending upon model or data type
        Plotcolors=idlayout('plotcol');
        if isa(usd,'idnlarx')
            set(ax2,'color',Plotcolors(9,:));
        elseif isa(usd,'idnlhw')
            set(ax2,'color',Plotcolors(10,:));
        elseif isa(usd,'idfrd')
            set(ax2,'color',Plotcolors(8,:));
        elseif isa(usd,'iddata') && strcmpi(usd.Domain,'frequency')
            set(ax2,'color',Plotcolors(7,:));
        end
        
        set(hl2,'xdata',get(hl1,'xdata'),'ydata',get(hl1,'ydata'),...
            'color',get(hl1,'color'),'userdata',usd,...
            'vis',get(hl1,'vis'),...
        ...
        if isa(Model,'idnlarx') || isa(Model,'idnlhw')
            % broadcast model addition event for the benefit of nonlinear
            % plots and estimation GUI
            ...
            ed.Info = struct('Model',Model,'isActive',get(hl2,'linewidth')>0.5,...
                'Color',get(hl1,'color'));
