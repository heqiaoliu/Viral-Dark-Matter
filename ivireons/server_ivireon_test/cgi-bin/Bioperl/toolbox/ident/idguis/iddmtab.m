...
%   $Revision: 1.9.4.11 $  $Date: 2008/10/31 06:11:59 $
...
fig = figure('Unit','pix','Position',[number*10 number*10 FigW FigH], ...
    ...
    'CloseRequestFcn',@localWarnAndExecute,...
...
for i=4:-1:1,
    Axis(6*(i-1)+1) = axes('Parent',fig,'Position',[H2 V2 AWH],'NextPlot','add',...
        'tag',['data ',int2str(kd)]);kd=kd-1;
    text('Parent',Axis(6*(i-1)+1),'pos',[0.5 0],'units','norm','tag','name',...
        'verticalalignment','bottom');
    line('Parent',Axis(6*(i-1)+1),'color',map(rem(kd,20)+1,:),'vis','off',...
        'tag','dataline0');
    Axis(6*(i-1)+2) = axes('Parent',fig,'Position',[H1 V2 AWH],'NextPlot','add',...
        'tag',['data ',int2str(kd)]);kd=kd-1;
    text('Parent',Axis(6*(i-1)+2),'pos',[0.5 0],'units','norm','tag','name',...
        'verticalalignment','bottom');
    line('Parent',Axis(6*(i-1)+2),'color',map(rem(kd,20)+1,:),'vis','off',...
        'tag','dataline0');
    for kpos=3:6
        Axis(6*(i-1)+kpos) = axes('Parent',fig,'Position',[HPOS(kpos+1) V2 AWH],...
            'NextPlot','add','tag',['model',int2str(km)]);km=km-1;
        text('Parent',Axis(6*(i-1)+kpos),'pos',[0.5 0],'units','norm','tag','name',...
            'verticalalignment','bottom');
        line('Parent',Axis(6*(i-1)+kpos),'color',map(rem(km,20)+1,:),'vis','off',...
            'tag','modelline0');
    end
    V2 = V2 + AWH(2) + Voff;
end
...
%--------------------------------------------------------------------------
function localWarnAndExecute(es,varargin)
Name = get(es,'Name');
ButtonName = questdlg(sprintf('Do you really want to close summary board ''%s''?',Name),...
    'System Identification Tool','Cancel');

if strcmpi(ButtonName,'Yes')
    localCloseThisBoard([],[],es)
end

%--------------------------------------------------------------------------
function localCloseThisBoard(es,ed,h,varargin)
