...
%   $Revision: 1.14.4.8 $  $Date: 2010/03/26 17:24:15 $
...
if isempty(posfig)
    ...
    posXY=max(pos1(3:4)-posWH-(number-1)*[40 40]-[0,50],[5 5]);
    posfig=[posXY posWH];
end
...
if any(number == [1 3 13])
    ...
    %if length(Exp)>1 
    iduiexp('set',number); % Always show experiment menu (r.s., March 2010)
    %end
end
...
if any(number==[1 2 6 13,40]) % Two axes plots
    ...
    xax(2,1)=axes('parent',hw,'position',pos(2,:),'box','on',...
...
elseif any(number==[3,9,10])   % Single plots with buttons
    ...
    set(get(xax(1,1),'ylabel'),'fontsize',fz,'color',ftcol,'fontweight',fw);
    if number==3
        %        xtab=axes('parent',hw,'position',[0.72 0.15 0.27 0.75],'box','on',...
        xtab=axes('parent',hw,'position',[pos(1,1)+pos(1,3)+0.025 pos(1,2) 1-pos(1,3)-pos(1,1)-0.05 pos(1,4)],'box','on',...
    ...
    pos = idlayout('axes',number);
    xax=axes('parent',hw,'position',pos,'box','on',...
        'drawmode','fast','tag','axis1','color',AxesColor,...
        'xcolor',fticol,'ycolor',fticol,'fontsize',fz);
...
idlayoutscript
pos=[0.9*mStdButtonWidth,2*mStdButtonHeight];
uicontrol(hw,'pos',[0 0 pos],'vis','off','style','text',...
    'max',2,'tag','infobox','HorizontalAlignment','left',...
...
iduimbcb('setzoom',number,hw);
iduiaxes('grid',1,hgrid,hw);
iduital(number);
set(hw,'vis','on');
