...
% $Revision: 1.22.4.14 $ $Date: 2009/10/16 04:56:10 $
...
if strcmp(arg,'open')
    ...
    if (strcmpi(usd.dom,'f') && usd.ts==0)
        errordlg('Estimation of Process Models using continuous-time frequency-domain data is not supported. Set sampling interval of data to be greater than zero.',...
            'Error Dialog','modal')
        return
    elseif nu==0 || ny>1
    ...
    if ~idfigflag(FigName,0)
        iduistat('Opening the Process Model dialog ...')

        idlayoutscript
        %butw = mStdButtonWidth;
        wadj=20; %Adjusting the width to fit largest formula
        FigW = iduilay2(5)+wadj;
        ...
        XID.iter(15) = uicontrol(f,'pos',pos(3,:),'style','text','string','Fit:',...
        ...
        XID.iter(18) = uicontrol(f,'pos',pos(5,:),'style','check','string','Display',...
            'tooltip','Check to view full details of iterations in Command Window.');
        ...
        uicontrol(f,'pos',pos(11,:),'style','push','string','Options...',...
            'callback','iduiopt(''dlg_ioopt'',''ioopt'',1,1:7,1);',...
            'tooltip','Change options for the iterative estimation algorithm.');

        %LEVEL 4 RIGHT The table
        lev4 = pos(1,2)+pos(1,4);
        posf = iduilay1((FigW-wadj)*3/5,50,11,lev4);%just for the frame
        posf2 = iduilay1((FigW-wadj)*2/5+wadj,55,11,lev4);
        uicontrol(f,'pos',posf(1,:)+[(FigW-wadj)*2/5+wadj 0 0 0],'style','frame');
        uicontrol(f,'pos',posf2(1,:),'style','frame');

        %Level 4.a Initial model and initial guess

        posr = iduilay1((FigW-wadj)*3/5,55,11,lev4,[],[0.35 0.35 0.65 0.65 0.7]);
        posr(:,1) = posr(:,1)+(FigW-wadj)*2/5+wadj;
        ...
        uicontrol(f,'pos',posr(4,:),'style','text','string','Value',...
            'tooltip',ttval);
        uicontrol(f,'pos',posr(5,:)+[0 0 2 0],'style','text','string','Initial Guess',...
            'tooltip',ttvali);
        ...
        XID.procest(6,1) = uicontrol(f,'pos',pos(8,:)+[-10 1+addh sum(pos(2:4,3))-pos(8,3)+20+wadj -7],'style','text',...
            'string','G(s)=');
        XID.procest(6,2) = uicontrol(f,'pos',pos(8,:)+[-10 -12+addh sum(pos(2:4,3))-pos(8,3)+20+wadj -7],'style','text',...
            'string','G(s)=');
        XID.procest(6,3) = uicontrol(f,'pos',pos(8,:)+[-10 -25+addh sum(pos(2:4,3))-pos(8,3)+20+wadj -7],'style','text',...
            'string','G(s)=');
        ...
        FigXY=max([sumbpos(1) sumbpos(2)-FigH-35],[0 50]); %-35 is header
...
elseif strcmp(arg,'struc')
    ...
    nop = evalin('base',type(2));
    nopp = nop; %actual number of different poles. To be used
    % when double and tripel are implemented
    if any(call==['a','p'])
...
elseif strcmp(arg,'noise') %adjust the focus popup
    if (~strcmp(get(XID.procest(2,8),'style'),'text')) && get(XID.procest(2,8),'value')>1
        ...
        tt1 = ['Prediction/Simulation: Optimize the fit for prediction or simulation.',...
...
elseif strcmp(arg,'estimate')
    ...
    % Search Method
    ssear = deblank(usd(7,:)); searind=evalin('base',ssear);
    if ~isempty(searind) && (searind~=1)
        SearchFields = {'Auto','gn','gna','lm','lsqnonlin','grad'};
        argin = [argin,{'SearchMethod'},{SearchFields{searind}}];
        text =[text,',''SearchMethod'',''',SearchFields{searind},''''];
    end
    
    % Criterion
    scrit = deblank(usd(6,:)); crit=evalin('base',scrit);
    if ~isempty(crit) && (crit~=1)
        argin =[argin,{'Criterion'},{'Trace'}];
        text =[text,',''Criterion'',''Trace'''];
    end
    
    % Weighting
    sweight = deblank(usd(5,:)); wtg=evalin('base',sweight);
    if ~isempty(wtg) && (crit~=1)
        argin =[argin,{'Weighting'},{wtg}];
        text =[text,',''Weighting'',',sweight];
    end
    
    % Limit Error
    slim=deblank(usd(1,:));
    lim = evalin('base',slim);
    if ~isempty(lim)
        argin = [argin,{'Lim'},{lim}];
        text =[text,',''Lim'',',slim];
    end
    ...
    % Tolerance
    stol=deblank(usd(2,:));tol=evalin('base',stol);
    if ~isempty(tol)
        argin=[argin,{'Tol'},{tol}];
        text = [text,',''Tol'',',stol];
    end
    ...
    % MaxIter
    smaxiter=deblank(usd(3,:));maxiter=evalin('base',smaxiter);
    if ~isempty(maxiter)
    ...
    % FixedParameter
    sindex=deblank(usd(4,:));index=evalin('base',['[',sindex,']']);
    if ~isempty(index)
    ...
    if estim
        if XID.BusyAction.Busy
            % do not start iterative estimation if one is already going on
            return;
        else
            XID.BusyAction.Busy = true;
        end
        try
            ...
            % store optimization messenger
            m0 = pvset(m0,'OptimMessenger',XID.OptimMessenger);

            displayVal = idGetDisplayVal(XID.procest(1),Xsum);

            % estimate
            LASTM = pem(eDat,m0,argin{:},'Display',displayVal);
            warmess = lastwarn;
            ...
            XID.BusyAction.Busy = false;
            errordlg(idlasterr(E),'Error  Using PEM','Modal')
            %disp(lasterr)
            return
        end
        XID.BusyAction.Busy = false;
...
elseif strcmp(arg,'parobj')
    ...
    tg = get(llo,'tag');
    if length(tg)==6
        pnr = evalin('base',tg(6));
        tg = tg(1:5);
    end
    if length(tg)==7
        pnr = evalin('base',tg(7));
        tg = tg(1:6);
    end
    if strcmp(tg,'status')% known-checkbox
        v = get(llo,'value');
        if v
            res = 'fixed';
            ...
            if isempty(deblank(get(XID.procest(3,pnr),'string')))
                ...
                val = evalin('base',get(XID.procest(3,pnr),'string'));
            end
            % check bounds
    ...
    elseif strcmp(tg,'value')
        if isempty(get(llo,'string')) || strcmpi(get(llo,'string'),'auto')
            res = NaN;
        else
            set(XID.procest(8,[1 3]),'value',0)% The radio buttons
            set(XID.procest(8,2),'value',1)
            try
                res = evalin('base',get(llo,'string'));
            catch
                res =[];
            end
            if isempty(res) || ~isnumeric(res)
                errordlg('Invalid value entered for the parameter.','Error Dialog','Modal')
                set(llo,'string','')
                return
            end
        end
        ...
        %%LL At this point one should check against bounds
        if size(usd,1)==2
            if ~under
                usd(1,ku) = res;
                usd(2,ku) = 0;
    ...
    elseif strcmp(tg,'bounds')
        try
            ...
            errordlg('Invalid value for bounds','Error Dialog','modal')
            usd2gui(XID.procest);
            return
        end
        if size(res,2)~=2
...
% sets all the GUI features right according the stored type.
...
try
    ...
    errordlg(idlasterr(E),'Error Dialog','Modal');
end
...
if val>pmax
    val = pmax;
    wrnm = 'Value exceeds upper bound.';
end
if val<pmin
    val = pmin;
    wrnm = 'Value below lower bound.';
end