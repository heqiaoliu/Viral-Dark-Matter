...
%   $Revision: 1.20.4.6 $ $Date: 2006/12/27 20:53:07 $
...
if strcmp(tag2(1:5),'selva')
    ...
    if dom=='f'&nu==0
        ...
        if ~(isempty(lla)&isempty(llb))
            ...
            % this should really not happen if there are no models
    ...
    % fire identGUIChange event ---------------------------------
    messenger = nlutilspack.getMessengerInstance('OldSITBGUI');
    ed = nlutilspack.idguievent(messenger,'vDataChanged');
    ed.NewValue = data; %only new data
    ...
    % fire identGUIChange event ---------------------------------
    messenger = nlutilspack.getMessengerInstance('OldSITBGUI');
    ed = nlutilspack.idguievent(messenger,'eDataChanged');
    ed.NewValue = data;
    messenger.send('identguichange',ed);
    %-------------------------------------------------------------
end
