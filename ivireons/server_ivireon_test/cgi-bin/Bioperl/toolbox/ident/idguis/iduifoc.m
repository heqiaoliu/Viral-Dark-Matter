...
%   $Revision: 1.2.4.8 $ $Date: 2009/07/09 20:52:13 $
...
if strcmp(arg,'open')
    ...
    if ~idfigflag(figname,0)
        ...
        uicontrol(f,'pos',pos(4,:),'style','push','string','Help','callback',...
            'iduihelp(''idfoc.hlp'',''Help: Estimation Focus'');')
...
elseif strcmp(arg,'range')
    ...
    if ~(isa(rge,'double')||isa(rge,'lti')||isa(rge,'idmodel'))
        errordlg('The variable you entered  is neither a matrix nor a linear model.',...
            'Error Dialog','modal');
        return
    end
    if isa(rge,'double')
