...
%   $Revision: 1.18.4.10 $ $Date: 2008/12/29 02:07:49 $
...
elseif strcmp(arg,'estimate')
    ...
    if popv==6, iduicra('open');%iduinpar('cra');%eval('iduicra(''open'');')
    elseif popv==5, eval('iduispa(''open'');')
    elseif popv==2, eval('idparest(''open'');')
    elseif popv==7, eval('iduiqs;')
    elseif popv==3, idprocest('open');
    elseif popv==4
        Znlr = iduigetd('e');
        if ~isa(Znlr,'iddata') || ~strcmpi(get(Znlr,'Domain'),'time')
            errordlg('Nonlinear model estimation requires time domain data.',...
                'Invalid Estimation Data','modal');
        elseif size(Znlr,3)<1
            errordlg(['Nonlinear model estimation using time series data is not supported in GUI.',...
                ' Use IDNLARX/NLARX in MATLAB Command Window to create such models.'],...
                'Unsupported Estimation Data','modal');
        else
            wb = waitbar(0.5,'Opening nonlinear models estimation window...');
            nlutilspack.getNLBBGUIInstance(true,true);
            if idIsValidHandle(wb), waitbar(1,wb); end
