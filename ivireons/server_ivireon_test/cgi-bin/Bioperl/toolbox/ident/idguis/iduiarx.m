...
%   $Revision: 1.30.4.8 $ $Date: 2009/10/16 04:56:11 $
...
elseif strcmp(arg,'estimate')
    ...
    if nargin==1
        ...
        if any(sl1==':')
            ...
            NN=idarxstr(sl1);
            if isempty(NN)
                errordlg(sprintf(['Orders cannot be evaluated.',...
            ...
            vDatc=vDat(:,kys,kus); %funkar ej för idfrd, inte heller forts
            [N,nye,nue] = size(eDat);
            ...
            try
                ...
                errordlg({'Order selection failed.',MExcep.message},'Order Selection Error','modal')
                return
            end
            if time>60,
        ...
        if strcmp(method,'arx')
            ...
            if ny==1&nu<2
                ...
                mod_info=str2mat(eDat_info,pretext,nastr,nbstr,nkstr,...
                    [' ',model,' = arx(',eDat_n,',[na,nb,nk]',comma,...
                    text1,')']);
            end
        else
            if nu>0
...
elseif strcmp(arg,'insert')
    ...
    if err
        errordlg(sprintf(['Cannot find the Parametric Models Window.',...
...
elseif strcmp(arg,'close_NG')
    ...
    delete(findobj(allchild(0),'name','FAKE'));
end
