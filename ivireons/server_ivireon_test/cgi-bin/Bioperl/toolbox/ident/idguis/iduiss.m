...
%   $Revision: 1.29.4.15 $ $Date: 2009/04/21 03:22:45 $
...
elseif strcmp(arg,'ordedit')
    ...
    if length(nn)>1 && ~isequal(get(XID.ss(1,2),'value'),0)
    ...
    try
        set(XID.ss(1,8),'string',int2str(nk));
    end
...
elseif strcmp(arg,'estimate')
    ...
    if strcmp(method,'pem')
         if ~isposintscalar(nn)
            errordlg('Specify a positive integer for model order.',...
                'Invalid model order','modal')
            return
        end
        ...
        % Search Method
        ssear = deblank(usd(7,:)); searind=eval(ssear);
        ...
        % Criterion
        scrit = deblank(usd(6,:)); crit=eval(scrit);
        ...
        % Weighting
        sweight = deblank(usd(5,:)); wtg=eval(sweight);
        ...
        % LimitError
        slim = deblank(usd(1,:));%deblank(usd(1,:));
        lim = eval(slim);
        if ~isempty(lim)
            argin = [argin,{'Lim'},{lim}];
            text =[text,',''Lim'',',slim];
        ...
        % Tolerance
        stol = deblank(usd(2,:)); %deblank(usd(2,:));
        tol=eval(stol);
        if ~isempty(tol)
            argin=[argin,{'Tol'},{tol}];
            text = [text,',''Tol'',',stol];
        ...
        try
            ...
            LASTM = pem(eDat,nn,argin{:},'OptimMessenger',XID.OptimMessenger);
            warmess = lastwarn;
            set(XID.iter(7),'userdata',LASTM,'string','Continue iter','callback',...
                'idparest(''continue'');')
        catch E
            XID.BusyAction.Busy = false;
            errordlg(idlasterr(E),'Estimation Error','Modal')
        ...
        if ~(isvector(nn) && isposintmat(nn))
            errordlg({'Specify a positive integer for model order.',...
                'Specify a vector of positive integers for trying various model orders.'},...
                'Invalid model order','modal')
            return
        end
        if length(nn)>1
        ...
        if Ncap<(ceil(max(nn)/ny+1)+ceil((max(nn)-ny+1)/(nu+ny)))*(1+nu+ny)%1.5*ny+2.5*nu)*(max(nn)+2)+ny
            errordlg({'There are too few data points for this choice of orders.',...
                'You need at least twice as many data samples as the model order.'},'Error Dialog','modal');
            iduistat('No model estimated.');
            return
        end
        %}
        
        if (2*ny+nu)*max(nn)>200 && isequal(get(XID.parest(19),'value'),1)
            kor = questdlg({['With the chosen order(s), the calculation of covariance information',...
                ' may take some time'],'Do you want to continue?',...
                '','Push YES to carry out the calculation',...
        ...
        % The commented code below will update the order editor with the
        % latest optional variables.
        %         try
        %             set(XID.ss(1,7),'string',['[',int2str(LASTM.es.N4Horizon),']']);
        %             n4w = LASTM.es.N4Weight;
        %             if strcmp(n4w,'CVA')
        %                 n4val = 3;
        %             else
        %                 n4val = 2;
        %             end
        %             set(XID.ss(3,7),'value',n4val);
        %         end
        if isempty(LASTM)
...
elseif strcmp(arg,'specsel')
    ...
    idparest('mse');
    XID = get(Xsum,'Userdata');
    n4h = LASTM.es.N4Horizon;
    if ischar(n4h)
        n4h = '';
    else
        n4h = mat2str(n4h);
    end
    set(XID.ss(1,7),'string',n4h);
    n4w = LASTM.es.N4Weight;
    if strcmp(n4w,'CVA')
        n4val = 3;
    elseif strncmpi(n4w,'Not',3)
        n4val = 1; % set to "auto" if "not applicable" such as for freq domain data
    else
        n4val = 2; % MOESP
    end
    set(XID.ss(3,7),'value',n4val);
    ...
    model=get(XID.parest(7),'string');
    if isempty(findstr(text,'N4H')) && ~isempty(n4h)
        text = [text,',''N4Horizon'',',n4h];
    end
...
if any(sl1==':') && strcmp(method,'pem')
    errordlg(str2mat('PEM does not allow several orders.',...
        'Change the selected order or change Method to N4SID.'),'Error Dialog','modal');
    iduistat('Error.');
    err = 1;
    return
end
...
if ~isempty(auxord),
    if min(auxord)<max(nn+1)
        if length(auxord)>1
            auxord = auxord(auxord>nn);
        else
            auxord = max(nn+1);
        end
        errordlg(sprintf(['N4Horizon must be larger than ORDER.',...
...
try
    if ~isempty(auxord)
        set(XID.ss(1,7),'string',int2str(auxord));
    end
...
if foc >1
    ...
    if strcmp(foc,'si')
        ...
        text = [text,',''Focus'',''Sim'''];
    elseif strcmp(foc,'st') % Stability
        argin = [argin,{'Focus'},{'Stability'}];
        text = [text,'''Focus'',''Stab'''];
    elseif strcmp(foc,'fi')
...
if isempty(starnr)
    ...
    pnr2 = find(nnstr==']');
    if isempty(pnr2)
        errordlg('The input delay must be enclosed in square brackets: [nk].','Error Dialog','modal')
        return
    end
