...
%   to System Identification Toolbox ("System Identification Tool").
...
%      choices will be used at the next ident session. Using DEFAULT
%      in this options menu reverts back to the defaults defined in
%      the current MATLAB file.

%   L. Ljung 4-4-94
%   Copyright 1986-2010 The MathWorks, Inc.
%   $Revision: 1.9.4.9 $ $Date: 2010/03/22 03:48:52 $
...
elseif strcmp(swt,'plotcol')
    ...
    if get(0,'screendepth')==1
        ...
        fdcolor = AxesColor;
        idfrdcolor = AxesColor;
        nlarxcolor = AxesColor;
        nlhwcolor = AxesColor;
    elseif strcmp(plotmode,'white')
        ...
        fdcolor =[0.8 1 1]; %Data in the frequency Domain
        idfrdcolor = [1 1 0.7]; % IDFRD data
        nlarxcolor = [0.87 0.89 0.96]; %idnlarx model's axes color
        nlhwcolor  = [0.976 0.922 0.945]; %idnlhw model's axes color
    else
        % For Color Screens
        col=[0.2 0.2 0.2];
        ftcol=[1 1 1]; % white
        ...
        nlarxcolor = [0 0.13 0.1]; %idnlarx model's axes color
        nlhwcolor  = [0.12 0 0.1]; %idnlhw model's axes color
    end
    map=[col;ftcol;fticol;AxesColor;AxesFrame;TextColor;fdcolor;idfrdcolor;nlarxcolor;nlhwcolor];
...
elseif strcmp(swt,'figdefs')
    ...
    % ERASMODE-DEFAULT
    map(1) = NaN;  % 1 means  'normal' and 0 means 'xor' (not in use anymore)
...
elseif strcmp(swt,'plotdefs')
    ...
    map(5) = 1;  % 1 means comparison of simulated and measured output
    % 2 means comparison of predicted and measured output

    map(6) = 0;  % 0 means that measured and simulated/predicted curves
    % are shown together,
    % 1 means that their difference is plotted
    ...
    map(9) = 0; % Number of samples for which the response is
    % calculated
