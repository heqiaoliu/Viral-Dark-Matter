...
%DFEVALASYNC  Asynchronous Distributed FEVAL
%   Job = DFEVALASYNC(F,numArgOut, x1,...,xn,pv1,...,pvn) is equivalent to 
%   DFEVAL except this function returns immediately with a single output 
%   argument containing the job object that has been sent to the cluster.
%   The numArgOut input specifies the number of output arguments that F
%   generates.
%
%   Results associated with the job may be obtained by executing the
%   command:
...
%   $Revision: 1.1.8.8 $  $Date: 2010/05/10 17:03:00 $

% Check arguments, but don't use standard nargchk/nargoutchk so users don't
% get confused between input/output arguments to dfevalasync and nargin/nargout
% required by their function handle.  Use the detailed message option for
% checkNumberOfArguments.
parallel.internal.cluster.checkNumberOfArguments('output', 1, 1, nargout, mfilename, true);
parallel.internal.cluster.checkNumberOfArguments('input', 3, inf, nargin, mfilename, true);
