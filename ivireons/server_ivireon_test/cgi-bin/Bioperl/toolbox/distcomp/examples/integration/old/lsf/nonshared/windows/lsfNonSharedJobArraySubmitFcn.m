function lsfNonSharedJobArraySubmitFcn(scheduler, job, props, ...
                                        clusterHost, remoteDataLocation)
%LSFNONSHAREDJOBARRAYSUBMITFCN Submits a MATLAB job to a LSF scheduler in the
% absence of a shared file system between the MATLAB client and the
% LSF scheduler.
%
% See also lsfNonSharedJobArrayDecodeFcn.
...
decodeFcn = 'lsfNonSharedJobArrayDecodeFcn';
...
% Each TaskLocation begins with a string prefix followed by a unique 
% number. Obtain the prefix from the first task location.
firstTaskLocation = props.TaskLocations{1};
taskLocationPrefix = firstTaskLocation(1:end-1);
...
localScript = createLSFSubmitScript(commandToRun, decodeFcn, ...
    ...
    jobLocation, taskLocationPrefix, numberOfTasks, quote);
...
function filename = createLSFSubmitScript(commandToRun, decodeFunction, ...
        ...
        taskLocationPrefix, numberOfTasks, quote)
...
logFileLocation = [quote remoteDataLocation '/' taskLocationPrefix ...
                   '%%I.log' quote];
