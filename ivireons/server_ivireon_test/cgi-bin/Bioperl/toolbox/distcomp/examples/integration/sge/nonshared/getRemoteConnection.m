function remoteConnection = getRemoteConnection(scheduler, clusterHost, remoteDataLocation)
...
if ~ischar(clusterHost)
    error('distcompexamples:SGE:IncorrectArguments', ...
...
if isempty(scheduler.UserData)
    ...
    if ~isstruct(scheduler.UserData)
        error('distcompexamples:SGE:IncorrectUserData', ...
    ...
    if isfield(scheduler.UserData, 'RemoteConnection')
        ...
        if isempty(remoteConnection)
            ...
            elseif ~(strcmpi(remoteConnection.Hostname, clusterHost) && ...
                    ...
                    strcmpi(remoteConnection.DataLocation, remoteDataLocation))
                % The connection stored in the user data does not match the cluster host 
                % and remote location requested
                warning('distcompexamples:SGE:DifferentRemoteParameters', ...
                    ['The current scheduler is already using cluster host %s and remote data location %s.\n', ...
                    'The existing connection to %s will be terminated.'], ...
                    remoteConnection.Hostname, remoteConnection.DataLocation, remoteConnection.Hostname);
...
remoteConnection.connect(clusterHost, remoteDataLocation);
...
if isempty(usernameResponse)
    % User hit cancel
    error('distcompexamples:SGE:UserCancelledOperation', 'User cancelled operation.');
end
