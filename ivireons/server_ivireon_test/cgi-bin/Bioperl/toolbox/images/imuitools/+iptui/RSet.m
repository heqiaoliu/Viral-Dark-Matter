            ...
            for r = rows
                for c = cols
                    ...
                    if (numel(rows) < numel(cols))
                        cdata = horzcat(cdata, tmpData);
                    else
                        cdata = vertcat(cdata, tmpData);
                    end
                end
            end
...
if (fid < 0)
    error('images:RSet:fileNotFound', ...
          'Could not open R-Set file "%s" for reading: %s.', ...
          rsetFilename, msg);
end
fclose(fid);
...
if (~tf)
    error('images:RSet:notRSetFile', ...
          'File is not in the R-Set format.');
end

% Is the R-Set compatible?
if (~supported)
    error('images:RSet:unsupportedRSetVersion', ...
          'This .rset file is too new.\nOnly backward-compatibility versions of %0.2f or earlier are supported.', ...
          iptui.RSet.maxSupportedRSetVersion());
end
