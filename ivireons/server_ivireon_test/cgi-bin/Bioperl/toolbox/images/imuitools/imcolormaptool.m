...
%   $Revision: 1.1.6.5 $  $Date: 2010/05/13 17:36:43 $
...
if ~isempty(strfind(selectedStr,'<custom>'))
    selectedStr = '';
else
    selectedStr = strrep(selectedStr, ' (original)', '');
...
lastCustom = [];
    ...
    function refreshFcn(varargin)
        ...
        if isempty(cdata) || ndims(cdata) ~= 2
            deleteFcn();
        end
    ...
    function changeFcn(varargin)
        ...
        for i = 1:length(cmapStr)
            ...
            isExpressionString = ~isempty(strfind(curString,'<expression>'));
            if ~isExpressionString
                
                isCustomString = ~isempty(strfind(curString,'<custom>'));
                if ~isCustomString
                    
                    if isequal(feval(strrep(curString, ' (original)', ''),256),cmap)
                        isFound = true;
                        fcnInd = i;
                    end
                    ...
                    if isempty(strfind(curString,'original'))
        ...
        if isFound
            ...
            set(hObj,'String',sprintf('%s(%d)',strrep(cmapStr{fcnInd}, ' (original)', ''),256))
        else
            if isempty(customInd)
                % Add a non-original <custom> to list here
                set(hList,'String',['<custom>'; cmapStr(:)]);
                set(hList,'Value',1)
                set(hObj,'String','')
            else
    ...
    function showPanel(src,evt) %#ok<INUSL>
        ...
        if strcmp(get(selectedPanel,'Tag'),'cmapVarPanel')
            ...
            hVarList = findobj(hCMapFig,'Tag','cmapVarList');
            workspaceVariables = evalin('base','whos');
            varInd = filterWorkspaceVars(workspaceVariables,'colormap');
            ...
            if ~isempty(get(hFcnList,'Value'))
                ...
                varExist = strmatch(prevVar, varList, 'exact');
                if varExist
                    % If previously selected workspace variable still
                    % exists in the workspace, select that item in the
                    % listbox
                    set(hVarList,'String',varList);
                    set(hVarList, 'Value', varExist);
                else
                    if isempty(strfind(prevVar,'<cleared>'))
                        ...
                        % to the listbox.
                        varList = [['<cleared> ' prevVar] varList];
                        set(hVarList,'String',varList);
    ...
    function hPanel = createCMapFcnPanel
        ...
        for i = 1:length(cmapStr)
            if isequal(cmapStr{i,2}(256),get(hClientFig,'Colormap'))
                cmapStr{i,1} = [cmapStr{i,1} ' (original)'];
                isFound = true;
                break;
            end
        end
        ...
        if isFound
            ...
            set(hList,'String',['<custom> (original)'; cmapStr(:,1)]);
        end
    ...
    function hPanel = createCMapVarPanel
        ...
        uimenu(hContextMenu,'Label','Refresh',...
    ...
    function createButtonPanel
        ...
        button_strs_n_tags = {'OK', 'okButton';...
            'Cancel','cancelButton'; 'Help', 'helpButton'};
    ...
    function status = getColormap
        ...
        elseif isCallerListbox
            ...
            if isFunctionList
                ...
                if ~isempty(strfind(selectedStr,'<custom>'))
                    if ~isempty(strfind(selectedStr,' (original)'))
                ...
                elseif ~isempty(strfind(selectedStr,'<expression>'))
                    selectedStr = '';
        ...
        if ~isdeployed
            ...
            if isempty(selectedStr) || ~isempty(strfind(selectedStr,'<cleared>'))
                return;
            end
            ...
            if strcmp(panelTag,'cmapFcnPanel')
                if isCallerEditbox
                    ...
                    for i = 1:length(cmapStr)
                        ...
                        if ~isExpressionString
                            ...
                            isCustomString = ~isempty(strfind(curString,'<custom>'));
                            if ~isCustomString
                                if isequal(feval(strrep(curString, ' (original)', ''),256),cmapOut)
                                    isFound = true;
                                    break;
                                end
                            end
                    ...
                    if ~isFound
                        selectedInd = strmatch('<expression>',cmapStr(:));
                        if isempty(selectedInd)
                            % If <expression> item does not exist in the
                            % listbox, create one.
                            set(hList,'String',['<expression>'; cmapStr]);
                            selectedInd = 1;
                        end
                ...
                if isCallerEditbox
                    selectedInd = strmatch('<expression>',cmapFcnStr(:));
                    if isempty(selectedInd)
                        % If <expression> item does not exist in the
                        % listbox, create one.
                        set(hFcnList,'String',['<expression>'; cmapFcnStr]);
                        selectedInd = 1;
                    end
                    lastExpression = cmapOut;
    ...
    function stat = setFigColormap(hFig,map)
        ...
        try
            ...
            if strcmp(ME.identifier,'Images:setFigColormap:badMapValues')
                error_str = 'Colormap cannot have values outside the range [0,1]';
            elseif strcmp(ME.identifier,'Images:setFigColormap:badMapMatrix')
                error_str = 'Colormap must be nonempty, double, 2-D matrices with 3 columns';
            else
...
for i = 1:length(imHandle)
    % verify image is 2D
    if ndims(get(imHandle(i),'CData')) == 2
        hImage = imHandle(i);
        break
    end
end
if isempty(hImage)
    eid = sprintf('Images:%s:noValidImage',mfilename);
    msg = sprintf(['%s expects a current figure containing a ' ...
        'grayscale or indexed image.'],upper(mfilename));
    error(eid,'%s',msg);
end
