...
%   $Revision: 1.1.8.16 $  $Date: 2008/12/22 23:47:21 $
...
if (nargout==1)
    ...
    function initializePanel
        ...
        if ndims(get(targetImage, 'CData')) == 2
        ...
        linkImageCData = linkprop([targetImage hOverviewIm],'CData');
        setappdata(hOverviewIm,'linkImageCDataListener',linkImageCData);
    ...
    function new_pos = constrainDetail(pos)
        ...
        xdata = get(hOverviewIm,'XData');
        ydata = get(hOverviewIm,'YData');
        
        % compute pixel span in each direction
        dx = getDeltaOnePixel(xdata,imW);
        dy = getDeltaOnePixel(xdata,imH);
