...
%   $Revision: 1.1.10.30 $  $Date: 2010/05/13 17:36:49 $
 ...
 suppressPlotTools(hFig);
  ...
  if (nargout > 0)
   ...
   function addImageToImtool(varargin)
     ...
     try
       ...
       if any(strncmp(classCdata, {'int8','uint32','int32'}, ...
           ...
           error(eid, '%s cannot display an %s image.\nPlease see %s', ...
                upper(mfilename), classCdata, ...
                'the help for allowed datatypes.');
       end
     ...
     if ~isempty(rset)
           ...
           if isrset(common_args.Filename)
               ...
               try
                   ...
                   if out_of_mem_large_tif
                         dlg_message = sprintf('This image is too large to fit into memory. You can:\n\n * Create a reduced resolution dataset (R-Set) viewable in IMTOOL\n * Browse for an existing R-Set\n * Cancel\n ');
                         rset_button = questdlg(dlg_message,...
                                        'Image too large for memory','Create...','Browse...','Cancel','Create...');
                       if isempty(rset_button) || strcmpi(rset_button,'Cancel')
                           % User cancelled or hit "X" button when asked to
                           % create RSet.
                           rethrow(ME)
                       elseif strcmpi(rset_button,'Browse...')
           ...
           function [cdata,xdata,ydata,map] = yesButtonPressed(ME)
               ...
               while repeat_dialog_display
                   ...
                   try
                       ...
                       % non-writable directory.
                       h_dlg = errordlg('Cannot save or create this file. Make sure the disk you want to save the file on is not full or write-protected.',...
                           'Cannot write R-Set','modal');
                       uiwait(h_dlg);
                   end
           ...
           function [cdata,xdata,ydata,map] = browseButtonPressed(ME)
               ...
               while repeat_dialog_display
                   ...
                   if is_valid_rset
                       ...
                       h_dlg = errordlg('Invalid R-Set file selected.',...
                           'Invalid R-Set','modal');
                       uiwait(h_dlg);
                   end
       ...
       function refreshImageModel(obj,evt) %#ok<INUSD>
           ...
           % reset figure name
           imtool_name = sprintf('Image Tool %d',tool_number);
           set(hFig,'Name',imtool_name);
    ...
    function switchModes = reactToModeChange
        ...
        if cropRectOverImage
            ...
            buttonName = questdlg('Do you want to complete the cropping operation?',...
                'Crop Image?', 'Yes', 'No', 'Cancel', 'Yes');
   ...
   function toolbar = createMenusAndToolbar
     ...
     % Tools menu 
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     toolsMenu = uimenu(hFig, 'Label','&Tools','Tag','tools menu');
     ...
     t.properties.TooltipString   = 'Navigate image using overview';
     ...
     t.properties.TooltipString   = 'Inspect pixel values';
     ...
     t.properties.TooltipString   = 'Display image information';
     ...
     t.properties.TooltipString   = 'Adjust contrast';
     ...
     t.properties.TooltipString   = 'Help';
     ...
     t.properties.TooltipString   = 'Crop Image';
     ...
     t.properties.TooltipString   = 'Measure distance';
     ...
     t.properties.Parent          = toolbar;
     t.properties.TooltipString   = 'Adjust contrast/brightness via mouse motion';
