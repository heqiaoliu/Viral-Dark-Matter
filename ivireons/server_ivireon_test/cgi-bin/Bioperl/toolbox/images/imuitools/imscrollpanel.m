...
%   $Revision: 1.1.8.10 $  $Date: 2010/05/13 17:36:48 $
...
% Must use these Action events to get continuous events fired as slider
...
setappdata(hScrollpanel,'API',api);
    ...
    function replaceImage(varargin)
        ...
        if initialMagSpecified && preserveViewValue
            ...
            error(eid,'%s%s','Cannot specify both an ',...
                '''InitialMagnification'' and ''PreserveView''.');
        end
    ...
    function validatePreserveViewProperty(preserve_view)
        ...
        if  ~islogical(preserve_view) && ~isnumeric(preserve_view)
            eid = sprintf('Images:%s:preserveViewInvalidValue',mfilename);
            error(eid,['Value for ''preserveView'' property must '...
                'logical or numeric.']);
        end
...
isXDataDefault = isequal(get(hIm,'XData'),defaultXData);
isYDataDefault = isequal(get(hIm,'YData'),defaultYData);
