...
%   $Revision: 1.1.6.11 $ $Date: 2008/12/22 23:47:17 $
...
function [h_group,draw_api] = imfreehandAPI(varargin)
  ...
  % All other tools except imfreehand accept an initial position.
  % roiParseInputs is permissive of a position input, so must check for
  % presence of a position argument in imfreehand.
  if nargin >1    
      position_matrix_specified = isnumeric(varargin{2});
      xy_position_vectors_specified = (nargin > 2) && ...
          isnumeric(varargin{2}) && ...
          isnumeric(varargin{3});
      
      initial_position_specified = position_matrix_specified ||...
          xy_position_vectors_specified;
      
      if initial_position_specified
          error(sprintf('Images:%s:invalidSyntax',mfilename),...
                'Invalid syntax. IMFREEHAND does not accept an initial position.');
      end
  end
    
  [commonArgs,specificArgs] = roiParseInputs(0,1,varargin,mfilename,{'Closed'});
  ...
  try
    ...
    error(sprintf('Images:%s:invalidHandle',mfilename), ...
