...
%   $Revision: 1.1.6.27 $ $Date: 2010/05/13 17:36:47 $
    ...
    methods
        ...
        function setPosition(obj,pos)
            ...
            if invalidPosition
                ...
                error(error_id,...
                    'Invalid position specified. Position has the form [XMIN YMIN WIDTH HEIGHT].');
            end
...
if xy_position_vectors_specified || invalid_position_matrix;
    error(sprintf('Images:%s:invalidPosition',mfilename),...
        'Position must be specified in the form [x y width height].');
end
...
try
    ...
    error('Images:imrect:failureToParent', ...
...
[start_vert,start_x,start_y,dragFcn,drag_motion_callback_id,...
 ...
 connected_vertices,is_vertical_side,is_shift_click,start_position,h_hit] = deal([]);
...
% Note: The Following API syntaxes have been grandfathered.  Each
% grandfathered syntax is aliased to a current syntax to support clients
% calling grandfathered API functions.
...
%-----------------------------------
    function buttonDownEventFcn(hObj,ed) %#ok later argument ed is needed
        ...
        % Cache the CurrentPoint field of the event data of the
...
%-----------------------------------
    ...
    function startDrag(src, varargin) %#ok varargin needed by HG caller
        ...
        if interactive_placement
            ...
            % placement, define start_x,start_y in terms of the initial button
            % down position. Want tool to grow from where buttonDown occurred at
            % the beginning of interactive placement. Fixes g405610.
            start_x = position(1);
            start_y = position(2);
        ...
        % cache drag function to enable all types of drags to share same stopDrag
        % function for ButtonUp.
        dragFcn = [];
        
        % store callback ids at function scope for drag callbacks to be unwired by stopDrag.
        drag_motion_callback_id = [];
        drag_up_callback_id = [];
        
        % setupCornerResize and setupSizeResize need to setup other variables at
        % startDrag function scope to be used in startDrag and stopDrag. These
        % variables are shown as output variables of each respective setup
        % function.
        if draw_api.isBody(h_hit)
            
            setupTranslateRect();
            
        elseif draw_api.isWing(h_hit)
            
            setupTranslateRect();
            
        elseif draw_api.isCorner(h_hit)
            
            [connected_vertices_x,connected_vertices_y,selected_vertex] = ...
             setupCornerResize();
            
        elseif draw_api.isSide(h_hit)
            
            [connected_vertices,is_vertical_side] = setupSideResize();
            
        else
            % Shouldn't reach this point in the code. If we have, re-enable
            % pointer manager to prevent cursor from getting stuck.
        ...
        % check. This is a fix for g413211.
        if buttonUp
            stopDrag();
        end
    ...
    %---------------------------------------------------------------
    function [connected_vertices,is_vertical_side] = setupSideResize
    ...
    %----------------------------------------------------------------------
    function [connected_vertices_x,connected_vertices_y, selected_vertex] = setupCornerResize
