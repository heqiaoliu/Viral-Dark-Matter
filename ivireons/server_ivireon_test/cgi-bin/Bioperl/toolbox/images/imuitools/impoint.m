...
%   $Revision: 1.1.6.23 $ $Date: 2009/07/06 20:35:32 $
    ...
    methods
        ...
        function setPosition(obj,varargin)
            ...
            if length(varargin) == 1
                ...
                if invalidPosition
                    error(error_id,'Invalid position specified. Position has the form [X Y].');
                end
            elseif length(varargin) == 2
                ...
                if isInvalidCoord(x) || isInvalidCoord(y)
                    error(error_id,'Invalid position specified. X and Y must be numeric scalars.');
                end
                pos = [x,y];
            end
...
try
    ...
    error('Images:impoint:failureToParent', ...
