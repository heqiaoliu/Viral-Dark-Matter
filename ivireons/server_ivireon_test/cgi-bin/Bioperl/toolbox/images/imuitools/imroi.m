classdef imroi < handle
    ...
    %   $Revision: 1.1.6.13 $ $Date: 2009/12/02 06:43:31 $
    ...
    methods (Access = 'protected')
        ...
        function [obj,h_im] = parseInputsForCreateMask(varargin)
            ...
            if nargin ==1
                ...
                if numel(h_im) > 1
                    eid = sprintf('Images:%s:createMask:mustSpecifyImage',mfilename);
                    msg = ['Abiguous syntax. Associated axes contains more than one image.'...
                        ' Specify image handle as input argument to resolve ambiguity.'...
            ...
            if isempty(h_im)
                eid = sprintf('Images:%s:createMask:noImage',mfilename);
                msg = 'createMask expects a current figure containing an image.';
                error(eid,'%s',msg);
            end
            
            iptcheckhandle(h_im,{'image'},'createMask','h_im',2)
