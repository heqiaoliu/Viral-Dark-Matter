...
%   $Revision: 1.1.6.7 $  $Date: 2006/10/04 22:38:31 $
...
iptchecknargin(3,6,nargin,mfilename);
...
% P
P = varargin{2};
iptcheckinput(P, {'double'}, {'real', 'vector', 'nonsparse','integer',...
...
if nargin > 3
  CONN = varargin{4};
  iptcheckinput(CONN, {'double'}, {}, mfilename, 'CONN', 4);
  if (CONN~=4 && CONN~=8)
...
string = iptcheckstrs(FSTEP, validStrings, mfilename, 'FSTEP', 3);
for i=1:CONN
...
if nargin > 4
  N = varargin{5};
  iptcheckinput(N, {'double'}, {'real','nonnan','nonnegative','nonzero'}, ...
                mfilename, 'N', 5);
  if ~isinf(N)
    iptcheckinput(N, {'double'}, {'integer','scalar'}, ...
                  mfilename, 'N', 5);
    if N<2
  ...
  if nargin > 5
    ...
    string = iptcheckstrs(DIR, validStrings, mfilename, 'DIR', 6);
    switch string
     case 'clockwise'
      DIR = true;
