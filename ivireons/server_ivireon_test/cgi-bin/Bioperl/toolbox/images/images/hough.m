...
%   D = sqrt((numRowsColsInBW - 1)^2 + (numColsInBW - 1)^2).
%
...
%   $Revision: 1.1.8.5 $  $Date: 2010/03/22 03:49:11 $
...
function validateValue(idx, nargin, mfilename, inputStr)

if idx > nargin
    eid = sprintf('Images:%s:valFor%sMissing', mfilename, inputStr);
    msg = sprintf('Parameter ''%s'' must be followed by a value.',...
        inputStr);
    error(eid,'%s', msg);
end
...
if (rhoResolution >= norm(size(BW)))
    eid = sprintf('Images:%s:invalidRhoRes', mfilename);
...
if (min(theta) < -90)
    eid = sprintf('Images:%s:invalidThetaMin', mfilename);
    msg = sprintf(['Value of parameter,''%s'', must be in the '...
        'range [-90, (90-ThetaResolution)].'], inputStr);
    error(eid,'%s', msg);
end

if (max(theta) >= 90)
    eid = sprintf('Images:%s:invalidThetaMax', mfilename);
    msg = sprintf(['Value of parameter,''%s'', must be in the '...
        'range [-90, (90-ThetaResolution)].'], inputStr);
    error(eid,'%s', msg);
end
...
function validateThetaStrings(inputStrings, mfilename)

if (length(strmatch('Theta', inputStrings)) > 1)
    eid = sprintf('Images:%s:multipleThetaParams', mfilename);
    msg = 'Parameters ''Theta'' and ''ThetaResoultion'' cannot be used together.';
    error(eid,'%s', msg);
end
