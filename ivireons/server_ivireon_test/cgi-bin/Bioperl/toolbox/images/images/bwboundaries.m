...
%   $Revision: 1.1.6.7 $  $Date: 2006/10/04 22:38:29 $
...
iptcheckinput(BW, {'numeric','logical'}, {'real','2d','nonsparse'}, ...
              mfilename, 'BW', 1);
...
if nargin < 2
  ...
  if ischar(varargin{2})
    ...
    conn = varargin{2};
    iptcheckinput(conn, {'double'}, {}, mfilename, 'CONN', 2);
    if (conn~=4 && conn~=8)
...
if firstStringToProcess
  ...
  for k = firstStringToProcess:nargin
    % check for options
    string = iptcheckstrs(varargin{k}, validStrings, mfilename, 'OPTION', k);
    switch string
     case 'noholes'
      findholes = false;
     case 'holes'
