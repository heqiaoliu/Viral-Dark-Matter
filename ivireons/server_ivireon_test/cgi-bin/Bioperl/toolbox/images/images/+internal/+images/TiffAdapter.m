...
%   $Revision: 1.1.6.6 $  $Date: 2010/04/15 15:18:12 $
    ...
    methods
        ...
        function obj = TiffAdapter(filename, mode, varargin)
            ...
            if strcmpi(mode, 'w')
                ...
                if nargin ~= 4
                    error('Images:TiffAdapter:invalidInputParamCount', ...
                        'TiffAdapter expects 4 arguments when Mode is ''w''.');
                end
                ...
                if length(obj.ImageSize) < 2 || length(obj.ImageSize) > 3 ||...
                        ...
                        obj.ImageSize(3) ~= 3)
                    error('Images:TiffAdapter:invalidDims', ...
                        'Input size must be an MxN or MxNx3 vector.');
                end
                ...
                if any(strcmpi(obj.Datatype,non_writable_types))
                    error('Images:TiffAdapter:invalidType', ...
                        'TiffAdapter does not support writing ''%s'' datatype.', ...
                        obj.Datatype);
                end
            ...
            elseif strcmpi(mode, 'r')
                ...
                if is_compressed
                    wid = sprintf('Images:%s:CannotWriteCompressed',mfilename);
                    warning(wid,'The TiffAdapter class cannot write to compressed TIFF images.  Opening the adapter in read-only mode.  See the documentation for IMWRITE or the Tiff class for information on creating uncompressed TIFF images.');
                    obj.ReadOnly = true;
                    % for performance, we also use IMREAD here...
                    obj.UseImread = true;
                end
                if is_planar_config_separate
                    wid = sprintf('Images:%s:planarConfigurationError',mfilename);
                    warning(wid,'The TiffAdapter class cannot write to TIFF images with the ''Separate'' PlanarConfiguration tag.  Opening the adapter in read-only mode.  See the documentation for the Tiff class for more information on Tiff tags.');
                    obj.ReadOnly = true;
                    obj.UseImread = true;
                end
                if is_YCbCr
                    wid = sprintf('Images:%s:photometricError',mfilename);
                    warning(wid,'The TiffAdapter class cannot write to TIFF images with the ''YCbCr'' Photometric tag.  Opening the adapter in read-only mode.  See the documentation for the Tiff class for more information on Tiff tags.');
                    obj.ReadOnly = true;
                    obj.UseImread = true;
                end
        ...
        function result = readRegion(obj, start, count)
            ...
            if (start(1) + count(1) - 1 > obj.ImageSize(1) || ...
                ...
                eid = sprintf('Images:%s:countOutOfRange',mfilename);
                error(eid,'Error reading from TiffAdapter.  Region size exceeds image bounds.');
            end
        ...
        function [] = writeRegion(obj, start, data)
            ...
            if obj.ReadOnly
                error('Images:TiffAdapter:readOnly', ...
                    'Attempted to write to a read-only TiffAdapter.');
            end
            ...
            if (start(1) + count(1) - 1 > obj.ImageSize(1) || ...
                    start(2) + count(2) - 1 > obj.ImageSize(2))
                error('Images:TiffAdapter:countOutOfRange', ...
                    'Error writing to TiffAdapter.  Data will exceed image bounds.');
            end
    ...
    methods (Access = private)
        ...
        function [] = create(obj)
            ...
            elseif length(obj.ImageSize) == 2
                ...
                error('Images:TiffAdapter:unsupportedNumChannels', ...
                    'TiffAdapter only supports creating 1 or 3 channel images.');
            end
...
elseif strcmp(datatype, 'double')
    ...
    error('Images:TiffAdapter:unsupportedDatatype', ...
        'Datatype not supported by TIFF format handler.');
end
