...
%   is the noise-to-signal power ratio of the additive noise. NSR can
%   be a scalar or an array of the same size as I. Specifying 0 for the NSR
%   is equivalent to creating an ideal inverse filter.
%
...
if prod(sizePSF)<2,
    eid = sprintf('Images:%s:mustHaveAtLeast2Elements',mfilename);
    error(eid,'In function %s, PSF must have at least two elements.', ...
          mfilename);
elseif all(PSF(:)==0),
    eid = sprintf('Images:%s:psfMustNotBeZeroEverywhere',mfilename);
    error(eid,'In function %s, PSF cannot be zero everywhere.', ...
          mfilename);
end
...
if any(sizeI(numNSdim) < sizePSF(numNSdim))
    ...
    error(eid, ...
          'In function %s, size of the PSF must not exceed the image size in any nonsingleton dimension.', mfilename);
end
