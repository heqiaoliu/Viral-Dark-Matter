...
%   $Revision: 1.5.4.11 $  $Date: 2008/09/13 06:55:44 $
...
if  fcnflag == 1
    if numregion == 0
      ...
      if ~isreal(cmap) || any(cmap(:) > 1) || any(cmap(:) < 0) || ...
        ...
        error(eid, ...
            'function handle MAP must return a n x 3 colormap array');
      end
    end
else
    cmap = map;
...
for i=1:numregion
  if isequal(zerocolor,cmap(i,:))
    ...
    warning(wid, ...
        'Region number %d has the same color as the ZEROCOLOR.', i);
  end
end
cmap = [zerocolor;cmap];
...
if ~ischar(Zerocolor)
    ...
    if ~isreal(Zerocolor) || ~isequal(size(Zerocolor),[1 3]) || ...
      ...
      error(eid, 'Invalid RGB triple entry for ZEROCOLOR.');
    end
else    
...
elseif nnz(idx) > 1
    error(eid, 'Ambiguous string for ORDER: %s.', Order);
else
