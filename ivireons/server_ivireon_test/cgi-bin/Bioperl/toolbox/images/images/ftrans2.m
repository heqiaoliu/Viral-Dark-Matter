...
%   H = FTRANS2(B,T) produces the 2-D FIR filter H that corresponds to the
%   1-D FIR filter B using the transform T. (FTRANS2 returns H as a
%   computational molecule, which is the appropriate form to use with
%   FILTER2.) B must be a 1-D type I (even symmetric, odd-length) filter
%   such as can be returned by FIR1, FIR2, or REMEZ in the Signal
%   Processing Toolbox.  The transform matrix T contains coefficients that
...
%   $Revision: 5.14.4.5 $  $Date: 2009/07/06 20:34:44 $
...
iptcheckinput(b,{'double'},{'nonsparse','nonzero'},mfilename,'B',1);

if nargin < 2,
    t = [1 2 1;2 -4 2;1 2 1]/8;
else
    iptcheckinput(t,{'double'},{'nonsparse','nonzero'},mfilename,'T',2);
