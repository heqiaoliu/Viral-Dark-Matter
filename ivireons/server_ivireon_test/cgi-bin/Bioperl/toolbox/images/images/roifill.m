...
%
%   See also IMPOLY, ROIFILT2, ROIPOLY.

%   Copyright 1993-2007 The MathWorks, Inc.
%   $Revision: 1.22.4.12 $  $Date: 2007/12/10 21:37:29 $
...
switch nargout
    ...
    otherwise
        ...
        if (nargout >= 7)
            ...
            error(eid, 'Too many output arguments.');
        end
end
...
switch nargin
    case 0
        ...
        iptcheckinput(I, {'uint8', 'uint16', 'int16', 'single', 'double'}, ...
            {'2d', 'nonsparse'}, mfilename, 'I', 1);
        if (flag == 0)
            eid = sprintf('Images:%s:currentAxesMissingImage',mfilename);
            error(eid, 'Current axes does not contain an image.');
        end
        if (flag == 1)
            eid = sprintf('Images:%s:indexedNotSupported',mfilename);
            error(eid, 'Indexed images are not supported for ROIFILL.');
        end
    ...
    case 2
        ...
        if ~isequal(size(mask),size(I))
            ...
            error(eid, 'MASK must be the same size as I.');
        end
...
if (min(size(I)) < 3)
    ...
    error(eid, 'I must contain at least 3 rows and 3 columns.');
end
...
if ~XiYiAreVectors || XiYiUnequalLengths
    ...
    error(eid, 'X and Y must be vectors with the same length.');
end
