...
%   $Revision: 1.1.6.6 $  $Date: 2009/03/09 19:15:17 $
...
iptchecknargin(1, 12, nargin, 'decorrstretch');
...
iptcheckinput(A, {'double','uint8','uint16','int16','single'},...
...
for k = 2:2:nargin
    propName = iptcheckstrs(varargin{k}, validPropertyNames,...
                            mfilename, 'PARAM', k);
    switch propName
      case 'Mode'
        checkExistence(k, nargin, mfilename, 'mode string', propName);
        mode = iptcheckstrs(varargin{k+1}, {'correlation','covariance'},...
                            mfilename, 'mode', k+1);
...
if (position + 1 > nargs)
    eid = sprintf('Images:%s:missingParameterValue',fcnName);
    error(eid,...
...
iptcheckinput(targetMean, {'double'},...
...
iptcheckinput(targetSigma, {'double'},...
...
if (numel(targetSigma) > 1) && (size(targetSigma,2) ~= nbands)
    eid = sprintf('Images:%s:targetSigmaWrongSize',mfilename);
    error(eid,...
...
iptcheckinput(rowsubs, {'double'},{'nonempty','integer','positive'},...
              mfilename, 'ROWSUBS', position);

iptcheckinput(colsubs, {'double'},{'nonempty','integer','positive'},...
