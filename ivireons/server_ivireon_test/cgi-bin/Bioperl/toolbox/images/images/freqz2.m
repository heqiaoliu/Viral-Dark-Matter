...
%   $Revision: 5.20.4.4 $  $Date: 2005/12/12 23:19:59 $
...
% frequency method.
if any(size(a) > [Ny Nx]) && strcmp(spacing, 'equal')
    spacing = 'meshgrid';
...
a = varargin{1};
iptcheckinput(a,{'numeric'},{},mfilename,'h',1);
...
switch nargin
  ...
  case 3
    ...
    if (numel(varargin{2}) == 1) && (numel(varargin{3}) == 1)
        % FREQZ2(H,Nx,Ny)
        spacing = 'equal';
        Nx = varargin{2};
        Ny = varargin{3};
...
switch spacing
  ...
  case 'meshgrid'
    if (min(numel(Fx)) ~= 1) || (min(numel(Fy)) ~= 1)
