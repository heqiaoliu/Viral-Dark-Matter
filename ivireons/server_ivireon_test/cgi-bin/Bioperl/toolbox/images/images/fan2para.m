...
%   $Revision: 1.1.6.8 $  $Date: 2008/11/24 14:58:09 $
...
if isMaxPthetaTooBig || isMinPthetaTooSmall
    error('Images:fan2para:cannotComputePtheta', '%s %s',...
          'Internal problem: Desired PTHETA range cannot be computed ',...
          'from given THETA range.');
end
...
if isAnyThetaTooSmall || isAnyThetaTooBig     
    error('Images:fan2para:illegalTheta', '%s %s',...
          'Internal problem: THETA values must be in the range ',...
          '[min(GAMMA),360) degrees.');
end
...
msg1 = 'Inconsistency detected among the parameters D, FAN_SENSOR_SPACING,';
msg2 = 'and FAN_SENSOR_GEOMETRY. Check your input parameters.';
...
if d < maxFloc 
    msg3 = sprintf('Try increasing D to be at least %d.',ceil(maxFloc));
...
iptcheckinput(F, {'double','single'}, ...
              {'real', '2d', 'nonsparse'}, ...
              mfilename, 'F', 1);

iptcheckinput(d, {'double','single'},...
