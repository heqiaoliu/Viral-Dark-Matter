...
%   $Revision: 1.6.4.3 $  $Date: 2004/08/10 01:46:04 $
...
if isa(in, 'strel')
    ...
    if ~( isnumeric(in) || islogical(in) )
        ...
        msg1 = sprintf('Function %s expected its %s input argument, ',...
                       func_name, ...
                       iptnum2ordinal(arg_position));
        ...
        if ~isempty(in)
            ...
            if any(bad_elements(:))
                ...
                msg1 = sprintf('%s, the %s input argument to function %s,', ...
                               arg_name, iptnum2ordinal(arg_position), ...
                               func_name); 
