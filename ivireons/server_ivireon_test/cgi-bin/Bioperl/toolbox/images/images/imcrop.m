...
%  $Revision: 5.21.4.13 $  $Date: 2009/12/02 06:43:23 $
...
switch nargin
    case 0
        ...
        [x,y,a,flag,cm] = validateTargetHandle(hIm);
    ...
    case 1
        ...
        if isscalar(a) && ishghandle(a)
            % IMCROP(H)
            h = a;
            [x,y,a,flag,cm] = validateTargetHandle(h);
...
if (flag == 0)
    ...
    error(eid,'%s','No image found in the current axes');
end
if (flag == 1)
...
if ~right_type || ~(is_2d || is_rgb)
    ...
    error(eid,'%s','Invalid input image.');
end
