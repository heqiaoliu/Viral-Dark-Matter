...
%   $Revision: 1.1.6.10 $  $Date: 2010/04/15 15:18:41 $
...
if valid_file
    ...
    if ~(is_tiff || is_nitf)
        error('Images:rsetwrite:unsupportedInputFormat',...
            'Function RSETWRITE expects input files to be in the TIFF or NITF format.');
    end
...
elseif valid_adapter
    ...
    if numel(varargin) < 1
        error('Images:rsetwrite:mustSpecifyOutputFile',...
            'When the source image is an Image Adapter object, you must specify an output file to RSETWRITE.');
    end
    ...
    if ~isempty(map)
        if ~isequal(ndims(map),2) || ~isequal(size(map,2),3) || ~isnumeric(map)
            ...
            error(eid,'%s','An image adapter object'' Colormap property must be an m-by-3 numeric matrix.');
        end
    end
    
else
    error('Images:rsetwrite:invalidSourceImage','%s%s%s',...
        'Function RSETWRITE expected its first input, sourceImage, ',...
        'to be a filename or an ImageAdapter object.  See the ',...
        'documentation for RSETWRITE and ImageAdapter for more information.');
end
...
if (samplesPerPixel > 1)
    ...
    if (samplesPerPixel ~= 3)
        warning('Images:rsetwrite:samplesPerPixel', ...
            'The input image has %d samples.  IMTOOL may not display all samples.', ...
            samplesPerPixel);
    end
...
if ((info.Height < 3) || (info.Width < 3))
    ...
    error('images:rsetwrite:imageTooSmall', ...
        'The input image must have at least 3 pixels in each dimension')
    
elseif ((numSpanningTilesRows < 2) || (numSpanningTilesCols < 2))
    
    warning('images:rsetwrite:smallImage', ...
        'One of the original image dimensions was smaller than the R-Set tile size.')
...
if (isequal(details.mlType, 'logical'))
    ...
    error('Images:rsetwrite:logicalImage', ...
        'Binary images are not supported.')
...
if (~success)
    ...
    error('Images:rsetwrite:nonexistentOutputDir', ...
        'The output directory ''%s'' does not exist.', outputDir)
    
elseif (~attributes.UserWrite)
    
    error('Images:rsetwrite:outputDirPerms', ...
        'Cannot write to output directory ''%s''.', outputDir)
...
if (~tf && ~isequal(eid, 'Images:%s:nitfNumberOfBands'))
