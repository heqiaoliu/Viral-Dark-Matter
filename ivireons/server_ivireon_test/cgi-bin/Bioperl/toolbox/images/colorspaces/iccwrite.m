...
%   $Revision: 1.1.6.3 $  $Date: 2009/07/06 20:34:37 $ Poe
%   Original authors: Scott Gregory, Toshia McCabe, Robert Poe; 12/20/03


% Check input arguments
iptchecknargin(2, 2, nargin, 'iccwrite');
iptcheckinput(filename, {'char'}, {'nonempty'}, 'iccwrite', 'FILENAME', 2);
if ~isicc (p)
    eid = 'Images:iccwrite:invalidProfile';
    msg = 'Invalid or incomplete profile structure.';
    error(eid, '%s', msg);
end
...
if (fid < 0)
    eid = 'Images:iccwrite:errorOpeningFile';
    error(eid, '%s', msg);
end
...
if isempty(tagheap)
    fclose(fid);
    eid = 'Images:iccwrite:InvalidTags';
    msg = 'Profile tags corrupted or invalid';
    error(eid, '%s', msg);
end
...
if isempty(headerheap)
    fclose(fid);
    eid = 'Images:iccwrite:InvalidHeader';
    msg = 'Profile header corrupted or invalid';
    error(eid, '%s', msg);
end
tableheap = uint8(encode_tagtable(tagtable));
if isempty(tableheap)
    fclose(fid);
    eid = 'Images:iccwrite:InvalidTagTable';
    msg = 'Tag table corrupted or invalid';
    error(eid, '%s', msg);
end
...
for k = 1:numtags
    out((offset + 1):(offset + 4)) = uint8(tagtable{k, 1});
...
for k = 1:length(profile_fields)
    ...
    if ~isempty(pdx)
        ...
        if isempty(tag)
            eid = 'Images:iccwrite:DefectiveTag';
            msg1 = sprintf('Error in %s tag.', profile_fields{k});
            warning(eid, '%s', msg1);
            tagheap = [];
            return;
        end
...
for k = 1:length(mattrc_fields)
    ...
    if ~isempty(mdx)
        ...
        if isempty(tag)
            eid = 'Images:iccwrite:DefectiveTag';
            msg1 = sprintf('Error in %s tag.', mattrc_fields{k});
            warning(eid, '%s', msg1);
            tagheap = [];
            return;
        end
...
for k = 1:privatecount
    ...
    if isempty(tag)
        eid = 'Images:iccwrite:DefectiveTag';
        msg1 = sprintf('Error in private tag %s.', signature);
        warning(eid, '%s', msg1);
        tagheap = [];
        return;
    end
...
if count ~= numel(a)
    pos = ftell(fid) - count;
    fclose(fid);
    eid = 'Images:iccwrite:fileWriteFailed';
    msg = sprintf('File write failed: %d bytes at position %d.\n', n, pos);
    error(eid, '%s', msg);
end
