...
%   $Revision: 1.1.6.4.4.1 $  $Date: 2010/06/07 13:33:31 $ Poe
...
transform_type = iptcheckstrs(varargin{1}, valid_strings, mfilename, ...
                           'TRANSFORMTYPE', 1);
...
iptcheckinput(varargin{1},{'char'},{'nonempty'},mfilename,'TRANSFORMTYPE',1);
...
if nargin == 2
    error('Images:makecform:wrongNumInputs', ...
...
if nargin > 1
        ...
        prop_string = iptcheckstrs(varargin{2}, valid_property_strings, ...
                                mfilename, 'PARAM', 2);
        value_string = iptcheckstrs(varargin{3}, valid_value_strings, ...
                                mfilename, 'PARAM', 3);
...
switch nargin
  ...
  case 3
    ...
    error(eid,'PropertyName/PropertyValue pair expected.');
  case 4
...
iptchecknargin(5, 5, nargin, mfilename);

if isempty(propname) || isempty(propvalue)
    wp = whitepoint;
else
    iptcheckstrs(propname, valid_strings, mfilename, 'PROPERTYNAME', varpos);
    wp = propvalue;
    iptcheckinput(wp, {'double'}, ...
                    {'real', '2d', 'nonsparse', 'finite', 'row', 'positive'}, ...
                    mfilename, varname, varpos + 1);
    if size(wp, 2) ~= 3
        eid = 'Images:makecform:invalidWhitePointData';
        error(eid, 'White-point data must have 3 columns.');
    end
end
...
if ~isfield(profile,luttype)
    ...
    error(eid,'Profile does not contain the requested lut tag.');
end
...
elseif luttag.MFT <= 4  % mft2, mAB , mBA 
    ...
    eid = 'Images:makecform:invalidLuttag';
    error(eid, 'Unsupported tag type.');
end
...
elseif isstruct(varargin{1})
    ...
    eid = 'Images:makecform:invalidArgument';
    msg = 'Invalid first argument; must be profile or MatTRC';
    error(eid, '%s', msg);
end
...
if ~is_valid_mattrc(mattrc)
    eid = 'Images:makecform:invalidMattrc';
    error(eid,'MatTRC structure is invalid.');
end

% Check direction arguments
valid_propname_strings = {'direction'};
valid_propvalue_strings = {'forward', 'inverse'};
iptcheckstrs(varargin{2}, valid_propname_strings, ...
                    mfilename, 'PROPERTYNAME', 2);
direction = iptcheckstrs(varargin{3}, valid_propvalue_strings, ...
...
elseif nargin > 4
    ...
    valid_propvalue_strings = {'relativecolorimetric', ...
                               'absolutecolorimetric'};
    iptcheckstrs(varargin{4}, valid_propname_strings, ...
                    mfilename, 'PROPERTYNAME', 4);
    intent = iptcheckstrs(varargin{5}, valid_propvalue_strings, ...
...
if strcmpi(intent, 'absolutecolorimetric')
    if isempty(pf)
        eid = 'Images:makecform:invalidArgument';
        error(eid, 'AbsoluteColorimetric intent requires profile input.');
    else
...
if ~isfield(pf, 'GrayTRC')
    eid = 'Images:makecform:invalidGraytrc';
    error(eid, 'No GrayTRC in profile.');
else
    graytrc = pf.GrayTRC;
end
...
if invalidGraytrc 
    eid = 'Images:makecform:invalidGraytrc';
    error(eid, 'Profile''s GrayTRC field is invalid.');
end
...
if ~isfield(pf, 'NamedColor2')
    eid = 'Images:makecform:invalidNamedColor';
    error(eid, 'No NamedColor2 tag in profile.');
elseif ~isfield(pf.NamedColor2, 'NameTable')
    eid = 'Images:makecform:invalidNamedColor';
    error(eid, 'No NameTable in namedColor2Tag.');
else
...
elseif strcmpi(cdata.Space, 'device')
    ...
    eid = 'Images:makecform:invalidInputData';
    error(eid, 'Invalid SPACE; must be ''PCS'' or ''Device''.');
end
...
if nargin > 2
    for k=3:2:nargin
        prop_string = iptcheckstrs(varargin{k}, valid_property_strings, ...
                                mfilename, 'PROPERTYNAME', k);
        value_string = iptcheckstrs(varargin{k+1}, valid_value_strings, ...
...
if ~isicc(source_pf)
    ...
    error(eid, 'Invalid source profile.');
end
if strcmp(source_pf.Header.DeviceClass, 'device link')
    eid = 'Images:makecform:invalidSourceProfile';
    error(eid, 'DeviceLink Profile can''t be used as source.');
end
...
if ~isicc(dest_pf)
    ...
    error(eid, 'Invalid destination profile.');
end
if strcmp(dest_pf.Header.DeviceClass, 'device link')
    eid = 'Images:makecform:invalidDestinationProfile';
    error(eid, 'DeviceLink Profile can''t be used as destination.');
end
...
elseif isfield(source_pf, 'GrayTRC')
    ...
    eid = 'Images:makecform:invalidProfile';
    error(eid, 'Invalid source profile. No AToBx, MatTRC, or GrayTRC found.');
end
...
elseif isfield(dest_pf, 'GrayTRC')
    ...
    eid = 'Images:makecform:invalidProfile';
    error(eid, 'Invalid destination profile. No BToAx, MatTRC, or GrayTRC found.');
end
...
if source_absolute && ~dest_absolute
    ...
    error(eid, ...
        'Destination rendering intent must be absolute if source is absolute.');
else
...
if nargin < 6
    ...
    iptcheckstrs(varargin{6}, {'AdaptModel'}, mfilename, 'PROPERTYNAME', 6);
    if nargin < 7
        adapttype = 'bradford';
    else
        adapttype = iptcheckstrs(varargin{7}, {'VonKries' 'Bradford'}, ...
...
elseif strcmpi(adapttype, 'VonKries')
    ...
    eid = 'Images:makecform:AdaptModelNotSupported';
    error(eid, '%s adaptation model not supported', adapttype);
end
