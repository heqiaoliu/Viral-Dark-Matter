...
%   $Revision: 1.1.6.1 $  $Date: 2008/04/03 03:10:08 $ Poe
%   Original author:  Robert Poe 10/15/05

% Check input arguments
iptchecknargin(2, 4, nargin, 'applycurve');
iptcheckinput(in, {'double'}, ...
...
if isstruct(curve)
    if isfield(curve, 'FunctionType')
        ...
        eid = 'Images:applycurve:private:invalidInputData';
        msg = 'No function specified for parametricCurveType';
        error(eid, '%s', msg);
    end
    if isfield(curve, 'Params') & isa(curve.Params, 'double')
        ...
        eid = 'Images:applycurve:private:invalidInputData';
        msg = 'No valid parameters for parametricCurveType';
        error(eid, '%s', msg);
    end
...
% Handle curveType
    ...
    elseif scale == 65535.0                   % power law
        ...
        eid = 'Images:applycurve:private:invalidInputData';
        msg = '8-bit LUT must have 256 entries';
        error(eid, '%s', msg);
    end
end
...
if ~isvector(xin) | length(xin) < 2
    eid = 'Image:applygraytrc_inv:BadInput';
    msg = 'Xi must be a vector of length at least 2';
    error(eid, '%s', msg);
end
...
if size(yin, 1) ~= nin
    eid = 'Image:applygraytrc_inv:BadInput';
    msg = 'Yi must have same number of rows as Xi.';
    error(eid, '%s', msg);
end
