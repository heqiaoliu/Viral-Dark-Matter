...
%   $Revision: 1.1.6.2 $  $Date: 2009/04/15 23:09:48 $ 
%   Original author:  Scott Gregory, 10/20/02

% Check input arguments
iptchecknargin(2, 4, nargin, 'applyclut');

in = varargin{1};
iptcheckinput(in,{'uint8','uint16'},{'real','2d','nonsparse','finite'},...
           'applyclut','IN',1);

% Check the luttag
luttag = varargin{2};
iptcheckinput(luttag,{'struct'},{'nonempty'},'applyclut','LUTTAG',1);
...
if (isa(in, 'uint8') && luttagbytes == 2) || ...
   (isa(in, 'uint16') && luttagbytes == 1)
    eid = 'Images:applyclut:private:datatypeMismatch';
    msg = 'Input data type does not match luttag.';
    error(eid, '%s', msg);
end
...
if  ~isfield(luttag, 'MFT') || ...
    ...
    error('Images:applyclut:private:invalidLuttag', ...
          'Invalid or incomplete luttag.');
end
%--------------------------------------------------------------------------
...
if size(in, 2) ~= chan_in
    eid = 'Images:applyclut:private:invalidInputData';
    msg = 'Number of input channels does not match number of shapers.';
    error(eid, '%s', msg);
end
...
if size(in, 2) ~= chan_in
    eid = 'Images:applyclut:private:dataMismatch';
    msg = 'Number of channels in input data doesn''t match CLUT.';
    error(eid, '%s', msg);
end
