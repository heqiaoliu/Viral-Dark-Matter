...
%   $Revision: 1.1.6.3.8.1 $  $Date: 2010/06/07 13:33:29 $ 
...
if ischar(in) 
    if strcmp(c.encoding, 'name')
        ...
        eid = 'Images:applycform:invalidNamedColorCform';
        error(eid, 'Cform structure is invalid for Named Color.');
    end
end

% Check to make sure IN is of correct class and attributes
iptcheckinput(in,{'double','uint8','uint16'}, ...
...
elseif ndims(in) == 3
    ...
    error(eid, 'IN must be M-by-K or M-by-N-by-K.');
end
...
if bad_data
    ...
    error(eid, 'Cform structure is invalid.');
end
