...
%   $Revision: 1.1.6.4 $  $Date: 2009/07/06 20:34:36 $ 
%   Original authors: Scott Gregory, Toshia McCabe 10/18/02


% Check input argument
iptchecknargin(1,1,nargin,'iccread');
iptcheckinput(filename,{'char'},{'nonempty'},'iccread','FILENAME',1);
...
if exist(filename,'file') ~= 2
    ...
    if (exist(fullfile(repository, filename)) == 2)
...
if (fid < 0)
    eid = 'Images:iccread:errorOpeningProfile';
    error(eid,'%s',msg);
end
...
if isfield(s.Header, 'ProfileID') && any(s.Header.ProfileID)
    ...
    if ~isequal(digest, s.Header.ProfileID)
        warning('Images:iccread:badChecksum', ...
                'Profile checksum does not match the stored value; profile may be corrupt.');
    end
end
...
if isempty(idx)
    fclose(fid);
    msg1 = 'Unrecognized Profile/Device Class in profile header.'; 
    msg2 = '     See ICC Specification, Clause 6.1.4.';
...
if isempty(idx)
    ...
    msg1 = 'Unrecognized Color Space in profile header.'; 
    msg2 = '     See ICC Specification, Clause 6.1.5.';
    error(eid, '%s\n%s', msg1, msg2);
else
...
if strcmp(header.DeviceClass, 'device link')
    ...
    if isempty(idx)
      ...
      msg1 = 'Unrecognized Connection Space in profile header.'; 
      msg2 = '     See ICC Specification, Clause 6.1.6.';
      error(eid, '%s\n%s', msg1, msg2);
    else
...
if ~strcmp(header.Signature, 'acsp')
    ...
    msg1 = 'Unrecognized File Signature in profile header.'; 
    msg2 = '     See ICC Specification, Clause 6.1.';
    error(eid, '%s\n%s', msg1, msg2);
end
...
if data_size < 8
    eid = 'Images:iccread:invalidDataSize';
    msg1 = sprintf('Insufficient data size in %s tag.', tagname);
    warning(eid, '%s\n', msg1);
    out = [];
    return;
end
...
if ~strcmp(tagtype, 'sf32')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''sf32''.'];
    msg2 = '     See Clause 6.5.14 (v. 2) or 6.5.19 (v. 4).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
if ~strcmp(xyztype, 'XYZ ')
    fclose(fid);
    eid = 'Images:iccread:invalidTagType';
    msg = sprintf('Invalid %s tag type:  ', tagname);
    msg = [msg 'Must be ''XYZ ''.'];
    error(eid, '%s', msg);
end
fseek_check(fid, offset + 8, 'bof');
...
if rem(num_values,3) ~= 0
    fclose(fid);
    eid = 'Images:iccread:invalidTagSize';
    msg1 = sprintf('Unexpected data size in %s tag.', tagname);
    msg2 = '     See Clauses 5.3.10 and 6.5.26 for v. 2 or 6.5.30 for v. 4.';
    error(eid, '%s\n%s', msg1, msg2);
end
out = fread_check(fid, num_values, 'int32') / 65536;
out = reshape(out, 3, num_values/3)';
...
function out = read_curve_type(fid, offset, data_size, version, tagname)
...
elseif strcmp(curvetype, 'para') & version > 2 % New v. 4 type
    ...
    switch out.FunctionType
        ...
        case 4
            n = 7;
        otherwise
            eid = 'Images:iccread:invalidFunctionType';
            msg = sprintf('Unknown FunctionType %d in %s tag.', ...
                          out.FunctionType, tagname);
            error(eid, '%s', msg);
    end
    ...
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''curv'' or (v. 4) ''para''.'];
    msg2 = '     See Clauses 6.5.3 (v. 2) or 6.5.5 and 6.5.16 (v. 4).';
    error(eid, '%s\n%s', msg1, msg2);
end
...
if ~strcmp(tagtype, 'view')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''view''.'];
    msg2 = '     See Clause 6.5.25 (v. 2) or 6.5.29 (v. 4).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
elseif strcmp(luttype, 'mBA ') & version > 2 % New v. 4 lut type
    ...
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''mft1'', ''mft2'', ''mAB '' (v. 4), or ''mBA '' (v. 4).'];
    msg2 = ['     See Clauses 6.5.7 and 6.5.8 for v. 2 ' ...
           'or 6.5.9, 6.5.10, 6.5.11, 6.5.12 for v. 4.'];
    error(eid, '%s\n%s', msg1, msg2);
end
...
if out.MFT < 3
    ...
    % Handle newer lutAtoBType and lutBtoAType
      ...
      if boffset == 0
          fclose(fid);
          eid = 'Images:iccread:invalidLuttag';
          msg1 = sprintf('Invalid %s tag:  ', tagname);
          msg1 = [msg1 'lutAtoBType and lutBtoAType must have "B" curves.'];
          msg2 = '     See Clauses 6.5.11 and 6.5.12.';
          error(eid, '%s\n%s', msg1, msg2);
      end
      ...
      elseif xoffset == 0
          ...
          eid = 'Images:iccread:invalidLuttag';
          msg1 = sprintf('Invalid %s tag:  ', tagname);
          msg1 = [msg1 'lutAtoBType and lutBtoAType can''t have "M" curves without Matrix.'];
          msg2 = '     See Clauses 6.5.11 and 6.5.12.';
          error(eid, '%s\n%s', msg1, msg2);
      else
      ...
      if coffset == 0
          ...
          elseif datasize == 2
              ...
              fclose(fid);
              eid = 'images:iccread:invalidLuttag';
              msg1 = sprintf('Invalid CLUT precision in %s tag:  ', tagname);
              msg1 = [msg1 'Must be 8-bit or 16-bit.'];
              msg2 = '     See Clauses 6.5.11 and 6.5.12.';
              error(eid, '%s\n%s', msg1, msg2);
          end
      ...
      elseif coffset == 0
          ...
          eid = 'Images:iccread:invalidLuttag';
          msg1 = sprintf('Invalid %s tag:  ', tagname);
          msg1 = [msg1 'lutAtoBType and lutBtoAType can''t have "A" curves without CLUT.'];
          msg2 = '     See Clauses 6.5.11 and 6.5.12.';
          error(eid, '%s\n%s', msg1, msg2);
      else
...
if ~strcmp(tagtype, 'meas')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''meas''.'];
    msg2 = '     See Clause 10.12 (v. 4.2).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
function out = read_named_color_type(fid, offset, data_size, pcs, tagname)
...
if ~strcmp(tagtype, 'ncl2')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''ncl2''.'];
    msg2 = '     See Clause 10.14 (v. 4.2).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
if ~strcmp(tagtype, 'pseq')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''pseq''.'];
    msg2 = '     See Clause 10.16 (v. 4.2).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
for p = 1:n
    ...
    if strcmp(techsig, char(uint8([0 0 0 0])))
        ...
        if isempty(idx)
            eid = 'Images:iccread:invalidTechnologySignature';
            msg1 = sprintf('Unrecognized technology signature in %s tag', ...
                           tagname);
            msg2 = '     See ICC Specification, Clause 9.2.35 (v. 4.2).';
            warning(eid, '%s\n%s', msg1, msg2);
...
if ~strcmp(tagtype, 'rcs2')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''rcs2''.'];
    msg2 = '     See Clause 10.17 (v. 4.2).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
for i = 1 : numtypes
    out(i) = struct('MeasurementCode', [], 'MeasurementType', [], ...
                    'SolidXYZs', [], 'ResponseArray', []);
...
if strcmp(typesig, 'sig ')
    ...
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''sig ''.'];
    msg2 = '     See Clause 10.19 (v. 4.2).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
end
...
if strcmp(chartype, 'desc')
  ...
  % optional Unicode or ScriptCode data, saved but not interpreted
    ...
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'In v. 2 must be ''desc''.'];
    msg2 = '     See Clause 6.5.17.';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
end
...
function out = read_unicode_type(fid, offset, data_size, tagname)
...
if strcmp(chartype, 'mluc') % New v. 4 type
    ...
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'In v. 4 must be ''mluc''.'];
    msg2 = '     See Clause 6.5.14.';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
end
...
if ~strcmp(tagtype, 'text')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''text''.'];
    msg2 = '     See Clause 6.5.18 (v. 2) or 6.5.22 (v. 4).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
if data_size < 20
    eid = 'Images:iccread:invalidTagSize';
    msg1 = sprintf('Invalid data size in %s tag.', tagname);
    warning(eid, '%s\n', msg1);
    out = [];
    return;
end
...
if ~strcmp(tagtype, 'dtim')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''dtim''.'];
    msg2 = '     See Clause 6.5.5 (v. 2) or 6.5.7 (v. 4).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
if ~strcmp(tagtype, 'crdi')
    eid = 'Images:iccread:invalidTagType';
    msg1 = sprintf('Invalid %s tag type:  ', tagname);
    msg1 = [msg1 'Must be ''crdi''.'];
    msg2 = '     See Clause 6.5.2 (v. 2) or 6.5.4 (v. 4).';
    warning(eid, '%s\n%s', msg1, msg2);
    out = [];
    return;
end
...
if ~strcmp(class(bytes), 'uint8')
    eid = 'Images:iccread:trim_string:invalidInputData';
    msg = 'Input must be uint8.';
    error(eid, '%s', msg);
end
endbyte = findstr(0, bytes);
...
if fseek(fid, n, origin) < 0
    ...
    msg = sprintf('File seek failed: %d bytes at position %d.', n, pos);
    error(eid,'%s',msg);
end
...
if count ~= n
    ...
    msg = sprintf('File read failed: %d bytes at position %d.', n, pos);
    error(eid,'%s',msg);
end 
