...
% $Revision: 1.1.6.6 $ $Date: 2009/12/28 04:11:05 $
%#eml
...
if (~isfi(a0) || ~isfi(b0))
    eml_assert(0,'No method ''sub'' with matching signature found for class ''embedded.fimath''.');
end
...
if isfixed(a0) || isfixed(b0)
    ...
    eml_assert((isslopebias_in&&iscomplex_in)==0,...
               'Function ''minus'' is not defined for complex-value FI objects with slope and bias scaling.');
    ...
    if fullPrecSum || cb4sum
        if (isslopebias_in)
            c = eml_fimathislocal(eml_minus(eml_cast(a,ta,f), eml_cast(b,tb,f), tc, f),false);
        else
            c = eml_fimathislocal(eml_minus(eml_cast(a,tc,f), eml_cast(b,tc,f), tc, f),false);
        end
...
elseif ( isfloat(a0) || isfloat(b0))
    ...
    c = eml_fimathislocal(eml_minus(ain,bin,t,f),false);
else
