...
%#eml
%   $Revision: 1.1.6.10 $  $Date: 2009/03/05 18:46:52 $

fnname = 'rol';

% To make sure the number of the input arguments is right
eml_assert(nargin == 2, 'no method ''rol'' with matching signature found for class ''embedded.fi''.');
...
eml_assert(isfi(ain), 'first operand to ''rol'' must be a fi object.');
...
eml_assert(~eml_isslopebiasscaled(ain), 'bitrol does not support slope-bias scaled fis.');
...
aNT = numerictype(ain);
eml_assert(aNT.WordLength > 1, 'bitrol not supported for 1-bit input operand');
...
if isfixed(ain)
    ...
    for m = 1:numel(a)
        % real
        cr = eml_bit_rol(real(a(m)),uint8(k));

        % if a is complex
        if ~isreal(a)
            ci = eml_bit_rol(imag(a(m)),uint8(k));
            c(m) = complex(cr,ci);
        else
            c(m) = cr;
        end
    ...
    eml_fi_assert_dataTypeNotSupported('BITROL','fixed-point');
end
