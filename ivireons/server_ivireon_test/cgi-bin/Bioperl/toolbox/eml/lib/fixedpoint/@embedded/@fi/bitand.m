...
%
% BITAND Returns bitwise AND
% C = bitand(a,b) returns the bitwise AND of argument a and b, 
% where a and b are:
...
%
% Note: Difference from the MATLAB Toolbox BITAND:
%   When a and b are both non-fi, the MATLAB Toolbox BITAND restricts a and b 
%   to be unsigned (uint8, uint16, uint32, ...). Here we loose the restriction 
%   because the Fixed-Point Toolbox BITAND accepts the signed case.

%   Copyright 2002-2009 The MathWorks, Inc.
%#eml
%   $Revision: 1.1.6.7 $ $Date: 2009/03/05 18:46:48 $ 
...
if isfixed(a) || isfixed(b)
    ...
    if isreal(a) || isreal(b)
        c_uint = eml_bitand(real(a),real(b));
        ...
        c_uint_r = eml_bitand(real(a),real(b));
        c_uint_i = eml_bitand(imag(a),imag(b));
    ...
    eml_fi_assert_dataTypeNotSupported('BITAND','fixed-point');
end
