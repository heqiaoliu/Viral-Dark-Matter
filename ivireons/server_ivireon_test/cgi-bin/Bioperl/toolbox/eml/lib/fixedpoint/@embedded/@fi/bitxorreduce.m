function c = bitxorreduce(a,lidx,ridx)
% Embedded MATLAB Library function.
%
% BITXORREDUCE Perform bitwise-xor reduction on a range of bits in a fixed point word.
%
%    C = BITXORREDUCE(A,LIDX, RIDX) returns a bit after perform bitwise xor
...
%#eml
%   $Revision: 1.1.6.7 $ $Date: 2009/03/05 18:46:58 $
...
if (nargin > 1)
    eml_assert(~isempty(lidx), 'empty input is not allowed in ''bitxorreduce''');
end
if (nargin > 2)
    eml_assert(~isempty(ridx), 'empty input is not allowed in ''bitxorreduce''');
end
...
if isfixed(a)
    ...
    eml_bitop_index_checks('bitxorreduce', a,lidx,ridx)
    ...
    for k = 1:eml_numel(c)
        c(k) = eml_bitxorreduce(a(k), eml_const(lidx-1), eml_const(ridx-1));
    end

else
    % non fi-fixedpoint not supported
    eml_fi_assert_dataTypeNotSupported('BITXORREDUCE','fixed-point');
end
