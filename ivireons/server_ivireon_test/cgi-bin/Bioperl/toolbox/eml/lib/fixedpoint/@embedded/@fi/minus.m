function c = minus(a0,b0)
% Embedded MATLAB function the @fi/minus operation

% $INCLUDE(DOC) toolbox/eml/lib/fixedpoint/@embedded/@fi/minus.m $
% Copyright 2002-2009 The MathWorks, Inc.
%#eml
% $Revision: 1.1.6.21 $  $Date: 2010/01/25 21:35:47 $
...
if eml_ambiguous_types
    ...
    if isrealC
        c = eml_not_const(ctemp);
    else
        c = eml_not_const(complex(ctemp,ctemp));
    end
    return;
end
...
if ( (isfi(a0) && isfixed(a0)) || ...
    ...
    eml_assert(eml_const((isslopebias_in&&iscomplex_in)==0),...
               'Function ''minus'' is only supported for complex-value FI objects that have an integer power of 2 slope, and a bias of 0.');
    ...
    if fullPrecSum || cb4sum
        if (isslopebias_in)
            c = eml_fimathislocal(eml_minus(a, b, tc, f),cHasLocalFimath);
        else
            c = eml_fimathislocal(eml_minus(eml_cast(a,tc,f), eml_cast(b,tc,f), tc, f),cHasLocalFimath);
        end
...
elseif ( isfi(a0) && isfloat(a0) ) || ...
    ...
    c                  = eml_minus(ain,bin,t,f);
    
else
    % FI datatype not supported
    eml_fi_assert_dataTypeNotSupported('MINUS','fixed-point,double, or single');
end
