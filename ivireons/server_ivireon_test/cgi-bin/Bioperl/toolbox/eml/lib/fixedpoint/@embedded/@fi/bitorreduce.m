function c = bitorreduce(a,lidx,ridx)
% Embedded MATLAB Library function.
%
% BITORREDUCE Perform bitwise-or reduction on a range of bits in a fixed point word.
%
%    C = BITORREDUCE(A,LIDX, RIDX) returns a bit after perform bitwise or
...
%#eml
%   $Revision: 1.1.6.7 $ $Date: 2009/03/05 18:46:51 $
...
if (nargin > 1)
    eml_assert(~isempty(lidx), 'empty input is not allowed in ''bitorreduce''');
end
if (nargin > 2)
    eml_assert(~isempty(ridx), 'empty input is not allowed in ''bitorreduce''');
end
...
if isfixed(a)
    ...
    eml_bitop_index_checks('bitorreduce', a,lidx,ridx)
    ...
    for k = 1:eml_numel(c)
        c(k) = eml_bitorreduce(a(k), eml_const(lidx-1), eml_const(ridx-1));
    end


else
    % non fi-fixedpoint not supported
    eml_fi_assert_dataTypeNotSupported('BITORREDUCE','fixed-point');
end
