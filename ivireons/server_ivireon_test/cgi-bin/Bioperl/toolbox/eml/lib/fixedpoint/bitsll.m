...
%#eml
%   $Revision: 1.1.6.5 $  $Date: 2009/12/28 04:10:40 $
    ...
    elseif isinteger(u)
        switch class(u)
          ...
          case 'uint64'
            temp = fi(u,0,64,0);
          otherwise
            eml_lib_assert(0,'fi:eml:bitsll:UnrecognizedInteger','Unrecognized integer type.');
        end
        w  = int(bitsll(temp, v));
    else
        eml_assert(false,['Function ''bitsll'' is not defined for a first argument of class ',class(u) '.']);
        w = 0;
    end
end
