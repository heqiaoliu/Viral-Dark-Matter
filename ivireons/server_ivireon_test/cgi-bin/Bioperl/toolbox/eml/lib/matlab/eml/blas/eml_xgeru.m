function A = eml_xgeru(m,n,alpha1,x,ix0,incx,y,iy0,incy,A,ia0,lda)
...
%   2. ALPHA1, X, or Y may be real when A is complex.
%   3. Dispatches to xGER if alpha1, x, y, and A are real.
%   4. m, n, ix0, incx, iy0, incy, ia0, lda from any numeric class.
%   5. To avoid copies, pass in [] for x or y when they represent
...
if eml_option('Developer')
    ...
    eml_assert(isscalar(alpha1) && isa(alpha1,'float'), ...
        'ALPHA1 must be scalar and ''double'' or ''single''.');
    ...
    eml_assert(~isreal(A) || (isreal(alpha1) && isreal(x) && isreal(y)), ...
        'A must be complex if ALPHA1, X, or Y is complex.');
...
if isreal(alpha1) && isreal(x) && isreal(y) && isreal(A)
    A = eml_xger(m,n,alpha1,x,ix0,incx,y,iy0,incy,A,ia0,lda);
else
    A = eml_blas_xgeru(m,n,alpha1,x,ix0,incx,y,iy0,incy,A,ia0,lda);
end
