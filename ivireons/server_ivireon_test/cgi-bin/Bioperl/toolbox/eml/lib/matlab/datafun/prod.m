...
eml_assert(isa(x,'float'), ...
    ['Function ''prod'' is not defined for values of class ''' class(x) '''.']);
if nargin == 1 && eml_is_const(size(x)) && isequal(x,[])
    y = cast(eml_scalar_eg(x)+1,output_class(x));
    return
end
...
if nargin == 1 || (nargin == 2 && ischar(in2))
    eml_lib_assert(eml_is_const(size(x)) || ~isequal(x,[]), ...
        'EmbeddedMATLAB:prod:specialEmpty', ...
        'PROD with one variable-size matrix input of [] is not supported.');
    dim = eml_const_nonsingleton_dim(x);
    eml_lib_assert(eml_is_const(size(x,dim)) || ...
        isscalar(x) || ...
        size(x,dim) ~= 1, ...
        'EmbeddedMATLAB:prod:autoDimIncompatibility', ...
...
eml_assert(nargin == 1, ...
    'The ''native'' and ''double'' flags are not supported.');
if nargin == 2
    if ischar(cls) && strcmp(cls,'native')
        eml_assert(isreal(x) || ~isinteger(x), ...
            'Complex integer multiplication is not supported.');
        outcls = class(x);
