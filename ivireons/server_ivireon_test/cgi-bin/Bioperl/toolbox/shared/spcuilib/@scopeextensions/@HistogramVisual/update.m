...
% UPDATE Update the histogram display

%   Copyright 2009-2010 The MathWorks, Inc.
%   $Revision: 1.1.6.7 $    $DATE:$    
...
if this.NTXFeaturedOn
    ...
    this.NTExplorerObj.update(rawData);
else
    ...
    if isempty(range);this.screenMsg(uiscopes.message('emptyHistogramDataMessage'));return; end
...
% by the histogram up to the minimum bin resulting from the selected data
% type.
inrange_line = y_re;
...
% resulting from the selected data type up to the maximum bin returned by
% the histogram.
inrange_line = y_re;
