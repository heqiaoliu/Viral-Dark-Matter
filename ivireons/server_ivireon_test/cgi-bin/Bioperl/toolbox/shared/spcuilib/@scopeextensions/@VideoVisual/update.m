...
%   $Revision: 1.1.6.10 $  $Date: 2010/03/31 18:43:18 $

% Make sure that the screen message text object is invisible.
% set(this.MessageText, 'Visible', 'Off');
...
if sizePrevFrame(1) ~=0 && sizeNewFrame(1) ~=0
    if  ~(length(sizeNewFrame) == length(sizePrevFrame)) ...
        ...
        this.OldDimensions= sizeNewFrame;
        [msg id] = DAStudio.message('vipblks:svipvidrend2:videoBandsSzChanged');
