...
%   $Revision: 1.1.8.1 $  $Date: 2009/10/16 06:32:07 $
...
switch action
   case 'init'
      ...
      if hChildren(idx) == hittest(HostFig)
         X = unitconv(Constr.Origin, 'deg', XUnits);
      ...
      Constr.AppData = struct('Origin0', Constr.Origin);
   ...
   case 'update'
      ...
      Constr.Origin = sgnPhaOrig * (abs(dX) + 180 - rem(abs(dX),360));
      ...
      Origin  = unitconv(Constr.Origin, 'deg', XUnits);
      LocStr = sprintf('Margin moved from %0.3g to %0.3g %s.', ...
   ...
   case 'finish'
      ...
      Constr.TextEditor.Origin = Constr.Origin;
