...
% EVAL  Method to evaluate the dampingratio of a linear system
%
% Inputs:
%          this - a srorequirement.dampingratio object.
%          Sys  - An LTI object.
% Outputs: 
%          c - a nx1 double giving the damping ratios of the system, 
%          interpreted as the sin of the real axis value to the 
%          magnitude of the pole on a continuous pole-zero plot. Unstable
%          poles have a zero damping ratio.
 
% Author(s): A. Stothert 31-May-2005
%   Copyright 2005-2009 The MathWorks, Inc.
% $Revision: 1.1.8.1 $ $Date: 2009/10/16 06:34:36 $
...
if Model.Ts == 0
   %Continuous time
   c = -real(pSys)./max(eps,abs(pSys));
else
   %Discrete time
   r = max(eps,abs(pSys)); t = angle(pSys);
