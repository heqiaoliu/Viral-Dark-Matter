classdef PZLocation < editconstr.absEditor
    % PZLOCATION  Editor panel class for a pz-location constraint
    %
    
    % Author(s): A. Stothert 07-Jan-2009
    % Copyright 2009 The MathWorks, Inc.
    % $Revision: 1.1.8.1 $ $Date: 2009/10/16 06:30:53 $
    
    methods
        function this = PZLocation(SrcObj)
            this = this@editconstr.absEditor(SrcObj);
            this.Activated = true;
            this.setDisplayUnits('xunits','abs');  %x=real, y=imag
        ...
        function widgets = getWidgets(this,Container)
            ...
            columnNames(1) = java.lang.String(ctrlMsgUtils.message('Controllib:graphicalrequirements:lblPWLEditorReal'));
            columnNames(2) = java.lang.String(ctrlMsgUtils.message('Controllib:graphicalrequirements:lblPWLEditorImaginary'));
...
%% Manage changes in the Real/Imaginary table columns
...
nEdge   = size(this.Data.getData('xData'),1);
NewEdge = min(eventData.Source.getSelectedRows+1,nEdge);
