...
%   $Revision: 1.1.8.1 $  $Date: 2009/10/16 06:18:15 $
if ~isempty(cv.MagPoints) && strcmpi(Event,'postlim')
   ...
   if any(isne)
      ...
      if strcmp(AxGrid.XScale,'log')
         YDot = utInterp1(log(Freq(ind)),Mag(ind),log(XDot));  
      else
   ...
   if nPM>0
      ...
      if strcmp(AxGrid.XScale,'log')
         PhaseWcp = utInterp1(log(Freq(ind)),Phase(ind),log(XDot));  
      else
         PhaseWcp = utInterp1(Freq,Phase,XDot);  
      end 
   ...
   if any(isne)
      if strcmp(AxGrid.XScale,'log')
         YDot = utInterp1(log(Freq(ind)),Phase(ind),log(XDot));  
      else
