...
%   $Revision: 1.1.8.2 $  $Date: 2010/04/21 21:45:01 $
...
%---Frame
s.Frame = MWFrame(ctrlMsgUtils.message('Controllib:gui:strPropertyEditor'));
s.Frame.setLayout(MWBorderLayout(0,0));
s.Frame.setFont(Prefs.JavaFontP);
h = handle( s.Frame, 'callbackproperties');
set(h,'WindowClosingCallback',@(es,ed) localClose(this));

%---Open an empty Frame with some status text
s.Frame.setSize(java.awt.Dimension(360,280));
centerfig(s.Frame);
Status = MWLabel(ctrlMsgUtils.message('Controllib:gui:strOpeningPropertyEditor'),MWLabel.CENTER);
s.Frame.add(Status,MWBorderLayout.CENTER);
awtinvoke(s.Frame,'setVisible(Z)', true);
s.Frame.toFront;
...
%---Close
Close = MWButton(ctrlMsgUtils.message('Controllib:general:strClose')); 
ButtonPanel.add(Close);
Close.setFont(Prefs.JavaFontP);
h = handle( Close, 'callbackproperties');
set(h,'ActionPerformedCallback',@(es,ed) localClose(this));
%---Help
Help = MWButton(ctrlMsgUtils.message('Controllib:general:strHelp')); 
ButtonPanel.add(Help);
Help.setFont(Prefs.JavaFontP);
h = handle( Help, 'callbackproperties');
set(h,'ActionPerformedCallback',@(es,ed) localHelp);
%---Store handles
