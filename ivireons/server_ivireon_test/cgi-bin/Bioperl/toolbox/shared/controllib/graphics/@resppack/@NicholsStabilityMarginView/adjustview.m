...
%   $Revision: 1.1.8.1 $  $Date: 2009/10/16 06:18:40 $
...
if ~isempty(cv.MagPoints) && strcmp(Event,'postlim')
   ...
   doComparePhase = strcmp(cv.Parent.ComparePhase.Enable, 'on');
   if doComparePhase
       ax = AxGrid.getaxes; %Revisit
       h = gcr(ax(1));
       Freq = unitconv(rData.Frequency,rData.FreqUnits,h.FrequencyUnits);
       idx = find(Freq>cv.Parent.ComparePhase.Freq,1,'first');
       if isempty(idx)
           idx = 1;
       end
       % If compare Phase(idx) is nan find nearest phase which is not
       ...
       n = round(abs(Phase(idx)-cv.Parent.ComparePhase.Phase)/(2*Pi));
       Phase = Phase-sign(Phase(idx)-cv.Parent.ComparePhase.Phase)*n*2*Pi;
   end
   ...
   XDot = reshape(utInterp1(Freq,Phase,GMFrequency),size(GMFrequency));
   ...
   XDot = reshape(utInterp1(Freq,Phase,PMFrequency),size(PMFrequency));
