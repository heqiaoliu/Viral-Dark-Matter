...
%   $Revision: 1.1.8.2 $  $Date: 2010/04/21 21:45:04 $
...
Main = com.mathworks.mwt.MWGroupbox(ctrlMsgUtils.message('Controllib:gui:strTimeResponse'));
...
s.SettlingTime = com.mathworks.mwt.MWLabel(ctrlMsgUtils.message('Controllib:gui:strShowSettlingTimeLabel')); 
...
s.SettlingTime.setFont(Prefs.JavaFontP);
   ...
   s.SettlingTimeThreshold = com.mathworks.mwt.MWTextField(3); 
   s.R2E.add(s.SettlingTimeThreshold);
   s.SettlingTimeThreshold.setFont(Prefs.JavaFontP);
   s.R2EL1 = com.mathworks.mwt.MWLabel(sprintf('%%')); 
   s.R2E.add(s.R2EL1);
   s.R2EL1.setFont(Prefs.JavaFontP);

%---RiseTime
s.R3 = com.mathworks.mwt.MWPanel(com.mathworks.mwt.MWBorderLayout(0,0)); s.RC.add(s.R3);
s.RiseTime = com.mathworks.mwt.MWLabel(ctrlMsgUtils.message('Controllib:gui:strShowRiseTimeLabel')); 
s.R3.add(s.RiseTime,WEST);
s.RiseTime.setFont(Prefs.JavaFontP);
   ...
   s.R3EL1 = com.mathworks.mwt.MWLabel(ctrlMsgUtils.message('Controllib:gui:strTo')); s.R3E.add(s.R3EL1);
...
case 'SettlingTimeThreshold'
   ...
   val = evalnum(char(eventSrc.getText))/100;
   if isequal(h,val)
      %---No change: return
      return;
   elseif isempty(val)
      %---Invalid number: revert to original value
      eventSrc.setText(num2str(oldval*100));
      return;
   else
      val = max(min(val,1),0);
      eventSrc.setText(num2str(val*100));
