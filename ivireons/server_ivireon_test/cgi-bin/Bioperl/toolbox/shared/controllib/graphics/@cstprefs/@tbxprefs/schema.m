...
%   $Revision: 1.1.8.1 $  $Date: 2009/10/16 06:13:11 $
...
ComparePhase = struct( ...
    ...
    'Freq', 0, ...
    'Phase', 0);
localCreateProp(c,'ComparePhase',           'MATLAB array', ComparePhase);

%---Minimum Gain Limit
MinGainLimit = struct(...
   'Enable', 'off',...
   'MinGain', 0);
localCreateProp(c, 'MinGainLimit',          'MATLAB array', MinGainLimit);

%---PID Tuner Options
PIDTunerPreferencesFactoryValues = struct( ...
    'PhaseMargin',60,...
    'DisturbanceLocation','input',...
    'D2CMethod','zoh',...
    'TunedColor',[0,0,1],...
    'BlockColor',[0.7,0.7,0.7],...
    'DefaultDesignMode','basic',...
    'DefaultTableMode','off',...
    'DefaultPlotType','tracking',...
    'DefaultLegendMode','on',...
    'DefaultWelcomeDialog','on');
localCreateProp(c, 'PIDTunerPreferences',   'MATLAB array',  PIDTunerPreferencesFactoryValues);
