...
%   $Revision: 1.1.8.2 $  $Date: 2010/04/21 21:45:27 $
...
GridState = this.Grid;
s.Grid.setState(strcmpi(GridState,'on'));
...
s.Main = com.mathworks.mwt.MWGroupbox(ctrlMsgUtils.message('Controllib:gui:strGrid'));
s.Main.setLayout(java.awt.GridLayout(1,1,0,3));
s.Main.setFont(Prefs.JavaFontB);

%---Checkbox to toggle grid visibility
s.Grid = com.mathworks.mwt.MWCheckbox(ctrlMsgUtils.message('Controllib:gui:strShowGrid'));
...
set(hc,'ItemStateChangedCallback',@(es,ed) localWriteProp(es,ed,OptionsBox));
...
case 'Grid'
    if eventSrc.getState
        OptionsBox.Target.Grid = 'on';
    else
        OptionsBox.Target.Grid = 'off';
    end
end
