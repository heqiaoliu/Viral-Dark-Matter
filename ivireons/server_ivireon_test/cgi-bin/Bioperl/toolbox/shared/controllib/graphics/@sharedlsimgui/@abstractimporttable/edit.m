...
% $Revision: 1.1.8.1 $ $Date: 2009/10/16 06:25:35 $
...
if strcmp(h.Type,'io') % build importSelector GUI if empty
    if isempty(h.getImportSelector)
        ...
        if nargin==1
            ...
            importSelector.importhandles.importDataFrame = MJDialog(varargin{1},sprintf('Data Import'));
        end
...
elseif strcmp(h.Type,'state')
    ...
    h.getImportSelector.frame = MJDialog(frame, ...
        sprintf('Import State From Workspace'));
...
awtinvoke(importSelector.importhandles.importDataFrame,'setVisible(Z)', ...
...
if ~isempty(currentRows)
    ...
    if length(selectedvar.size)~=2 || min(selectedvar.size)>1
        msgbox(sprintf('Variable must be a vector'),sprintf('State Import'),'modal')
    end
