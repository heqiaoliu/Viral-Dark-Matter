...
%  $Revision: 1.1.8.1 $  $Date: 2009/10/16 06:23:27 $
...
if all(GridSize([1 2])==1) && ~(NoInput || NoOutput)
   ...
   if isempty(OutputNames{1}) && isempty(InputNames{1})
      ...
      InputLabels = strcat({xlate('From: ')},LocalFillName(InputNames,xlate('In')),...
         {xlate('  To: ')},LocalFillName(OutputNames,xlate('Out')));
   end
   ...
   elseif all(GridSize([1 3])==1) & isempty(OutputNames{1})
      ...
      OutputLabels = strcat({xlate('To: ')},LocalFillName(OutputNames,xlate('Out')));
   end
end
