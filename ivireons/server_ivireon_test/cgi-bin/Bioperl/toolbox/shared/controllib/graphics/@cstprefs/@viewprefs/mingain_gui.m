...
%   $Revision: 1.1.8.2 $  $Date: 2010/04/21 21:45:20 $
...
s.MinGainListener = [handle.listener(h,CLS.findprop('MinGainLimit'),'PropertyPostSet',{@localReadProp,s}); ...
    handle.listener(h,CLS.findprop('MagnitudeUnits'),'PropertyPreSet',{@LocalMagUnitsChanged s, h}); ...
    handle.listener(h,findprop(Prefs,'MagnitudeUnits'),'PropertyPreSet',{@LocalMagUnitsChanged s, h})];
s.MinGainBox.setName('MinGainBox');
MinGainBoxCB = handle( s.MinGainBox, 'callbackproperties' );
MinGainBoxCB.ItemStateChangedCallback = {@localWriteProp,h,s};

s.MinGainEdit.setName('MinGainEdit');
MinGainEditCB = handle( s.MinGainEdit, 'callbackproperties' );
MinGainEditCB.ActionPerformedCallback = {@LocalWriteMinGain,h};
MinGainEditCB.FocusLostCallback = {@LocalWriteMinGain h};
