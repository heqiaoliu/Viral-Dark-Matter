...
%   $Revision: 1.1.8.2 $  $Date: 2010/04/21 21:45:21 $
...
Main = com.mathworks.mwt.MWGroupbox(ctrlMsgUtils.message('Controllib:gui:strTimeVector'));
...
s.Manual1 = com.mathworks.mwt.MWCheckbox(ctrlMsgUtils.message('Controllib:gui:strDefineStopTime'),GRP,0); 
s.R2.add(s.Manual1);
...
case {'Stop','Vector'}
   if eventSrc.isEnabled
      oldval = get(eventData.getSource,'UserData');
      oldstr = get(eventData.getSource,'Tag');
      ...
      if isempty(newval)
         eventSrc.setText(oldstr);
      elseif ~isequal(oldval,newval)
         newstr = makestr(newval);
         eventSrc.setText(newstr)
         set(eventData.getSource,'Tag',newstr,'UserData',newval);
         h.TimeVector = newval;
      end
   end
end
