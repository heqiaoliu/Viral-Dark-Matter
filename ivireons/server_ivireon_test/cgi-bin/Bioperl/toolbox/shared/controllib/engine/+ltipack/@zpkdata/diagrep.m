function D = diagrep(D,k)
% Replicates model along the diagonal.
%
%   D = DIAGREP(D,K) forms the block-diagonal model Diag(D,...,D) with 
%   D repeated K times.

%   Copyright 1986-2010 The MathWorks, Inc.
%	 $Revision: 1.1.8.1 $  $Date: 2010/02/08 22:48:11 $
[ny,nu] = size(D.k);
z = cell(k*ny,k*nu);  z(:) = {zeros(0,1)};
p = cell(k*ny,k*nu);  p(:) = {zeros(0,1)};
k = zeros(k*ny,k*nu);
iod = zeros(k*ny,k*nu);
ix = 0; jx = 0;
for j=1:k,
   z(ix+1:ix+ny,jx+1:jx+nu) = D.z;
   p(ix+1:ix+ny,jx+1:jx+nu) = D.p;
   k(ix+1:ix+ny,jx+1:jx+nu) = D.k;
   iod(ix+1:ix+ny,jx+1:jx+nu) = D.Delay.IO;
   ix = ix+ny;  jx = jx+nu;
end
D.z = z;
D.p = p;
D.k = k;
D.Delay.Input = repmat(D.Delay.Input,[k 1]);
D.Delay.Output = repmat(D.Delay.Output,[k 1]);
D.Delay.IO = iod;
