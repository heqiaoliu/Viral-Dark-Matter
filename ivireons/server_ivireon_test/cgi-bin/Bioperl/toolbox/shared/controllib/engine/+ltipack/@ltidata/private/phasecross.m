...
%   $Revision: 1.1.8.2 $  $Date: 2010/03/26 17:49:14 $
...
if Td==0 && all(isder) && all(isint)
   if abs(mod(PhaseDC,2*pi)-pi)>pi/4
      % Constant phase is not -180
      wc = zeros(1,0);
   elseif DCdeg~=0
      % k*s^DCdeg with -180 phase: pick crossing where gain=1
      wc = abs(k)^(-1/DCdeg);  
   else
      % Constant gain k: pick any frequency
      wc = 1;
   end
   return
end
...
while ~isempty(intset)
   ...
   for wint=intset
      ...
      if abs(y2-y1)>=2*pi && x2-x1>2*eps*(1+abs(x1)+abs(x2))
