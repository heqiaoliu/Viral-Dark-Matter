...
%   $Revision: 1.1.8.2 $  $Date: 2010/03/26 17:49:12 $
...
tolsep = 2*eps*(1+abs(lnx1)+abs(lnx2)); % min x-sep

% Check if exhausting floating point resolution
if lnx2-lnx1<tolsep
   if y1*y2<=0 || abs(y1)<rtol || abs(y2)<rtol
      % RE: Y1 and Y2 could both be large if resonant mode cancellation
      % skews error estimates
      wcnew = x1;
   end
   intnew = zeros(2,0);
   ...
   elseif (xce(2)<=x1) || (xce(1)>=x2) || (ymax+eymax<rtol && (y1*y2>0 || ymax<1e3*eps))
      ...
      if LogScale
         if xce(2)-xce(1)<tolsep
            intnew = zeros(2,0); % exhausted x-resolution
         elseif xce(2)-xce(1)<(x2-x1)/2
            intnew = xce(:);
         else
            xc = (xce(1)+xce(2))/2;
         ...
         dlogxce = log(xce(2)/xce(1));
         if dlogxce<tolsep
            intnew = zeros(2,0); % exhausted x-resolution
         elseif dlogxce<(lnx2-lnx1)/2
            intnew = xce(:);
         else
            xc = sqrt(xce(1)*xce(2));
            intnew = [xce(1) xc;xc xce(2)];
