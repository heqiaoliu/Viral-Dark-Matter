function [k,r] = gainrloc(a,b,c,d,Zero,Pole,Gain,Ts)
%GAINRLOC Adaptively generates root locus gain for system.
%
%   [K,R] = GAINRLOC(A,B,C,D,ZEROS,POLES,GAIN,TS) adaptively picks 
...
%   $Revision: 1.1.8.1 $  $Date: 2009/11/09 16:30:36 $   
...
kmult = rlocmult(Zero,Pole,Gain,a,b,c,Ts);
...
if m>0 || Gain>0,
   ...
   % Estimate relevant gain range [kStart,kEnd] in (0,inf)
   % and generate initial vector of gains K
   kStart = log10(initgain(Zero,Pole,Gain,[]));
   kEnd = -log10(initgain(Pole,Zero,1/Gain,[]));
   if m>0,
      % Asymptotes for K->Inf
      kasym = getAsymptoteStart(Zero,Pole,Gain);
      kmax = max([kEnd,log10(2*kmult),log10(kasym)]);   
   ...
   ksing = -1/d;  % Gain = d
   [PoleSing,GainSing] = sszero(a,b,c,0,[],Ts);
   if GainSing==0
      % DEN = scalar * NUM
      k = [0 1];
      r = [Pole , Pole];
...
while ~Done && kinit(1)<kmax && i<200
   ...
   if any(k(i-1)==kmult) || ...
      ...
      kinit = [midK , kinit]; %#ok<AGROW>
      rinit = [genrloc(a,b,c,d,midK,Zero,Pole) , rinit]; %#ok<AGROW>
   end
...
if m>0, 
   ...
   if isfinite(klarge) && klarge>k(i-1)
      k(i) = klarge;
...
% Check if the curves joining the three sets of roots v1,v2,v3 are smooth
% enough
...
function ga = getAsymptoteStart(z,p,k)
% Estimate the gain GA where the asymptotes begin (for loop gain > GA, 
% the unbounded roots for move along straight lines)
wmax = max(abs([z;p]));
if ~any(wmax)
   wmax = 1;
end
% Using that g(s)~Gain/s^m for |s|>alpha*wmax, estimate the gain GA
% where asymptotes begin
m = length(p)-length(z);
alpha = 3;
ga = (alpha*wmax)^m/abs(k);
