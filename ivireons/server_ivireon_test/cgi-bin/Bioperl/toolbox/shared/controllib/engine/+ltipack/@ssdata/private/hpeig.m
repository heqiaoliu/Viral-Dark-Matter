...
%   $Revision: 1.1.8.1 $  $Date: 2009/11/09 16:32:17 $
...
% Balancing
[h,junk,junk,j] = aebalance([h11 h12;h21 h22],[],[],...
   blkdiag(j11,zeros(ny+nu)),'noperm','fullbal');
...
mag = max(abs(a),abs(e));
maglt = tril(mag,-2);
magld = mag(2:nx+1:end);  % diag(mag,-1);
if norm(maglt,1)==0 && ~any(magld(1:nx-2) & magld(2:nx-1))
