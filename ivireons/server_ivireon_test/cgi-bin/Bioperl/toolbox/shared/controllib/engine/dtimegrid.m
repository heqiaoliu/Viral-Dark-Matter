function n = dtimegrid(a,x0,c,d)
%DTIMEGRID  Estimates simulation horizon for discrete-time responses. 
%
%   N = DTIMEGRID(A,X0,C,D) estimates the number N of simulation
%   steps needed to reach adequate settling of the time response
%   of the affine difference equation:
%      x[k+1] = Ax[k]       x[0] = x0
%        y[k] = Cx[k] + D
%
%   DTIMEGRID attempts to produce a simulation time N that ensures the
%   output responses have decayed to approximately 1% of their initial
%   or peak values. 

%   Author: P. Gahinet, 5-7-96
%	 Copyright 1986-2004 The MathWorks, Inc.
%	 $Revision: 1.1.8.1 $  $Date: 2009/10/16 06:11:00 $
...
elseif ~isempty(rns),
   % Marginally stable
   n = 50*max(1,1/(1+max(abs(log(rns)))));  
   return
end
...
if rcond(v) > eps^(2/3),
   ...
   contrib = max(1.5*st,contrib(idom));    % so that st/contrib<1
