...
%   $Revision: 1.1.8.1 $  $Date: 2009/11/09 16:30:32 $
...
% RE: Watch out for numerics near resonant modes, see g308017
psi = atan2(1,2*s);
rho(rho==0) = eps;
alpha = 1./rho - rho;
beta = max(0,1-(alpha.*s).^2)./(1+4*s.^2);
as1 = asin(min(1,sqrt(beta)));
as2 = -as1;
as1(idxNegS) = pi - as1(idxNegS);
as2(idxNegS) = pi - as2(idxNegS);
