...
% D and RHS are @tfdata objects with the same sampling time

%   Author(s): P. Gahinet
%   Copyright 1986-2010 The MathWorks, Inc.
%	 $Revision: 1.1.8.2 $  $Date: 2010/02/08 22:48:08 $

%RE: Assumes SYS(I,J) is a subsystem of SYS (no size growing)
if isempty(rhs)
   % SYS(i,:) = [] or SYS(:,j) = []
   D.num(rowIndex,colIndex) = [];
   D.den(rowIndex,colIndex) = [];
   ...
   elseif ~isempty(rowIndex) && ~isempty(colIndex)
      ...
      % Update data
      D.num(rowIndex,colIndex) = rhs.num;
      D.den(rowIndex,colIndex) = rhs.den;
   end
end
