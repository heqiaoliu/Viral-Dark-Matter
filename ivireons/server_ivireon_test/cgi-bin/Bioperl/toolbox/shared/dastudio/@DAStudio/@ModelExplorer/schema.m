...
%   $Revision: 1.1.6.3 $  $Date: 2008/12/01 07:38:47 $
...
function sv = setSelected(h, inputV)
    ...
    if (~isempty(h.jModelExplorer))
        ...
        for i = 1:length(inputV)
            ...
            if isempty(daparent)
                error('DAStudio:ModelExplorer:ParentScopeRequired', 'All selections must have a valid parent scope');
            else
            ...
            if isempty(scope)
                ...
                if parent ~= scope
                    error('DAStudio:ModelExplorer:SameScopeRequired', 'Multiple selections must all have the same scope');
                end
            end
        end
        setScope(h, scope);
...
function sv = setTab(h, inputV)
    ...
    if (~isempty(h.jModelExplorer))
        ...
        if ~isequal(newTab, inputV)
            error('DAStudio:ModelExplorer:InvalidTab', '%s is not a valid ModelExplorer tab', inputV);
        end
    end
