...
sorttable = {
  ...
  makeSortable: function(table) {
        // Modified for difftool usage: the column headers we want
        // to sort by are in the second row, not the first.
        if (table.getElementsByTagName('thead').length == 0) {
            // table doesn't have a tHead. Since it should have, create one and
            // put the second table row in it.
            ...
            if (!headrow[i].className.match(/\bsorttable_nosort\b/)) { // skip this col
                ...
                dean_addEvent(headrow[i],"click", function(e) {
                        ...
                        for (var j=0; j<rows.length; j++) {
                            ...
                            var colcount = 0;
                            for (var q=0; q<=col; q++) {
                                thiscell = rows[j].cells[q];
                                if (colcount==col) {
                                    // This is the column we need.
                                    row_array[row_array.length] = [sorttable.getInnerText(thiscell), rows[j]];
                                    break;
                                } else if (colcount>col) {
                                    // We've skipped the column we need.  Insert an empty entry.
                                    row_array[row_array.length] = ["", rows[j]]; // empty cell
                                    break;
                                }
                                colcount += thiscell.colSpan;
                            }
                        }
