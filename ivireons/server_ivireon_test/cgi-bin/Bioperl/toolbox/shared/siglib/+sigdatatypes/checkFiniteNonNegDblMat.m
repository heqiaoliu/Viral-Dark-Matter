function checkFiniteNonNegDblMat(h, prop, value, reqSize)
...
%   $Revision: 1.1.6.1 $  $Date: 2008/03/31 17:10:55 $
...
if  (m~=reqSize(1)) || (n~=reqSize(2)) || ~isa(value, 'double') ...
    ...
    if ischar(h)
        msg = sprintf(['The %s input argument of %s must be a finite '...
            'non-negative double %s of size %dx%d.'], prop, h, type, ...
            reqSize(1), reqSize(2));
    else
        msg = sprintf(['The ''%s'' property of ''%s'' must be a finite '...
            'non-negative double %s of size %dx%d.'], prop, class(h), type, ...
             reqSize(1), reqSize(2));
    end
