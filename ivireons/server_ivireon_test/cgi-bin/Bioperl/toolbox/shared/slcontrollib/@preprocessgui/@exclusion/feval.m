...
% $Revision: 1.1.6.3 $ $Date: 2007/11/09 21:02:06 $
...
if boundsactive
    ...
    if ~isempty(h.Ylow) && (length(h.Ylow)~=1 && length(h.Ylow)~=s(2))
      ctrlMsgUtils.error( 'SLControllib:preprocessgui:ExclusionYLowBounds' );
    end
    if ~isempty(h.Yhigh) && (length(h.Yhigh)~=1 && length(h.Yhigh)~=s(2))
      ctrlMsgUtils.error( 'SLControllib:preprocessgui:ExclusionYHighBounds' );
    end
    if (~isempty(h.Xlow) && ~isscalar(h.Xlow)) && (~isempty(h.Xhigh) && ~isscalar(h.Xhigh))
      ctrlMsgUtils.error( 'SLControllib:preprocessgui:ExclusionXBounds' );
    end
    % Bounds comparison
    if ~isempty(h.Xlow)
