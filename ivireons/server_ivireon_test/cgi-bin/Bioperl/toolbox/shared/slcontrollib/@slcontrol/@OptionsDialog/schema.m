...
% $Revision: 1.1.6.6 $ $Date: 2008/03/13 17:38:41 $
...
p = schema.prop( c, 'showSimOptions', 'MATLAB array' );
p.FactoryValue = true;
p.AccessFlags.PublicSet = 'off';
...
p = schema.prop( c, 'showOptimOptions', 'MATLAB array' );
p.FactoryValue = true;
p.AccessFlags.PublicSet = 'off';
p.AccessFlags.PublicGet = 'off';

p = schema.prop( c, 'showParallelOptions', 'MATLAB array' );
p.FactoryValue = false;
p.AccessFlags.PublicSet = 'off';
p.AccessFlags.PublicGet = 'off';

p = schema.prop(c,'RunDependencyCheckerCallback', 'MATLAB array');
p.FactoryValue = {};
...
p.FactoryValue =  { 'VariableStepDiscrete', 'ode45', 'ode23', 'ode113', ...
...
p.FactoryValue = { 'FixedStepDiscrete', 'ode5', 'ode4', 'ode3', ...
                   'ode2', 'ode1', 'ode14x' };
p.AccessFlags.PublicSet = 'off';
p.Description = 'Names of fixed-step solvers from simset structure';

% Events
p = schema.event( c, 'OptionsChanged' );
