...
% $Revision: 1.1.6.1 $ $Date: 2007/04/25 03:19:46 $

% R2007b move opcond.StateReportSimMech to opcond.StateReport objects
if this.Version < 2.0
    states = this.States(:);
    for ct = numel(states):-1:1
        if isa(states(ct),'opcond.StateReportSimMech')
            newstates = handle(NaN(states(ct).Nx,1));
            for ct2 = 1:states(ct).Nx
                newstates(ct2) = opcond.StateReport;
                ...
                newstates(ct2).dx = states(ct).dx(ct2);
