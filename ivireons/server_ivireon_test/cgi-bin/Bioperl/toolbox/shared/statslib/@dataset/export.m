...
%   $Revision: 1.1.6.7 $  $Date: 2010/02/08 22:53:53 $
%   SAS is a registered trademarks of SAS Institute Inc.

pnames = {'file' 'xlsfile' 'xptfile' 'writevarnames' 'writeobsnames' };
dflts =  {   []        []      []      true            true  };
[eid,errmsg,fileArg,xlsfileArg,xptfileArg,writevarnames,writeobsnames,otherArgs] = ...
    dataset.getargs(pnames, dflts, varargin{:});
if ~isempty(eid)
    error(sprintf('stats:dataset:export:%s',eid),errmsg);
end

if ~isscalar(writevarnames) || ...
                ~(islogical(writevarnames) || isnumeric(writevarnames))
    error('stats:dataset:export:InvalidWriteVarNames', ...
          'WRITEVARNAMES must be TRUE or FALSE.');
end
if ~isscalar(writeobsnames) || ...
                ~(islogical(writeobsnames) || isnumeric(writeobsnames))
    error('stats:dataset:export:InvalidWriteObsNames', ...
          'WRITEOBSNAMES must be TRUE or FALSE.');
end

% Only write obsnames if asked to, and if they exist.
writeobsnames = (writeobsnames && ~isempty(a.obsnames));

if ~isempty(fileArg) || (isempty(xlsfileArg) && isempty(xptfileArg))
...
elseif ~isempty(xlsfileArg)
    ...
    xptwrite(a,xptfileArg,'WriteObsNames',writeobsnames,otherArgs{:});
end
...
otherwise
  error('stats:dataset:export:UnrecognizedDelimiter', ...
        'Unrecognized delimiter character: ''%c''.',delimiter(1));
end
...
pnames = {'sheet' 'range'};
dflts =  {     1     'A1'};
[eid,errmsg,sheet,range] = dataset.getargs(pnames, dflts, varargin{:});
if ~isempty(eid)
    error(sprintf('stats:dataset:export:%s',eid),errmsg);
end
