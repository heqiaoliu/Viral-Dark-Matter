...
%      'RightKeys'  - specifies the variables to use as kesy in B.
%
...
if nargin < 4
    ...
    dflts =  {'simple'    []         []         []           []         []          []};
    [eid,errmsg,type,key,leftkey,rightkey,mergeKeys,leftvars,rightvars] ...
...
if isempty(mergeKeys)
    mergeKeys = false;
elseif ~(islogical(mergeKeys) && isscalar(mergeKeys))
    error('stats:dataset:join:InvalidMergeKeys', ...
          'The ''MergeKeys'' parameter must be true or false.');
end
...
if length(leftkey) ~= length(rightkey)
    error('stats:dataset:join:UnequalNumKeyVars', ...
          'The number of key variables on the left and right must be the same.');
end
...
if mergeKeys
    ...
    [~,~,mergedkey] = intersect(leftkey,leftvars);
end
...
for j = 1:nkeys
    if size(leftkeyvals{j},2) ~= size(rightkeyvals{j},2) % already know these are 2-D
         error('stats:dataset:join:KeyVarSizeMismatch', ...
               'Left and right key variables ''%s'' and ''%s'' do not have the same number of columns.', ...
               leftkeynames{j},rightkeynames{j});
    elseif iscell(leftkeyvals{j}) ~= iscell(rightkeyvals{j})
         error('stats:dataset:join:KeyVarCellMismatch', ...
               'Left and right key variables ''%s'' and ''%s'' are not comparable because one is a non-cell.', ...
                leftkeynames{j},rightkeynames{j});
    end
    try
        lrkey_j = [leftkeyvals{j}; rightkeyvals{j}];
    catch me
         error('stats:dataset:join:KeyVarTypeMismatch', ...
               'Left and right key variables ''%s'' and ''%s'' have incompatible types.', ...
               leftkeynames{j},rightkeynames{j});
    end
    if size(lrkey_j,2) > 1
        ...
        try
            ...
            if strcmp(me.identifier,'MATLAB:CELL:UNIQUE:InputClass')
                error('stats:dataset:join:KeyVarUniqueError', ...
                      'Left and right key variables ''%s'' and ''%s'' include cells containing non-string values.', ...
                      leftkeynames{j},rightkeynames{j});
            else
                error('stats:dataset:join:KeyVarUniqueError', ...
                      'Error when finding unique values for left and right key variables ''%s'' and ''%s'':\n%s.', ...
                      leftkeynames{j},rightkeynames{j},me.message);
            end
        end
    end
end
...
if simpleJoin
    ...
    % A are just 1:n.
    ir = simplejoin(leftkeyvals,rightkeyvals);
    ...
    if nargout > 1
        if nargout > 2
            error('stats:dataset:join:TooManyOutputs', ...
                  'Too many outputs.');
        end
...
function rindex = simplejoin(lkey,rkey)
...
if isempty(i)
    error('stats:dataset:join:InvalidType', ...
          'TYPE must be ''inner'', ''leftouter'', ''rightouter'', or ''fullouter''.');
end
leftOuter = (i == 2) || (i >= 4);
rightOuter = (i >= 3);
...
elseif isa(x,'categorical')
    y = x(1:0);
    y(1:n,p) = char(''); % literal '' treated as "delete on assignment"
elseif iscell(x)
    if iscellstr(x)
        y = repmat({''},sz);
    else
