...
%   $Revision: 1.1.6.4 $  $Date: 2009/10/10 20:11:00 $

% Since [] is a valid value for groupvars, cannot use it as a default to
% detect whether the arg was passed in or not.  For now, do this.
pnames = {'groupvars' 'constvars' 'newdatavarnames' 'aggregationfun'};
dflts =  {       -99          []                []               [] };

[eid,errmsg,groupVars,constVars,wideVarNames,fun] ...
    = dataset.getargs(pnames,dflts,varargin{:});
if ~isempty(eid)
    error(sprintf('stats:dataset:unstack:%s',eid),errmsg);
end
...
if ~isscalar(indicatorVar)
    error('stats:dataset:unstack:MultipleDataIndVar','DATAINDVAR must be a single variable.');
end

if ~isempty(fun) && ~isa(fun,'function_handle')
    error('stats:dataset:unstack:InvalidAggregationFun','AGGREGATIONFUN must be a function handle.');
end
...
if isequal(groupVars,-99)
    ...
    if ~isempty(intersect(groupVars,dataVars))
        error('stats:dataset:unstack:ConflictingGroupAndDataVars','GROUPVARS and DATAVAR must not have variables in common.');
    end
end

% indicatorVar must not appear in groupVars or dataVars.
if ismember(indicatorVar,groupVars) || ismember(indicatorVar,dataVars)
    error('stats:dataset:unstack:ConflictingIndVar','INDVAR must not be included in GROUPVARS or DATAVAR.');
end
...
if ~isempty(constVars)
    ...
    if any(ismember(constVars,groupVars)) || any(ismember(constVars,dataVars)) || any(ismember(constVars,indicatorVar))
        error('stats:dataset:unstack:ConflictingConstVars','CONSTVARS must not have variables in common GROUPVARS, INDVAR, or DATAVAR.');
    end
end
...
useDfltWideVarNames = isempty(wideVarNames);
if useDfltWideVarNames
    wideVarNames = genvalidnames(dfltWideVarNames(:)');
end
...
for t = 1:ntallVars
    ...
    if isnumeric(tallVar) || islogical(tallVar) % but not char
        ...
        if isempty(fun)
            ...
            if ~(isnumeric(funVal) || islogical(funVal))
                error('stats:dataset:unstack:BadAggFunValueClass', ...
                      ['AGGREGATIONFUN must return a numeric or logical value ' ...
                       'that can be be cast to the class of the data variable ''%s''.'], ...
                      aNames{dataVars(t)});
            elseif isfloat(tallVar) && isfloat(funVal)
                fillVal = NaN(class(funVal));
            else % isinteger(tallVar) || islogical(tallVar)
                fillVal = cast(zeros,class(funVal));
            end
    ...
    % Handle non-numeric types indirectly
        ...
        if isempty(fun)
            ...
            if any(cellCounts(:) > 1)
                error('stats:dataset:unstack:MultipleObs',...
                      'You must specify AGGREGATIONFUN for multiple observations in non-numeric data.');
            end
                ...
                for i = 1:nobsWide
                    ...
                    try
                        ...
                        if size(val,1) > 1
                            ...
                            error('stats:dataset:unstack:NonscalarAggFunValue', ...
                        ...
                        elseif ~isequal(size(val),szFunOut)
                            ...
                            error('stats:dataset:unstack:BadAggFunValueSize', ...
                                  'AGGREGATIONFUN must return a value whose size conforms to elements of the data variable ''%s''.', ...
                                  aNames{dataVars(t)});
                        else
                            throw(addCause(MException('stats:dataset:unstack:AssignmentError', ...
                                  'Error when assigning output of aggregation function to dataset variable ''%s''.', ...
                                  aNames{dataVars(t)}),ME));
                        end
                    end
                end
    ...
    if useDfltWideVarNames
        uniqueNames = genuniquenames([b.varnames wideNames],j0+1);
        wideNames = uniqueNames((j0+1):end);
    end
    ...
    try
        ...
        if isequal(me.identifier,'stats:dataset:setvarnames:DuplicateVarnames') ...
            ...
            error('stats:dataset:unstack:ConflictingNewDataVarNames','NEWDATAVARNAMES conflicts with variables in CONSTVARS.');
        else
            rethrow(me);
        end
    end
