...
%   $Revision: 1.1.6.2 $  $Date: 2009/05/07 18:27:40 $
...
pnames = {'writeobsnames'};
dflts =  { true };
[errid,errmsg,writeObsFlag] = ...
    dataset.getargs(pnames, dflts, varargin{:});
if ~isempty(errid)
    error(sprintf('stats:xptwrite:%s',errid), errmsg);
end

if ~isa(data,'dataset')
    error('xptwrite:dataNotDataset',...
        'DATA must be a dataset array.');
end
...
blanks8 = blanks(8);
[fid,message] = fopen(filename,'wt','b');
if fid == -1
    error('stats:xptwrite:fileopenerror',...
...
nextHeaderLine = [blanks(64),createdString];
fwrite(fid,nextHeaderLine,'char');
...
% VAR block
nextHeaderLine = ...
    ['HEADER RECORD*******MEMBER  HEADER RECORD!!!!!!!000000000000000001600000000',num2str(blockSize) '  '];
fwrite(fid,nextHeaderLine,'char');
...
dataSetName = padString(upper(inputname(1)),8,true);
nextHeaderLine = [sasString,dataSetName,blanks8,sasVersion,OSString,blanks(24),createdString];
fwrite(fid,nextHeaderLine,'char');
...
for count = 1:numVariables
    ...
    theClass = class(data.data{count});
    varTypes{count} = theClass;
    switch theClass
        case {'cell'}
            ntype = 2;
            
            if ~iscellstr(data.data{count})
                fclose(fid);
                delete(filename);
                error('stats:xptwrite:CellVar',...
                    'Cannot export cell-valued variable %s; only cell arrays of strings supported.',...
                    theVars{count}) 
            end
            nlng = max(cellfun(@length,data.data{count}));
            charLengths(count) = nlng;
            varTypes{count} = 'char';
            isVarChar(count) = true;
        case {'nominal','ordinal'}
            ntype = 2;
            nlng =  max(cellfun(@length,getlabels(data.data{count})));
            charLengths(count) = nlng;
            varTypes{count} = 'char';
            isVarChar(count) = true;
        case {'int8','uint8'}
    ...
    if ~strcmp(theXPTVars{count},theVars{count})
        warning('stats:xptwrite:truncatedVarName',...
            'The variable name ''%s'' was truncated to ''%s'' as the XPT format only allows 8 character long variable names.',...
            theVars{count}, theXPTVars{count})
    end
...
fwrite(fid,blanks(rem(numVariables*140,80)),'char');
    ...
    for inner = 1:numVariables
        if isVarChar(inner)
            ...
            if ~isscalar(theVal)
                fclose(fid);
                delete(filename);
                error('stats:xptwrite:obsNotScalar',...
                    'The variable %s contains non-scalar observations.',...
                    theVars{inner})
            end
...
try
    ...
    delete(filename);
    if strLen>paddedLen
        error('stats:xptwrite:padString:strTooLong','String to be padded is too long.');
    else
...
if numel(sortedNames) ~= numNames
    ...
    fclose(fid);
    delete(filename);
    error('stats:xptwrite:truncateVarNames:CannotTruncateNames',...
        'Unable to create unique variable names by truncation. Please rename the variables using fewer than 8 characters per name.')
end
