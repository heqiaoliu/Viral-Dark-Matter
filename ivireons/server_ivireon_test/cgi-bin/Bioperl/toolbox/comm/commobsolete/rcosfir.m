...
%   $Revision: 1.1.6.1 $   $Date: 2009/04/21 03:05:45 $

%routine check
if nargin < 1
    error('comm:rcosfir:NotEnoughInputs','Not enough input variables for RCOSFIR')
elseif nargin < 2
...
if ~isscalar(r) || (r < 0) || (r > 1) || ~isreal(r)
    error('comm:rcosfir:InvalidR','The Rolloff factor in RCOSFIR must be a positive integer in the range, [0, 1].')
end;
...
if ~isscalar(rate) || (rate <= 1) || (ceil(rate) ~= rate)
    error('comm:rcosfir:InvalidRate','RATE in RCOSFIR must be an integer greater than 1')
end
