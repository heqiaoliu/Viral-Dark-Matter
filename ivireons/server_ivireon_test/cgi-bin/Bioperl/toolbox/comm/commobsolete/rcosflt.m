...
%
%WARNING: This is an obsolete function and will be removed in the future.
%         Please use the fdesign.pulseshaping instead.
%
...
%   $Revision: 1.1.6.2 $  $Date: 2009/05/23 07:50:15 $

%Too few input variables.
if nargin < 3
    error('comm:rcosflt:NotEnoughInputs','Too few input variables for RCOSFLT.')
end;
...
elseif ~ischar(type_flag) && ~isempty(type_flag)
    error('comm:rcosflt:InvalidType_Flag','TYPE_FLAG in RCOSFLT must be a string.');
end;
...
if ~filter_flag
    % 'filter' flag is not used.  Following arguments, if exist, are rolloff
    % factor, delay, and tolerance.
    %default rolloff factor
    if nargin < 5
        R = .5;
    elseif R < 0
        error('comm:rcosflt:NegativeRolloff','The rolloff factor in RCOSFLT cannot be a negative number.')
    elseif R > 1
        error('comm:rcosflt:InvalidRollOff','The rolloff factor in RCOSFLT cannot be greater than 1.')
    end;
    ...
    elseif isempty(Delay)
        Delay = 3;
    elseif Delay <= 0
        error('comm:rcosflt:InvalidDelay','DELAY must be a positive integer in RCOSFLT.')
    elseif ceil(Delay) ~= Delay
        error('comm:rcosflt:NonIntegerDelay','DELAY in RCOSFLT must be an integer.')
    end;
...
FsDFd = Fs/Fd;
if ceil(FsDFd) ~= FsDFd
    error('comm:rcosflt:InvalidRatio','Fs/Fd must be an integer.')
end;
...
if findstr(type_flag, 'fir')
    ...
    if filter_flag
        ...
        if nargin < 5
            error('comm:rcosflt:MissingNum','Too few input variables. ''NUM'' must be defined for the FIR filter')
        else
            num = R;
        end
...
elseif findstr(type_flag, 'iir')
    ...
    if filter_flag
        ...
        if nargin < 6
            error('comm:rcosflt:MissingNumAndDen','Too few input variables. ''NUM'' and ''DEN'' must be defined for the IIR filter')
        else
            num = R;
            den = Delay;
        end
