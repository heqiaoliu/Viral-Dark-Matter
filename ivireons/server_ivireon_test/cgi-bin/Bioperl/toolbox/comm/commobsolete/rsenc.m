...
%         Please use the FEC.RSENC object instead.
%
%
...
% $Revision: 1.1.6.1 $  $Date: 2007/08/20 16:22:35 $ 
...
if ~isgfobject(msg)
   error('comm:rsenc:MsgNonGaloisArray','MSG must be a Galois array.');
end
if isempty(N) || ~isnumeric(N) || ~isscalar(N) || ~isreal(N) || N~=floor(N) || N<3
    error('comm:rsenc:InvalidN','N must be a real integer scalar equal to or larger than 3.');
end
if isempty(K) || ~isnumeric(K) || ~isscalar(K) || ~isreal(K) || K~=floor(K) || K<1
    error('comm:rsenc:InvalidK','K must be a real positive integer scalar.');
end
...
% --- msg
if isempty(msg.x)
    error('comm:rsenc:MsgEmptyGaloisArray','MSG must be a nonempty Galois array.');
end;

if N > 65535,
    error('comm:rsenc:InvalidNVal','N must be between 3 and 65535.');
end

% --- width of msg
if K ~= n_msg
    error('comm:rsenc:InvalidMsgForm','MSG must be either a K-element row vector or a matrix with K columns.');
end

% --- msg.m and its relationship with N
if M < 3
    error('comm:rsenc:InvalidMsgSymbols','Symbols in MSG must all have 3 or more bits.');
end
if N > 2^M-1
    error('comm:rsenc:InvalidNVal','N must not be greater than (2^MSG.m-1).');
end

% --- t
if floor(t)~=t || t<1,
    error('comm:rsenc:InvalidNKDiff','N and K must differ by a positive even integer.');
end
...
if ~isempty(varargin)
    ...
    if isempty(varargin{1}) 
        if(~isnumeric(varargin{1}))
            error('comm:rsenc:InvalidDefaultGenPoly','The default generator polynomial should be marked by [].');
        end;
        varargin{1} = genpoly;
    end;
    ...
    for num=1:nvarargin,
        ...
        elseif(ischar(varargin{num}))
            ...
            error('comm:rsenc:InvalidOptionalArg','Only strings and Galois vectors are allowed as optional parameters.');
        end;
    end;
    ...
    case 1
        if(objArg == [1])
            ...
            error('comm:rsenc:InvalidSyntax','Illegal syntax.  To specify GENPOLY, it must be provided as the third argument.')
        end;
        
    otherwise
        error('comm:rsenc:InvalidSyntax','Illegal syntax.No more than one Galois vector is allowed in the optional parameter list.');
    end;
    ...
    for num=1:length(strArg)
        ...
        case {'end' 'beginning'}
            if(~parityposSet)
                ...
                error('comm:rsenc:ParityPosSetOnce','The parity position must only be set once.');
            end;
        otherwise
            error('comm:rsenc:InvalidOption','Unknown option passed in.');
        end;
    end;
...
if genpolySet 
    if ~isequal(msg.prim_poly,genpoly.prim_poly)
        error('comm:rsenc:InvalidMsgGenPoly','MSG and GENPOLY must be Galois arrays with the same prim_poly field.');
    end
    
    if ~isequal(M,genpoly.m)
        error('comm:rsenc:InvalidMsgGenPoly','MSG and GENPOLY must be Galois arrays with the same m field.');
    end

    if ~isgfvector(genpoly)
        error('comm:rsenc:InvalidGenPolyForm','The generator polynomial must be a Galois row vector.');
    end
    
    if ~isequal(length(genpoly),T2+1)
        error('comm:rsenc:InvalidGenPolyDegree','The generator polynomial must be of degree (N-K).');
    end

    [b bEcode] = genpoly2b(genpoly(:)', genpoly.m, genpoly.prim_poly);
    if bEcode
        if isequal(bEcode,2)
            error('comm:rsenc:GenPolyNotMonic','The generator polynomial must be monic.')
        else
            error('comm:rsenc:InvalidGenPoly','The generator polynomial must be the product\n (X+alpha^b)*(X+alpha^(b+1))*...*(X+alpha^(b+N-K-1)), where b is an integer.');
        end
    end
end    
