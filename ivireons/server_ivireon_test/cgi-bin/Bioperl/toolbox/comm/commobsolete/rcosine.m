...
if nargin < 5
    Delay = 3;
elseif Delay <= 0
    error('comm:rcosine:InvalidDelay','DELAY must be a positive integer in RCOSINE.')
elseif ceil(Delay) ~= Delay
    error('comm:rcosine:NonIntegerDelay','DELAY in RCOSINE must be an integer.')
end;

%default rolloff factor
if nargin < 4
    R = .5;
elseif isempty(R) || (R < 0) || (R > 1) || ~isreal(R)
    error('comm:rcosine:InvalidR','The Rolloff factor, R, in RCOSINE must be a real number in the range, [0, 1].')
end;
...
elseif ~ischar(type_flag) && ~isempty(type_flag)
    error('comm:rcosine:InvalidType_Flag','TYPE_FLAG in RCOSINE must be a string.');
end;

%not enough input variables.
if nargin < 2
    error('comm:rcosine:notEnoughInputs','Not enough input variables for RCOSINE.')
end;
...
if (ceil(FsDFd) ~= FsDFd) || (FsDFd <= 1) || ~isreal(FsDFd)
    error('comm:rcosine:InvalidRatio','Fs/Fd in RCOSINE must be an integer greater than 1.')
end;
