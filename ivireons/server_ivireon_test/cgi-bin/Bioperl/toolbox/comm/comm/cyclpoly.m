...
%   $Revision: 1.10.2.2 $ $Date: 2007/08/03 21:17:24 $

error(nargchk(2,3,nargin,'struct'));

if (floor(n) ~= n) || (n<1)
    error('comm:cyclpoly:InvalidN','The codeword length must be a positive integer greater than 0.');
end

if (floor(k) ~= k) || (k<1)
    error('comm:cyclpoly:InvalidK','The message length must be a positive integer greater than 0.');
end
...
if m < 0
    error('comm:cyclpoly:NLessThanK','Message length cannot be shorter than code word length.');
elseif m == 0
    pr = 1;
elseif m == 1
    pr = [1 1];
...
if isempty(pr)
    warning('comm:cyclpoly:NoCycGenPolyFound',...
        'No cyclic generator polynomial satisfies the given constraints.');
end;
