...
%   $Revision: 1.1.6.2 $  $Date: 2007/09/14 15:58:04 $
...
if ~isnumeric(x)
    error('comm:channel:buffer:buffer_update:inputNumeric', ...
...
LB = h.BufferSize;
KR = h.DownsampleFactor;  % Downsampling not yet supported.
...
if nOverLength>0
   ...
   idxNext = nOverLength;
   if idxNext>LB
       error('comm:channel:buffer:buffer_update:BufferOverflow','Input signal too long for buffer.');
   end
