...
%   $Revision: 1.1.6.6 $  $Date: 2008/04/28 03:08:59 $

if (Ts<=0.0)
    error('comm:channel_intfiltgaussian_setrates:ts', ...
        'Output sample period must be greater than zero.');
end

if (fc<0.0)
    error('comm:channel_intfiltgaussian_setrates:fc', ...
...
for i_fc = 1:length(fc)
    [KI(i_fc,:), N(i_fc,1)] = intfiltgaussian_intfactor(Ts, fc(i_fc), NTarget, h.CutoffFrequencyName{i_fc});
end
