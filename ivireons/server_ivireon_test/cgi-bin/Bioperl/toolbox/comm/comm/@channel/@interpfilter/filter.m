...
%   $Revision: 1.1.6.2 $  $Date: 2004/12/10 19:19:44 $

% If first call, load up filter to initialize interpolation.
if f.NumSamplesProcessed==0
   x = generateoutput(s, f.SubfilterLength); 
   reset(f, x);
end

% Linear interpolation factor.
KI = f.LinearInterpFactor;
...
if KI==1
    ...
    prevOutputs = f.LastFilterOutputs(:, end-idx);
...
y = zeros(f.NumChannels, NS); 
x = zeros(f.NumChannels, 0);
...
f.FilterInputState = u.';

% Update polyphase filter phase.
% The additional -1 and +1 are because phase is 1-based,
% but mod is 0-based.
f.FilterPhase = mod(startPhase+(NS-1) - 1, R) + 1;

% Store last *two* outputs for each path. 
% Needed for linear interpolation.
if NS==1
    f.LastFilterOutputs = [f.LastFilterOutputs(:, end) y];
else
    f.LastFilterOutputs = y(:, end-[1 0]);
end
