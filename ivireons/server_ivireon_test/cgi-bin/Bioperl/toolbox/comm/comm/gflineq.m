...
%   $Revision: 1.13.4.6 $   $Date: 2009/03/30 23:24:08 $                
...
elseif ( isempty(p) || numel(p)~=1 || abs(p)~=p || floor(p)~=p || ~isprime(p) )
    error('comm:gflineq:InvalidP','The field parameter P must be a positive prime integer.');
end;
...
if ( isempty(a) || ndims(a) > 2 )
    error('comm:gflineq:InvalidA','A must be a two-dimensional matrix.');
end
if ( isempty(b) || ndims(b) > 2 || n_b > 1 )
    error('comm:gflineq:InvalidB','B must be a column vector.');
end
if ( m_a ~= m_b )
    error('comm:gflineq:InvalidAB','A and B must have the same number of rows.');
end
if ( any(any( abs(a)~=a | floor(a)~=a | a>=p )) )
    if ( p == 2 )
        error('comm:gflineq:InvalidAForP2','Entries in A must be either 0 or 1 for P=2.');
    else
        error('comm:gflineq:InvalidAElements','Entries in A must be real nonnegative integers between 0 and P-1.');
    end
end
if ( any(any( abs(b)~=b | floor(b)~=b | b>=p )) )
    if ( p == 2 )
        error('comm:gflineq:InvalidBForP2','Entries in B must be either 0 or 1 for P=2.');
    else
        error('comm:gflineq:InvalidBElements','Entries in B must be real nonnegative integers between 0 and P-1.');
    end
end    
