...
%  $Revision: 1.1.6.5 $ $Date: 2005/12/12 23:13:15 $
...
if isa(data,'gf')
    ...
    data = double(gf_data);  % Convert gf_data from uint32 to double
    input_data = data;
end
    ...
    for i = 1:size(varargin{1}.value,2)   
        ...
        if ~isnumeric(orig_data)
            ...
            elseif isa(varargin{1}.value{i},'gf')
                ...
                varargin{1}.value{i} = double(gf_state.value{i});   % Convert INIT_STATE from uint32 to double
            end
        end
    end      
