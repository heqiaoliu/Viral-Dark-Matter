...
%
%   See also BERCODING, ISCATASTROPHIC, ISTRELLIS, POLY2TRELLIS.

%   Copyright 1996-2010 The MathWorks, Inc.
%   $Revision: 1.1.6.6 $  $Date: 2010/01/25 21:28:15 $
...
if istrellis(varargin{1}),
    ...
    error('comm:distspec:InvalidFirstArg','The first input to DISTSPEC must be a valid trellis. See ISTRELLIS for more information.');
end

%
% Check to see if the code specified by the trellis is catastrophic
%
if iscatastrophic(trellis)
    error('comm:distspec:InvalidTrellis','The specified trellis corresponds to a catastrophic code. ');
end
...
if nargin == 2,
    Nterms = varargin{2};
    if (~isscalar(Nterms) || ~isnumeric(Nterms) || ~is(Nterms, 'integer') || Nterms<1)
        error('comm:distspec:InvalidSecondArg','The second input to DISTSPEC must be a positive integer scalar value.');
    end
else
    Nterms = 1;
end
...
weightTable = sum(de2bi(0:(maxbits-1)), 2);
...
while isempty(spect.dfree);
    spect = tree_search(dtest, numInputs, nextStates, outputs, weightTable, distTable);
    dtest = dtest + 1;
end
...
%
spect = tree_search(spect.dfree+Nterms-1, numInputs, nextStates, outputs, weightTable, distTable);
return

%%%%%%%%%%%% end of main body of DISTSPEC function %%%%%%%%%%%%


function spect = tree_search(dMax, numInputs, nextStates, outputs, weightTable, distTable)
