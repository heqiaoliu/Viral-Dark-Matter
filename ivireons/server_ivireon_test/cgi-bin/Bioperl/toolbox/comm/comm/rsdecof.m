...
if ~(ischar(file_in) && ischar(file_out))
    error('comm:rsdecof:InvalidFileString','FILE_IN and FILE_OUT must be strings.')
end
if nargin < 3
...
fid = fopen(file_in, 'r');
if fid == -1
    errID1 = 'comm:rsdecof:CannotOpenFile';
    error(errID1,sprintf('Cannot open file %s.', file_in));
end
code = fread(fid, 'uchar');
if fclose(fid) ~= 0
    errID2 = 'com:rsdecof:CannotCloseFile';
    error(errID2,sprintf('Cannot close file %s.', file_in));
end
...
if fid == -1
    errID3 = 'comm:rsdecof:CannotOpenFile';
    error(errID3,sprintf('Cannot open file %s.', file_out));
end
fwrite(fid, msg(1:codeRow*K-padded), 'uchar');
if fclose(fid) ~= 0
    errID4 = 'com:rsdecof:CannotCloseFile';
    error(errID4,sprintf('Cannot close file %s.', file_out));
end
