...
%   $Revision: 1.3.4.2 $ $Date: 2008/08/01 12:17:27 $
...
if (length(find(size(seq)==1)) ~= 1)
    eStr.eID  = 'comm:arithenco:InvalidSymbolSequenceParameter';
    eStr.emsg =  'The symbol sequence parameter must be a vector of positive finite integers.';
    eStr.ecode = 1; return;    
end
...
if max(seq) > length(counts)
    eStr.eID  =  'comm:arithenco:InvalidSymbolSequenceParameter';
    eStr.emsg =  'The symbol sequence parameter can take values only between 1 and the length of the symbol counts parameter.';
    eStr.ecode = 1; return;    
end
