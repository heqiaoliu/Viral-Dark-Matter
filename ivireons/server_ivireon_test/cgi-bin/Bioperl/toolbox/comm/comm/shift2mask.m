...
%   $Revision: 1.2.4.4 $  $Date: 2008/08/01 12:17:42 $
...
if isscalar(prpoly)
    if (prpoly < 3) % => [1 1]
        error('comm:shift2mask:InvalidPrpolyParameter','Invalid primitive polynomial parameter specified.');
    end
...
if ( ~( isvector(prpoly) && ~isempty(prpoly) && ~isscalar(prpoly) ) | ...
    ...
    error('comm:shift2mask:InvalidPrpoly','The primitive polynomial must be a binary vector of coefficients in descending powers.');
end

% Check for shift argument
if ( ~isscalar(shift) | ~isreal(shift) | floor(shift) ~= shift )
    error('comm:shift2mask:ShiftNotScalarInt','The shift parameter must be a scalar real integer.');
end

% Compute degree/order of polynomial
ord = length(prpoly)-1;
if ord > 53
    error('comm:shift2mask:OrderGreaterThan53','Only primitive polynomials of degree 53 or less are allowed.');
end
