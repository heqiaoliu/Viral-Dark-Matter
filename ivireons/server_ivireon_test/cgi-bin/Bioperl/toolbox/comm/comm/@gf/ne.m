...
%    $Revision: 1.3.4.1 $  $Date: 2007/09/14 15:59:03 $ 


if ~isa(x,'gf'), x = gf(x,y.m,y.prim_poly); end
if ~isa(y,'gf'), y = gf(y,x.m,x.prim_poly); end
if x.m~=y.m
  error('comm:gf_ne:MismatchOrders','Orders must match.')
elseif x.prim_poly~=y.prim_poly
  error('comm:gf_ne:MismatchPrim_Poly','Primitive polynomials must match.')
end
c = x.x ~= y.x;
