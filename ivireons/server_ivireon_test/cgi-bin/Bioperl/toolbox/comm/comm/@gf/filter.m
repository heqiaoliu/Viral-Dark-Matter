...
%   $Revision: 1.6.2.2 $  $Date: 2009/07/27 20:09:34 $
...
if (B.m~=A.m | B.m~= X.m)
    error('comm:gf_filter:MismatchOrder','Orders must match.')
elseif( B.prim_poly~=A.prim_poly | B.prim_poly ~= X.prim_poly)
    error('comm:gf_filter:MismatchPrim_Poly','Primitive polynomials must match.')
end

% a,b,x must be vectors
if ((size(X,1)>1 & size(X,2)>1) | length(size(X)) == 3  )
    error('comm:gf_filter:XNotAVector','X must be a vector.')
end

if ((size(A,1)>1 & size(A,2)>1) | length(size(A)) == 3  )
    error('comm:gf_filter:ANotAVector','A must be a vector.')
end

if ((size(B,1)>1 & size(B,2)>1) | length(size(B)) == 3  )
    error('comm:gf_filter:BNotAVector','B must be a vector.')
end

% Check that the first value of A is not zero
if(A(1)==0)
    error('comm:gf_filter:ZeroFirstElement','The first element of A cannot be zero.');
end
