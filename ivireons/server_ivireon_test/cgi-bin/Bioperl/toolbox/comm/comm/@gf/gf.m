...
%    $Revision: 1.11.4.10 $  $Date: 2009/01/05 17:45:36 $ 
...
elseif isa(x,'gf')
    ...
    if  ~all(all(isfinite(double(x))))
        error('comm:gf:ElementsOfXInfOrNaN','X must not contain Inf or NaN elements');                    
    end
    ...
    if mod(m,round(m))~=0
        error('comm:gf:NonIntegerM','M must be an integer');
    end
    
    if numel(m)~=1
        error('comm:gf:NonScalarM','M must be a scalar.')
    end
    
    if m<1 || m>16
        error('comm:gf:RangeM1To16','M must be an integer in the range 1 to 16.')
    end
    
  
if(any(any(x<0 | x>2^m-1)))
    error('comm:gf:InvalidX','X must be between 0 and 2^m-1');
end
  p.x = uint32(x);
  p.m = m;
  ...
  if need2CheckPoly
      ...
      if(numel(prim_poly)>1)
          error('comm:gf:NonScalarPrim_Poly','PRIM_POLY must be a scalar');
      end
