...
%   $Revision: 1.5.2.2 $  $Date: 2009/03/09 19:06:06 $

if ~isa(x,'gf'), x = gf(x,p.m); end
if ~isa(p,'gf'), p = gf(p,x.m); end
if p.m~=x.m
  error('comm:gf_polyval:MismatchOrders','Orders must match.')
elseif p.prim_poly~=x.prim_poly
  error('comm:gf_polyval:MismatchPrim_Poly','Primitive polynomials must match.')
end
...
y = gf(zeros(m,n), p.m, p.prim_poly);
if ~isempty(p)
    S.type='()';
    S.subs={1};
    G.type='()';
