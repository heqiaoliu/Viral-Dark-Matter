...
%    $Revision: 1.4.4.2 $  $Date: 2007/09/14 15:59:00 $ 
...
if size(A.x,1)>size(A.x,2)  % tall matrix
    if(det(A'*A)==0);
        error('comm:gf_mldivide:CannotFindSol','A''*A is not invertible, so MLDIVIDE cannot find a solution. However, a solution may still exist.');
    end
    z = (A'*A)\(A'*b);      
    return
...
if A.m~=b.m
    error('comm:gf_mldivide:MismatchOrders','Orders must match.')
elseif A.prim_poly~=b.prim_poly
    error('comm:gf_mldivide:MismatchPrim_Poly','Primitive polynomials must match.')
end
...
elseif size(A.x,1)~=size(b.x,1)
    error('comm:gf_mldivide:InvalidMatrixDims','matrix dimensions must agree')
else
    z = A;
    z.x = uint32(zeros(size(A.x,2),size(b.x,2)));
    [L,U,P] = lu(A);
    for i = 1:size(b.x,2)
