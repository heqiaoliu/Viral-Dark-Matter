...
%    $Revision: 1.2.4.4 $  $Date: 2007/09/14 15:59:12 $ 
...
global GF_TABLE_M GF_TABLE_PRIM_POLY GF_TABLE1 GF_TABLE2
    ...
    if U.m~=b.m
      error('comm:gf_usolve:MismatchOrders','Orders must match.')
    elseif U.prim_poly~=b.prim_poly
      error('comm:gf_usolve:MismatchPrim_Poly','Primitive polynomials must match.')
    end

    n=length(U.x);
    if (length(b.x) ~= n),
        error('comm:gf_usolve:InvalidUbDims',' USOLVE: Inputs U and b must be of compatible dimensions');
    end;

    if any(( any(double(U.x)-double(triu(U.x))) )), 
        error('comm:gf_usolve:UNotUpperTriangularMtx',' USOLVE: Input matrix U must be upper-triangular');
    end;

    if ( any(diag(double(U.x))==0) ), 
        error('comm:gf_usolve:SingularU',' USOLVE: Input matrix U must be nonsingular');
    end;
