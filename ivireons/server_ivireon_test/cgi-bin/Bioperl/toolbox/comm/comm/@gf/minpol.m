...
%    $Revision: 1.4.4.4 $  $Date: 2007/09/14 15:58:58 $ 

% X must be a GF object
if(~isa(x,'gf'))
    error('comm:gf_minpol:InvalidX','X must be a GF object');
end

% X must be a column vector.
if(size(x,2) >1)
    error('comm:gf_minpol:XNotScalarOrClmnVector','X must be a scalar or a column vector');
end
