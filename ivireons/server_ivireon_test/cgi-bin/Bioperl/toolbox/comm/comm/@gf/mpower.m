...
%    $Revision: 1.4.4.3 $  $Date: 2009/05/23 07:49:21 $ 

global GF_TABLE_M GF_TABLE_PRIM_POLY GF_TABLE1 GF_TABLE2


if isa(y,'gf'), 
    error('comm:gf_mpower:yNotgf','x^y not defined for y in GF(2^M).') 
end

if(~isfinite(y))
    error('comm:gf_mpower:Infinitely','Exponentiation by Inf or NaN not defined for Galois Fields');
end
...
if size(x.x,1)~=size(x.x,2)
    error('comm:gf_mpower:InpsNotSqrMatrixOrScalar','^ only defined for square matrices (and scalars)')
elseif numel(y)~=1
    error('comm:gf_mpower:NonScalary','in x^y, y must be a scalar')
else
