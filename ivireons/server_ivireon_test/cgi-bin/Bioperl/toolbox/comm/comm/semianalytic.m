...
%   and can have the values 'psk/nondiff', 'psk/diff', 'qam', 'dpsk', 'oqpsk',
%   'msk', or 'msk/precoded'.  Note that 'msk' is equivalent to conventional
%   MSK.  M is the order of the modulation, the value of which is dependent on
%   MODTYPE as follows: 
%
%                    MODTYPE                        M
%                 -------------    ----------------------------------------
...
%
%   Example:  
%       % To determine the BER of a 16QAM system with square pulse shaping,
...
%   $Revision: 1.1.6.13 $  $Date: 2010/05/20 01:58:04 $
%
...
% txSig
if (isreal(txSig) || ~isnumeric(txSig) || ~isvector(txSig))
    error('comm:semianalytic:complexVector',...
          'TXSIG must be a complex vector.');
end

% rxSig
if (isreal(rxSig) || ~isnumeric(rxSig) || ~isvector(rxSig))
    error('comm:semianalytic:complexVector',...
...
valModType = {'psk/diff' 'psk/nondiff' 'qam' 'dpsk' 'oqpsk' 'msk' ...
              'msk/precoded'};
compModType = {'msk/diff' 'msk/nondiff'}; % Compatibility mode mod types
if (~any(strcmp(modType, valModType))) && (~any(strcmp(modType, compModType)))
    error('comm:semianalytic:invalidModtype', 'Invalid MODTYPE specified');
end

if any(strcmp(modType, compModType))
    warning('comm:semianalytic:mskDataEnc', ...
        ['''msk/diff'' is equivalent to ''msk'' and ''msk/nondiff'' is '...
        'equivalent to\n''msk/precoded''.  ''msk/diff'' and ''msk/nondiff'' '...
        'are obsolete options.  Please use\n''msk'' or ''msk/precoded'' '...
        'instead.']);
end

if ((ismember(modType, [{'msk' 'msk/precoded'} compModType]) ...
        || ismember(modType, compModType)) && length(rxSig)/Nsamp<4)
...
switch modType
    ...
    case [{'msk' 'msk/precoded'} compModType]
        if (M~=2)
            error('comm:semianalytic:invalidM',...
...
valMSKmodType = [{'msk' 'msk/precoded'} compModType];
if (~ismember(modType, valMSKmodType))
    ...
    % Get delay, in sec
    [~, index] = max(abs(xcorr(rxSig, txSig)));
...
valMSKmodType = [{'msk' 'msk/precoded'} compModType];
if (~ismember(modType, valMSKmodType))
    [~, index] = max(abs(xcorr(filtSig, txSig)));
    sigLength = length(txSig);
...
switch modType    
    case [{'msk' 'msk/precoded' 'oqpsk'} compModType]      % align I and Q over 2Tsym
...
switch modType
    case {'psk/diff' 'psk/nondiff' 'dpsk'}
        ...
        posSig = abs(real(decSig)) + 1i*abs(imag(decSig));
        if (M == 4)
            posSig = max(real(posSig), imag(posSig)) + 1i*min(real(posSig), imag(posSig));
        elseif (M > 4)
            posSig = posSig .* exp(1i*pi/M);
            mag = abs(posSig);
            newSigAngle = mod(angle(posSig), pi/(M/2)) - pi/M;
            posSig = mag .* exp(1i*newSigAngle);
        end
    ...
    case 'oqpsk'
        ...
        posSig = abs(real(decSig)) + 1i*abs(imag(decSig));   % to first quadrant
        posSig = posSig .* exp(-1i*pi/4);                    % to positive I-axis
    ...
    case [{'msk' 'msk/precoded'} compModType]
        ...
        rotateSig = mag .* exp(1i*newSigAngle);
        
        % Find the distance to (cos(pi/M, sin(pi/M))
        idealPoint = (1+1i) / sqrt(2);
    ...
    switch lower(modType)
        case 'psk/diff'
            ...
            elseif (M == 4)
                ...
                newRxSig = abs(real(rxSig)) + 1i*abs(imag(rxSig));
                newRxSig = max(real(newRxSig), imag(newRxSig)) + ...
                               1i*min(real(newRxSig), imag(newRxSig));
                newRxSig = newRxSig * exp(1i*pi/4);
        ...
        case 'psk/nondiff'
            ...
            elseif (M == 4)
                ...
                % constellation has symbols at +/-0.707 +/-j*0.707.)
                newRxSig = abs(real(rxSig)) + 1i*abs(imag(rxSig));
                newRxSig = max(real(newRxSig), imag(newRxSig)) + 1i*min(real(newRxSig), imag(newRxSig));
                newRxSig = newRxSig * exp(1i*pi/4);
            ...
            elseif (M > 4)
                ...
                newRxSig = rxSig .* exp(1i*pi/M);
                mag = abs(rxSig);
                newSigAngle = mod(angle(newRxSig), pi/(M/2)) - pi/M;
                newRxSig = mag .* exp(1i*newSigAngle);
        ...
        case 'qam'
            ...
            for sigIndx = 1:sigLength
                minIndxI = find(decBdryI < rxSigI(sigIndx), 1, 'last' );
                sigBdryI(sigIndx, 1) = decBdryI(minIndxI);
                sigBdryI(sigIndx, 2) = decBdryI(minIndxI+1);
                
                minIndxQ = find(decBdryQ < rxSigQ(sigIndx), 1, 'last' );
...
%   modulation types. The function assumes the I and Q QAM signaling points are
%   1, 3, 5, etc., because the received QAM signal is collapsed into the first
%   quadrant. The computed decision boundaries include a very large maximum
%   threshold to maintain the same form of BER equation for all input signal
%   types.  
