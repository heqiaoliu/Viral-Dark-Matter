...
% $Revision: 1.7.4.1 $  $Date: 2007/08/03 21:18:14 $

if isempty(h)
    error('comm:syndtable:EmptyH','The parity check matrix cannot be empty.');
end

[n_k,n]=size(h);                    % n_k denotes n-k
if n_k >= n || ~isequal(size(size(h)),[1 2]),
    error('comm:syndtable:LessRowsMoreColumns','There must be more columns than rows in the parity check matrix.');
end

if ~isnumeric(h) || max(max(h < 0)) || max(max(h)) > 1 || (max(max(floor(h) ~= h))),
    error('comm:syndtable:NonBinaryNosInH','The parity check matrix must contain only binary numbers.');
end
