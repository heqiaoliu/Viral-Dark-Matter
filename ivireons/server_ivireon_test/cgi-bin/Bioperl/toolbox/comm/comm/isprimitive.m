...
%   $Revision: 1.4.4.3 $   $Date: 2007/09/14 15:57:28 $

[r,c]=size(inp);

if (r>1) & (c>1)
    error('comm:isprimitive:InvalidInput','Input cannot be a matrix');
end

%Make the input row vector if its entered as a coulmn vector
if c>1
    error('comm:isprimitive:InvalidAForm','A must be a column vector');
end

if any(inp<=1) | any(inp-floor(inp))
    error('comm:isprimitive:InvalidA','A must be nonnegative integer(s) greater than one');
end
