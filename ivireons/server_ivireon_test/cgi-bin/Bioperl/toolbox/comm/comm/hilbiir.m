...
%   group delay is one second. The ideal Hilbert transform filter impulse
...
%   singular values in the calculation.
%
%   [A, B, C, D] = HILBIIR(...) outputs the state-space solution.
%
%   [A, B, C, D, SV] = HILBIIR(...) outputs the state-space solution and
%   singular values.
%
%   See also GRPDELAY.
...
if dly <= 0
    error('comm:hilbiir:DlyThanZero','The time delay factor for designing HILBIIR filter must be larger than 0')
end;
