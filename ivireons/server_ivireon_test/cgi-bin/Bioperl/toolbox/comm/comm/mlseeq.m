...
%     modObj = modem.qammod(4);
%     modmsg = modulate(modObj,[2 2 1 3 1 1 3 3 2 2 1 3 1 0 1]);
%     const = modObj.Constellation;
...
%     msg = [preamble data];
%     modObj = modem.qammod(4);
%     modmsg = modulate(modObj,msg); const = modObj.Constellation;
%     tblen = 10; chcffs = [1 ; 0.25];
%     filtmsg = filter(chcffs,1,modmsg);   
...
% $Revision: 1.1.6.6 $  $Date: 2010/05/20 01:58:01 $
...
for nn=1:nvarargin
   if(isnumeric(varargin{nn}))
      numArg(size(numArg,2)+1) = nn; %#ok<AGROW>
   else
    ...
    case 3
      ...
      if (opmodeNum == RST)
          ...
          if isempty(inputPreamble)            
            ...
            validateattributes(inputPreamble, {'numeric'}, ...
              {'vector', 'real', '>=', 0, '<', numel(const)}, ...
              'MLSEEQ', 'PREAMBLE')
          ...
          if isempty(inputPostamble)
            ...
            validateattributes(inputPostamble, {'numeric'}, ...
              {'vector', 'real', '>=', 0, '<', numel(const)}, ...
              'MLSEEQ', 'POSTAMBLE')
...
if ~isempty(x)
    ...
    if ~isnumeric(x) || ~isvector(x) || any(~isfinite(x))
...
if (isempty(cplxchanest) || ~isvector(cplxchanest) || ...
...
if isempty(cplxconstpts) || ~isvector(cplxconstpts) || length(cplxconstpts)<2 ...
...
if (initTableSet == 1)
    ...
    elseif ~isnumeric(initmetric) || (~isreal(initmetric))  || ...
        ...
        error('comm:mlseeq:initmetric',['The initial state metrics must be a real vector with length ' ,...
              'equal to the number\n' ,...
