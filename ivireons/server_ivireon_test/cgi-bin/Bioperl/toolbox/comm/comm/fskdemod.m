...
%   $Revision: 1.1.6.6 $  $Date: 2009/07/27 20:09:26 $ 
...
if (~isreal(M) || ~isscalar(M) || M<2 || (ceil(M)~=M) || ~isnumeric(M))
    error('comm:fskdemod:Mreal', 'M must be a scalar positive integer greater than or equal to 2.');
end

% Check that M is of the form 2^K
if(~isnumeric(M) || ceil(log2(M)) ~= log2(M))
    error('comm:fskdemod:Mpow2', 'M must be a positive power of 2. ');
end
