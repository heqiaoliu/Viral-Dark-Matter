...
%   work in GF(2^M), you can also use the ROOTS function with Galois arrays.
%
%   See also GFPRIMDF, GFLINEQ.

%   Copyright 1996-2007 The MathWorks, Inc.
%   $Revision: 1.18.4.5 $ $ Date: $
...
if nargin < 2 % Syntax doesn't specify the extension field.
    ...
    if ord==0 % Polynomial is a constant and syntax does not specify the field.
       error('comm:gfroots:FieldNotSpecified','For zero-degree polynomials, use the second input to specify the field.')
    end    
end;
...
if ( isempty(f) || ndims(f)>2 || any(any( abs(f)~=f | floor(f)~=f | f>=p )) )
    if (p == 2)
        error('comm:gfroots:InvalidFForP2','Polynomial coefficients must be either 0 or 1 for P=2.');
    else
        error('comm:gfroots:InvalidF','Polynomial coefficients must be real integers between 0 and P-1.');
    end
end
