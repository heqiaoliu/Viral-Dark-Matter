...
%   Gray-coded ordering.
%
%   See also FSKDEMOD, MODEM.PSKMOD, MODEM.PSKDEMOD, MODEM/MODULATE, 
%   MODEM/DEMODULATE.

%   Copyright 1996-2009 The MathWorks, Inc.
%   $Revision: 1.1.6.11 $  $Date: 2009/07/27 20:09:27 $ 


% Error checks -----------------------------------------------------------------
if (nargin < 4)
    error('comm:fskmod:numarg', 'Too few input arguments.');
end
...
if (~isreal(M) || ~isscalar(M) || M<2 || (ceil(M)~=M) || ~isnumeric(M))
    error('comm:fskmod:Mreal', 'M must be a scalar positive integer greater than or equal to 2.');
end

% Check that M is of the form 2^K
if(~isnumeric(M) || ceil(log2(M)) ~= log2(M))
    error('comm:fskmod:Mpow2', 'M must be a positive power of 2. ');
end
...
if(nargin >= 4 && nargin <= 6 )    
   Symbol_Ordering = 'bin';         % default
    ...
    if (~ischar(Symbol_Ordering)) || (~strcmpi(Symbol_Ordering,'GRAY')) && (~strcmpi(Symbol_Ordering,'BIN'))
        error('comm:fskmod:SymbolOrder','Invalid symbol set ordering.');    
    end
end
...
if (strcmpi(Symbol_Ordering,'GRAY'))
    [x_gray,gray_map] = bin2gray(x,'fsk',M);   % Gray encode
