...
if ~(ischar(file_in) & ischar(file_out))
    error('comm:rsencof:InvalidFileString','FILE_IN and FILE_OUT must be strings.')
end
if nargin < 3
...
fid = fopen(file_in, 'r');
if fid == -1
    errID1 = 'comm:rsencof:CannotOpenFile';
    error(errID1, sprintf('Cannot open file %s.', file_in));
end
msg = fread(fid, 'uchar');
if fclose(fid) ~= 0
    errID2 = 'com:rsencof:CannotCloseFile';
