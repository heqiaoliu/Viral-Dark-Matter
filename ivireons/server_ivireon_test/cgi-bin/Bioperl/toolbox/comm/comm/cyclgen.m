...
% Check the P parameter. P cannot be zero if it is not a polynomial.
if(length(p)==1 && p==0)
    error('comm:cyclgen:InvalidP','Invalid generator polynomial specified.');
end

% Verify that the polynomial is irreducible.
[pc, r] = gfdeconv([1 zeros(1, n-1) 1], p);
if max(r) ~= 0
    error('comm:cyclgen:InvalidP','The generator polynomial P cannot produce a cyclic code generator matrix.');
end;
