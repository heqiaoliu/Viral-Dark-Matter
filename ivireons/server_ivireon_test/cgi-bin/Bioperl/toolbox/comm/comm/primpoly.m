...
%   $Revision: 1.4.4.4 $   $Date: 2009/03/30 23:24:15 $

% Error checking.
error(nargchk(1,3,nargin));

% Error checking - M.
if ( isempty(m) || ~isreal(m) || m<1 || floor(m)~=m || numel(m)~=1 || m>16)
    error('comm:primpoly:InvalidMVal','M must be a real positive scalar greater than 0 and lesser than or equal to 16.');
end
...
elseif nargin==2
    ...
    if ischar(fd_flag)
        ...
        if ~( strcmp(fd_flag,'min') || strcmp(fd_flag,'max') || strcmp(fd_flag,'all') || strcmp(fd_flag,'nodisplay') || strcmp(fd_flag,'one') )
            error('comm:primpoly:InvalidStringInp','Invalid string input. Type help primpoly for more information on usage.');
        end
    ...
    elseif ( isempty(fd_flag) || floor(fd_flag)~=fd_flag || ~isreal(fd_flag) || numel(fd_flag)~=1 || fd_flag<2 || fd_flag>m+1 )
        error('comm:primploy:InvalidOPT','OPT parameter must be either a string, or a real integer greater than one and less than M+1.');
    else
        disp_flag='';
...
elseif nargin>2
    if ischar(disp_flag)
        if ~( strcmp(disp_flag,'nodisplay') )
            error('comm:primpoly:InvalidStringInp',['Invalid string input '...
                'for polynomial display option.\nChoose ''nodisplay'' to '...
                'turn off polynomial form of display.']);
        end
    else 
        error('comm:primpoly:InvalidThirdArg','Third parameter must be the string ''nodisplay'' to turn off the display the polynomials naturally');
    end
end
...
elseif isnumeric(fd_flag)
    ...
    error('comm:primpoly:LNotPosScalar','Weight L must be a real positive scalar.');
    end
