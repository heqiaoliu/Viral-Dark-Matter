...
%   work in GF(2^M), you can also apply the - operator to Galois arrays of equal
%   size.
%
%   See also GFADD, GFCONV, GFMUL, GFDECONV, GFDIV, GFTUPLE.

%   Copyright 1996-2007 The MathWorks, Inc.
%   $Revision: 1.19.4.5 $   $Date: 2007/08/03 21:17:46 $
...
% Error checking - LEN.
if (nargin > 3 && (isempty(len) || floor(len)~=len || real(len)~=len || numel(len)~=1))
    error('comm:gfsub:LenNotScalarInt','Length parameter must be a real integer scalar.');
end

% Error checking - A & B.
if ( ( (m_a > 1) && (n_a > 1) ) || ( (m_b > 1) && (n_b > 1) ) )
    if ( (m_a ~= m_b) || (n_a ~= n_b) )
        error('comm:gfsub:InputsNotSameSize','If inputs are matrices, they must have the same size.');
    end
elseif ( ((m_a > 1) && (n_b > 1)) || ((m_b > 1) && (n_a > 1)) )
    error('comm:gfsub:InvalidOrientation','Orientations of input vectors must be the same.');
end
...
if m_field*n_field == 1
    ...
    if ( ndims(a)>2 || ndims(b)>2 )
        error('comm:gfsub:InvalidInputDims','Inputs must not have more than two dimensions.');
    end
    if ( numel(p)~=1 || isempty(p) || ~isreal(p) || p<=1 || floor(p)~=p || ~isprime(p) )
        error('comm:gfsub:InvalidP','The field parameter P must be a positive prime integer.')
    end
    ...
    if ( isempty(a) || isempty(b) || any(any( a~=floor(a) | real(a)~=a | b~=floor(b) | real(b)~=b )) )
        error('comm:gfsub:InputElementsNotInt','Input elements must be real integers.')
    end

    % Error checking - A & B.
    if any(any( a<0 | b<0 | a>=p | b>=p ))
        if ( p == 2 )
            error('comm:gfsub:InvalidInputElementsP2','Input elements must be either 0 or 1 for subtraction over GF(2).')
        else
            error('comm:gfsub:InvalidInputElements','Input elements must be between 0 and P-1 for subtraction over a prime field.')
        end
    end;
    ...
    if (nargin > 3)
        ...
        elseif (len < 0)
                c = c( : , 1:max([find(sum(c~=0,1),1,'last'),1]) );
        elseif (n_a > len)
            c = c( : , 1:len );
        end
    end
    ...
    if ( isempty(field) || any(any( floor(field)~=field | abs(field)~=field )) )
        error('comm:gfsub:InvalidFieldVal','Field parameter values must be real nonnegative integers.')
    end
    ...
    if ( ~isprime(p) || (m_field ~= q) || (n_field ~= m) )
        error('comm:gfsub:InvalidField','Field parameter is not valid. See GFTUPLE.');
    end
    ...
    if any(any( ( a > (p^m)-2 ) | ( b > (p^m)-2 ) ))
        error('comm:gfsub:InvalidInputElements','Input elements must be between -Inf and P^M-2 for subtraction over an extension field.')
    end;
