...
%   $Revision: 1.2.2.3 $ $Date: 2008/08/01 12:17:26 $
...
if (length(find(size(code)==1)) ~= 1)
    eStr.eID = 'comm:arithdeco:InvalidCodeParameter';
    eStr.emsg = 'The binary arithmetic code parameter must be a double array of zeros and ones only.';
    eStr.ecode = 1; return;
end
if (length(find(size(counts)==1)) ~= 1)
    eStr.eID  = 'comm:arithdeco:InvalidSymbolCountParameter';
    eStr.emsg = 'The symbol counts parameter must be a vector of positive finite integers.';
    eStr.ecode = 1; return;
end
...
% entered for COUNTS
if ~all(counts > 0) || ~all(isfinite(counts)) || ~isequal(counts, round(counts)) ||~isreal(counts)
    eStr.eID = 'comm:arithdeco:InvalidSymbolCountParameter';
    eStr.emsg = 'The symbol counts parameter must be a vector of positive finite integers.';
    eStr.ecode = 1; return;
end
...
if ~isequal(size(len),[1 1])
    eStr.eID = 'comm:arithdeco:InvalidLength';
