...
%   $Revision: 1.14.4.4 $ $Date: 2007/10/05 18:33:06 $

% default values.
if nargin < 2
    b = 'X';
elseif ( ~ischar(b) || isempty(b) )
    error('comm:gfpretty:InvalidStr','The second input variable must be a string.');
end;
...
if ( (size(a,1) ~= 1) || ndims(a)~=2 )
    error('comm:gfpretty:ANotRowVector','The input polynomial must be a row vector.');
end

if ( any(floor(a) ~= a) || any(~isreal(a)) || any(a<0) )
    error('comm:gfpretty:InvalidCoeffA','The polynomial coefficients must be nonnegative integers.');
end
...
if length(a) <= 1
    ...
    for i=1:length(a)
        if a(i) ~= 0
            ...
            diu = [diu, char(ones(1,spa)*32), pow]; %#ok
            did = [did, char(ones(1,length(pow))*32)]; %#ok
        end;
    end;
end;
