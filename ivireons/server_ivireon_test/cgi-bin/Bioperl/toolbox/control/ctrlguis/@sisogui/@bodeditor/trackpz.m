...
%   $Revision: 1.35.4.6 $  $Date: 2007/08/20 16:25:27 $
...
switch event
   case 'init'
      ...
      if Ts~=0 && MovedGroup.beyondnf(PZID,0.1)
      ...
      if ActiveAxes==1
         Yinit = Editor.Magnitude * getZPKGain(Editor.EditedBlock,'mag');  % absolute
      else
      ...
      EventMgr.poststatus(LocalTrackStatus(MovedGroup,PZID,Ts,FreqUnits));
   ...
   case 'acquire'
      if ~InvalidMove
         ...
         if ActiveAxes==1
            ...
            Y0 = utInterp1(FreqInit,Yinit,X);            % initial phase at w=X (in rad)
         ...
         if AutoScaleX || AutoScaleY,
            ...
            MovePtr = Editor.reframe(PlotAxes(ActiveAxes),'xy',X,Y);
            if MovePtr
               % Reposition mouse pointer 
               moveptr(PlotAxes(ActiveAxes),'move',X,Y);
            end
         end
      end
...
case 'Complex'
   ...
   Status = ...
      ...
      Text,real(R),'+/-',abs(imag(R)),Spacing,Zeta,Spacing,Wn,FreqUnits);
