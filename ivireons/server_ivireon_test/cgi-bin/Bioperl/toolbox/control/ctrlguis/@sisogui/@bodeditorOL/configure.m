...
%   $Revision: 1.1.8.4 $  $Date: 2010/05/10 16:59:13 $
if strcmp(this.Visible,'on')
   [L,idxL] = getL(this);
   % Update title
   if isempty(L.Name) || strcmp(L.Name,L.Identifier)
      this.Axes.Title = sprintf('Open-Loop Bode Editor for %s',L.Identifier);
   else
      this.Axes.Title = sprintf('Open-Loop Bode Editor for %s (%s)',L.Name,L.Identifier);
   end
