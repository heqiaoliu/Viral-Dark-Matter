...
%   $Revision: 1.12.4.4 $  $Date: 2010/04/21 21:10:39 $
...
if isempty(Frame)
    ...
    % Don't clear listener property
    AllProps = AllProps(~strcmp(AllProps,'Listeners'));
    ...
    %---Help
        ...
        set(hc,'ActionPerformedCallback',@(es,ed) ctrlguihelp('sisotool_preferences'));
    %---Apply
    ...
    %---Time Delay Options
    s.Pade = pade_gui(s.NewPrefs);
    s.TimeDelayTab = localMakeTab(s.Pade);
    s.TabPanel.addPanel(...
        ctrlMsgUtils.message('Control:compDesignTask:strTimeDelaysLabel'), ...
        s.TimeDelayTab);
    %---Style
