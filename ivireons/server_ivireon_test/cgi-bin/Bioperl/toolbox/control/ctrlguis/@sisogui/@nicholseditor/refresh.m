...
%   $Revision: 1.1.8.5 $  $Date: 2010/05/10 16:59:21 $
...
case 'init'
   ...
   hPZ = Editor.HG.Compensator;  % handles of x and o
   Wpz = get(hPZ,{'UserData'}); 
   Wpz = cat(1,Wpz{:});
   ...
   % Hide "fixed" poles and zeros (their number may change while editing external comp)
...
% Update nichols plot
OLMag   = mag2db(LoopGain*OLMag);
OLPhase = unitconv(OLPhase,'deg',Editor.Axes.XUnits);
set(Editor.HG.NicholsPlot, 'Xdata', OLPhase(1:nf), 'Ydata', OLMag(1:nf))
...
hPZ = Editor.HG.Compensator;
for ct=1:length(hPZ)
   set(hPZ(ct),'Xdata', OLPhase(nf+ct), 'Ydata',OLMag(nf+ct))
end
