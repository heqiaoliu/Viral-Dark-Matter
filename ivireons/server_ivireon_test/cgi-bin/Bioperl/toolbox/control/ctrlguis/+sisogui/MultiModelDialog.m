classdef (Hidden = true) MultiModelDialog < handle
    % @MultiModelDialog class definition
    
    %   Author(s): C. Buhr
    %   Copyright 2009-2010 The MathWorks, Inc.
    %	 $Revision: 1.1.8.1.2.1 $  $Date: 2010/07/01 20:42:22 $
    properties
        Parent  % sisodb
        Frame  % MJFrame
        NominalPanel % Struct
    ...
    methods
        %%
        function this = MultiModelDialog(sisodb)
            % Constructor
            this.Parent = sisodb;
...
%             Btnpanel.add(Btn1);
        ...
        function NomPanel = buildNominalModelPanel(this)
            ...
            h.FocusLostCallback = {@localNominalEditorChange this};
        ...
        function FreqPanel = buildFreqSpecPanel(this)
           ...
           % Label1 = MJLabel(sprintf('Specify frequencies for multi-model computations:'));
            ...
            h.ActionPerformedCallback = @(es,ed) localSetFreq(this);
...
% ------------------------------------------------------------------------%
function localSetFreq(this)

% Revisit need error data handling here
try
    Data = evalin('base',char(this.FrequencySpecPanel.ManualSelectEditField.getText));
catch ME
    f = errordlg(ctrlMsgUtils.message('Control:compDesignTask:strMultiModelInvalidFreqVector'), ...
        ctrlMsgUtils.message('Control:compDesignTask:strMultiModelFreqPanelTitle'), 'modal');
    return
end
...
if isnumeric(Data) && isvector(Data) && isreal(Data) && (numel(unique(Data))>1) && all(Data>0)
    ...
    this.FreqSpecData.UserModeData = unique(Data(:));
else
    f = errordlg(ctrlMsgUtils.message('Control:compDesignTask:strMultiModelInvalidFreqVector'), ...
        ctrlMsgUtils.message('Control:compDesignTask:strMultiModelFreqPanelTitle'), 'modal');
end
