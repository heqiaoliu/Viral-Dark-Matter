...
%   $Revision: 1.7.4.11 $  $Date: 2010/03/22 03:40:47 $
...
Frame.setName('LTIViewerExportDialog');
...
set(handle(Frame,'callbackproperties'),...
...
% If this Frame is not visible then don't do anything    
...
% Make sure all Export Names are valid variable names
for ct = 1:length(s.ModelList)
    if ~isvarname(s.ModelList(ct).ExportName)
       s.ModelList(ct).ExportName = ['sys_',num2str(ct)];
    end
end
...
% Table view
Table = MWTable(20,4);
Table.setName('ModelExportTable');
