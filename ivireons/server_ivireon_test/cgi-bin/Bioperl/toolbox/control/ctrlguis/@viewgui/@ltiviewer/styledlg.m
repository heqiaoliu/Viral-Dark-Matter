...
%   $Revision: 1.10.4.6 $  $Date: 2008/08/01 12:19:01 $
...
%
src = get(eventSrc,'Userdata'); % Get Figure handle from the Style dialog handle
this = get(src.Parent,'Userdata'); % Get  Ltiviewer handle
if ~isempty(this)
...
set(Buttons(3),'Callback','ctrlguihelp(''lti_viewer_line_styles'');');
...
% Revisit: Workaround to do layout management g479467
% If extents extend past the width of the uicontrol increase the height.
extents = get(b,'Extent');
rows = floor(extents(3)/44);
pos4 = 0;
if rows>0
    pos4 = pos4+ floor(extents(3)/44)*get(b,'FontSize');
end
set(b,'position',[208,279,44,14+pos4])
