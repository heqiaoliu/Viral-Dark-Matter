...
%   $Revision: 1.17.4.6 $  $Date: 2008/12/29 01:47:18 $
...
if nargin==1
    ...
    CharProps = [Prefs.findprop('SettlingTimeThreshold');...
          ...
          Prefs.findprop('MinGainLimit')];
    L = [L;...
          handle.listener(Prefs, UnitProps, 'PropertyPostSet', {@LocalApplyUnits this});...
          handle.listener(Prefs, CharProps, 'PropertyPostSet', {@LocalApplyOptions this})];
end
...
%LOCALSAPPLYOPTIONS%
%%%%%%%%%%%%%%%%%%%%
function LocalApplyOptions(eventsrc,eventdata,this)
% PostSet listener for options change change (generic)
for v=this.Views(ishandle(this.Views))'
   p = getoptions(v);
   mapCSTPrefs(p,this.Preferences);
   setoptions(v,p);
end
