...
%
%   [X1,X2,D,L] = DARE(A,B,Q,...,'factor') returns two matrices 
...
%   $Revision: 1.20.4.8 $ $Date: 2008/01/15 18:46:54 $
...
try
   [Q,R,S,E,Flags,OldSyntax] = arecheckin('dare',A,B,Q,varargin{:});
catch ME
   throw(ME)
end
[n,m] = size(B);
...
if DescFlag && rcond(E)<eps,
    ctrlMsgUtils.error('Control:foundation:ARE09','dare')
end
...
if any(strcmp(Flags,'factor'))
   ...
   if no<4 && ~any(strcmp(Flags,'report'))
      switch Report
         case -1
