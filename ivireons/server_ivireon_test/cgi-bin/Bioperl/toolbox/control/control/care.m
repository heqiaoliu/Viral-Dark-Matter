...
%   $Revision: 1.21.4.7 $ $Date: 2008/01/15 18:46:51 $
...
try
   [Q,R,S,E,Flags,OldSyntax] = arecheckin('care',A,B,Q,varargin{:});
catch ME
   throw(ME)
end
[n,m] = size(B);
...
if DescFlag
   ...
   if rcond(E)<eps,
      ctrlMsgUtils.error('Control:foundation:ARE09','care')
   end
elseif isequal(R,diag(diag(R)))
   % R is diagonal
   D = diag(R);
   if any(D==0)
      ctrlMsgUtils.error('Control:foundation:ARE08','care')
   else
