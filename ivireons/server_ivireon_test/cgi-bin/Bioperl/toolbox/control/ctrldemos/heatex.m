...
%   $Revision: 1.7.4.10 $  $Date: 2010/01/25 21:30:38 $
...
GUI_Data = struct(...
    ...
    'FBSwitch','heatex_sim/Feedback Control C/FB on-off Switch', ...        
    'FFSwitch','heatex_sim/Feedforward Control F/FF on-off Switch', ...
    'FFGain','heatex_sim/Feedforward Control F/FF Gain', ...
    'FFDelay','heatex_sim/Feedforward Control F/FF Delay'));
...
lblColor = frmColor;
boxColor = frmColor;
bottom = 0.01;        top = 1- 0.01;                     % Reference definitions
left = 0.01;        right = 1-left;                      % Reference definitions
...
set(YLabelHdl,'String','Tank Temperature','FontSize',7)
TitleHdl = get(GUI_Data.AnimData.GraphHdl,'Title');
...
% Create a legend s
legend(GUI_Data.AnimData.GraphHdl,'d','T','V',4);
...
if u(1)~=0 & GUI_Data.AnimData.DisplayFlag == 1
    set(GUI_Data.StatusText,'String','A disturbance in the inlet flow temperature has occurred');
end
...
% Customize the plot depending on the type of control used
n = 2*GUI_Data.CtrlStrct(1) + GUI_Data.CtrlStrct(2) + 1;
switch n
...
case 1
    ...
    text(x0-3,y0,'V  ','horiz','right','fontweight','normal','fontsize',8+2*isunix,'parent',ax);
    % The Plant
    ...
    text(x0+20.5,y0,'  T','horiz','left','fontweight','normal','fontsize',8,'parent',ax);
    ...
    text(x0-4.5,y0+7,'d  ','horiz','left','fontweight','normal','fontsize',8,'parent',ax);
...
case 2
    ...
    sumblock('position',[x0-0.5,y0],'label',{'+140','-235'},'radius',.5,...
        'LabelRadius',1.5,'fontsize',10,'parent',ax);
    text(x0-3,y0,'T_{sp}','horiz','right','fontweight','normal','fontsize',8,'parent',ax);
    % The Plant
    sysblock('position',[x0+11.4 y0-1.5 4.5 3.5],'name','Gp',...
        'fontweight','normal','facecolor','y','fontsize',8,'parent',ax);
    wire('x',x0+[16 17.5],'y',y0+[0 0],'parent',ax,'arrow',0.5);
    text(x0+20.5,y0,'  T','horiz','left','fontweight','normal','fontsize',8,'parent',ax);
    text(x0+9.7,y0+1,'V','horiz','left','fontweight','normal','fontsize',8,'parent',ax);
    wire('x',x0+[18 20.5],'y',y0+[0 0],'parent',ax,'arrow',0.5,'parent',ax);
    % The Disturbance
    wire('x',x0+[-3.2 11.2],'y',y0+[7 7],'parent',ax,'arrow',0.5,'parent',ax);
    sysblock('position',[x0+11.4 y0+5.5 4.5 3.5],'name','Gd',...
    ...
    sysblock('position',[x0+2 y0-1.5 4.5 3.5],'name','C',...
...
case 3
    ...
    sysblock('position',[x0+2 y0+2.6 4.5 3.5],'name','F',...
        'fontweight','normal','facecolor','c','fontsize',8,'parent',ax);
...
if GUI_Data.AnimData.RefreshMimic == 1
    ...
    % draw another patch on top which increases when valve is open
    dp = 0;
    GUI_Data.AnimData.dp = dp;
