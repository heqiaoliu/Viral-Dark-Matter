...
%       $Revision: 1.8.4.2 $  $Date: 2006/12/15 19:29:27 $
...
elseif nrc==4*N*N
  for k=1:N
    for l=1:N
      cgxu(k,:)=cgxu(k,:)+c(1+4*(k-1+N*(l-1)),:).*gxu(l,:)+ ...
          c(3+4*(k-1+N*(l-1)),:).*gyu(l,:);
      cgyu(k,:)=cgyu(k,:)+c(2+4*(k-1+N*(l-1)),:).*gxu(l,:)+ ...
          c(4+4*(k-1+N*(l-1)),:).*gyu(l,:);
    end
  end
else
