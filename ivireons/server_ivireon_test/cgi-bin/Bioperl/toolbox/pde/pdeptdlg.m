...
%   $Revision: 1.13.4.12 $  $Date: 2010/04/11 20:33:42 $
...
if strcmp(action,'initialize')
  ...
  if isempty(fig),
    ...
    PlotFcn = 'pdeptdlg(''savenoclose''), pdeptdlg(''plot'')';
    uicontrol(fig,'Style','push','String','Plot','Callback',PlotFcn, ...
        'Position',[Hspace mLineHeight/2 BWH]);
    CloseFcn = ['set(findobj(get(0,''Children''),''flat'',',...
      '''Tag'',''PDEPlotFig''),''Visible'',''off'');',...
      'figure(findobj(get(0,''Children''),''flat'',''Tag'',''PDETool''));',...
      'pdeptdlg(''close'');pdeinfo'];
    uicontrol(fig,'Style','push','String','Close',...
        'Callback',CloseFcn,'Position',[2*Hspace+BWH(1) mLineHeight/2 BWH]);
    ...
    hndls =[type_hndl' property_hndl' double(animopt_hndl) user_hndl' style_hndl' ...
            mesh_hndl' double(do_hndl) double(col_hndl) double(time_hndl) double(popup_hndl) ...
            double(lvl_hndl) appl];
...
elseif strcmp(action,'plot')
  ...
  if animate && ~replay && (pde_type==2 || pde_type==3)
    ...
    for i=1:length(figs)
      ...
      if npl(1)=='a',
        if isempty(findobj(get(figs(i),'Children'))),
          moviefig=figs(i);
          figure(moviefig)
          set(moviefig,'Colormap',cmap,'Nextplot','add')
          break;
        end
      elseif npl(1)=='r',
        moviefig=figs(i);
    ...
    if isempty(moviefig),
      moviefig=figure('Colormap',cmap,'Nextplot','add');
    end
    ...
    if ~strcmp(xystyle,'off')
      ...
      if size(xydtst,2)>1
        xydata=zeros(size(xydtst,2),n);
        trx=1;
      else
        xydata = zeros(size(uu,1),n);
      end
    elseif contflag,
      [xydtst,xystr]=...
    ...
    if ~strcmp(zstyle,'off')
      [zdtst,zstr]=...
         pdeptdata(plotflags(3),2,p,e,t,c,uu(:,1),appl,currparams,hndls,time);
      if size(zdtst,2)>1
        zdata=zeros(size(zdtst,2),n);
        trz=1;
      else
        zdata=zeros(size(t,2),n);
      end
    else
      zdata=zeros(1,n);
    end
  ...
  % Make the axis equal only for 2d plots
  ax = get(moviefig,'CurrentAxes');
  if all(rem(get(ax,'View'),90)==0) % True for 2D plots
      axis equal
  end
    ...
    if ~strcmp(xystyle,'off') || contflag,
      ...
      set(hc,'OuterPosition',[0.90   -0.0043    0.0896    1.0020])
...
elseif strcmp(action,'close') || strcmp(action,'savenoclose')
  % 'close' is used by 'Close' button; 'savenoclose' is used to apply
