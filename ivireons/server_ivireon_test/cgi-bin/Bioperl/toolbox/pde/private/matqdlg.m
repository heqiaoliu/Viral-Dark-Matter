...
%   $Revision: 1.1.6.5 $  $Date: 2010/04/11 20:33:49 $
...
elseif (strcmp(action, 'ws2buffer'))
  ...
  if (rem(nargin,2) ~= 1)
    error('pde:matqdl:invalidNumberOfArgs', 'Invalid number of input arguments for ws2buffer action.');
  end
...
elseif (strcmp(action, 'buffer2ws'))
  ...
  if (num_items == 0)
    ...
    error('pde:matqdl:emptyQueue','The queue is empty.');
  end
...
elseif (strcmp(action, 'get_entry'))
  ...
  error('pde:matqdl:invalidAction','Invalid action.');
...
switch(wsormat)
 ...
 otherwise
  error('pde:matqdl:okButtonCallback:unknownOption','unknown option');
