...
%       $Revision: 1.9.4.3 $  $Date: 2008/12/01 07:21:19 $
  ...
  if nf>1,
   ...
   fold=f;
    ...
    if any(size(f)-[1 1]) && any(size(f)-size(x))
      errstr=sprintf( ...
        '%s\n\nIn expression:\n%s\nwhen evaluating pde coefficients.\n', ...
        'Expression evaluates to wrong size. Must be a scalar or a row vector. In a system case, pass first or second row; for example, u(2,:).',fold);
      error('PDE:pdetexpd:InvalidSizeFromExpression', errstr);
    end
  end
end
