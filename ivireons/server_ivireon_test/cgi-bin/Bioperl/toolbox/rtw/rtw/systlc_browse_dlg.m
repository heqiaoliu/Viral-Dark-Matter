...
%
%   When you press the "Browse" button on Configuration parameters dialog
...
%   we don't want to allow user to manually change some related items like typing
...
switch Action
  ...
  otherwise
    ...
    %
    DAStudio.error('RTW:configSet:invalidCallSystlcBrowseDlg');
    return;
end
%endfunction
...
%
% Adjust the position of the figure if the screen size is big enough
%
...
%
% Pass Configuration Parameter Real-Time Workshop page's figure handle into
% UserData 
%
...
%
% Set the position of the selected file full name including its path.
%
...
%
% Set the position of 'name' label
%
...
%
% Set the position of 'description' label
%
...
%  path will be showed on the bottom of the window.
%
...
%
% Install callback for 'OK' and 'Cancel' button
%
...
errmsg = [];
try
    systlcFile = eval('systlc_browse(matlabroot,path);');
catch exc
    errmsg = exc.message;
end
...
if ~isempty(systlcFile)
  ...
  if ~isempty(sysChildren)
    existingTLC = strtrim(get(TargetFileEdit, 'String'));
    existingTMF = strtrim(get(MakeFileEdit, 'String'));
    existingMCD = strtrim(get(MakeCmdEdit, 'String'));
  end
  ...
  for i = 1:length(systlcFile)
    ...
    fileListStr = [ fileListStr systlcFile(i).description ]; %#ok<AGROW>
    
    tlcListStr = [ tlcListStr; { fileListStr } ]; %#ok<AGROW>
...
switch action
 ...
 case 'OK'
  ...
  if ~strcmp ( preTLCFileSet, currentTLCFileSet ) 
    %
    % Set the selected TLC file on Configuration Parameter GUI and in the
    % system.
    %
    if ~isempty(sysChildren)
      set(TargetFileEdit, ...
    ...
    if ~isempty(sysChildren)
      ...
      if ~isempty (sysChildren.TargetFileButton)
        set( simParamApplyButton, 'Enable', 'on' );
          ...
          DAStudio.error('RTW:configSet:invalidSimParamDialog');
       end
    end
 ...
 case 'Selection'
  ...
  if strcmpi(get(figHandle, 'SelectionType'), 'normal')
    %
  ...
  elseif strcmpi(get(figHandle, 'SelectionType'), 'open')
 ...
 case 'Reshow'
  SysTLCBrowse( figHandle );
 
  otherwise
    DAStudio.error('RTW:configSet:systlcBrowseDlgCallbackError');
end
