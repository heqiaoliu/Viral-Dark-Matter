...
%   $Revision: 1.1.8.16 $
    ...
    %getComponentIntegrationBuildInfo returns the integration BUILDINFO object
            ...
            % get some build args
            %
            baModelRefTargetType = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'MODELREF_TARGET_TYPE');

            baModelRefLinkLibs = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'MODELREF_LINK_LIBS');

            baRelativePathToAnchor = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'RELATIVE_PATH_TO_ANCHOR');

            baModelLib = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'MODELLIB');

            baSharedSrc = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'SHARED_SRC');

            baSharedSrcDir = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'SHARED_SRC_DIR');

            baSharedBinDir = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                'SHARED_BIN_DIR');
            
            baSharedLib = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
                    'SHARED_LIB');
            ...
            if topmodelbuild                
                ...
                %
                skipFiles = {'ert_main' % SIL / PIL has its own main
                             [buildInfo.ModelName '_sf'] % ERT S-fcn
                             [buildInfo.ModelName '_bio'] % g598514
                             [buildInfo.ModelName '_pt']}; % g598514
                modelModules = '';
                for srcFileIdx=1:length(prebuiltSrcFiles)
                    srcFile = prebuiltSrcFiles{srcFileIdx};
                    % skip anything on the skip list
                    [~, srcName] = fileparts(srcFile);
                ...
                baGenerateERTSfun = rtw.connectivity.MakefileBuilder.getBuildArg(buildInfo, ...
        ...
        function build(this, buildInfo)
            ...
            try            
                ...
                rtwVerboseParam = this.getComponentArgs.getParam('RTWVerbose');
                % convert to logical
                ...
                rootModel = this.getComponentArgs.getModelName;
