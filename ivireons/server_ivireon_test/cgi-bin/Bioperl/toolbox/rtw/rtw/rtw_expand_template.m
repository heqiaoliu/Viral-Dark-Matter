function rtw_expand_template(inFileName, outFileName)
...
try
  ...
  % Open input file using latin1 encoding to read bytes in using fgetl
  % latin1 maps 0-255 directly to 0-255 of utf-16 thus ensuring loseless
  % conversion for Japanese chars. Since the read in text is never
  % displayed but only written out as bytes this would guarantee
  % consistency.
  inId = fopen(inFileName,'r','n','latin1');
  if inId < 0
    ...
    for i = 1 : nTokens
      ...
      elseif LocIsTokenInList(token,requiredTokens)
        ...
        ostr = strrep(ostr,tokens{i},[...
            '%<LibGetSourceFileSection(fileIdx,"',token,'")>']);
        e = false;
        switch token
         case 'Includes'
  ...
  % Open output file using latin1 encoding to ensure saving byte in a time
  outId = fopen(outFileName,'w','n','latin1');
  if outId < 0
