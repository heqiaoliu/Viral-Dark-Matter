...
%   $Revision: 1.4.4.5 $
%   $Date: 2010/02/25 08:17:51 $

    % turn off unwanted "AnalyticLinearization" warning 
    warningID = 'Simulink:Engine:JacobianAnalyticLinNotEnabled';
    ...
    if (size(segment_handle,1) == 0)
      TargetCommon.ProductInfo.error('asap2', 'CANLibSignalNotFound');
    end
    ...
    if (trunk_segment_found == 0) 
      TargetCommon.ProductInfo.error('asap2', 'CANLibSignalNoTrunkSegment');
    end;
...
function [block, code] = i_Subsystem(udd_obj, VERBOSE, portnum)
    ...
    if (size(outport_handle, 1) ~= 1) 
      TargetCommon.ProductInfo.error('asap2', 'CANLibSignalNoOutportHandle');
    end;
