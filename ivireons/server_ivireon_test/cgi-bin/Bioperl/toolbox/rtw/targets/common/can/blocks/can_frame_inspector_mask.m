...
%   $Date: 2008/05/01 20:22:45 $
   ...
   case 'callback'
      ...
      if (strcmp(get_param(bdroot(block),'BlockDiagramType'),'library'))
        TargetCommon.ProductInfo.warning('can', 'OpeningCANMsgUnpackingMask');
          return;
      end;
      ...
      parentframe = awtcreate('javax.swing.JFrame');
      ...
      awtinvoke(parentframe, 'setIconImage(Ljava.awt.Image;)', icon);
      ...
      f = awtcreate('com.mathworks.toolbox.ecoder.canlib.CanDB.FrameBuilderMask', ...
                    'Ljava.awt.Frame;ZLcom.mathworks.toolbox.ecoder.canlib.CanDB.Message;Ljava.lang.String;Ljava.lang.String;Ljava.lang.String;', ...
                    parentframe,0,clonedMessage, ...
                    pwd, library_block_name,'Unpacks a CAN message into output data signals');

      f.setTitle(block_name);

      awtinvoke(f, 'pack');
      dialog_size = java.awt.Dimension(650, 700);
      awtinvoke(f, 'setSize(II)', dialog_size.width, dialog_size.height);
      ...
      awtinvoke(f, 'setLocation(II)', new_pos.width, new_pos.height);
   ...
   case 'init'
      ...
      try 
        ...
        TargetCommon.ProductInfo.warning('can', 'ReloadCANdbMessage', ...
...
function i_action_callback(source, event, block, mask_handle)
   ...
   switch char(event.JavaEvent.getActionCommand)
      ...
      case char( FrameBuilderMask.CANCEL_ACTION )
         ...
         i_dispose_mask(block,mask_handle);
     otherwise 
       TargetCommon.ProductInfo.error('common', 'UnsupportedAction', data.actionCommand);
   end
   ...
   if (isempty(message.signals)==0)
    ...
    for i=1:nSigs
        ...
        case 3
            lbl = [ lbl ' D'];
          otherwise
            TargetCommon.ProductInfo.error('can', 'UnsupportedSignalDatatype');
