...
if isempty(stylesheet_full_path)
    TargetCommon.ProductInfo.error('common', 'StyleSheetNotFound', stylesheet);
end
...
if isempty(xmlfile_full_path) 
    ...
    if ~exist(xmlfile_full_path, 'file')
        TargetCommon.ProductInfo.error('common', 'XMLFileNotFound', xmlfile);
    end
end

if rem(length(varargin),2)~=0
  TargetCommon.ProductInfo.error('common', 'ParameterPairsExpected');
end
