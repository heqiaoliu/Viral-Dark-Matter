...
#ifdef PROFILE_LOG_DATA
    uint_T numPoints;
    int_T loggedData[PROFILING_ARRAY_SIZE];
#endif
} prof;
...
void profile_task_start(int_T task_num) {
    ...
    if (logIdx < (PROFILING_ARRAY_SIZE-1) ) {
        prof.loggedData[logIdx++] = (task_num + (int_T)1);
        prof.loggedData[logIdx++] = (int_T)timer;
    }
#endif
#ifdef PROFILE_MAX_TURNAROUNDS
...
#ifdef PROFILE_LOG_DATA
    if (logIdx < (PROFILING_ARRAY_SIZE-1) ) {
        /* Use negative of task id to indicate end of section */
        /* (int_T)0 - is used in this expression to avoid use of unary - 
           operator due to compiler issues with this operator */
        prof.loggedData[logIdx++] = ((int_T)0 - (task_num + (int_T)1));
        prof.loggedData[logIdx++] = (int_T)timer;
    }
#endif
#ifdef PROFILE_MAX_TURNAROUNDS
...
void profile_section_end(int_T section_num) {
    ...
    if (logIdx < (PROFILING_ARRAY_SIZE-1) ) {
        ...
        /* (int_T)0 - is used in this expression to avoid use of unary - 
        operator due to compiler issues with this operator */
        prof.loggedData[logIdx++] =  ((int_T)0 - section_num);
        prof.loggedData[logIdx++] = timer;
    }
#endif
}    
