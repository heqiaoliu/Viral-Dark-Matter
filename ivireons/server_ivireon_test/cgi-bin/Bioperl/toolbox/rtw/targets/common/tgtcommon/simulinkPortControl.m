        ...
        case 'enable'
            ...
            switch state
                    ...
                    case 'on'
                        ...
                        i_enablePort(parentBlock, handle, type);                
              otherwise
                TargetCommon.ProductInfo.error('common', 'PortControlUnrecognisedState');
            end;                    
      otherwise
    ...
    switch lower(type)
        case 'inport'
            ...
            if (hOutputLine == -1)
                ...
                TargetCommon.ProductInfo.error('common', 'PortControlInportConnection');
            end;
            ...
            if (length(controlHandle)~=1)
              TargetCommon.ProductInfo.error('common', 'PortControlSingleControlBlock');
            end;
            ...
            switch type
              ...
              otherwise
                  TargetCommon.ProductInfo.error('common', 'PortControlInports');
            end;
        case 'outport'
            % follow the input
            ...
            if (hInputLine == -1)
                % not connected to anything!
                TargetCommon.ProductInfo.error('common', 'PortControlOutportConnection');
            end;
            ...
            switch type
                case 'outport'
                    % desired case - no action
              otherwise
                  TargetCommon.ProductInfo.error('common', 'PortControlOutports');
            end;
    end;
return;
...
function i_checkControlBlocks(parentBlock)
        ...
        if sscanf(get_param(controlBlocks(i),'isInport'), '%d')
            ...
            if sscanf(get_param(controlBlocks(i),'isConfigurable'), '%d')
                ...
                switch get_param(portHandle, 'BlockType')
                    ...
                    otherwise
                        ...
                        TargetCommon.ProductInfo.error('common', 'PortControlInportControlBlock');
                end;       
            else
                %%
                ...
                switch get_param(portHandle, 'BlockType')
                  ...
                  otherwise
                      TargetCommon.ProductInfo.error('common', 'PortControlInportControlBlockNoConfiguration');            
                end;
            end;
        else
            %%
            ...
            if sscanf(get_param(controlBlocks(i),'isConfigurable'), '%d')
                ...
                switch get_param(portHandle, 'BlockType')
                    ...
                    otherwise
                        % this control block is connected to an invalid block type!
                        TargetCommon.ProductInfo.error('common', 'PortControlOutportControlBlock');
                end;              
            else
                %%
                ...
                switch get_param(portHandle, 'BlockType')
                    case 'Outport'
                        % this is the expected case - do nothing    
                  otherwise
                      TargetCommon.ProductInfo.error('common', 'PortControlOutportControlBlockNoConfiguration');
                end;    
            end;
        end;
    ...
    if (length(inportNumArray)~=length(unique(inportNumArray)))
      TargetCommon.ProductInfo.error('common', 'PortControlInportNumbersUnique');
    end;
    % check range of inport numbers - must go up 1 by 1 from 1
    if ((min(inportNumArray) ~= 1) | (max(inportNumArray) ~= length(inportNumArray)))  
      TargetCommon.ProductInfo.error('common', 'PortControlInportNumberedConsecutively');
    end;
    % verify Outport Numbers are unique
    if (length(outportNumArray)~=length(unique(outportNumArray)))
      TargetCommon.ProductInfo.error('common', 'PortControlOutportNumbersUnique');
    end;
    ...
    if ((min(outportNumArray) ~= 1) | (max(outportNumArray) ~= length(outportNumArray)))  
      TargetCommon.ProductInfo.error('common', 'PortControlOutportNumberedConsecutively');
    end;
    % verify portNames are unqiue
    if (length(portNameCell)~=length(unique(portNameCell)))
      TargetCommon.ProductInfo.error('common', 'PortControlPortNamesNotUnique');
    end;
return;
...
function i_setControlBlockEnabled(block, enabled)
    ...
    switch (enabled)
      ...
      otherwise 
        TargetCommon.ProductInfo.error('common', 'PortControlValuesForState');
    end;
return;
...
function portHandle = i_getHandleOfBlockToManipulate(block, type)
    ...
    if (length(portHandle)~=1)
      TargetCommon.ProductInfo.error('common', 'PortControlControlBlockConnection');
    end;
return;
        ...
        case 'enable'
            switch lower(type)
                ...
                case 'outport'
                    newBlkType = 'Outport';
              otherwise
                  TargetCommon.ProductInfo.error('common', 'PortControlUnknownPortType');
            end;
        case 'disable'
            switch lower(type)
                case 'inport'
...
% return the block handle (and type) of the Control Block in the
...
function [blockhandle, type] = i_getControlBlockWithBlockRef(parentBlock, blockref)
    ...
    if ((length(inportControlBlock) + length(outportControlBlock)) ~= 1)
        ...
        TargetCommon.ProductInfo.error('common', 'PortControlSingleControlBlockNamedPort', blockref);
    end;
...
function [blockhandle, type, configurable] = i_getBlockHandle(parentBlock, blockref)
    if ishandle(blockref)      
        ...
        switch (maskType)
            ...
            case 'Non-Configurable Outport Control'
                ...
                configurable = 0;
          otherwise
            TargetCommon.ProductInfo.error('common', 'PortControlControlInvalidBlockReference');
        end;
    elseif ischar(blockref)
      ...
      TargetCommon.ProductInfo.error('common', 'PortControlBlockReferenceInvalid');
    end;
return;
