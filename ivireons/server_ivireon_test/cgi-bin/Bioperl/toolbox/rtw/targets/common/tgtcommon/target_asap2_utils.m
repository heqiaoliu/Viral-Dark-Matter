...
% TARGET_ASAP2_UTILS Utilities to help each target perform ASAP2 post processing
...
% check the RTWOptions to see if we generated an ASAP2 file that we do
% not want to keep
optExtMode = get_param(modelName, 'ExtMode');
optAsap2 = get_param(modelName, 'GenerateASAP2');
if strcmp(optExtMode, 'off') && strcmp(optAsap2, 'off')
  ...
  if ((strcmp(optExtMode, 'on') || strcmp(optAsap2,'on')) && ~generateCodeOnly)
