...
sub createDependencyFile {
    ...
    if (-e $depfile) {
      print "Merging dependencies into existing dependency file $depfile\n";
    } else {
      print "Creating new dependency file $depfile\n"; 
    }
    ...
    foreach $include ( @includes ) {
        if (!defined($dependencies{$include})){
            if ( $fullpath = findpath($include) ) {
                ...
                # discard absolute paths, which return "" from generate_relative_path
                if ($dependencies{$include} ne "") {
...
sub generate_relative_path {
   ...
   # old code would return an absolute path
   # return $end if !$common_volume;
   # 
   # now, return an empty string
   return "" if !$common_volume;
