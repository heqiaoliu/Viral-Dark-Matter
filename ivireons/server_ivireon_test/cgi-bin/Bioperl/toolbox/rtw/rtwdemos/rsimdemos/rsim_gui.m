...
%   $Revision: 1.3.4.2 $  $Date: 2006/06/20 20:22:43 $
...
case 'Run_Executable'
  ...
  elseif isunix == 1
      %handle directories with spaces with the call to unix
      run_file = regexprep(run_file,' ','\\ ');
      lastFile = regexprep(handles.LastFile,' ','\\ ');
      outMATFile = regexprep(handles.OutputMATFile,' ','\\ ');
      
	[err,msg] = unix([run_file,' -p ',lastFile,' -o ', ...
		outMATFile ' -v']);
  end
  if err > 1
