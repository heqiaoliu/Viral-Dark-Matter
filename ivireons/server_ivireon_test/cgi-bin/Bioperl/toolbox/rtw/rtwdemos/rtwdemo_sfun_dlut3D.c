/*
 *   rtwdemo_sfun_dlut3D.c Simple C-MEX S-function for function call.
 *
 *   ABSTRACT:
 *     The purpose of this sfunction is to call a simple legacy
 *     function during simulation:
 *
 *        single y1 = DirectLookupTable3D(single p1[][][], uint32 p2[3], uint32 u1[3])
 *
 ...
 */
#define S_FUNCTION_NAME                rtwdemo_sfun_dlut3D
...
static void mdlCheckParameters(SimStruct *S)
{
  ...
  if EDIT_OK(S, 0) {
    ...
    if (mxGetNumberOfDimensions(ssGetSFcnParam(S, 0)) < 3) {
      ssSetErrorStatus(S,"Parameter 1 must have 3 dimensions");
      return;
    }

    /* Check the parameter attributes */
    ssCheckSFcnParamValueAttribs(S, 0, "P1", DYNAMICALLY_TYPED, 3, dimsArray, 0);
  }
  ...
  if EDIT_OK(S, 1) {
    ...
    /* Parameter 2 must have 3 elements */
    if (mxGetNumberOfElements(ssGetSFcnParam(S, 1)) != 3) {
      ssSetErrorStatus(S,"Parameter 2 must have 3 elements");
      return;
    }
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (ssRTWGenIsCodeGen(S)) {
     ...
     */
    ssRegMdlInfo(S, (char*)"DirectLookupTable3D", MDL_INFO_ID_RESERVED, 0, 0,
                 (void*) ssGetPath(S));
  }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
   ...
   */
  *y1 = DirectLookupTable3D( p1, p2, u1);
}
