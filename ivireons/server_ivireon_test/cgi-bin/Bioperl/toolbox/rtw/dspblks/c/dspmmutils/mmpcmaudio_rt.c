 ...
 * $Revision: 1.1.6.6 $  $Date: 2007/08/03 21:34:53 $
 */
...
EXPORT_FCN COPY_PCM_SAMPLES_FUNC MWDSP_GetPCMAudioCopyFcn(AudioDataType dType, int_T numBits)
{
...
EXPORT_FCN COPY_PCM_SAMPLES_FUNC MWDSP_GetPCMAudioEmptyFcn(AudioDataType dType)
{
...
EXPORT_FCN OUTPUT_PCM_SAMPLES_FUNC MWDSP_GetPCMAudioOutputFcn(AudioDataType dType, int_T numBits)
{
    int_T offset = dType * NUM_AUDIO_BIT_DEPTHS + 4 - (numBits / 8);
    if(offset >= 0 && offset < (NUM_DTYPES * NUM_AUDIO_BIT_DEPTHS))
