...
%   $Revision: 1.8.2.4 $ $Date: 2008/05/31 23:18:35 $
...
%             'Position',Position, 'MaskValueString','fis');
        add_block('fuzwiz/FIS Wizard',sprintf('%s/FIS Wizard',CB),'Position',Position);
        set_param(sprintf('%s/FIS Wizard',CB),'MaskValueString','fis');
    end
else
    ...
    if ~isempty(FISWIZ)
        ...
        delete_block(FISWIZ);
        load_system('fuzwiz')
        add_block('fuzwiz/FIS S-function',sprintf('%s/FIS S-function',CB),'Position',Position);
    end
end
