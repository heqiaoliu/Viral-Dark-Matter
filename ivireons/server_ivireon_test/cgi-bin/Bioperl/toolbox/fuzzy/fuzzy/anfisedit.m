...
%   $Revision: 1.42.4.12 $  $Date: 2010/03/22 03:48:07 $
...
case '#initialize',
    ...
    mcwHndl=LocalBuildUi(mcwPos, 'radio', 'anfisedit #radioloadfrom', 'file', 'trndatin');
    ...
    lableHandle=LocalBuildUi(labelPos, 'radio', 'anfisedit #radiogenfis', 'Load from file', 'genfis');
...
case '#start',
    ...
    elseif ~isfield(fis, 'rule') | isempty(fis.rule)
        ...
        if strcmp(stopflag, 'Train Now')
            ...
            try
                [trnrows,trncols] = size(fis.trndata);
                if ~isequal(trncols, getfis(fis,'numinputs')+getfis(fis,'numoutputs'))                    
                    error('Training data has incorrect number of columns');
                end
                for i=1:numEpochs
...
case '#genfis',
    ...
    case 'Grid partition'
        ...
        if ~isempty(param)
            ...
            try
                ...
                uiwait(errordlg(ME.message,'Generate FIS Error', 'modal'))       
                fismat = [];
            end
        end
    ...
    case 'Load from worksp.'
        fismat=[];
        vname=inputdlg('input variable name:','Load from workspace');
...
case '#test',
    ...
    if ~isempty(testdata)
        ...
        try
            ...
            uiwait(errordlg(ME.message,'Test Error', 'modal'))                   
            return
        end
    ...
    for i=1:length(typeHndl)
        if get(typeHndl(i), 'Value')==1 
            ...
            if strcmp(out, 'Yes')
                ...
                case 'Testing',
                    fis.tstdata=[];
                    Line = findobj(gca,'type','line','tag','test');                    
                case 'Training',
                    fis.trndata=[];
                    Line = findobj(gca,'type','line','tag','train');                    
                case 'Checking',
                    fis.chkdata=[];
                    Line = findobj(gca,'type','line','tag','check');                                        
                case 'Demo',
                    Line = findobj(gca,'type','line','tag','demo');                                        
                end
...
listener = LocalAddHGListeners(MainFigHndl,SubFigHndl);
% Store the listener
...
for id = 1:numin
    for kid = 1:nummf(id)
        ...
        zcircle = 10*ones(1,length(ycircle));
        % Plot the connections between the inputmf nodes and the rule nodes
...
function listener = LocalAddHGListeners(MainFigHndl,SubFigHndl)
listener = addlistener(MainFigHndl, 'ObjectBeingDestroyed', ...
    @(x,y) LocalCloseMainFigCallback(x, y, SubFigHndl));
