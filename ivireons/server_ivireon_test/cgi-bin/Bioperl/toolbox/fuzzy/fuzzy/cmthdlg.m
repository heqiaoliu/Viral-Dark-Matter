...
%   $Revision: 1.16.2.3 $  $Date: 2008/05/31 23:18:34 $
...
elseif strcmp(action,'#okay'),
    ...
    oldmethodName = getfield(fis,methodType);
    try
        oldmethodName = getfield(fis,methodType);
        fis = setfield(fis,methodType,methodName);
        dummyinputs = [];
        for i= 1:1:length(fis.input)
            dummyinputs = [dummyinputs fis.input(1).range'];                                    
        end
        evalfis(dummyinputs, fis);
    catch ME
        errordlg(sprintf('Custom method %s is undefined',methodName), 'FIS Editor Error', 'modal');
        fis = setfield(fis,methodType,oldmethodName);
        set(editHndl,'String', oldmethodName);
    end
