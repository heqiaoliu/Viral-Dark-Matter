...
/* $Revision: 1.15.4.2 $  $Date: 2009/10/16 04:53:54 $  $Author: batserve $ */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	fis->trn_data_n = (int) mxGetM(TRN_DATA);
	if (nlhs == 5) {
		fis->chk_data = matlab2c(CHK_DATA);
		fis->chk_data_n = (int) mxGetM(CHK_DATA);
	}

	/* get training parameters */
	fis->epoch_n = (int) mxGetPr(TP)[0];
