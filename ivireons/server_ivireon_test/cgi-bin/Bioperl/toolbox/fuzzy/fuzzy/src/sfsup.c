...
/* $Revision: 1.1.8.1 $  $Date: 2009/10/16 04:53:59 $  */
...
FIS *matlab2cStr(const mxArray *matlabmat, int numofpoints)
{
	...
	fis->in_n = (int) mxGetN(fispointer);
	...
	for (i = 0; i < fis->in_n; i++){
        ...
        if (inputpointer!=NULL)
            in_mf_n[i] = (int) mxGetN(inputpointer);
        else
            in_mf_n[i] = 0;
        }
	io_list = fisBuildIoList(fis->in_n, in_mf_n);
	...
	for(i=0; i<fis->in_n; i++){
		...
		inputpointer=mxGetField(fispointer, i, "mf");
		if (inputpointer!=NULL)
			fis->input[i]->mf_n = (int) mxGetN(inputpointer);
		else
			fis->input[i]->mf_n = 0;
		...
		for(j=0; j<fis->input[i]->mf_n; j++){
            ...
            param_size = (int) mxGetN(mfpointer);
	...
	fis->out_n = (int) mxGetN(fispointer);
	...
	for (i = 0; i < fis->out_n; i++){
        inputpointer=mxGetField(fispointer, i, "mf"); 
        if (inputpointer!=NULL)
			out_mf_n[i] = (int) mxGetN(inputpointer);
		else
			out_mf_n[i] = 0;
        }
	io_list = fisBuildIoList(fis->out_n, out_mf_n);
	...
	for(i=0; i<fis->out_n; i++){
		...
		inputpointer=mxGetField(fispointer, i, "mf");
		if (inputpointer!=NULL)
           fis->output[i]->mf_n = (int) mxGetN(inputpointer);
		else
           fis->output[i]->mf_n=0;
        ...
        for(j=0; j<fis->output[i]->mf_n; j++){
			...
			if (strcmp(fis->type, "sugeno") == 0) {
				...
				*/
				...
				data_size = ((int) mxGetM(mfpointer)) * ((int) mxGetN(mfpointer));
				...
				/* Mamdani system */
				param_size = ((int) mxGetM(mfpointer)) * ((int) mxGetN(mfpointer));
				fis->output[i]->mf[j]->nparams = param_size;	
				fis->output[i]->mf[j]->params = (DOUBLE *)fisCalloc(param_size,sizeof(DOUBLE));
				for(k=0; k<param_size; k++)
					fis->output[i]->mf[j]->params[k] = *(real_data_ptr++);
    ...
    if (fispointer==NULL)
		...
		fis->rule_n = (int) mxGetN(fispointer);
...
{
	int m = (int) mxGetM(matlabmat);
	int n = (int) mxGetN(matlabmat);
