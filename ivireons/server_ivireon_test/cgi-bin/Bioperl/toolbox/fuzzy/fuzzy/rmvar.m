...
%   $Revision: 1.30.4.1 $  $Date: 2009/04/21 03:19:28 $
...
case 'i',
   ...
   for i=1:numRules,
    ...
    if strcmpi(fis.type,'sugeno') && isfield(fis,'output') && ~isempty(fis.output)
        for ct=1:length(fis.output.mf)
            if strcmpi(fis.output.mf(ct).type,'linear')
                fis.output.mf(ct).params(varIndex) = [];
            end
        end
    end
