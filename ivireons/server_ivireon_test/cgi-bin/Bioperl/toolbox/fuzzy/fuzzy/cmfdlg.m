...
%   $Revision: 1.20.2.3 $  $Date: 2008/05/31 23:18:33 $
...
elseif strcmp(action,'#okay'),
    ...
    try
        evalmf(fis.(varType)(varIndex).range, mfParams, mfType);
    catch ME
        mfIndex = -1;
        for i = 1:getfis(fis,varType, varIndex, 'nummfs')
            if isequal(fis.(varType)(varIndex).mf(i).name, mfName)
                mfIndex = i;                
            end            
        end        

        fis=rmmf(fis,varType,varIndex,'mf',mfIndex);
        errordlg(sprintf('%s\nCustom membership function cannot be added',ME.message), 'Membership Function Editor Error', 'modal');
