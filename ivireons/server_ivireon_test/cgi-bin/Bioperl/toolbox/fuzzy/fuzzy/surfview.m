...
%   $Revision: 1.49.2.8 $  $Date: 2009/12/05 02:01:12 $
...
if strcmp(action,'#initialize'),
    ...
    labelStr='Ref. Input:';
    pos=[left-spacing*.5 bottom btnWid btnHt];
    helpHndl=uicontrol( ...
        'Style','text', ...
        'BackgroundColor',frmColor', ...
        'HorizontalAlignment','left', ...
    ...
    name='refinputedit';
    callbackStr='surfview #refinputedit';
    pos=[left+btnWid-2*spacing bottom right-left-btnWid*2.75 btnHt];
...
elseif strcmp(action,'#refinputedit');
    ...
    % Replaced the following code with the code block below to sidestep
    % error thrown by eval.
    % newRefInput=eval(get(refInputHndl,'String'),'oldRefInput');
    if ~isempty(deblank(get(refInputHndl,'String')))
        newRefInput = eval(get(refInputHndl,'String'));        
    else
...
elseif strcmp(action,'#grids');
    ...
    try
        gridNum=eval(get(gridHndl,'String'));
    catch ME
        gridNum=15;
    end
...
elseif strcmp(action,'#evaluate');
    ...
    % retrieve number of sample points
    numPts=101;
    pthndl=findobj(gcf, 'Tag', 'numdisp');
    temp=str2double(get(pthndl, 'String'));
    if ~isempty(temp) && ~isnan(temp)
    ...
    [x,y,z]=gensurf(fis,[xIndex yIndex],zIndex,[xGrids yGrids],refInput,numPts);
