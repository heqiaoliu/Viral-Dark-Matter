...
%   $Revision: 1.13.2.6 $  $Date: 2010/03/22 03:48:03 $
...
elseif ~isempty(flag) & flag == 2,
   ...
   % ====== return nothing
   closeHandle=findobj(AnimTbuFigH, 'style', 'pushbutton', 'String', ' Close');
...
elseif ~isempty(flag) & flag == 0,
   ...
   if isempty(AnimTbuFigH),
      ...
      tmp = double([truckH f_wheelsH r_wheelsH wheelH stickH circleH delta1H delta2H]);
      tmp = reshape(tmp,[1 8]);
      tmp = [tmp -1 -1];
      set(AnimTbuFigH, 'userdata', [get(AnimTbuFigH, 'userdata'); tmp]);
