...
%   $Revision: 1.16.2.5 $  $Date: 2010/03/22 03:47:59 $
...
%    tmp = get(AnimBbFigH, 'userdata');
    ...
    % ====== return nothing
    closeHandle=findobj(AnimBbFigH, 'style', 'pushbutton', 'String', ' Close');
...
elseif ~isempty(flag) & flag == 0,
    ...
    if isempty(AnimBbFigH),
        ...
        forceH = line(real(force), imag(force), ...
            'erase', 'xor', 'color', 'c', 'clipping', 'off');
        ...
        tmp = double([ballH beamH forceH refH]);
        tmp = reshape(tmp,[1 4]);
        tmp = [tmp -1 -1 -1 -1 -1 -1];
        olduserdata=get(AnimBbFigH, 'userdata');
        newuserdata.tmp=[olduserdata.tmp; tmp];
