...
%   $Revision: 1.1.6.1 $ $Date: 2008/10/31 07:11:15 $ $Author: batserve $
  ...
  if strcmp(get(w, 'view'), 'on')
    ...
    while 1
      ...
      if etime(clock, t0) > timeout
        warning('VR:protocoltimeout', 'Timeout has occurred opening the remote viewer. Try increasing the "TransportTimeout" property.');
        break;
      end
    end
