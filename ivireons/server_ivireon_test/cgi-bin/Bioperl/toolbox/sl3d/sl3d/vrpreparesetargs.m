...
%   $Revision: 1.1.6.1 $ $Date: 2008/10/31 07:09:41 $ $Author: batserve $
...
while ~isempty(args)
  ...
  % property is a structure - field names are property names, field values are property values
  ...
  argname = [argname newname];   %#ok<AGROW> number of elements unknown in advance
  if size(newval, 1) == num
    argval = [argval newval];    %#ok<AGROW> number of elements unknown in advance
  elseif size(newval, 1) == 1
    argval = [argval newval(ones(1, num), :)];   %#ok<AGROW>  number of elements unknown in advance
  else
