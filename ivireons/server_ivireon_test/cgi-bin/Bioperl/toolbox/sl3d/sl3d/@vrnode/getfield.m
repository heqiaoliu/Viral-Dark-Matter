...
%   $Revision: 1.1.6.4 $ $Date: 2010/03/01 05:30:56 $ $Author: batserve $
...
if nargin<2
  ...
  for i=1:numel(fieldlist)
    ...
    if any(strcmp(acc, {'exposedField', 'field', 'eventOut'}))
...
for i=1:size(y, 1)
  ...
  for j=1:size(y, 2)
    ...
    [y{i,j}, nodeType] = vrsfunc('VRT3GetField', wid, node(i).Name, fieldname{j});

    
    % convert to VRNODE if needed
    if nodeType
      names = y{i,j};
      if isa(names, 'char')
        % SFNode 
       y{i,j} = vrnode(get(node, 'World'), names);  
      else
        % MFNode  
        nodes = vrnode([]);
        for k=1:numel(names)
          nodes(k) = vrnode(get(node, 'World'), names{k});  
        end
        y{i,j} = nodes;
      end
    end
