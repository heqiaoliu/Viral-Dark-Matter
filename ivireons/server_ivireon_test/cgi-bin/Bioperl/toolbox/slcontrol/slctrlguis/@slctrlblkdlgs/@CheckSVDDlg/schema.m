...
%   $Revision: 1.1.8.1 $ $Date: 2010/03/26 17:55:50 $
...
hThisClass = schema.class(hPackage, 'CheckSVDDlg', hParent);
...
% preApplySVDCallback
m = schema.method(hThisClass,'preApplySVDCallback');
m.Signature.varargin    = 'off';
m.Signature.InputTypes  = {'handle','handle'};
m.Signature.OutputTypes = {'bool','string'};
% postApplySVDCallback
m = schema.method(hThisClass,'postApplySVDCallback');
m.Signature.varargin    = 'off';
m.Signature.InputTypes  = {'handle','handle'};
m.Signature.OutputTypes = {'bool','string'};
