function getInspectorPanelData(this,linearizeroot)
%GETINSPECTORPANELDATA

%  Author(s): John Glass
%  Revised:
%   Copyright 1986-2010 The MathWorks, Inc.
% $Revision: 1.1.6.28 $ $Date: 2010/02/25 08:34:12 $
...
stateBlockPath = J1.stateBlockPath;
...
% Remove carriage return from linearizeroot
linearizeroot = regexprep(linearizeroot,'\n',' ');

% Remove references to hidden buffers
BlockNameList(HiddenBuffers) = [];
BlockHandles(HiddenBuffers) = [];
BlocksInPath(HiddenBuffers) = [];
BlockAnalyticFlags(HiddenBuffers) = [];

% Store the diagnostic data
DiagnosticMessages = struct('BlockName',getfullname(BlockHandles),...
                             'BlockType',get_param(BlockHandles,'BlockType'),...
                             'InPath',[], 'Type',[],'Message',[]);
for ct = 1:numel(BlockHandles)
    DiagnosticMessages(ct).InPath = BlocksInPath(ct);
    DiagnosticMessages(ct).Type = BlockAnalyticFlags(ct).jacobian.type;
    DiagnosticMessages(ct).Message = BlockAnalyticFlags(ct).jacobian.message;
end
this.DiagnosticMessages = DiagnosticMessages;

% Store the stripped down list of blocks that are in the path
this.BlocksInPathByName = BlockNameList(BlocksInPath);

% Tree Creation: Find the unique subsystem and normal mode model references
[MdlRefParents,MdlRefModels] = getNormalModeBlocks(slcontrol.Utilities,linearizeroot);
models = [linearizeroot;MdlRefModels];
...
UniqueParents = unique([linearizeroot;UniqueParents(:);MdlRefParents(:)]);

% If the system being linearized is a non-virtual block then do not
...
for ct = numel(UniqueParents):-1:1
    ...
    if strcmp(UniqueParents{ct},linearizeroot)
        this.InspectorNode = nodes(ct);
    end
...
for ct = 1:length(UniqueParents)
    ...
    switch class(get_param(UniqueParents{ct},'Object'))
        ...
        case 'Simulink.ModelReference'
            ...
            if isa(get_param(NodeParent,'Object'),'Simulink.BlockDiagram') && ...
                  ~strcmp(NodeParent,linearizeroot)
    ...
    for ct2 = 1:nnormal_child
        ...
        elements = strfind(stateBlockPath,getfullname(BlockHandles(ind_children(ct2))));
        state_ind = find(~cellfun(@isempty,elements));
