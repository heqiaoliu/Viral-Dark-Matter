...
% $Revision: 1.1.6.2 $ $Date: 2005/03/31 16:43:38 $
...
for ct = 1:length(rows)
    ...
    if (wantVisible && ~isVisible)
        ...
        rowelements = 1:size(VisibleResultTableModel.data,1);
        if length(rowelements) > 0
            VisibleResultTableModel.data(rowelements,row+1) = java.lang.Boolean(true);
        end
