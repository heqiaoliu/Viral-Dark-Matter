...
if ~isempty(states)
    ...
    state_table = cell(length(states)+total_states,6);
    ...
    for ct1 = 1:length(states)
        ...
        % Put in dummy data
        state_table(counter,2:end) = {'0',false,false,'0','0'};
        % Increment the counter
        counter = counter + 1;
        for ct2 = 1:states(ct1).Nx
            state_table(counter,:) = {sprintf('State - %d',ct2),num2str(states(ct1).x(ct2)),...
                           states(ct1).Known(ct2) == 1,states(ct1).SteadyState(ct2) == 1,...
                           num2str(states(ct1).Min(ct2)),num2str(states(ct1).Max(ct2))};
            counter = counter + 1;
        end
    end    
else
    ...
    state_ind = 0;
    state_table = {ctrlMsgUtils.message('Slcontrol:operpointtask:ModelHasNoStates',this.Model),...
                        '',false,false,'',''};
end