function S = MDTransferFcn(this,blockname,TunableParameters)
% MDTRANSFERFCN This is the configuration function for the discretized
% transfer function block.
%

% Author(s): Erman Korkut 22-Apr-2008
%   Copyright 2005-2008 The MathWorks, Inc.
% $Revision: 1.1.8.1 $ $Date: 2008/10/31 07:34:22 $
...
FilteredTunableParameters(3) = struct('Name','SampleTime',...
                              'Value',TunableParameters(strcmp('SampleTime',{TunableParameters.Name})).Value,...
                              'Tunable','off');

FilteredTunableParameters(4) = struct('Name','method',...
                          'Value',method,...
                          'Tunable','off'); 
                      
FilteredTunableParameters(5) = struct('Name','Wc',...
...
ts = TunableParameters(3).Value;
method = TunableParameters(4).Value;

sys = zpk(tf(num,den));
if strcmp(method,'prewarp')
  WcRad = TunableParameters(5).Value*2*pi;
...
sysd = zpk(z,p,k,TunableParameters_in(3).Value);
if strcmp(TunableParameters_in(4).Value,'prewarp')
  WcRad = TunableParameters_in(5).Value*2*pi;
  sys = d2c(sysd,TunableParameters_in(4).Value,WcRad);
else
  sys = d2c(sysd,TunableParameters_in(4).Value);
end
