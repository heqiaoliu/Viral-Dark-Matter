...
%   $Revision: 1.10.2.14 $  $Date: 2010/02/25 08:30:46 $
function update_assert_sys(block,varargin)
if (local_is_checker(block))
  type=get_param(block, 'masktype');
  if (nargin > 1 )
      if(strcmp(varargin(1),'updateSimulationCallBackEditField'))
          local_updateSimulationCallBackEditField(block);
          return
      end
  end
  ...
  switch (type)
   case 'Checks_Gradient'
    ...
    case 'Checks_Resolution'
      local_update_resolution(block, type);
   case { ...
  ...
  DAStudio.error('Simulink:blocks:unknownBlockType');
end;
...
function local_updateSimulationCallBackEditField(block)
% update simulation call back edit field without having to click 'Apply'

    isEnableCheckBoxOff = strcmp(get_param(block, 'enabled'), 'off');
    names        = get_param(block, 'MaskNames');
    enableStatus = get_param(block, 'MaskEnables');
    for i = 1:length(names)
        if strcmp(names{i}, 'enabled')
            enableStatus{i} = 'on';
        end    
    end
    ...
    for i = 1:length(names)
        if strcmp(names{i}, 'callback')
            if (isEnableCheckBoxOff)
                % If the checkbox for the "Enable Assertion"
                % is off then the "Simulation callback" edit
...
function local_update_enables(block, ~)
...
function local_update_boundary(block, type)
  ...
  switch (type)
   ...
   case 'Checks_DRange'
    ...
    relop_max = local_relop(max_included);
   otherwise
      DAStudio.error('Simulink:blocks:invalidType', type);
  end; % switch type
...
function local_update_icon(block)
  ...
  switch ( lower(mode) )
   case 'graphic'
    iconStr = local_set_graphic_icon(block);
   case 'text'
    iconStr = local_set_text_icon(block);
   otherwise
    DAStudio.error('Simulink:blocks:wrongArgument', mode, 'switch_icon');
  end
  
  userData = get_param(block, 'UserData');
  if (~userData.enabled)  && ~isOveride
      iconStr = [iconStr, sprintf('\n'), 'plot([0 1], [0 1]);', sprintf('\n'), 'plot([0 1],[1 0]);'];
  end
  ...
  if (~userData.enabled) && isOveride
      ...
      iconStr = [iconStr, sprintf('\n'), 'text(' xPosStr ',0.9,''Override'');'];
  end
  
  oldIconStr = get_param(block, 'MaskDisplay');
  newIconStr = sprintf(iconStr);
  if ~strcmp(oldIconStr,newIconStr)
  	set_param(block, 'MaskDisplay', newIconStr);
  end
   ...
   case 'Checks_Gradient'
    ...
    case 'initializing'
      ...
      if (~strcmp(solver, 'FixedStepDiscrete'))
          isGloballyEnabled = strcmp(get_param(bdroot(block), 'AssertionControl'),'EnableAll');
          isLocallyEnabled  = strcmp(get_param(bdroot(block), 'AssertionControl'),'UseLocalSettings') ...
              && strcmp(get_param(block, 'enabled'), 'on');
          if(isGloballyEnabled || isLocallyEnabled)
              DAStudio.error('Simulink:blocks:requiresFixedStepDiscreteSolver');
          end
      end
    otherwise
      % Don't check solver during edit time.
    end
   otherwise
    DAStudio.error('Simulink:blocks:invalidType', type);
  end
...
%
function local_update_resolution(block, type)

% Return early if model is running
simStatus   = get_param(bdroot(block), 'SimulationStatus');
simMode     = get_param(bdroot(block), 'SimulationMode');
isRunning   = strcmp(simStatus, 'running') || strcmp(simStatus, 'paused');
isExternSim = strcmp(simMode, 'external') && ...
                strcmp(simStatus, 'initializing') || ...
                strcmp(simStatus, 'terminating') || ...
                strcmp(simStatus, 'external');
if (isRunning || isExternSim)
    return;
end

resolution_length = length(str2num(get_param(block, 'resolution')));%#ok
switch (type)
  case 'Checks_Resolution'
