...
%   $Revision: 1.1.6.8 $ $Date: 2009/01/29 17:25:02 $
...
switch(nargin) 
 ...
 otherwise,
  DAStudio.error('Simulink:blocks:invalidNumInputs', num2str(nargin));
end
...
catch GotoDlgException
  applyNoError = 0;
  try
    % recover the parameter
    set_param(ud.blkHdl, 'GotoTag', prevTag);
  catch NewGotoDlgException
      GotoDlgException = GotoDlgException.addCause(NewGotoDlgException);
  end
  errmsg = [errmsg; {GotoDlgException.getReport('basic')}];
end
...
% Function: doHelp =============================================================
% Abstract:
%   This function will load online documentation for Goto block.
%
...
if ~isempty(listStr)
  ...
  if ~isempty(fromSrc)
    try
      ...
      msg = ['Unable to find From block named: "' fromSrc '". ' GotoDlgException.getReport('basic')];
...
if ~strcmp(get_param(bdroot(blkHdl),'Lock'), 'on')
  ...
  try
    ...
    errordlg(GotoDlgException.getReport('basic'));
  end
  set_param(ud.blkHdl, 'Figure', hdl);
