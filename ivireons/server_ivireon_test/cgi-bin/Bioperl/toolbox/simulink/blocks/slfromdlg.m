...
%   $Revision: 1.1.8.8 $ $Date: 2009/01/29 17:25:01 $
...
try
  ...
  ud = doRefresh(ud);
catch FromDlgException
  applyNoError = 0;
  try
    % recover the parameter
    set_param(ud.blkHdl, 'GotoTag', prevTag);
  catch NewFromDlgException
      FromDlgException = FromDlgException.addCause(NewFromDlgException);
  end
  errmsg = [errmsg; {FromDlgException.getReport('basic')}];
end
...
if ~isempty(deblank(gotoSrc))
  try
    ...
    msg = ['Unable to find Goto block named: "' gotoSrc '". ' FromDlgException.getReport('basic')];
...
% Function: doHelp =============================================================
% Abstract:
%   This function will load online documentation for From block.
%
