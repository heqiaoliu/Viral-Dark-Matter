...
elseif nargin == 1
    ...
    DAStudio.error(Simulink:utility:NumInputMismatch);
end
...
% Status is 0 if the user has pressed cancel on the waitbar
if status == 0
  return;
end
...
% For testing, in case of tlc profiler, if get_param(iModelName,'ProfileOutputDir')
...
if isempty(iOutputDir) || (isTLC && isempty(get_param(iModelName,'ProfileOutputDir')))
  if usejava('mwt') == 1
    ...
    if strcmp(pname, 'Simulink')
        dispMsg = DAStudio.message('Simulink:tools:slprofNotMATLABWebBrowser');
        disp(dispMsg);
    end
  ...
  switch status
   case 0
    % no action required
   case 1
        DAStudio.warning('Simulink:tools:slprofBrowserNotFound');
   case 2
        DAStudio.warning('Simulink:tools:slprofBrowserNotLaunched');
   otherwise
...
%endfunction slprofreportcancel
...
wbHandle = waitbar(0,'Generating function details profile report ...',...
                   'CreateCancelBtn',@slprofreportcancel);
...
% is the line time.  Third column is the line string.  P is the number
