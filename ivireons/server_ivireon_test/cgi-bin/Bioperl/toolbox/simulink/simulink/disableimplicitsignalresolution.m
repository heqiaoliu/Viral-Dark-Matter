...
%   $Revision: 1.1.6.10 $  $Date: 2008/12/01 07:51:14 $
  ...
  if strcmp(origSetting, 'UseLocalSettings')
    DAStudio.warning('Simulink:tools:TempImplicitSignalResolution');
  ...
  try
    ...
    if strcmp(origSetting, 'UseLocalSettings')
      disp(DAStudio.message('Simulink:tools:RestoreSignalResolutionControl'));
    end
    ...
    if (~displayOnly)
      ...
      for idx = 1:length(hPorts)
        ...
        if l_IsPortInsideStateflowChart(thisPort)
          ...
          % Set "PermitHierarchicalScoping" to 'ExplicitOnly' for Stateflow chart
