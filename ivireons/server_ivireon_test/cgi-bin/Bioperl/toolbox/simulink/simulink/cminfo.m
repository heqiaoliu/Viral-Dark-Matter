...
% Field can be 'AllInfo', 'Fields', 'Separators', 'Commands'
  ...
  if isempty(CMInList)
    ...
    if ~isempty( findstr(CM, 'rcs') ),
      y = rcsAllInfo;
    elseif ~isempty( findstr(CM, 'sourcesafe') ),
      y = msvsAllInfo;
    elseif ~isempty( findstr(CM, 'pvcs') ) || ~isempty( findstr(CM, 'merant') ),
        % PVCS has changed name a few times so we have multiple strings
        % mapping to one tool here:
      y = pvcsAllInfo;
    end
...
% Returns all configuration managers defined
  ...
  y = {'None'; 'RCS'; 'Microsoft Visual SourceSafe'; 'PVCS (Merant)';'Merant Version Manager';'PVCS'};
