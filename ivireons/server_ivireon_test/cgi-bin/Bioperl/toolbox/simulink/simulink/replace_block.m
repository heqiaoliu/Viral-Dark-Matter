...
%
if nargin < 3,
  error('Simulink:replace_block:InputsProblem', 'Not enough input arguments.');
end
...
try
  ...
  error('Simulink:replace_block:InvalidSystem', 'System must be a valid model or subsystem name.');
end
...
%   - the block is a built-in type specified with or without 'built-in'
...
if isempty(NewHandle),
    error('Simulink:replace_block:InvalidBlockHandle', ...
        ['Invalid NewBlock (' NewBlock  ...
...
if ~isempty(OldBlocks),
  ...
  if OK,
    ...
    if strcmp(get_param(bdroot(System),'Lock'),'on'),
      error('Simulink:replace_block:LockedLibrary', ...
      ['The model must be unlocked to make changes to it. ', CR, ...
             'To unlock the model, select "Unlock Library" from the ', ...
             'Edit menu.  ', CR, ...
             'Otherwise enter the command:', CR, ...
