...
%   $Revision: 1.3.2.5 $
  ...
  switch action,
    ...
    otherwise,
      DAStudio.error('Simulink:dialog:MAssertUnknownAction');
  end
...
% Calculate width and height of uicontrols.  Returns a position vector for 
...
function i_Sync(ud)
  ...
  switch(nameMode),
    ...
    case 'Custom',
      val = 2;
    otherwise,
      DAStudio.error('Simulink:dialog:DCDErrAssertionInvalidVal');
  end
...
function i_Apply(fig),
  ...
  try
    ...
    errordlg(ME.message, DAStudio.message('Simulink:dialog:ErrorText'),'modal');
  end
