...
switch (Action)
 case 'Create'
  ...
  if ~usejava('MWT')
    error(DAStudio.message('RTW:configSet:ConfigBlkError'));
  end
...
if ~isempty(blks)
  ...
  configIdx = strcmp(get_param(blks(subsysIdx), 'TemplateBlock'), '')==0;
...
for i = 1:length(choices)
  % Build list of inport and outport names
  mem = [sys '/' choices{i}];

  %% SUBSYSTEM BLOCK
  if strcmp(get_param(mem, 'BlockType'), 'SubSystem')
    % Get Inport names
    inports = find_system({mem},...
                          'Searchdepth', 1,...
                          'LookUnderMasks','on',...
                          'FollowLinks','on',...
                          'BlockType','Inport');
    inNames = [inNames; get_param(inports, 'Name')]; %#ok

    % Get Outport Names
    outports = find_system({mem},...
                           'Searchdepth', 1,...
                           'LookUnderMasks','on',...
                           'FollowLinks','on',...
                           'BlockType','Outport');
    outNames = [outNames; get_param(outports, 'Name')]; %#ok
    
  elseif strcmp(get_param(mem, 'BlockType'), 'ModelReference')
    %% MODEL REFERENCE block
    ins = get_param(mem, 'InputPortNames');
    inNames = [inNames; struct2cell(ins)]; %#ok

    outs = get_param(mem, 'OutputPortNames');
    outNames = [outNames; struct2cell(outs)]; %#ok

  else
    %% Built-in block
    ports = get_param(mem, 'Ports');
    ins   = ports(1);
    outs  = ports(2);
    for k = 1:ins
        inNames = [inNames; {['In', num2str(k)]}]; %#ok
    end
    for k = 1:outs
        outNames = [outNames; {['Out', num2str(k)]}]; %#ok
    end
...
% Make it unique
inNames = unique_unsorted(inNames);
inNames = strrep(inNames, sprintf('\n'), ' ');

outNames = unique_unsorted(outNames);
outNames = strrep(outNames, sprintf('\n'), ' ');
...
for i = 1:length(newOrder)
  idx = strcmp(outNames, newOrder{i});
