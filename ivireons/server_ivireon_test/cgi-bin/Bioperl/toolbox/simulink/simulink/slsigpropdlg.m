...
if nargin < 1,
  DAStudio.error('Simulink:dialog:NotEnoughInpArgs');
end

action = varargin{1};
ArgErrorStr = DAStudio.message('Simulink:dialog:InvalidArgsPerfAction', mfilename, action);
...
switch action,
 ...
 case 'Open',
  ...
  if strcmp(simStatus,'stopped') | strcmp(simStatus,'terminating') %#ok
 ...
 case 'Apply',
  ...
  %-------------------------------------------------------------
  if nargin ~= 2 error(ArgErrorStr); end,  %#ok
  figHdl = varargin{2};
 ...
 otherwise,
  ...
  %-------------------------------------------------------------
  DAStudio.error('Simulink:dialog:InvalidActionPerfAction', mfilename, action); 
end
...
%
function figHdl = CreateDlg(figHdl, portHdl); %#ok
...
% make sure that the dialog will stay inside the window
...
javaItems.figHdl.setTitle(DAStudio.message('Simulink:dialog:SignalProps', sigName));
...
elseif strcmp(get_param(portHdl, 'DataLoggingNameMode'), 'SignalName')
  ...
  idx = 0;
  warndlg(DAStudio.message('Simulink:dialog:SigLoggFeatureParamUnknown', 'DataLoggingNameMode'), ...
          DAStudio.message('Simulink:dialog:SigPropsDlgMsg'));
end
...
if ~isempty(docStr),
  ...
  warndlg(DAStudio.message('Simulink:dialog:cannotLaunchHTMLDocLinkEmpty'), ...
	  DAStudio.message('Simulink:dialog:SigPropWarn'), 'modal');
end
...
%
function LogNamePopupChangedCallback(figHdl);  %#ok
