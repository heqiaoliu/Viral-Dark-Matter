...
%   continuous derivatives indexed by IDX to DX(IDX). Derivatives not
%   indexed are free to vary.
%
%   [X,U,Y,DX]=TRIM('SYS',X0,U0,Y0,IX,IU,IY,DX,IDX,OPTIONS) allows
%   the optimization parameters to be set. For details, type DOC TRIM 
%   to open the TRIM reference page. 
%
...
% Trim is not supported for models with model reference
...
% If no initial guesses then use system's initial state
if isempty(x0), x0=state0; end

% If no initial guesses then use zeros
...
if numel(dx0) ~= sizes(1)
    % Error because TRIM only supports constraining continuous states.
