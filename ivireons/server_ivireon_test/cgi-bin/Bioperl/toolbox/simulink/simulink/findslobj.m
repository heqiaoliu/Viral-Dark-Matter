...
switch (Action)
  ...
  case 'Create'
    ...
    try
      % Fix for G410726. Invoke these methods on
      % the MATLAB event dispatch (GUI) thread to 
      % avoid deadlocks. Note that edtMethod is being
      % replaced by javaMethodEDT in R2008a.
      edtMethod('setVisible', FINDER_FRAME_HANDLE, 1);
      edtMethod('show', FINDER_FRAME_HANDLE);
  ...
  case 'DeleteFinder'
    try
      FINDER_PANEL_HANDLE = [];
      edtMethod('dispose', FINDER_FRAME_HANDLE);
      FINDER_FRAME_HANDLE = [];
      RESULTS = [];
    end
...
% avoid deadlocks.
%
panelH = edtMethod('CreateFindDlg', 'com.mathworks.toolbox.simulink.finder.Finder', system);
edtMethod('setVisible', panelH, 1);

% Make the Finder visible
frameH = panelH.getParent;
edtMethod('setVisible', frameH, 1);
edtMethod('show', frameH);
