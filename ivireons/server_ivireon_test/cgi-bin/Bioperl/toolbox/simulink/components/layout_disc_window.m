...
%   $Revision: 1.5.2.7 $  $Date: 2008/06/24 17:50:38 $
...
try
    foundMDL = find_system('type','block_diagram','name',systemname);
    if(isempty(foundMDL))
        return
    end
catch
...
% get the model original dirty flag(fix for geck358533)
dirtyflag = get_param(systemname, 'dirty');
if (strcmpi(command, 'layout'))
  ...
  % set the mode dirty flag to original one(fix for geck358533)
  newdirtyflag = get_param(systemname, 'dirty');
  if ~strcmpi(dirtyflag, newdirtyflag)
      set_param(systemname, 'dirty', dirtyflag);
  end
...
% subsystems              = find_system(systemname, 'LookUnderMasks', 'all', 'FollowLinks', 'on', 'BlockType', 'SubSystem');
subsystems              = find_system(systemname, 'BlockType', 'SubSystem');
