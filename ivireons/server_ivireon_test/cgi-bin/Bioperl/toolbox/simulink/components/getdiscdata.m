...
% $Revision: 1.8.4.5 $ $Date: 2005/06/24 11:10:59 $
...
if nargin == 2
    ...
    warningStateInfo  = warning;
    warningShadowedModelName = 'on';
    for(k=1:length(warningStateInfo))
        if(strcmp(warningStateInfo(k).identifier,'Simulink:ShadowedModelName'))
            warningShadowedModelName =  warningStateInfo(k).state;
            break;
        end
    end
    % suspend the warnings in case untitledx.x exists 
    warning('off','Simulink:ShadowedModelName');
    tempsys = new_system;
    % Restore the warning state.
    warning(warningShadowedModelName,'Simulink:ShadowedModelName')
end
% open_system(tempsys);
