...
% $Revision: 1.4.2.4 $ $Date: 2009/09/09 21:39:29 $
% Copyright 1990-2005 The MathWorks, Inc.

if nargin < 3
    error('Simulink:ModelDiscretizer:NoBlock', 'At least one block should be provided!');
end
...
try
  ...
  [srcBlks, curBlks, data] = getDefaultReturnValues;
  return;
end
...
for k = 1:n,
    ...
    if isconf || strcmpi(putInto, 'new')
        if strcmp(method, 'prewarp')
            [origBlk, newBlk,errorOccurred] = sldiscmdl(sys, strstoffset, method, cf, ...
            ...
            [origBlk, newBlk,errorOccurred] = sldiscmdl(sys, strstoffset, method, ...
                option, curBlk, confOption);
            if(errorOccurred)
              [srcBlks, curBlks, data] = getDefaultReturnValues;
              return
        ...
        if strcmp(method, 'prewarp')
            [origBlk, newBlk,errorOccurred] = sldiscmdl(sys, strstoffset, method, cf, option);
            if(errorOccurred)
              [srcBlks, curBlks, data] = getDefaultReturnValues;
              return
            end
        else
          [origBlk, newBlk,errorOccurred] = sldiscmdl(sys, strstoffset, method, option);
          if(errorOccurred)
            [srcBlks, curBlks, data] = getDefaultReturnValues;
            return
          end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function  [srcBlks, curBlks, data] = getDefaultReturnValues()
import java.lang.*;
