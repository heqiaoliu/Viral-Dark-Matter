function sldemo_clutchplot_if(Action)
%   SLDEMO_CLUTCHPLOT_IF plot clutch time histories.
%   SLDEMO_CLUTCHPLOT_IF plots the input and output time histories of the clutch
%   model with If block.  
%
...
% sldemo_clutch_if_output is the logged data structure in MATLAB workspace
Vars=evalin('base','whos(''sldemo_clutch_if_output'')');
...
if ~isempty(Vars) && ~isempty(Value),  
  ...
  for lp=1:10,          
      ...
      % Need to extract data from data structure sldemo_clutch_if_output
      y_data=evalin('base',['sldemo_clutch_if_output.' PlotMe '.Data']);        
      x_data=evalin('base',['sldemo_clutch_if_output.' PlotMe '.Time']);        
...
% If sldemo_clutch_if is simulated from the command line, don't open figures.
if ~isempty(find_system(0,'flat','name','sldemo_clutch_if')) && ...
   strcmp(get_param('sldemo_clutch_if','Open'),'off'),
  return
end

% If sldemo_clutch_if is not open, open it
if isempty(find_system(0,'flat','name','sldemo_clutch_if')),
  sldemo_clutch_if
end  
...
Name=find_system('sldemo_clutch_if','SearchDepth',1,'BlockType','Outport');
...
TempInputName=find_system('sldemo_clutch_if'     , ...
                          'SearchDepth',1, ...
...
ModelPos=get_param('sldemo_clutch_if','Location');

FigX=0;FigY=0;
%%% Create Everything %%%    
Fig=figure('Units'          ,'points'                                , ...
           'Position'       ,[FigX FigY FigWidth FigHeight]          , ...
           'CloseRequestFcn','sldemo_clutchplot_if Close'                      , ...
           'Menubar'        ,'none'                                  , ...
           'Colormap'       ,[]                                      , ...
           'NumberTitle'    ,'off'                                   , ...
...
for lp=1:length(Names),    
  UI(lp)=uicontrol('Style'   ,'checkbox'       , ...
                   ...
                   'Units'   ,'normalized'     , ...               
                   'String'  ,Names{lp}        , ...
                   'Callback','sldemo_clutchplot_if Plot', ...                       
                   'Tag'     ,Names{lp}          ...
                   );
end % for lp
