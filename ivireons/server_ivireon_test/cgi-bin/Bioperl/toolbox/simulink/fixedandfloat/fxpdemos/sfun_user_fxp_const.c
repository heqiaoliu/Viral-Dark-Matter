...
static void setValue(SimStruct *S, void *pVoidOut)
{
    ...
    switch ( storageContainer )
    {
      ...
      case FXP_STORAGE_OTHER_SINGLE_WORD:
        if( ssGetDataTypeFxpIsSigned( S, dataTypeId ) )
        {
            *(( int64_T *)pVoidOut) = ( int64_T)storedIntVal;
        }
        else
        {
            *(( uint64_T *)pVoidOut) = ( uint64_T)storedIntVal;
        }
        break;
      ...
      case FXP_STORAGE_MULTIWORD:
        {
            ...
            if ( storedIntVal < 0 )
            {
                 ...
                 *  to be zero 
                 */
                if(storageContainer == FXP_STORAGE_CHUNKARRAY)
                {
                ...
                {
                /* keep sign extension up to a whole chunk array
                 */
                    for(indexChunk = indexMostSignificantUsedChunk + 1; indexChunk< FXP_NUM_CHUNKS; indexChunk ++)
                    {                    
                        chunkArray.chunk[indexChunk] = FXP_ALL_ONES_CHUNK;
                    }
                }
            }
            if(storageContainer == FXP_STORAGE_CHUNKARRAY)
            ...
            {
                unsigned int idx;
                for (idx = 0; idx < FXP_NUM_CHUNKS; idx++)
                {
                    ssFxpSetU32BitRegion(S, pVoidOut, dataTypeId, chunkArray.chunk[idx], idx);
                }
            }
        }
        break;
