...
%   XSCALE    input scaling specified using Simulink Fixed Point conventions.  
...
%   % Example 1
%   % ERRMAX and NPTSMAX both specified
%   fixpt_look1_func_approx('sin(2*pi*x)',0,0.25,ufix(16),2^-16,sfix(16),2^-14, ...
%     'floor',0.01,11,'un');
%
%   % Example 2
%   % ERRMAX only is specified.
...
%     'floor',[],17,'un');
%
%   See also SFIX, UFIX, FIXPT_INTERP1, FIXPT_LOOK1_FUNC_PLOT.

% Copyright 1994-2009 The MathWorks, Inc.
% $Revision: 1.8.2.6 $  
% $Date: 2009/05/14 17:50:24 $
...
while (memory_Size > 1000000)
  ...
  % of memory and throw that out. If the memory usage still exceeds the
  % memory bound find the next biggest contributor to memory usage and clear
  % that and so on and so forth.
  mem_size = zeros(length(DATA),1);
  for jj = 1:length(DATA)
    mem_struct = DATA(jj); %#ok<NASGU>
...
if xMaxFix <= xMinFix
    DAStudio.error('Simulink:fixedandfloat:look1MaxAndMin');
end
...
if nargin < 11 || isempty(spacing)
   ...
   %   allowing less efficient modes automatically allows more efficient modes
   unevenAllowed =                 ( 'u' == lower(spacing(1)) );
   evenAllowed   = unevenAllowed | ( 'e' == lower(spacing(1)) );
   %
   if ~evenAllowed && isFloatDT(xdt)
     DAStudio.warning('Simulink:fixedandfloat:look1NotFixPt');
     evenAllowed = 1;
   end
end
...
elseif nPtsMaxValid
   ...
   DAStudio.error('Simulink:fixedandfloat:look1InvalidLimits');
end  
...
while ( pSmallestKnownBad - pLargestKnownGood ) > 1
    ...
    if errCur <= errMax
       ...
       if xDataTry(end) > (xMaxFix-0.5*xbit)
          ...
          xDataTry = [ xDataTry; xDataTry(end)+xSpacingTry ]; %#ok<AGROW>
           
          xDataTry = num2fixpt( xDataTry, xdt, xscale, 'Nearest', 'on' );
    
          ii = find( abs( diff( diff( xDataTry ) ) ) > xbit/16, 1 );
...
if ~isempty(ii)
  ...
  if any( abs( diff( diff( xdata ) ) ) > xbit/16 )
     DAStudio.error('Simulink:fixedandfloat:look1ScaleNotEven');
  end
end
...
function lsb = get_lsb(xdt,xscale,xmin,xmax)
  ...
  otherwise
      DAStudio.error('Simulink:fixedandfloat:look1DTNotSupport');
  end  
