function varargout = fixpt_extractbits_mask(action, numBits, bitIdxRange)
% fixpt_extractbits_mask.m - Mask helper function for Extract Bits block

% Copyright 1994-2003 The MathWorks, Inc.
% $Date: 2008/02/20 01:29:50 $ $Revision: 1.1.6.3 $
...
switch action
    case 'icon'
        varargout(1) = {getIconString(blockH, numBits, bitIdxRange)};
        
    case 'dynamic'
        handleDynamicDialogs(blockH);
        varargout = [];
...
%-------------------------------------------------------------------------
function str = getIconString(currBlock, numBits, bitIdxRange)
% Upper half
% Lower half
...
switch extBitsSpecStr
    ...
    case 'Range starting with most significant bit'
      try
          if isempty(numBits)
              ...
              numBitsStr     = [' ', num2str(double(numBits))];
          end
      catch errmsg %#ok!
          numBitsStr     = '';
      end
    ...
    otherwise
        % Range of bits
        try
            if isempty(bitIdxRange)
                bitIdxRngNum = [];
            else
                bitIdxRngNum = int32(double(bitIdxRange));
            end
        catch errmsg %#ok!
            bitIdxRngNum = [];
        end
        ...
        elseif (length(bitIdxRngNum) == 1)
            bitIdxStr = num2str(bitIdxRngNum);
