...
% $Date: 2006/12/20 08:32:36 $ $Revision: 1.1.6.3 $
...
switch bitsToExtract
    ...
    case { N_MS_Bits, N_LS_Bits }
        NumBitsMult = 0;
        NumBitsAdd  = double(numBits);
        
    otherwise
        NumBitsMult = 0;
        if (length(bitIdxRange) == 2)
            NumBitsAdd  = double(bitIdxRange(2)) - double(bitIdxRange(1)) + 1;
        else
            NumBitsAdd = 1;
        end
end
...
switch bitsToExtract
    case Arb_Bits
        MiddleSlopeMultExp = double(bitIdxRange(1));
