...
% $Revision: 1.7.2.2 $  
% $Date: 2010/04/05 22:45:59 $

if nargin < 2
    DAStudio.error('Shared:numericType:fixptbest1');
elseif nargin < 3
    fixedExponent = bestfixexp(RealWorldValue, double(TotalBits.WordLength), ...
                                               double(TotalBits.Signed));
else
    fixedExponent = bestfixexp(RealWorldValue, double(TotalBits), double(IsSigned));
end    
