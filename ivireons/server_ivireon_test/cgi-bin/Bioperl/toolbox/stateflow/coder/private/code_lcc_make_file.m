...
%   $Revision: 1.11.2.17 $  $Date: 2009/07/03 14:41:56 $
	...
	if gTargetInfo.codingMakeDebug
        ...
        if ~isempty(fileNameInfo.auxInfo.linkFlags)
            for i = 1:length(fileNameInfo.auxInfo.linkFlags)
                projectInfo.ldflags = ...
                    [projectInfo.ldflags ' ' fileNameInfo.auxInfo.linkFlags{i}];
            end
        end
   ...
   if (~isempty(fileNameInfo.dspLibInclude))
    ...
    if ~gTargetInfo.codingLibrary
        ...
        if gTargetInfo.codingSFunction || gTargetInfo.codingMEX
            ...
            projectInfo.libraries{end+1} = [fileNameInfo.matlabRoot,'\extern\lib\win32\lcc\libmwmathutil.lib'];
            projectInfo.libraries{end+1} = [fileNameInfo.matlabRoot,'\extern\lib\win32\lcc\libemlrt.lib'];
            if (~isempty(fileNameInfo.dspLibFile))
                projectInfo.libraries{end+1} = fileNameInfo.dspLibFile;
            end
            if (~isempty(fileNameInfo.blasLibFile))
                projectInfo.libraries{end+1} = fileNameInfo.blasLibFile;
            end
            if ~isempty(fileNameInfo.auxInfo.linkObjects)
                for i=1:length(fileNameInfo.auxInfo.linkObjects)
                    projectInfo.libraries{end+1} = fileNameInfo.auxInfo.linkObjects{i};
                end
            end
        end
    end
	...
	if ~gTargetInfo.codingLibrary && gTargetInfo.codingMEX
        ...
        for i = 1:length(fileNameInfo.chartSourceFiles)
            projectInfo.sourceFiles = [projectInfo.sourceFiles, fileNameInfo.chartSourceFiles{i}];
        end

	% fileNameInfo.auxSourceFiles is a Nx1 vector, while
    	% projectInfo.sourceFiles is a 1xN vector, so direct concatenation is
	% error-prone.
	for i=1:numel(fileNameInfo.auxInfo.sourceFiles)
		projectInfo.sourceFiles = ...
            [projectInfo.sourceFiles, fileNameInfo.auxInfo.sourceFiles{i}];
	end
	...
	if(gTargetInfo.codingLibrary)
		...
		if(gTargetInfo.codingMEX && ~isempty(sf('get',gMachineInfo.target,'target.mexFileName')))
			projectInfo.outputFileName = [sf('get',gMachineInfo.target,'target.mexFileName'),'.',mexext];
		else
	...
	lcc_make_gen(projectInfo,fileNameInfo);	

function lcc_make_gen(projectInfo,fileNameInfo)
...
fprintf(file,'\n');
	...
	includeDirString = '';
	if(~isempty(projectInfo.includeDirs))
    ...
    if ~isempty(fileNameInfo.auxInfo.includePaths)
        for i = 1:length(fileNameInfo.auxInfo.includePaths)
			...
			includeDirString = [includeDirString,' -I',quoteChar,path,quoteChar,' '];
        end
    end
