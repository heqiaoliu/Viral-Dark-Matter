...
fprintf(file,'	sf_debug_set_machine_data_scope(%s,%.17g,%.17g);\n',v1,v2,v3);
         v1 = gMachineInfo.machineNumberVariableName;
			v2 = dataNumber;
			v3 = sf('get',data,'data.name');
fprintf(file,'   sf_debug_set_machine_data_name(%s,%.17g,"%s");\n',v1,v2,v3);
			coderDataType = sf('CoderDataType',data);
			if(strcmp(coderDataType,'fixpt'))
				[fixptExponent,fixptSlope,fixptBias,nBits,isSigned] =...
					sf('FixPtProps',data);
				isFixedPoint = '1';
			...
			v4 = dataParsedInfo.complexity;
			v5 = isFixedPoint;
			v6 = biasStr;
			v7 = slopeStr;
			v8 = exponentStr;
fprintf(file,'	sf_debug_set_machine_data_type(%s,%.17g,%s,%.17g,%s,%s,%s,%s);\n',v1,v2,v3,v4,v5,v6,v7,v8);
			if(length(dataSizeArray)==0)
fprintf(file,'	sf_debug_set_machine_data_size(%s,%.17g,0,NULL);\n',gMachineInfo.machineNumberVariableName,dataNumber);
fprintf(file,'	sf_debug_set_machine_data_value_ptr(%s,%.17g,NULL);\n',gMachineInfo.machineNumberVariableName,dataNumber);
			else
fprintf(file,'	{\n');
fprintf(file,'		unsigned int dimVector[%.17g];\n',length(dataSizeArray));
				zeroIndexString = '';
				for i=0:length(dataSizeArray)-1
					zeroIndexString = ['[0]',zeroIndexString];
fprintf(file,'		dimVector[%.17g]= %.17g;\n',i,dataSizeArray(i+1));
				end
fprintf(file,'		sf_debug_set_machine_data_size(%s,%.17g,%.17g,&(dimVector[0]));\n',gMachineInfo.machineNumberVariableName,dataNumber,length(dataSizeArray));
fprintf(file,'		sf_debug_set_machine_data_value_ptr(%s,%.17g,NULL);\n',gMachineInfo.machineNumberVariableName,dataNumber);
fprintf(file,'	}\n');
			end
		end
