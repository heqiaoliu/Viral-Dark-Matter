...
%   $Revision: 1.54.4.8 $  $Date: 2008/05/20 00:37:00 $
...
entireView  = 1;         % object view is the default
...
switch (nargin),
    ...
    case 2,
        ...
        switch cmd,
          case 'setup',             
            print('-dsetup');            
            cudd = idToHandle(sfroot, chart);
            mudd = cudd.up;
            fig = sf('get', chart, '.hg.figure');
            set(mudd, 'PaperOrientation', get(fig, 'PaperOrientation'));
            chart_retile(cudd);
            return;
    ...
    otherwise, error('Stateflow:UnexpectedError','Too many args passed to sfprint!');
end;

%
% Execute print
%
out = print_this_chart_l(chart, device, output, entireView, showPreview);
%--------------------------------------------------------------------------
function out = print_this_chart_l(chart, device, output, entireView, showPreview)
%
%
%
...
sfBasedPrintJob = sf('get',portal,'.sfBasedPrintJob');
pStack = sf('get', portal, '.printStack');

% Directly called by sfprint in which case we have to see if the chart and
% the viewObject are the same
if(~sfBasedPrintJob && isempty(pStack)) 
    % We could be viewing subcharts or functions. Get the ID for the exact
    % object that we are viewing. If it is not the main chart, set the ID to
    % the ID of the object being viewed.
    % First get all the properties that the chart has. 'viewObj' will be
    % present only in case of Chart objects.
    viewObj = chart;
    paramsStr = evalc('sf(''get'',chart)');
    if(~isempty(strfind(paramsStr,'viewObj')))
        actualViewObj = sf('get',chart,'.viewObj');
        if(actualViewObj ~= viewObj)
            viewObj = actualViewObj;
        end
    end
% Printing via File->Print menu of the chart/model
% Print one by one in the print stack and pop as you print
else 
    viewObj = pStack(1);
    pStack(1) = [];
...
% Set the viewObject on the portal.
sf('set', portal, '.viewObject', viewObj);

if entireView
    sf('set', portal, '.viewMode','OBJECT_VIEW');
else
    sf('set', portal, '.viewMode','CURRENT_VIEW');
end
...
switch device,
    ...
    case 'hg',             % fallthrough
    otherwise, error('Stateflow:UnexpectedError',['Invalid device specified: ' device]);
end
...
if isequal(output, 'default')
    switch device
        case {'ps','psc', 'eps', 'epsc', 'tiff', 'tif', 'jpeg', 'jpg', 'png'}
            output = 'file';
        case {'meta', 'bitmap'}
            output = 'clipboard';
        case 'default'
        ...
        case 'hg'  % fallthrough
        otherwise
            error('Stateflow:UnexpectedError',['Invalid device specified: ' device]);
    end
end
...
switch output,
    ...
    case 'clipboard',
        sf('set', portal, '.output', 'TO_CLIPBOARD');
    case 'promptForFile',
        sf('set', portal, '.output', 'TO_FILE');
...
sfObj = get(figH, 'UserData');
if sf('get', sfObj, '.isa') == sf('get', 'default','portal.isa')
    sf('set', sfObj, '.margin', margin);
end
%--------------------------------------------------------------------------
