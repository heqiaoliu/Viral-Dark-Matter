...
% $Revision: 1.11.2.3 $  $Date: 2008/12/01 08:08:40 $
...
%   SET    = VSET( S )  returns the minimum set.
	...
	case '*' %  S = VSET( S1, '*', S2 )  returns the intersection of sets.
		if isempty(s1) || isempty(s2), if ischar(s1),s=''; else s = [];end,return, end
	...
	case '-' % S = VSET( S1, '-', S2 )  returns the differenece between the sets.
		...
		if isempty(s1),if ischar(s1),s=''; else s = [];end,return,end
		% Make sure s1 and s2 contain unique elements.
		s1 = vset(s1(:));
		s2 = vset(s2(:));
		% Find matching entries
	...
	otherwise
		error('Stateflow:UnexpectedError','Bad SET operation.');
	end
end
