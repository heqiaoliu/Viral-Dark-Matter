...
outPorts = find_system(blockH,'SearchDepth',1,'LookUnderMasks','on','BlockType','Outport');
inPorts = find_system(blockH,'SearchDepth',1,'LookUnderMasks','on','BlockType','Inport');
triggerPort = find_system(blockH,'SearchDepth',1,'LookUnderMasks','on','BlockType','TriggerPort');
...
assert_l((isempty(inEvents))==isempty(triggerPort),'Trigger port mismatch');
