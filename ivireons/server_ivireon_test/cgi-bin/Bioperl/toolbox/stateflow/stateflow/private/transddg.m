...
% Copyright 2002-2005 The MathWorks, Inc.
  ...
  execorderLabel.RowSpan = [5 5];
  execorderLabel.ColSpan = [1 1];
  execorderLabel.Type = 'text';
  execorderLabel.Tag = strcat('sfTransdlg_', execorderLabel.Name);
  
  %port pull down menu
  execorderPullDown.RowSpan = [5 5];
  execorderPullDown.ColSpan = [2 2];
  ...
  chartId = sf('get',h.id, 'transition.chart');
  ...
  dlgstruct.HelpArgs = {h,'TRANSITION_DIALOG'};
    ...
    switch sf('get',id,'.isa')
     ...
     case JUNCTION
      switch sf('get',id,'.type')
       ...
       otherwise
        objName = '(junction)';
        warning('Stateflow:UnexpectedError','Bad junction type.');
      end
     otherwise
      objName = sprintf('(#%d) ',id);
      warning('Stateflow:UnexpectedError','Bad object type.');
    end
  else
    objName = '~';
  end
...
s = DAStudio.message(['Stateflow:dialog:Transition' id],varargin{:});
