...
%   $Revision: 1.1.6.10 $  $Date: 2008/12/01 08:08:08 $
    ...
    if displayTitle
        if ~is_sf_chart(chartID)
            % eML/TT blocks
            statePath = sf('FullNameOf', chartID, '/');
        else
            statePath = sf('FullNameOf', stateID, '/');
        end
...
function iterator = iterate_truth_table_l(iterator, action)
    if isnumeric(iterator)
        ...
        % NOTE: "iterator.truthTableID = iterator;" is no longer valid
        % Empty var "iterator" before reassigning it into structure
        ttId = iterator;
        iterator = [];
