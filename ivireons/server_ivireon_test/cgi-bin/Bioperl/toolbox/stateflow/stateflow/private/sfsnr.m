...
%   $Revision: 1.72.4.17 $  $Date: 2009/03/05 19:13:03 $
...
function i = dismantle_property_l (property, field, i)
    % get the number of lines in this property
...
function response = inner_search_l(start, topId, searchExpr, searchMatrix, caseSensitive)
    ...
    if ~start
        ...
        otherwise,
            error('Stateflow:UnexpectedError','Bad object type passed to sfsnr()');
        end
    end
...
function replace_side_effect_l(newProperty, oldPropertyLength, matchIndex)
    ...
    formerExtent = get(thisMatch.textObject, 'Extent');
    set(thisMatch.textObject, 'String', newProperty(newLines(thisLine)+1:endIndex));
    latterExtent = get(thisMatch.textObject, 'Extent');
    s.sp.offset = s.sp.offset + length(newProperty) - oldPropertyLength;    
    set(fig, 'UserData', s);
    
    widthChange = latterExtent(3)-formerExtent(3);
    lastMatchIndex = find(s.hg.found.activeHighlights==under);
    nextMatchIndex = lastMatchIndex+1;
    numActiveHighlights = length(s.hg.found.activeHighlights);
    if lastMatchIndex < numActiveHighlights
        underPos = get(under, 'Position');
        for loop = nextMatchIndex:numActiveHighlights
            highlight = s.hg.found.activeHighlights(loop);
            highPos = get(highlight, 'Position');
            if highPos(2) ~= underPos(2)
                return;
            end
            highPos(1) = highPos(1) + widthChange;
            set(highlight, 'Position', highPos);
        end
    end

    
%---------------------------------------------------------------------------------------
% Replaces all additional occurrences of the search string with the replace string
% Parameters:
%   start           the object to start searching from
%   options         bitfield of search options
%   fields          bitfield of field types to search (see fields_toggle_l)
...
%---------------------------------------------------------------------------------------
% Replaces all occurrences of the search string within the current object
...
function response = init_GUI_l()
    ...
    % ---String-Labels----------------------------------------------------------------------------------------
    textColor = [0 0 0]; % Avoid using defaultTextColor!
...
function searchExpr = search_options_l(searchExpr, options)
    ...
    if ~(options(2) | options(1))
        searchExpr = regexptranslate('escape', searchExpr);
    end
