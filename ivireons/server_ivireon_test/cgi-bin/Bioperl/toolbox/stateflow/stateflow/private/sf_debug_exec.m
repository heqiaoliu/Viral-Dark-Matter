...
try
    ...
    switch cmd
        ...
        case {'whos'}
            if ~isEmlInferDbgMode
                if length(tokens) > 1
                    % For non singleton "whos" command, handle with TMW interpreter
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dataInfo = get_unknown_data_info(dataInfo, machineId)
...
dataInfo.class = [info.class ' array'];
...
for i = 1:numData
    ...
    if strcmp(thisDataInfo.class, 'unknown')
        thisDataInfo = get_unknown_data_info(thisDataInfo, machineId);
    end
