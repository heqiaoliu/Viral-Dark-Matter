...
%   $Revision: 1.19.2.6.8.1 $  $Date: 2010/07/23 15:46:06 $
...
switch(cmd),
   case 'memory_mode'
       if(nargin>1)
           sLogFileInfo.memoryMode = targetId; % unfortunate choice of name, but it will do for now.
       end
   ...
   otherwise,                  warning('Stateflow:UnexpectedError','bad parameter passed to log_file_manager');
end
...
%
   if(logFileInfo.memoryMode==0)
      if ~isempty(logFileInfo.logFileName), 
         try 
             sf_delete_file(logFileInfo.logFileName); 
         catch
         end; 
      end;
      logFileInfo.logFileName = '';
