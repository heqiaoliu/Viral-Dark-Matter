...
%  $Revision: 1.52.4.3 $  $Date: 2008/12/01 08:07:37 $
%
persistent sParenTable;
if(~isempty(varargin) && isequal(varargin{1},'reset'))
  sParenTable = [];
end
...
case 'c_parenthesization'
   ...
   return;
otherwise,
	warning('Stateflow:UnexpectedError','Unrecognized method ''%s'' is ignored.',method);
	return;
end
...
function load_custom_target_menus(defaultEditorFigure)
	...
	if(isempty(customTargetMenu))
		warning('Stateflow:UnexpectedError','Could not find Tools menu in sfcustom/load_custom_target_menus.');						
		return;
	end
...
function custom_save( machineId, fileName )
	...
	if fid<=2, warning('Stateflow:InternalError','Failed to open file %s.',fileName); end
	fprintf(fid,'\nCustomInfo {\n');
	fprintf(fid,'\t#This block contains custom Stateflow information\n');
	fprintf(fid,'\t#Date: %s\n',datastr(now,0));
	fprintf(fid,'}\n');
	fclose(fid);
	fprintf(1,'Saved customized model: %s.',fileName);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function custom_load( machineId, simulinkH )
	fsprintf(1,'Loading customized Stateflow model: %s.',get_param(simulinkH,'Filename'));
