...
pStr = regexprep([' ' pStr ' '], '([=,()])', ' $1 '); % separate all valid tokens
...
% Try to preserve the user input style of sample prototype in auto
...
% seperators "()[], " must immediately follow symbols (include "=")
...
styleP = '';
expr = '~|[_a-zA-Z]\w*|='; % Must be at most 1 '='
[s, e] = regexp(autoP, expr); 

if isempty(s)
    return;
end
[sSample, eSample] = regexp(noLcSampleP, expr); 
...
for i = 1:numSym
    ...
    % Replace 'unused' with '~'
    if i <= numel(sSample)
        sampleSymbol = noLcSampleP(sSample(i):eSample(i));
        if strcmp(sampleSymbol, '~') && numel(regexp(symbol,'unused\d*')) == 1
            symbol = sampleSymbol;
        end
    end
        
    styleP = [styleP symbol tailingStr]; %#ok<AGROW>
end
...
[s, e] = regexp(noLcSampleP, '~|[_a-zA-Z]\w*');
