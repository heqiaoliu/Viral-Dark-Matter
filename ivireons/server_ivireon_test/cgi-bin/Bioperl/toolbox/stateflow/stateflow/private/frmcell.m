function out = frmcell(in,cols,flshrght)
%OUT = FRMCELLN( IN, COLS, FLSHRGHT )

%   Copyright 1995-2008 The MathWorks, Inc.
%   $Revision: 1.12.2.3 $  $Date: 2008/12/01 08:06:01 $
...
if(numCols~=length(colWidths) && numCols~=length(flushDirection) )
    error('Stateflow:UnexpectedError','Formatting does not match cell array')
end
...
for k=1:numCols
    ...
    if(dims(2)>colWidths(k))
        if(flushDirection(k))
            ...
            testString = stradd(stringColumn,char(10));  % Insert line feeds for place keeping
