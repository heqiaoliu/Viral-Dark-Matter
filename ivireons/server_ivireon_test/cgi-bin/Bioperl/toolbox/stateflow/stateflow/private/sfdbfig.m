...
%   $Revision: 1.21.2.9.2.1 $  $Date: 2010/07/26 15:40:32 $
...
function gui_resize(fig, arg)
    ...
    if isempty( find( abs(posChange) > 0.4, 1 ) ) && ~flipPane && ~mouseMotion
        return;
    end
   ...
   if flipPane
    ...
    if mouseMotion
        ...
        % Turn off spurious warnings when debugger is docked (g298346)
        posWarnState = warning('off', 'MATLAB:Figure:SetPosition');
        set( fig, 'Position', newPos);
        warning(posWarnState.state, 'MATLAB:Figure:SetPosition');
        safe_set(fig,'UserData',data);
        %%%set(fig, 'UserData', data );
        currentPos = get( fig, 'Position' );
    ...
    if panes(2) == 1        
        ...
        if (findstr('ja', get(0, 'language')))
            % Japanese translation does not fit in the allocated space - see g323126
            evtCBW = 24; %
            stopBtnW = buttonHW(1)+3.0;

        else
            evtCBW = 20;
            stopBtnW = buttonHW(1)+1.0; 
        end
        evtCBX = chartCBW + chartCBX + 2;
...
if length( a ) > 1
    warning('Stateflow:InternalError','More than one debugger window open' );
end
zero = [ -1000 -1000 1 1];
