...
%   $Revision: 1.11.2.2 $  $Date: 2007/09/21 19:16:57 $

if nargin==0, return; end

if any(size(bmp)~=[32,32])
	warning('Stateflow:UnexpectedError','Bad Stateflow icon press button bitmap!');
	return
end

hitType = get(handle,'type');
if ~strcmp(hitType,'surface'),
	if ~strcmp(hitType,'axes'),	warning('Stateflow:UnexpectedError','Bad icon handle!'); end;
	return
end;
...
%		bmp(4:29,29:30) = 3;
	case 'release'
	otherwise,
		error('Stateflow:UnexpectedError','Bad command.');
end
