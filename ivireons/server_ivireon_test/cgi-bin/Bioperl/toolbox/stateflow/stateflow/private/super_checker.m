...
%   $Revision: 1.17.4.2 $  $Date: 2008/12/01 08:08:11 $
...
isa = sf('get', id, '.isa');
if isequal(isa, WIRE), x = true; else x = false; end;
...
duplicate = false;
for i=1:length(nags);
    if isequal(nags(i), nag) duplicate = true; end;
end;
...
if isempty(hadNags), hadNags = false; end;

if nargin == 1,
    switch arg,
        case 'flush', hadNags = false;
        case 'mark',  hadNags = true;
        otherwise,    hadNags = false;
    end;
end;
