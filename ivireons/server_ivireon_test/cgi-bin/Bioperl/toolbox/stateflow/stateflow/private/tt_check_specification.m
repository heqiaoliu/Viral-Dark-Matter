...
%  $Revision: 1.3.4.5 $ $Date: 2008/12/01 08:08:37 $
%
%
...
if ncFull > 0 && sum(find(tt(:,end) == dcval)) == sum(1:nr),
...
for m=1:nc,
  ...
  if length(ridx) == 1 && (numdc > 0) && num_NonSparseColumns_UsingThisRow == 0,
    ...
    setIdx = find(tts(ridx,:) >= 0);  % other sparse columns with T or F
                                      % entries in this row
    if isempty(sparse_companion_col),
      %
