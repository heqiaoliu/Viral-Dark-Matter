...
%	$Revision: 1.7.2.3 $
	if isempty(path1) || isempty(path2), error('Stateflow:UnexpectedError','bad args passed to rel_path'); end;
    ...
    if diffInd < 2, error('Stateflow:UnexpectedError','paths must share a root!'); end;
    ...
    if ~isempty(slashes),
        ...
        error('Stateflow:UnexpectedError','bad args passed to rel_path');
    end;
