...
%   $Revision: 1.6.2.4 $  $Date: 2008/08/26 18:39:32 $
...
linkChartSfunctions = find_system(linkCharts,'SearchDepth',1,'LookUnderMasks','on','FollowLinks','On','BlockType','S-Function');
...
for i=1:length(linkChartSfunTags)
   ...
   if(isempty(chartFileNumber))
      error('Stateflow:UnexpectedError',['Please run sfconv20 in this directory containing ',linkMachines{i},' to convert all old ', ...
            'library models.']);
   end
