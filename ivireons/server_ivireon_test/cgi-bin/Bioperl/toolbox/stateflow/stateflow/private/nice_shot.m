...
%   $Revision: 1.12.2.7 $ $Date: 2008/12/01 08:06:47 $
...
if length( id ) ~= 1
	error( 'Stateflow:UnexpectedError','first parameter must be an object id' );
end
recursive = 1;
...
switch(sf(GET, id, '.isa')),
 ...
 case ISA_TRANS,
	res = get_state_view( id, recursive );
 otherwise, error('Stateflow:UnexpectedError','other objects not handled yet!');
end;
...
%	for each of the object passed and their dependents
		...
		case ISA_STATE,
			...
			switch sf( GET, i, '.type' ),
				...
				case 1, 	% AND state. Dashed line, rounded corner	
					...
					t = [  sf_scalar2str( sf(GET, i, '.executionOrder') ) ];
...
switch sf( 'get', id, '.isa' )
	...
	otherwise
		error( 'Stateflow:UnexpectedError','other objects not handled yet' );
end
