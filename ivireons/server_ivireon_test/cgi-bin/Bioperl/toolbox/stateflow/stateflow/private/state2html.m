function htmlStr = state2html(stateID)
    ...
    if sf('get', stateID, '.isa')==sf('get', 'default','state.isa');
        ...
        elseif is_truth_table_chart(chartID)
            ...
            error('Stateflow:UnexpectedError',['Item ' chartID ' is neither a truth table nor an Embedded MATLAB function.']);
        end
    end
    ...
    elseif is_truth_table_fcn(stateID)
        ...
        error('Stateflow:UnexpectedError',['Item ' stateID ' is neither a truth table nor an Embedded MATLAB function']);
    end
...
function htmlStr = tt2html(chartID, truthTableID)
    ...
    for col = numCols+1:cols+1
        columnNames{col} = int2str(col-numCols);
    end
...
%   $Revision: 1.1.6.8 $  $Date: 2007/09/21 19:18:37 $
