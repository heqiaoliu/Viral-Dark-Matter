...
%   $Revision: 1.1.6.7 $  $Date: 2008/12/01 08:08:34 $
...
switch method
    ...
    otherwise
        warning('Stateflow:InternalError','Unknown method ''%s'' for truth_table_handle_man.', method);
end
...
if index == 0  % Not opened
    ...
    switch cache(index).status
        ...
        case 1  % vacant
            if ~jvmAvailable
                warning('Stateflow:InternalError','Truth table editor requires Java Swing and AWT components. One of these components in missing.');
                return;
            end
