...
%  $Revision: 1.6.4.16 $  $Date: 2009/09/28 20:52:58 $

% if dryRun is 1, then we just go through the motions
...
for i=1:length(inDataNames)
   ...
   inData(i).name = inDataNames{i}; %#ok<AGROW>
   inDataId = sf('find',funcData,'data.name',inData(i).name);
   if(~isempty(inDataId))
      inData(i).id = inDataId; %#ok<AGROW>
      funcData = vset(funcData,'-',inDataId);
   else
      inData(i).id = 0; %#ok<AGROW>
   end
end
...
for i=1:length(inData)
  ...
  if(inData(i).id==0)
    inData(i).id = obsInData(1); %#ok<AGROW>
    obsInData(1)=[];
  end
end
...
for i=1:length(outData)
  ...
  if(outData(i).id==0)
    outData(i).id = obsOutData(1); %#ok<AGROW>
    obsOutData(1)=[];
  end
end
