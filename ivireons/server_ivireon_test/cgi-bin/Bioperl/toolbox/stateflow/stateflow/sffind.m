...
% Is the selected system is a subsystem?
pos = findstr('/', system);
if( numel(pos) > 0 )
    subsystem = system(pos(1)+1 : end );
end

% Zero out all elements not in the correct subsystem:
if( ~isempty(pos) && ~isempty(subsystem) )
    for j = 1:length(searchSpace)
        parent = [resultsCell{3}(j,:) resultsCell{4}(j,:)];
        str1 = [subsystem, '/'];
        if( ~strncmp(str1, parent, numel(str1)) ) 
            searchSpace(j) = 0;
        end
    end
end
...
for i = 1:length(searchSpace)
  ...
  results(idx).Handle = handles(i); 
  results(idx).Type   = resultsCell{1}(i,:);
  results(idx).Name   = resultsCell{2}(i,:);
  results(idx).Parent = [resultsCell{3}(i,:) resultsCell{4}(i,:)];
  results(idx).Source = resultsCell{5}(i,:);
  results(idx).Dest   = resultsCell{6}(i,:);
  idx = idx+1;
end
