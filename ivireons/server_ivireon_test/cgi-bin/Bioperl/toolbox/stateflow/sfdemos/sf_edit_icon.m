...
%   $Revision: 1.8.2.4 $  $Date: 2009/06/16 05:47:49 $
...
%
    ...
    if imgsanitycheck((gs.handle.iconImg),uint8(im))   
    set (gs.handle.iconImg, 'CData', uint8(im));
    else
        error('Sanity Check for handle gs.handle.iconImg failed!\n');
    end
    
    set(gs.handle.icon, 'Visible', 'off');
    set(gs.figureHandle, 'CurrentAxes', gs.handle.canvas);
    
    if imgsanitycheck((gs.handle.canvasImg),uint8(im))           
    set (gs.handle.canvasImg, 'CData', uint8(im));
    else
        error('Sanity Check for handle gs.handle.canvasImg failed!\n');
    end  
...
%------
function bool = imgsanitycheck(handle,cdataval)
bool = false;
    if ~isempty(handle) && ishandle(handle)
        if strcmpi(get(handle,'Type'),'Image')
            if ~(any(find(cdataval > intmax('uint8'))))
                bool = true;
            else
                fprintf(1,'Garbage Values given for CDATA property of an Image!\n');
            end
        else
            fprintf(1,'We Expected the handle to be of Image Class - but its not!\n');
            fprintf(1,'The Properties of this handle are: \n');
            fprintf(1,'----------------\n');
            disp(eval('get(handle)'));
            fprintf(1,'----------------\n');
        end
    else
        fprintf(1,'Bad/Empty handle - not attempting to set CDATA!\n');
    end
...
% And, two wrapper functions, one to set and one to get.
...
% Open a file from disk (ask user for filename)
    ...
    if (file ~= 0)
        ...
        try
            ...
            msgbox(ME.getReport(), 'Read Error');
            return;
        end
