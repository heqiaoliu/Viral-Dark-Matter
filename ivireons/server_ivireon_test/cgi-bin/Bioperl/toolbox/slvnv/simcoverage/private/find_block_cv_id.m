...
%   $Revision: 1.1.6.9.2.1 $  $Date: 2010/07/06 14:42:39 $
    ...
    case 'cell'
        ...
        if length(block)>1
            switch(class(block{2})),
            case 'double'
                if block{2}~=floor(block{2})
                    errormsg = 'Element 2 of the BLOCK parameter should stateflow Id or handle';
                else
                    sfId = block{2};
                end
            otherwise,
    ...
    if ~isempty(sfId)
        if ~sf('ishandle',sfId)
            errormsg = ['The BLOCK parameter Stateflow ID (#' sfId ') is invalid'];
        end
        ...
        case sfIsa.data,
            ...
            otherwise,
                errormsg = 'The BLOCK parameter Stateflow object cannot be machine parented data';
            end
        ...
        case sfIsa.script 
            ...
            return;
        otherwise
            errormsg = 'The BLOCK parameter Stateflow object should be a state, transition, chart, or data';
        end
    end
    ...
    %when a path to atomic subchart is given
    ...
    if ~isempty(sfId)
        if isempty(blockH)    
            ...
            if length(instances)>1
                warning('SLVNV:simcoverage:find_block_cv_id:PotentialAmbiguity','Potential ambiguity, Stateflow chart #''%s''is multiply instantiated', sfChartId); 
            end       
        end
        if Sldv.utils.isAtomicSubchartSubsystem(blockH)
            allCvIds = cv('get',rootId,'.topSlsf');
            sfChartObjs = cv('DecendentsOf', allCvIds);
            chartIds = cv('find',sfChartObjs,'slsfobj.handle',sfChartId);
            parentCvIds = cv('get', chartIds, '.treeNode.parent'); 
            if numel(parentCvIds) > 1
                parentSfIds = cv('get', parentCvIds, '.handle');
                for idx = 1:numel(parentSfIds)
                    if ~isempty(sf('find', parentSfIds(idx), 'state.simulink.blockHandle', blockH))
                        blockCvId = parentCvIds(idx);
                        break;
                    end
                blockCvId = 0;
                end
            else
                blockCvId = parentCvIds;
            end
        else
    ...
    elseif  ~isempty(blockH)
        ...
        if blockCvId == 0
            allCvIds = cv('get',rootId,'.topSlsf');
            sfChartObjs = cv('DecendentsOf', allCvIds);
            blockCvId = cv('find',sfChartObjs,'slsfobj.handle',blockH);
            if isempty(blockCvId)
                blockCvId = 0;
            end
        end
...
function cvId = checkScript(rootId, name)
    ...
    if cv('get',modelcovId,'.isScript') && ...
        strcmpi(cv('get',modelcovId,'.name'), name)
