function out1 = softmax(varargin)
...
%
% <a href="matlab:doc softmax">softmax</a>('da_dn',N,A) returns the derivative of layer outputs A with
...
function a = apply(n,parameters)
  ...
  [S,Q] = size(n);
  numer = exp(n);
  notfinite = ~isfinite(numer);
  if any(notfinite)
    notfinite = find(notfinite);
    signs = sign(numer(notfinite));
    numer = zeros(S,Q);
    numer(notfinite) = signs;
  end
  denom = sum(numer,1); 
  denom(denom == 0) = 1;
  denom = 1./denom; 
