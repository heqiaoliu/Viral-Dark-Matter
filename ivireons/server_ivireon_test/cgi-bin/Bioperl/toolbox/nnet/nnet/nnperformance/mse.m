...
%
% MSE('dperf_dy',net,targets,outputs,errorWeights,perf,...parameters...)
% returns the derivative of performance with respect to outputs Y.
%
%   dy = <a href="matlab:doc mse">mse</a>('dperf_dy',net,targets,outputs,{1},'regularization',0.1)
%
% MSE('dperf_dwb',net,...parameters...) returns the derivative of
% performance with respect to the network weights and biases.
%
%   dwb = [mse]]('dperf_dwb',net,'regularization',0.1)
%
...
% TODO - Is Y necessary?
% TODO - Or should E by T?
% TODO - Economization with prioritization
% TODO - Prioritization over sample or time for training vs simulation
...
function name = function_name, name = 'Mean Squared Error'; end
...
function d = dperf_dwb(wb,param)
  if param.regularization == 0
    ...
    if ~isnumeric(wb), wb = getwb(wb); end
    d = (-2*param.regularization/length(wb)) * wb;
  end
end
