function [out1,out2] = mae(varargin)
...
%
%  <a href="matlab:doc mae">mae</a>('dperf_dy',net,targets,outputs,errorWeights,perf,...parameters...)
%  returns the derivative of performance with respect to outputs Y.
%
%    dy = <a href="matlab:doc mae">mae</a>('dperf_dy',net,targets,outputs,{1},'regularization',0.1)
%
%  MAE(net,targets,outputs,errorWeights,perf,...parameters...) returns the
%  derivative of performance with respect to the network weights and biases.
%
%    dwb = <a href="matlab:doc mae">mae</a>('dperf_dwb',net,targets,outputs,{1},'regularization',0.1)
%
...
function name = function_name, name = 'Mean Absolute Error'; end
...
function [perfwb,n] = performance_wb(net,param)
  if isnumeric(net)
    [perfwb,n] = meanabs(net);
  else
    [perfwb,n] = meanabs([net.IW net.LW net.b]);
  end
...
function d = dperf_dwb(wb,param)
  if param.regularization == 0
    ...
    if ~isnumeric(wb), wb = getwb(net); end
    d = (-param.regularization/length(wb)) * sign(wb);
  end
end
