function out1 = scalprod(varargin)
...
%
%  <a href="matlab:doc scalprod">scalprod</a>('size',S,R,FP) takes the layer dimension S, input dimension R,
%  and function parameters, and returns the weight size [1x1].
%
%  <a href="matlab:doc scalprod">scalprod</a>('dp',W,P,Z,FP) returns the derivative of Z with respect to P.
%  <a href="matlab:doc scalprod">scalprod</a>('dw',W,P,Z,FP) returns the derivative of Z with respect to W.
%
...
function name = function_name, name = 'Scalar Product'; end
...
function dim = weight_size(s,r,param)
  if (s ~= r)
    warning('nnet:scalprod:Dimensions',...
      'Source and layer sizes must be the same for SCALPROD.');
  end
  dim = [1 1];
end
...
function p = simulink_params(param)
  p = 'SCALPROD does not have a Simulink block version.';
end
