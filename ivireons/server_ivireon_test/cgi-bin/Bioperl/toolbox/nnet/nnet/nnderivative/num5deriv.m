...
function gWB = calc_gradient(net,data,fcns)
  ...
  for i=1:length(WB)
    perf1 = nntraining.perf_only(setwb(net,seti(WB,i,WB(i)+2*delta)),data,fcns);
    perf2 = nntraining.perf_only(setwb(net,seti(WB,i,WB(i)+delta)),data,fcns);
    perf3 = nntraining.perf_only(setwb(net,seti(WB,i,WB(i)-delta)),data,fcns);
    perf4 = nntraining.perf_only(setwb(net,seti(WB,i,WB(i)-2*delta)),data,fcns);
...
function jWB = calc_jacobian(net,data,fcns)
  ...
  for i=1:net.numWeightElements
    y1 = nntraining.y_only(setwb(net,seti(WB,i,WB(i)+2*delta)),data,fcns);
    ...
    y3 = nntraining.y_only(setwb(net,seti(WB,i,WB(i)-delta)),data,fcns);
    e3 = gsubtract(data.T,y3);
    e3 = remove_dont_care_errors(e3);
    e3 = fcns.perform.adjust_errors(net,e3,data.EW,fcns.perform.param);
    e3 = cell2mat(e3);
    
    y4 = nntraining.y_only(setwb(net,seti(WB,i,WB(i)-2*delta)),data,fcns);
    e4 = gsubtract(data.T,y4);
    e4 = remove_dont_care_errors(e4);
    e4 = fcns.perform.adjust_errors(net,e4,data.EW,fcns.perform.param);
    e4 = cell2mat(e4);
