...
% $Revision: 1.6.2.2.2.1 $ $Date: 2010/07/14 23:38:02 $
...
[perfb,Y,Ac,N,Zb,Zi,Zl] = nnsim.perf2(net_temp,[],Pd,T,Ai,{1},Q,TS,fcns);
g_flag = 0;
cnt1 = cnt1 + 1;
...
while retcode>3
  ...
  if (perfb <= perfa + alpha*lambda*dperfa)         %CONDITION ALPHA IS SATISFIED
    ...
    if (g_flag == 0)
      gX_temp = -calcgxmodref(net_temp,X_temp,Pd,Zb,Zi,Zl,N,Ac,Y,T,perfb,Q,TS);
      dperfb = gX_temp'*dX;
    end
...
if perfb <= perfa
   if (g_flag == 0)
      gX = -calcgxmodref(net_temp,X_temp,Pd,Zb,Zi,Zl,N,Ac,Y,T,perfb,Q,TS);
   else
      gX = gX_temp;
   end
