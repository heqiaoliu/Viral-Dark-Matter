...
%    Here we evaluate the network open loop response.
...
%    Here we evaluate the networks closed loop response.
...
states(:,1) = state;
for i=2:length(time)
  new_state = state + mnet([state; force]);
  states(:,i) = new_state;
