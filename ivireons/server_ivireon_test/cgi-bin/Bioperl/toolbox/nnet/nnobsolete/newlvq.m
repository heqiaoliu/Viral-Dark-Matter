...
% $Revision: 1.1.8.1 $ $Date: 2010/03/22 04:07:42 $
...
if (~isa(s1,'double')) || ~isreal(s1) || any(size(s1) ~= 1) || (s1<1) || (round(s1) ~= s1)
  nnerr.throw('Number of hidden neurons is not a positive integer.')
end
if (~isa(pc,'double')) || (~isreal(pc)) || (size(pc,1) ~= 1) || (abs(sum(pc)-1) > 1e-10)
  nnerr.throw('Class percentages is not a row vector whose elements sum to 1.0.')
end
if (~isa(lr,'double')) || ~isreal(lr) || any(size(lr) ~= 1) || (lr < 0) || (lr > 1)
  nnerr.throw('Learning rate is not a real value between 0.0 and 1.0.')
end
