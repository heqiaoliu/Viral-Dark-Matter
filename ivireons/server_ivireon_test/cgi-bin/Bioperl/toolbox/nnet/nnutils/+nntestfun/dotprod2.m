function out1 = dotprod2(varargin)
...
function name = function_name, name = 'Test Dot Product 2'; end
...
function d = dz_dw(w,p,z,param)
  ...
  d = cell(1,S);
  for i=1:S
     d{i} = 2*(z1(i,:)'*ones(1,R))'.*p;
  end
end

function p = simulink_params(param)
  p = 'DOTPROD2 does not have a Simulink block version.';
end
