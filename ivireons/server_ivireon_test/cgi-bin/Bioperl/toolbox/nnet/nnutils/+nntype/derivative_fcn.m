...
function err = type_check(x)
  ...
  outputs = net(inputs);
  perf1 = perform(net,targets,outputs);
  
  % Jacobian
  net.derivFcn = x;
  net2 = train(net,inputs,targets);
  outputs = net2(inputs);
  perf2 = perform(net2,targets,outputs);
  if (perf2 >= perf1)
    err = [upper(x) ' results in worse performance when training with Jacobian derivatives.'];
    return
  end
  ...
  perf2 = perform(net2,targets,outputs);
  if (perf2 >= perf1)
    err = [upper(x) ' results in worse performance when training with gradient derivatives.'];
    return
  end
end
