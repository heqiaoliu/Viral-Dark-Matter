...
%   $Revision: 1.1.6.6 $  $Date: 2008/05/31 23:25:58 $
...
if any(fcuts > 1/2),
    error(generatemsgid('InvalidRange'),'Band-edge frequencies must be less than the Nyquist frequency.');
end
if any(fcuts < 0),
    error(generatemsgid('MustBePositive'),'Band-edge frequencies must be positive.');
end
...
if length(mags) ~= length(devs)
    error(generatemsgid('InvalidDimensions'),'A and DEV must be vectors of the same length.')
end
