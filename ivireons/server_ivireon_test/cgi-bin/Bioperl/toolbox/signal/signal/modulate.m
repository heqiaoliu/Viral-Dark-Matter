...
%   $Revision: 1.9.4.4 $  $Date: 2009/05/23 08:13:37 $
...
elseif strcmpi(method,'pwm')
    x_max = max(max(x));
    x_min = min(min(x));
    if (x_max > 1) || (x_min < 0), error(generatemsgid('InvalidRange'),'  X out of range [0,1] '), end
    t = (0:(Fs/Fc*len)-1)';
    if nargin < 5
        P1 = 'left';
    end
    ...
    elseif strcmpi('left',P1)
        ...
        error(generatemsgid('InvalidParam'),' 5th input parameter not recognized')
    end
    t = t/Fs;
elseif strcmpi(method,'ptm') || strcmpi(method,'ppm')
    ...
    if (P1 >= 1) || (P1 <= 0), error(generatemsgid('InvalidRange'),'  Pulse duration out of range (0,1) '), end
    if (x_max > 1-P1), warning(generatemsgid('PulseOverlapped'),'Some pulses may overlap!'), end
    t = (0:(len*Fs/Fc)-1)'/Fs;
...
elseif strcmpi(method,'qam')
    if nargin < 5
        error(generatemsgid('SignalErr'),'  For ''qam'', a 5th input parameter is required.')
    end
    x2 = P1;
    r2 = size(x2, 1);
    if r2 == 1
    ...
    if any(size(x2)~=size(x)),
            error(generatemsgid('InvalidDimensions'),'  For ''qam'', input signals must be the same size')
    end
