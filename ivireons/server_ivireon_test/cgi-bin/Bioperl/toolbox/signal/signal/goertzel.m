...
%   $Revision: 1.6.4.7 $  $Date: 2007/12/14 15:04:59 $
...
if ~(exist('goertzelmex', 'file') == 3), 
	error(generatemsgid('NotSupported'),'Unable to find goertzelmex executable');
end
...
if isempty(INDVEC),
	...
	INDVEC = INDVEC(:);
	if max(INDVEC) > siz(1),
		error(generatemsgid('IdxOutOfBound'),'Maximum Index exceeds the dimensions of X.');
	elseif min(INDVEC) < 1
		error(generatemsgid('IdxOutOfBound'),'Index must greater than zero.');
	elseif all(INDVEC-fix(INDVEC))
		error(generatemsgid('MustBeInteger'),'Indices must be integer values.');
	end
end
