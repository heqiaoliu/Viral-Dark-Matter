...
%   $Revision: 1.10.4.7 $  $Date: 2008/05/31 23:26:02 $
...
if max(fcuts) >= fsamp/2,
    error(generatemsgid('InvalidRange'),'Band edge frequencies must be between 0 and half the sampling frequency Fs.');
end
...
if size(mags,1) ~= size(devs,1)
    error(generatemsgid('InvalidDimensions'),'Requires A and DEV to be the same length.');
end
if( min(abs(mags)) )
   error(generatemsgid('SignalErr'),'Stopbands must be zero.');
end
dmags = abs(diff(mags));
if( any(dmags~=dmags(1)) )
    error(generatemsgid('SignalErr'),'All passbands must have same height.');
end
if( any(diff(fcuts)<0) )
    error(generatemsgid('InvalidFreqVec'),'Bandedges must be strictly increasing.');
end


if mf ~= 2*(nbands-1)
    error(generatemsgid('InvalidDimensions'),'Length of F must be 2*length(A)-2.');
end
