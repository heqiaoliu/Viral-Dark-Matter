...
%   $Revision: 1.7.4.6 $  $Date: 2009/05/23 08:13:35 $
...
[M,N] = size(a);
if ~xor(M==1,N==1)
    error(generatemsgid('InvalidDimensions'),' A must be vector.')
end
[M,N] = size(b);
if M > 1 && N > 1
    error(generatemsgid('InvalidDimensions'),' B must be vector.')
end
...
if length(b) > length(a)
    error(generatemsgid('InvalidRange'),'Numerator B(s) degree must be no more than denominator A(s) degree.')
