...
%   $Revision: 1.8.4.4 $  $Date: 2010/05/20 03:09:39 $
...
elseif (strcmp(pf,'trace')),
    ...
    error(generatemsgid('NotSupported'),'Trace flag ''%s'' not recognizable',pf);
end
...
if any( (w>pi) | (w<0) ) && realFlag 
    ...
    warning(generatemsgid('InvalidRegion'),warnStr) %#ok<SPWRN>
end
...
while all([norm(gndir)>tol l<maxiter st~=1])
    ...
    if (verb),
        home
        fprintf('      ITERATION # %s\n',int2str(l));
        fprintf('Current fit:  %s  Previous fit:  %s\n',num2str(V1),num2str(Vcap));
        disp('Current par prev.par   GN-dir');
        disp([t1 t gndir])
        fprintf('Norm of GN-vector: %s\n',num2str(norm(gndir)));
        if st==1,
