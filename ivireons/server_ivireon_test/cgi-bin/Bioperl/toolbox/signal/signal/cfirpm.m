...
%   $Revision: 1.1.6.10 $  $Date: 2010/05/20 03:09:27 $
...
for ii=1:length(varargin),
  ...
  elseif ~ischar(varargin{ii}),
    ...
    otherwise
      error(generatemsgid('InvalidParam'),'Invalid argument "%s" specified.',ppv);
    end
  end
end
...
if strcmp(h_sym,'unspecified'),
   ...
   if any(edges < 0),
     ...
     if ~any(strcmp(h_sym,sym_opts)),
       error(generatemsgid('InvalidParam'),...
           'Invalid default symmetry option "%s" returned from response function "%s".  Must be one of ''none'',''real'',''even'', or ''odd''.',...
           h_sym,func2str(filt_call));
     end
  end
end
