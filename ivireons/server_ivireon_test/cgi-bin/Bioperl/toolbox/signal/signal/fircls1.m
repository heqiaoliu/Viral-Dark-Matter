...
%   $Revision: 1.11.4.6 $  $Date: 2008/05/31 23:25:56 $
...
if nargin < 4
    error(generatemsgid('Nargchk'),'You did not supply enough input parameters.')
elseif prod([size(n), size(wo), size(dp), size(ds)]) > 1
    error(generatemsgid('InvalidDimensions'),'Each of N, WO, DP, and DS must be a scalar.')
elseif ( wo <= 0) || ( wo >= 1)
    error(generatemsgid('InvalidRange'),'W0 must lie between 0 and 1.')
elseif (dp <= 0) || (ds <= 0)
    error(generatemsgid('MustBePositive'),'Both DP and DS must be positive.')
    ...
    if ischar(varargin{1})
        switch lower(varargin{1}(1))
            ...
            case 'b'    % as in 'both'
                TEXT_PF = 1; PLOT_PF = 1;
            otherwise
                error(generatemsgid('InvalidParam'),'Unrecognized string input.')
        end
    else
        wt = varargin{1};
        EDGE = 1;
...
if EDGE
    if isempty(wt),
        error(generatemsgid('Empty'),'WT cannot be empty.');
    elseif numel(wt) > 1
        error(generatemsgid('InvalidDimensions'),'WT must be a scalar.')
    elseif (wt <= 0) || (wt >= 1)
        error(generatemsgid('InvalidRange'),'WT must lie between 0 and 1.')
    elseif wt == wo
        error(generatemsgid('SignalErr'),'WT must not equal W0.')
    elseif wt < wo
        if pass_type == LOW
            edge_type = PASS;
        else
...
if WEIGHTS
    if pass_type == LOW
        if (wp > wo) || (ws < wo)
            error(generatemsgid('InvalidRange'),'For lowpass filters, WP < W0 < WS is needed.')
        end
    else
        if (ws > wo) || (wp < wo)
            error(generatemsgid('InvalidRange'),'For highpass filters, WS < W0 < WP is needed.')
        end
    end
end
