...
%   $Revision: 1.14.4.4 $  $Date: 2007/12/14 15:05:19 $
...
case 2,
    ...
    if ischar(V),
        ...
        otherwise
            error(generatemsgid('InvalidEnum'),'Lattice type must be ''fir'', ''allpole'' or ''allpass''.');
        end
...
if isempty(K),
    ...
    if ~strcmpi(ph,'fir') && max(abs(K)) > 1,
        warning(generatemsgid('InvalidParam'),'Reflection coefficients are greater than one.');
    end
...
elseif isempty(K) && length(V) == 1,
    ...
    if ordiff>0,
        K = [K; zeros(ordiff,1)];
        % error(generatemsgid('InvalidDimensions'),'length(V) must be <= 1+length(K).');
    elseif ordiff<0,
        V = [V; zeros(-ordiff,1)];
    end
