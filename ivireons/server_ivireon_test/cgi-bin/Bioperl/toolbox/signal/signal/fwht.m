...
%   $Revision: 1.1.6.4 $  $Date: 2009/08/11 15:47:42 $
...
if ~isempty(find(char(num2str(log2(N)))=='.', 1)) || (N < 1)
    msgid = generatemsgid('InvalidTransformLength');
    error(msgid,'Transform length must be a non-negative integer that is a power of 2');
end
...
if ~ischar(ordering) || ~any(strcmpi(ordering,{'sequency','hadamard','dyadic'}))
    msgid = generatemsgid('InvalidOrderingType');
    error(msgid,'Output ordering must be sequency, hadamard or dyadic');
end
