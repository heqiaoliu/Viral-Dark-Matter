...
%    	$Revision: 1.8.4.4 $  $Date: 2008/05/31 23:26:05 $
...
%================= convert B/A to partial fractions =========
  if( a(1) == 0 )
     error(generatemsgid('SignalErr'),'First coefficient in A vector must be non-zero.')
  end
...
%============= partial fractions ---> B(z)/A(z) ============
  ...
  if( LR ~= LP )
     error(generatemsgid('InvalidDimensions'),'Length of R and P vectors must be the same.')
  end
  if( LP == 0 )
     p = []; r = t(:).';     %--- no poles!
     return
