...
%   $Revision: 1.8.4.4 $  $Date: 2008/05/31 23:27:29 $
...
if any(bw < 0),
   edges
   error(generatemsgid('InternalError'),'Internal error: obtained a negative bandwidth');
end
...
if any(diff(iext) == 0),
   error(generatemsgid('InternalError'),'Internal error: two extremal frequencies at the same grid point')
end
fext = grid(iext);
...
[M,N] = size(x);
if dBrange <= 0, error(generatemsgid('InvalidRange'),'dBrange must be > 0'); end
