...
for k=2:2:length(F)-2
    if F(k) == F(k+1)
        error(generatemsgid('InvalidFreqVec'),'Adjacent bands not allowed.')
    end
end
if length(F) ~= length(A)
    error(generatemsgid('InvalidDimensions'),'Frequency and amplitude vectors must be the same length.')
end
