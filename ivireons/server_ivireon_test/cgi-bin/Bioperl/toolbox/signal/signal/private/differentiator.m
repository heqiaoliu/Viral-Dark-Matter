...
%   $Revision: 1.7.4.5 $  $Date: 2007/12/14 15:15:05 $
...
if Fsamp<=0,
  error(generatemsgid('MustBePositive'),'Sample rate Fs must be > 0');
end
...
elseif Le == 4,
  if F(2)*F(3) > 0,
    error(generatemsgid('InvalidParam'),['Transition band must include DC for 2-band ' ...
           'Differentiator designs']);
  end
else
  error(generatemsgid('InvalidFreqVec'),'There must be either 2 or 4 band edges for differentiator designs.')
end
