...
%   $Revision: 1.8.4.5 $  $Date: 2007/12/14 15:15:16 $
...
if (Le == 4),
  if any(F < 0),
    error(generatemsgid('MustBePositive'),'Band edges must be non-negative for 2-band Lowpass designs.');
  end
elseif (Le == 6),
  if F(3)*F(4) > 0,
    error(generatemsgid('InvalidFreqVec'),'Passband must include DC for 3-band Lowpass designs.');
  end
else
  error(generatemsgid('InvalidDimensions'),'There must be either 4 or 6 band edges for Lowpass designs.')
end
