...
%   $Revision: 1.1.4.5 $  $Date: 2009/05/23 08:16:21 $
...
if nargin < 3
   error(generatemsgid('Nargchk'),'fminbnd requires three input arguments.')
end
...
case 0
   if maxfun <= maxiter
      disp(' ')
      warning(generatemsgid('DidNotConverge'),...
          'Maximum number of function evaluations has been exceeded - increase MaxFunEvals option.');
   else
      disp(' ')
      warning(generatemsgid('DidNotConverge'),...
          'Maximum number of iterations has been exceeded - increase MaxIter option.');
