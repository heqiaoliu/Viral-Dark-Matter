...
%   $Revision: 1.7.4.5 $  $Date: 2007/12/14 15:15:09 $
...
if (Le == 4),
  if any(F < 0),
    error(generatemsgid('MustBePositive'),'Band edges must be non-negative for 2-band Highpass designs.'); 
  end
elseif (Le == 6),
  if F(3)*F(4) > 0,
    error(generatemsgid('InvalidFreqVec'),'Stopband must include DC for 3-band Highpass designs.');
  end
else
  error(generatemsgid('InvalidDimensions'),'There must be either 4 or 6 band edges for Highpass designs.')
end
