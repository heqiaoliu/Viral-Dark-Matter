...
%   $Revision: 1.8.4.1 $  $Date: 2007/12/14 15:15:14 $

% Some preliminaries first
if nargout>=2 & nargin<2
   error(generatemsgid('Nargchk'),'Insufficient number of input arguments');
end
...
knxt = anxt(end);
if knxt == 1.0,
   error(generatemsgid('InvalidParam'),['At least one of the reflection coefficients is equal to one.', sprintf('\n'), 'The algorithm fails for this case.']);
end
