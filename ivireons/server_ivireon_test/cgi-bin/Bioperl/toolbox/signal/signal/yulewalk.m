...
%   $Revision: 1.7.4.4 $  $Date: 2007/12/14 15:06:39 $

if (nargin < 3 || nargin > 5)
   error(generatemsgid('Nargchk'),'Wrong number of input parameters.')
end
if (nargin > 3)
...
if abs(ff(1)) > eps || abs(ff(nbrk) - 1) > eps
   error(generatemsgid('InvalidFreqVec'),'The first frequency must be 0 and the last 1.')
end
