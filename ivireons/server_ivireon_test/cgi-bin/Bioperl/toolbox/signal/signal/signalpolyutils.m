...
%   $Revision: 1.15.4.9 $  $Date: 2009/07/14 04:00:09 $ 
...
if ~isempty(indx),
    ...
    % All zeros
    error(generatemsgid('SignalErr'),'Polynomial cannot contain only zeros.');
end
...
if ~isvector(b) || ~isvector(a)
  error(generatemsgid('InvalidDimensions'),'Numerators and denominators must be vectors.');
end
