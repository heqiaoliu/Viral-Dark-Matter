...
%   $Revision: 1.7.4.5 $ $Date: 2009/08/11 15:47:47 $
%
%   Reference:
...
for m = 1:nchannels
    ...
    if (~isreal(lsf)),
        error (generatemsgid('MustBeReal'),'Line spectral frequencies must be real.');
    end

    if (max(lsf) > pi || min(lsf) < 0),
        error (generatemsgid('InvalidRange'),'Line spectral frequencies must be between 0 and pi.');
    end
