...
%   $Revision: 1.26.4.4 $  $Date: 2008/05/31 23:26:07 $
...
switch firflag,
    case 1,
        [K,V,msg] = fir2latc(num,phaseOpt,nargout);
        if ~isempty(msg), error(generatemsgid('SigErr'),msg); end
    case 0,
        if length(num) == 1,
...
%-------------------------------------------------------------------
function [K,V,msg] = fir2latc(num,phaseOpt,nout)
...
if nout == 2,
    ...
    if ~strcmpi(phaseOpt,'none') && (max(abs(K)) > 1),
        warning(generatemsgid('NotMinOrMaxPhase'),...
            'The filter does not appear to be strictly %sphase.',phaseOpt);
    end
end
