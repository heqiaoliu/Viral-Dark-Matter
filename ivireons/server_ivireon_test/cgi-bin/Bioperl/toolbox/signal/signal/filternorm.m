...
%   $Revision: 1.6.4.5 $  $Date: 2009/08/11 15:47:36 $
...
% Check for stability
if ~isstable(f),
    error(generatemsgid('SignalErr'),'The filter must be stable.');
end
...
elseif pnorm == 2, 
    ...
    if isfir(f),
        ...
        s = norm(b,pnorm);
        if nargin == 4,
            warning(generatemsgid('Ignore'),'Tolerance is only used when computing the 2-norm of an IIR filter.');
        end
    else
        % Default tolerance
        ...
        if maxradius >= 1,
            error(generatemsgid('SignalErr'),'Filter is unstable within numerical precision.');
        end
