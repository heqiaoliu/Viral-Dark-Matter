...
%   $Revision: 1.7.4.2 $ $Date: 2007/12/14 15:05:40 $
%
%   Reference:
...
a  = a(:);
if ~isreal(a),
    error(generatemsgid('NotSupported'),'Line spectral frequencies are not defined for complex polynomials.');
end
...
if (max(abs(roots(a))) >= 1.0),
    error(generatemsgid('SignalErr'),'The polynomial must have all roots inside of the unit circle.');
end
