...
%   $Revision: 1.1.6.2 $  $Date: 2007/12/14 15:04:12 $
...
if (edge_pairs(1) == -0.5) && (edge_pairs(end) == 0.5),
   ...
   if new_freq <= edge_pairs(end-1),
     ...
     if (new_freq >= edge_pairs(2)),
       error(generatemsgid('InvalidParam'),['Both -1 and 1 have been specified as frequencies ' ...
...
otherwise
  error(generatemsgid('InternalError'),'Internal error: domain must be "whole" or "half".');
end
