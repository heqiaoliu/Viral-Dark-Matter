...
%   $Revision: 1.15.4.3 $  $Date: 2007/12/14 15:06:10 $
...
if ~isnumeric(s) || ndims(s)~=2 || size(s,2)~=6,
   error(generatemsgid('InvalidDimensions'),'SOS matrix must be specified as a matrix with 6 columns.')
end
m = size(s, 1);
...
if ~isempty(x) && ~isnumeric(x) && ~isvector(x)
    warning(generatemsgid('InvalidParam'),'Input must be a numeric vector.')
end
