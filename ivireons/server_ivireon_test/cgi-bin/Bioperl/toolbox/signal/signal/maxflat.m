...
%   $Revision: 1.10.4.9 $  $Date: 2009/09/03 05:27:49 $
...
if ischar(P)
	...
	P = 0;
	if rem(Z,2)==1
	    error(generatemsgid('oddOrder'),'For symmetric FIR filters, only even orders are allowed.');
    elseif Z >= 140,
        error(generatemsgid('orderTooLarge'),'For symmetric FIR filters, the order must be less than 140.');
	else % (Z is even)
	    Z = Z/2;
	end
end
if nargin > 3
	if ~ischar(PF)
		error(generatemsgid('invalidPlotFlag'),'Plot flag must be ''trace'', ''plots'', or ''both''.');
	end
	switch lower(PF(1))
	case 't'
		TEXT_PF = 1;
...
%%%%%%%%%%%%%%%%%%%% error %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isempty(k)
    error(generatemsgid('invalidCutoff'),...
        'For NB=%g and NA=%g, Wn must be between %g and %g.',...
...
if abs(sum(b2)) < 100*eps
   % terminate program
   error(generatemsgid('SignalErr'),'A pole-zero pair cancels at z=1. Use a lower order.')
else
