...
%   $Revision: 1.5.4.5 $  $Date: 2007/12/14 15:04:08 $
...
if ~any(size(x) == 1),
   error(generatemsgid('InvalidDimensions'),'Input signal must be a vector.');
end

if any(size(M) > 1),
   error(generatemsgid('InvalidDimensions'),'M must be a scalar.');
elseif round(M)~=M,
   error(generatemsgid('MustBeInteger'),'M must be an integer.');
end
