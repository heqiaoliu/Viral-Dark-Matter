...
%   $Revision: 1.1.6.1 $  $Date: 2005/06/16 08:35:34 $
...
if abs(f1)>eps || abs(f2 - 1) > eps,
    if this.NormalizedFrequency,
        error(generatemsgid('InvalidFrequencies'), ...
            'The first element of the ''Frequencies'' vector must be 0 and the last element 1.')
    else
        error(generatemsgid('InvalidFrequencies'), ...
            'The first element of the ''Frequencies'' vector must be 0 and the last element Fs/2.')
    end
end
