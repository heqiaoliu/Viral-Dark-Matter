...
%   $Revision: 1.11.4.3 $  $Date: 2007/12/14 15:06:01 $

%   Some preliminaries first

a = a(:);                   % Convert to a column vector if not already so
if a(1)~=1,
   warning(generatemsgid('InvalidParam'),['First coefficient of the prediction polynomial ' ...
            'was not unity.']);
   a = a/a(1);
end
...
if p < 2
   error(generatemsgid('InvalidDimensions'),'Polynomial should have at least two coefficients');
end
