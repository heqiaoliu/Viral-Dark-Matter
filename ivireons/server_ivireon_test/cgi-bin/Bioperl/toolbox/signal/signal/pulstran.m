...
%   $Revision: 1.8.4.6 $  $Date: 2009/08/11 15:47:52 $

if nargin<3,
  error(generatemsgid('Nargchk'),'Too few input arguments.');
end
% Check that d is either a vector, or an Mx2 matrix.
[dm,dn]=size(d);
if dm>1 && dn>2,
  error(generatemsgid('InvalidDimensions'),'Delay D must either be a vector, or an Mx2 matrix.');
end
...
if ischar(func) || isa(func,'function_handle'),
  ...
  if ~isvector(P),
    error(generatemsgid('InvalidDimensions'),'Pulse P must be a vector.');
  end
