...
%   $Revision: 1.17.4.6 $  $Date: 2005/06/16 08:18:42 $
...
%     end
    ...
    if length(nfft) > 1,
        ...
        [h{end+1}, w{end+1}, p{end+1}] = zerophase(cFilt, nfft, fs);
        if opts.showref && any(isquantized(cFilt))
            [hr, wr, pr] = zerophase(reffilter(cFilt), nfft, fs);
            h{end} = [h{end} hr];
            p{end} = [p{end} pr];
        end
        ...
        if opts.showref && any(isquantized(cFilt))
            [hr, wr, pr] = getresponse(reffilter(cFilt), inputs{:});
            h{end} = [h{end} hr];
            p{end} = [p{end} pr];
        end
    end
...
% -------------------------------------------------------------------------
function [h, w, p] = getresponse(Hd, nfft, unitcircle, fs, wmin, wmax)

nfft = max(4, round(nfft*fs/(wmax-wmin)));

% Get the response of the entire filter normalized.
[h, w, p, opts] = zerophase(Hd, nfft, 'whole');
