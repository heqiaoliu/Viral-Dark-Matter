...
%       $Revision: 1.6.4.3 $
%   $Revision: 1.6.4.3 $  $Date: 2007/12/14 15:04:53 $
...
if bw<=0, error(generatemsgid('MustBePositive'),'Fractional bandwidth BW must be > 0.'); end
if fc<0, error(generatemsgid('MustBePositive'),'Frequency FC must be >= 0 Hz.'); end
if bwr>=0, error(generatemsgid('InvalidRange'),'Bandwidth reference level BWR must be < 0 dB.'); end
...
if compute_cutoff,
  ...
  if nargout>1, error(generatemsgid('SignalErr'),'Too many return parameters.'); end
  ...
  % Return RF pulses:
  if nargin>4, error(generatemsgid('Nargchk'),'Too many input arguments.'); end
  if isempty(t),
    ye=[]; yc=[]; ys=[];
    return
  end
