...
while ind < length(varargin)
    ...
    elseif iscell(varargin{ind})  % cell array of properties
        ...
        if length(fields)~=length(vals)
            error(generatemsgid('InvalidDimensions'),'When using cell syntax, number of props must match values.')
        end
        ind = ind + 2;
    else % property name
        fields = varargin(ind);
        ...
        case {'callback', 'radiogroup'}
            if ~isstr(vals{i})
                str = sprintf('Value for property ''%s'' must be a string.',...
                              fields{i});
                error(generatemsgid('SigErr'),str)
            end
            objud = setfield(objud,fields{i},vals{i});
        case 'integer'
            if ~isequal(vals{i},0) & ~isequal(vals{i},1)
                error(generatemsgid('SigErr'),'''integer'' property must be 0 or 1.')
            end
            objud = setfield(objud,fields{i},vals{i});
        case {'editable','visible'}
            if ~isequal(vals{i},'on') & ~isequal(vals{i},'off')
                error(generatemsgid('SigErr'),sprintf('''%s'' property must be ''on'' or ''off''.',...
                               fields{i}))
            end
            objud = setfield(objud,fields{i},vals{i});
