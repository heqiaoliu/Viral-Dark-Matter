...
%    save_fig = gcf;
    ...
    if ~isempty(findobj(fig,'type','uimenu','tag','markersmenu'))  % only create the first time
      ...
      error(generatemsgid('GUIErr'),'before calling ruler(''init''), must create marker menu via ruler(''makemenu'')')
    end    
...
case 'allowtrack'
    ...
    otherwise
        error(generatemsgid('InvalidEnum'),'allowTrack must be ''on'' or ''off''.')
    end
