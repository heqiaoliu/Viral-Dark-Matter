...
case 'show_ith_array_signal'
    ...
    ud = get(fig,'userdata');
    if isempty(ud),
        error(generatemsgid('Empty'),'ud is empty');
    end
    ...
    if i<0,
        ...
        if isempty(rel_idx),
            warning(generatemsgid('GUIWarn'),'Chosen array signal not currently being displayed by Signal Browser.');
            return;
        end
        i = rel_idx;
    end
...
otherwise
    if ischar(varargin{1})
        warning(generatemsgid('InvalidAction'),'Action ''%s'' not recognized',varargin{1});
    else
        warning(generatemsgid('InvalidParam'),'Sigbrowse: takes no arguments');
    end
end
