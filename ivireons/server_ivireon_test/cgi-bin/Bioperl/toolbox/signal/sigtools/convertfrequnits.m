...
%   $Revision: 1.4.4.4 $  $Date: 2008/05/31 23:27:35 $

% Parse the inputs
[Frequency, Fs, Previous, Current, All, err] = parse_inputs(varargin{:});
if ~isempty(err), error(generatemsgid('SigErr'),err); end
...
% normalized units
if isempty(Fs)
    error(generatemsgid('InvalidFreqVec'),'Converting to or from normalized units requires that FS be specified')
end
...
if ~isa(inputFrequency,'cell') & ~isa(inputFrequency,'double') & ~isa(inputFrequency,'char')
    error(generatemsgid('InvalidParam'),'Valid data types are ''cell'', ''double'' and ''char''')
end
...
elseif isa(Opts,'double')
    ...
    error(generatemsgid('MustBeAString'),'OPTS should be a cell array of strings or a double')
end
...
if strncmpi(pu,'norm',4) | strncmpi(cu,'norm',4)
    if isempty(fs)
        if isempty(err)
            err = 'Converting to or from normalized units requires that FS be specified';
        end
    end
end
