...
$Revision: 1.1.6.26.2.1 $  $Date: 2010/06/24 19:34:10 $
-->
  ...
  <xsl:variable name="link">
    ...
    <xsl:choose>
      <xsl:when test="type and file">
          <xsl:variable name="localFile">			<xsl:choose>
...
<xsl:with-param name="filepath" select="file" />
      ...
      <xsl:otherwise>
        ...
        <xsl:text>',</xsl:text>
	    <xsl:value-of select='$overrideDefaultLang'/>
        <xsl:text>)</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
      ...
      <td width="80" class="demopanel-type" align="left">
        <xsl:choose>
          <xsl:when test="type='M-file'">
            ...
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_m-file.png" align="middle"/><xsl:value-of select="$mfile"/>
          </xsl:when>
          <xsl:when test="type='M-GUI'">
            <xsl:attribute name="title"><xsl:value-of select="$mguidesc"/></xsl:attribute>
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_m-gui.png" align="middle"/><xsl:value-of select="$mgui"/>
          </xsl:when>
          <xsl:when test="type='model'">
            <xsl:attribute name="title"><xsl:value-of select="$modeldesc"/></xsl:attribute>
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_model.png" align="middle"/><xsl:value-of select="$model"/>
          </xsl:when>
          <xsl:when test="type='video'">
            <xsl:attribute name="title"><xsl:value-of select="$videodesc"/></xsl:attribute>
            <img src="{$urlbase}{$matlabroot}/toolbox/matlab/demos/private/type_video.png" align="middle"/><xsl:value-of select="$video"/>
          </xsl:when>
        </xsl:choose>
      </td>
    </tr>
...
<xsl:template name="buildLocalizedPath">
   ...
   <xsl:choose>
      ...
      <xsl:otherwise>
		<xsl:value-of select="$language" />/<xsl:value-of select="$filepath" />	  </xsl:otherwise>
   </xsl:choose>
</xsl:template>
