...
%   $Revision: 1.1.6.8 $  $Date: 2008/08/20 23:00:52 $
...
if ~errCheck         
    switch funfcn{1} % evaluate objective functions and possibly gradients
        ...
        otherwise
            error('optim:goalcon:UndefinedCalltypeFgoalattain','Undefined calltype in GOALCON.')
    end
        ...
        case 'fun_then_grad'
            ...
            try
                ...
                optim_ME = MException('optim:goalcon:GradientError', ...
                    'Failure in initial user-supplied objective gradient function evaluation. Optimization cannot continue.');
                userFcn_ME = addCause(userFcn_ME,optim_ME);
                rethrow(userFcn_ME)
            end
        otherwise
        ...
        case 'fun'
            try
                ...
                optim_ME = MException('optim:goalcon:NonlconError', ...
                    'Failure in initial user-supplied nonlinear constraint function evaluation. Optimization cannot continue.');
                userFcn_ME = addCause(userFcn_ME,optim_ME);
                rethrow(userFcn_ME)
            end
            c = ctmp(:); ceq = ceqtmp(:);
        ...
        case 'fun_then_grad'
            ...
            try
                ...
                optim_ME = MException('optim:goalcon:NonlconGradError', ...
                    'Failure in initial user-supplied nonlinear constraint gradient function evaluation. Optimization cannot continue.');
                userFcn_ME = addCause(userFcn_ME,optim_ME);
                rethrow(userFcn_ME)
            end
        case ''
    ...
    if ~isempty(gf) && ~all(size(gf) == [numVar,nGoalCnstr])
        gradSizeExcept = MException('optim:goalcon:InvalidSizeOfGF', ...
            'Gradient of the objectives must have size %i-by-%i.',numVar,nGoalCnstr);
    ...
    if ~isempty(gc) && (cgrow ~= numVar || cgcol ~= non_ineq)
        cGradSizeExcept = MException('optim:goalcon:InvalidSizeOfGC', ...
            'Gradient of the nonlinear inequality constraints must have size %i-by-%i.',numVar,non_ineq);
    ...
    if ~isempty(gceq) && (ceqgrow ~= numVar || ceqgcol ~= non_eq)
        ceqGradSizeExcept = MException('optim:goalcon:InvalidSizeOfGCeq', ... 
            'Gradient of the nonlinear equality constraints must have size %i-by-%i.',numVar,non_eq);
