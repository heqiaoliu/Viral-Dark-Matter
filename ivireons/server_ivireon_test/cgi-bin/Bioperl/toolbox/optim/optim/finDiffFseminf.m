function [gradf,cJac,NEWLAMBDA,OLDLAMBDA,s,numEvals] = finDiffFseminf(xCurrent,...
             xOriginalShape,funfcn,lb,ub,fCurrent,cCurrent,...
             DiffMinChange,DiffMaxChange,typicalx,finDiffType,variables,...
             LAMBDA,NEWLAMBDA,OLDLAMBDA,POINT,FLAG,s,startnlineq,varargin)
...
%   $Revision: 1.1.6.1 $  $Date: 2008/09/13 06:57:29 $
...
if fwdFinDiff % forward differences
    CHG = sqrt(eps)*nonzerosign(xCurrent).*max(abs(xCurrent),abs(typicalx));
else % central differences
    CHG = (eps^(1/3))*max(abs(xCurrent),abs(typicalx));
end
%
...
if ischar(variables)
   if strcmp(variables,'all')
      ...
      error('optim:fseminf:InvalidVariables', ...
