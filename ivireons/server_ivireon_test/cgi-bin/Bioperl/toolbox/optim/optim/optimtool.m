...
%   $Revision: 1.1.6.10 $  $Date: 2010/03/22 04:17:52 $
...
elseif isstruct(inputArg) % Input argument is either a problem structure or options structure
    ...
    if validProblemStruct
        ...
        if ~isempty(optimGUI)
            button = questdlg(xlate('Do you want to replace the current problem and option values?'), ...
...
elseif ischar(inputArg)
    if strcmpi(inputArg, 'close')
        ...
        % Make sure that the GUI is not open; else values will be replaced
        if ~isempty(optimGUI)
            button = questdlg(xlate('The Optimization Tool is already open. Do you want to replace the current values?'), ...
                'Replace Current Values', 'Yes', 'No', 'No');
            if ~strcmp(button, 'Yes')
                javaMethodEDT('toFront',optimGUI);
                return;
