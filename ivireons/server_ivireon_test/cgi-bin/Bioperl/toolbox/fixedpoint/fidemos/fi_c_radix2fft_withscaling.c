/** 
    FI_C_RADIX2FFT_WITHSCALING Fixed-Point radix 2 FFT, skipping multiply by W^0.

    Y = fi_radix2fft_withscaling(X, W, Wfractionlength) computes a radix
    2, fixed-point FFT of int16 input X with int16 twiddle factors W
    with scaling after each section.
    ...
    @version $Revision: 1.1.6.1 $ $Date: 2005/02/23 02:43:35 $ 
*/
#include "mex.h"
...
#define USAGE "\nUsage:  Y = fi_radix2fft_mex(X, W, Wfractionlength)"

void bitreverse(int16_T* xr, int16_T* xi, int n);
bool isPowerOfTwo(int n);
int log2(int n);

#include "fi_c_radix2fft_withscaling.h"
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    fi_c_radix2fft_withscaling(xr, xi, wr, wi, n, nw, t, Wfraclen);
