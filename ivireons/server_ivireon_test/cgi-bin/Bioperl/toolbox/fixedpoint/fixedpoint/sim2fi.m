...
%   $Revision: 1.1.6.12 $  $Date: 2010/05/20 02:17:41 $


doRobust = robustRequest(varargin{:});

if doRobust
    T = getNumericType(varargin{1:(end-1)});
else
    T = getNumericType(varargin{1:end});
end

if T.isdouble || checkoutFiLicense

    A = embedded.fi.simfi(T,IntArray);

elseif doRobust

    warning('fixedpoint:license:resorttodoubles','Cannot construct fi object; converting to double');
    A = localConvert(IntArray,T);
else
    error('fixedpoint:license:required', ...
          'A Fixed-Point Toolbox license is required to create a fi object with any data type other than double.');
end

%---------------------------------------------------------------
function res = robustRequest(varargin)
%---------------------------------------------------------------
% If last argument is 'robust'
% then the caller is requesting that the results
% be 'robust' to the lack of a fixed point toolbox
% license.  A cruder algorithm using doubles will 
% be employed if necessary

res = (nargin > 1) && ischar( varargin{end} ) && strncmpi('robust',varargin{end},6);

%---------------------------------------------------------------
function gotIt = checkoutFiLicense
%---------------------------------------------------------------
...
%
% If new formula calcs 4 chunks, then old or new is moot
%
...
switch nargin
  case 1
    ...
    T = varargin{1};
    if ~isnumerictype(T)
      error('fixedpoint:sim2fi:invalidInputs',...
            ['Calling SIM2FI with 2 input arguments, the second argument ' ...
             'must be a NUMERICTYPE object.']);
    end
  case 2
    error('fixedpoint:sim2fi:incorrectNumberOfInputs',...
          ['SIM2FI must have 2, 4, or 5 input arguments. You have ' ...
           'entered 3.']);
  case 3
    % A = SIM2FI(IntArray, Signed, WordLength, FractionLength)
    T = numerictype;
...
if T.WordLength > 128
  error('fixedpoint:sim2fi:invalidFixedPointWordlength',...
