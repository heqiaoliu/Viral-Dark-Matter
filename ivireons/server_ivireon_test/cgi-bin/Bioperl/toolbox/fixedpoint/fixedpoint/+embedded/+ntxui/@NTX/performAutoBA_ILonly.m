...
%   $Revision: 1.1.6.3.2.1 $     $Date: 2010/07/06 14:39:15 $
...
if dlg.BAGraphicalMode % Interactive overflow mode
    ...
    newOver = floor(log2(dlg.BAILMagInteractive));
    ...
    switch dlg.BAILMethod
      case 1
        ...
        case 2 % Largest Magnitude mode
            ...
            newOver = 1+floor(log2(dlg.BAILSpecifyMagnitude));
            if dlg.extraMSBBitsSelected
                newOver = newOver + dlg.BAILGuardBits;
                ntx.wasExtraMSBBitsAdded = true;
            end
      ...
      otherwise
        ...
        error(errID, 'Unsupported BAILMethod (%d)',dlg.BAILMethod);
    end
end
