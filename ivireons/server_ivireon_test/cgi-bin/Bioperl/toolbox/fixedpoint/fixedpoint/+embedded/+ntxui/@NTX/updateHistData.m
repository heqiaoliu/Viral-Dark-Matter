...
% quantizer applied to it i.e., real and imaginary parts cannot be quantized
% differently. 
if ~isreal(data)
    data_re = real(data(:));
    data_im = imag(data(:));
...
% If we removed all the data, there's nothing more to do
if isempty(data)
    return
end
...
% Determine vector of quantized (double-precision) values, and whether each
...
%
% Get power-of-2 exponent of abs(data)
% Leave in floating point (do not round)
data2 = log2(abs(data));


% Determine min and max range based on absolute values
% Compute reasonable bin limits.
xmax_exp = ceil(max(data2)); 
xmin_exp = floor(min(data2))-1;     % DO NOT REMOVE...see comment below!
...
%         localCenters(k) < Vals <=localCenters(k+1)
%
% The underflow cursor always goes to the right of the Fraction length or
% xmin_exp because the values contained in the bin contains the (2^-(FL-1)
% 2^-FL] value. We need an additional bin center to account for this.
%
% Compute local pos and neg data value histograms
localCenters   = xmin_exp : xmax_exp; % exponents (N), not values (2^N)
localPosCounts = computeHist(data2(posVals),localCenters);
localNegCounts = computeHist(data2(negVals),localCenters);

% xxx Bug fix for histc:
%  If data=[], histc returns an empty (e.g., 0xN)
%  It should return an Nx1 always, perhaps with zero-count entries
%  So if counts are empty, fix them
if isempty(localPosCounts)
    localPosCounts = zeros(size(localCenters));
end
if isempty(localNegCounts)
