...
%   $Revision: 1.1.8.1 $  $Date: 2009/09/09 21:07:27 $
...
if (nargin == 2)
    if ~isnumeric(dim)
        ...
        error('fi:mean:dimMustBeNumeric',...
            'Dimension input to ''mean'' must be of type ''numeric''.');
    elseif (~isscalar(dim)||(dim <= 0)||~isreal(dim)||~isequal(floor(dim), dim))

        error('fi:mean:dimMustBeRealPosScalarInt', ...
            'Dimension input to ''mean'' must be a positive, real-valued integer.');
    end
else
