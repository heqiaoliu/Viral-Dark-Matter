%ISSCALINGUNSPECIFIED Determine whether numerictype object has unspecified scaling
%   ISSCALINGUNSPECIFIED(T) returns 1 if numerictype object T is fixed-point
%   or scaled double and its scaling has not been specified.
%
%   See also EMBEDDED.NUMERICTYPE/ISFIXED,
%            EMBEDDED.NUMERICTYPE/ISFLOAT,
%            EMBEDDED.NUMERICTYPE/ISBOOLEAN,
%            EMBEDDED.NUMERICTYPE/ISDOUBLE,
%            EMBEDDED.NUMERICTYPE/ISSINGLE,
%            EMBEDDED.NUMERICTYPE/ISSCALEDDOUBLE,
%            EMBEDDED.NUMERICTYPE/ISSCALEDTYPE,
%            EMBEDDED.NUMERICTYPE/ISSCALINGBINARYPOINT,
%            EMBEDDED.NUMERICTYPE/ISSCALINGSLOPEBIAS,
%            EMBEDDED.FI/ISSCALINGUNSPECIFIED

%   Copyright 2010 The MathWorks, Inc.
%   $Revision: 1.1.6.1 $  $Date: 2010/04/21 21:22:14 $
