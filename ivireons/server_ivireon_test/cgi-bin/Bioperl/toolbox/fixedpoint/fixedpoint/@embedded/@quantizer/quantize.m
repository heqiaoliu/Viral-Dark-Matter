...
%   $Revision: 1.1.6.8 $  $Date: 2007/12/10 21:33:14 $
...
for k=1:nargs
  ...
  if ~isempty(x)
    ...
    elseif isfi(x)
      warning(warn_state);
      error('quantizer:quantize:valueCannotBeFi','Value to quantize cannot be a fi.');
    end
  end
  % Set the output
  varargout{k} = x;
...
omsg = overflowmsg(q,oldnover);
if ~isempty(omsg)
    warning('fi:overflow',omsg);
end
