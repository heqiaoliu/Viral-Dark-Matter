...
%   first nonsingleton dimension as vectors, returning an array of row vectors.
...
%   $Revision: 1.1.6.7 $  $Date: 2009/10/24 19:04:12 $
...
if ~isempty(dim) 
  if ~(length(dim)==1 && floor(dim(1))==dim(1) && dim(1)>0 && dim(1)<2^31)
    error('fixedpoint:getdimarg:dimensionMustBePositiveInteger',...
          'Dimension argument must be a positive integer scalar in the range 1 to 2^31.');
  end
end
...
switch lower(doublenativestr)
  ...
  otherwise 
    error('fixedpoint:sum:unknownFlag','Trailing string input must be ''double'' or ''native''.');
end
