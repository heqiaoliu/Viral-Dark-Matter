...
while loopcounter <= length(varargin)
        ...
        switch length(varargin)
          ...
          otherwise
            ...
            if loopcounter>length(varargin)
              error('quantizer:setquantizer:invalidPVPairs','Invalid parameter/value pair arguments');
            end
            value=varargin{loopcounter};
        end
      case stringvalues
  ...
  elseif isa(value,'embedded.quantizer')
    ...
    error('quantizer:setquantizer:invalidValueClass',...
          ['Values of class ',class(value),[' are not allowed in ' ...
