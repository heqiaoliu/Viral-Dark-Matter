...
%   $Revision: 1.1.8.3 $  $Date: 2010/04/05 22:16:15 $
...
% accumulation
[T,errmsg] = emlGetNTypeForMTimes(numerictype(b),numerictype(x),fm,isreal(b),isreal(x),...
    L,true,65535,'FILTER');
if ~isempty(errmsg)
    error('fi:filter:GetNTypeForMTimes', errmsg);
end

% Check that the states have the correct fixed-point settings
if isfixed(T) && ~(isempty(zi) || isfi(zi) && isequal(numerictype(zi), T))
    error('fi:filter:IncorrectStateNumerictype',...
        ['The states must be a FI object matching the output numerictype, ',...
        'which is DataType = %s, Signedness = %s, WordLength = %d, FractionLength = %d.'],...
        get(T,'DataType'), get(T,'Signedness'), get(T,'WordLength'), get(T,'FractionLength'));
end
...
if isfloat(T) || isscaleddouble(T) || ...
    ...
    if isfixed(T) && ...
        ...
        for k = 1:size(x,2),
            ...
            y.subsasgn(s1,tmp.subsref(s2));
        end
        
    else
        % General case
        % Set appropriate fimath
        [Tm,errmsg] = emlGetNTypeForTimes(numerictype(b),numerictype(x),fm,isreal(b),isreal(x),65535);
        if ~isempty(errmsg)
            error('fi:filter:GetNTypeForMTimes', errmsg);
        end
        ...
        if size_zf_2d(1) ~= L-1 || prod(size_zf(2:end)) ~= size_zf_2d(2)
            error('fi:filter:invalidInitialConditionSize',...
...
if ~isnumeric(dim) || ~isscalar(dim) || ~isreal(dim) || dim<1 || ...
        dim~=floor(dim)
    error('fi:filter:badDim',...
        'Dimension argument must be a positive integer scalar.');
end


if ~isvector(b)
    error('fi:filter:numVectorOnly', ...
        'The numerator must be a vector.');
end
...
if ~isscalar(a) || a ~= 1,
    error('fi:filter:supportFIROnly', ...
...
if ~isequal(get(b,'datatype'), get(x,'datatype')) || ...
    ...
    error('fi:filter:notSameDataTypes',...
        ['The filter coefficients, input data, and initial conditions ',...
