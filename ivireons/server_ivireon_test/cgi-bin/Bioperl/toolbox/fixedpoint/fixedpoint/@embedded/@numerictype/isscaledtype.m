%ISSCALEDTYPE Determine whether numerictype object has fixed-point or scaled double data type
%   ISSCALEDTYPE(T) returns 1 when the DataType property of numerictype object 
%   T is 'Fixed' or 'ScaledDouble', and 0 otherwise.
%
%   See also EMBEDDED.NUMERICTYPE/ISFIXED,
%            EMBEDDED.NUMERICTYPE/ISFLOAT,
%            EMBEDDED.NUMERICTYPE/ISBOOLEAN,
%            EMBEDDED.NUMERICTYPE/ISDOUBLE,
%            EMBEDDED.NUMERICTYPE/ISSINGLE,
%            EMBEDDED.NUMERICTYPE/ISSCALEDDOUBLE,
%            EMBEDDED.NUMERICTYPE/ISSCALINGBINARYPOINT,
%            EMBEDDED.NUMERICTYPE/ISSCALINGSLOPEBIAS,
%            EMBEDDED.NUMERICTYPE/ISSCALINGUNSPECIFIED,
%            EMBEDDED.FI/ISSCALEDTYPE

%   Copyright 1999-2010 The MathWorks, Inc.
%   $Revision: 1.1.6.2 $  $Date: 2010/04/21 21:22:11 $
