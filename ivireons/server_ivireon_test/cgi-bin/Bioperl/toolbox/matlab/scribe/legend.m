...
if isHGUsingMATLABClasses
    ...
    elseif nargout == 2
        [leg,labelhandles] = legendHGUsingMATLABClasses(varargin{:});
    elseif nargout == 3
        [leg,labelhandles,outH] = legendHGUsingMATLABClasses(varargin{:});
    else
        [leg,labelhandles,outH,outM] = legendHGUsingMATLABClasses(varargin{:});
    end
else
    narg = nargin;
    ...
    elseif nargout<=4
        [leg,labelhandles,outH,outM] = find_legend_info(ha);
    elseif nargout>4
        error(id('BadNumberOfOutputs'),'Must have 0,1,2,3 or 4 outputs.');
    end
    ...
    if ischar(argin{n})
        if strcmpi(argin{n},'orientation')
            if (n < nargs) && ...
                ...
                error(id('UnknownParameter'),...
                    'Invalid argument to orientation. Type ''help legend'' for more information.');
            end
        elseif strcmpi(argin{n},'location')
            ...
            elseif (n < nargs) && ...
                ...
                error(id('UnknownParameter'),...
                    'Invalid argument to location. Type ''help legend'' for more information.');
            end
            n = n + 1; % skip 'Location'
...
for i=1:2:n
    if ~isprop('scribe','legend',args{i})
        error(id('UnknownProperty'),'Unknown property: %s',args{i});
    elseif strcmpi(args{i},'Parent')
        if ~isa(handle(args{i+1}),'hg.figure') && ~isa(handle(args{i+1}),'hg.uipanel')
            error(struct('identifier',id('InvalidParent'),...
                'message',sprintf('A Legend can not be a child of an object of type %s.',get(args{i+1},'Type'))));
        end
    end
end
