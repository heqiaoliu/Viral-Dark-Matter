...
schema.event(h,'LegendInfoChanged');
schema.event(h,'LegendConstructorDone');
...
p = schema.prop(h,'EdgeColor','axesXColorType');
p.AccessFlags.Init = 'on';
p.FactoryValue = get(0,'DefaultAxesXColor');
p.SetFunction = @edgeColorSetter;
...
%----------------------------------------------------------------%
function val = edgeColorSetter(h,val) %#ok
set(h,'XColor',val);
set(h,'YColor',val);
