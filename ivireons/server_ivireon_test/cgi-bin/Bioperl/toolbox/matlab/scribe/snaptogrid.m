...
if nargin==2
    if isscalar(varargin{1}) && ishghandle(varargin{1},'figure')
        ...
        error(id('InvalidFirstFigureHandle'),'first argument must be a figure');
    end
    if ischar(varargin{2}) 
        if ~( strcmpi(varargin{2},'on') || strcmpi(varargin{2},'off') || ...
            ...
            error(id('InvalidAction'),'string must be either ''on'',''off'',''togglesnap'',''toggleview'' or ''noaction''');
        else
            action = varargin{2};
        end
    else
...
elseif nargin==1
    if isscalar(varargin{1}) && (ishandle(varargin{1}) || isobject(varargin{1}))
        if ishghandle(varargin{1},'figure')
            ...
            error(id('InvalidFigureHandle'),'handle argument must be a valid figure');
        end
    elseif ischar(varargin{1})
        fig = figcheck;
        if ~(strcmpi(varargin{1},'on') || strcmpi(varargin{1},'off') || strcmpi(varargin{1},'toggle'))
            error(id('InvalidOnOff'),'string must be either ''on'' or ''off''');
        else
            action = varargin{1};
        end
    else
...
if length(findobj(0,'type','figure'))<1
    error(id('NoFigures'),'must have a figure to enable snap to grid');
end
