...
if any(subject == char(10)) || any(subject == char(13))
    error('MATLAB:sendmail:InvalidSubject',  ...
...
if numel(attachments) == 0
    ...
    for iAttachments = 1:numel(attachments)
        ...
        [unused,fileName,fileExt] = fileparts(fullName);
...
try
    ...
    if isempty(niceError)
        ...
        error('MATLAB:sendmail:SmtpError','%s',niceError)
    end
end
...
try
    ...
    error('MATLAB:sendmail:AddressError','%s',stripJavaError(exception.message));
end
