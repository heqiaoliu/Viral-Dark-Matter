...
% $Revision: 1.1.6.8 $   $Date: 2008/03/17 22:16:53 $
...
try   
   ...
   cleanup(gunzipFilename, outStream, gzipInStream, fileInStream);   
   eid = sprintf('MATLAB:%s:javaCopyStreamError',mfilename);
   error(eid, ...
      'Unexpected end of input stream when attempting to GUNZIP the file "%s".',  ...
      gunzipFilename);
end
