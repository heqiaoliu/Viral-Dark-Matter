...
%   $Revision: 1.23.4.33.2.1 $  $Date: 2010/06/24 19:34:37 $
%=============================================================================
rawData = {};
Sheet1 = 1;
if nargin < 2
...
if strcmpi(mode, 'basic') 
    warning('MATLAB:xlsread:Mode',...
        'XLSREAD has limited import functionality on in basic mode.  Refer to HELP XLSREAD for more information.');
        basicMode = true;
else
    try
...
if basicMode 
    if ~isempty(range)
        warning('MATLAB:xlsread:RangeIncompatible',...
            'Range cannot be used in ''basic'' mode.  The entire sheet will be loaded.');
    end
    if isequal(sheet,1)  
        sheet = '';
    elseif isequal(sheet,-1)
        sheet = '';
        warning('MATLAB:xlsread:InteractiveIncompatible',...
            'Interactive selection cannot be used in ''basic'' mode.  First sheet will be loaded.');
    elseif ~ischar(sheet)
...
if nargin >= 5 
    ...
    if basicMode
        warning('MATLAB:xlsread:Incompatible',...
         'Custom functions cannot be used in ''basic'' mode.  The custom function argument will be ignored.')
    elseif ~isa(customFun,'function_handle')
        warning('MATLAB:xlsread:NotHandle', 'The fifth argument to XLSREAD must be a function handle.');
    else
        custom = true;
    end
end 
