...
% $Revision: 1.1.6.19 $  $Date: 2009/11/05 17:00:42 $
...
try
    ...
    if ~isempty(exception)
        ...
        if strcmp(exception, ...
            ...
            error('MATLAB:createClassFromWsdl:ConnectionRefused','Connection refused.');
        end
        if strcmp(exception, ...
                'ice.net.URLNotFoundException: Document not found on server')
            error('MATLAB:createClassFromWsdl:UrlNotFound','The requested URL was not found on this server.')
        end
        host = regexp(exception,'java.net.UnknownHostException: (.*)','tokens','once');
        if ~isempty(host)
            error('MATLAB:createClassFromWsdl:UnknownHost','Unknown host: %s',host{1})
        end
        sax = regexp(exception,'org.xml.sax.SAXException: (.*)','tokens','once');
        if ~isempty(sax)
            error('MATLAB:createClassFromWsdl:BadXml','Could not parse XML: %s',sax{1})
        end
        error('MATLAB:createClassFromWsdl:Exception',exception)
    else
        rethrow(e)
    end
end
        ...
        if (~isempty(msgPart.getName()) || ~isempty(msgPart.getTypeName)) 
            ...
            if (~isempty(qname))
                ...
                if (isempty(msgPart.getName()))                     
					error('MATLAB:createClassFromWsdl:BadXml', ...
						'WSDL Parsing Failure: unsupported wsdl');
                end
...
if (count~=(sum(cellfun('length',C))+length(C))) || (status==-1)
    error('MATLAB:createClassFromWsdl:WriteError','Error writing file %s',fname)
end
