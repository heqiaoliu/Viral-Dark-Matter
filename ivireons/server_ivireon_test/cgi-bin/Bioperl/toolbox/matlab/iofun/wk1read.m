...
%   array B. 
% 
%   WK1READ will be removed in a future release. 
%
%   See also WK1WRITE, CSVREAD, CSVWRITE. 

%   Copyright 1984-2009 The MathWorks, Inc.
% 
%   $Revision: 5.21.4.9 $  $Date: 2009/12/31 18:51:31 $ 

warning('MATLAB:wk1read:FunctionToBeRemoved', ...
    'WK1READ will be removed in a future release.'); 
...
% Get Filename 
if ~isstr(filename)
    error('MATLAB:wk1read:FilenameMustBeString', 'Filename must be a string.');
end 

% do some validation 
if isempty(filename)
    error('MATLAB:wk1read:FilenameIsEmpty', 'Filename must not be empty.');
end 
...
if ~isequal(exist(filename), 2)
    error('MATLAB:wk1read:FileNotFound', 'File not found.')
end 

% open the file Lotus uses Little Endian Format ONLY 
fid = fopen(filename,'rb', 'l'); 
if fid == (-1) 
    error('MATLAB:wk1read:FailedToOpen', 'Could not open file %s.',filename); 
end 
...
if(header(1) ~= LOTWK1BOFSTR) 
    error('MATLAB:wk1read:InvalidFile', 'Not a valid WK1 file.'); 
end 
...
elseif nargout < 2 | foundrng == 0 
    if foundrng == 0 
        error('MATLAB:wk1read:InvalidRange', 'Invalid range string or unknown named range argument.'); 
    else 
        error('MATLAB:wk1read:MissingNumericData', 'No numeric data in WK1 file - use two output args to look for text data.'); 
    end 
elseif exist('n','var') 
    m = zeros(size(n)); 
else 
