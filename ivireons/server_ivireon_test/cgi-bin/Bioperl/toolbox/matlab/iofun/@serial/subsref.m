...
%   $Revision: 1.6.4.10 $  $Date: 2009/10/16 05:01:59 $
...
for i=1:length(index1)
    if ~isnumeric(index1{i}) && (~(ischar(index1{i}) && (strcmp(index1{i}, ':'))))
        if ischar(index1{i})
            ...
            throwAsCaller(MEexception('MATLAB:serial:subsref:UndefinedFunction', ['Function ''subsindex'' is not defined for values of class ''' class(index1{i}) '''.']));
        end
    end
end
...
if any(cellfun('isempty', index1))
    for i = 1:length(index1)
        if ~isempty(index1{i}) && index1{i} > size(obj, i)
            throwAsCaller(MException('MATLAB:serial:subsref:exceedsdims', 'Index exceeds matrix dimensions.'));
        end
    end
    result = [];
    return;
elseif length(index1{1}) ~= (numel(index1{1}))
    thowAsCaller(MException(errorId, 'Only a row or column vector of instrument objects can be created.'));
