...
%   $Revision: 1.7.4.11 $  $Date: 2009/10/16 05:01:58 $
...
try
    ...
    if length(type) ~= numel(type)
        newExc = MException('MATLAB:serial:subsasgn:nonMatrixConcat', 'Only a row or column vector of instrument objects can be created.');
...
try
    ...
    exception = MException('instrument:subsasgn:badsubscript','Index exceeds matrix dimensions.');
end
