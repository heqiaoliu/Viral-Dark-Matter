...
%   $Revision: 1.6.4.7 $  $Date: 2008/05/19 23:18:10 $
...
if(isvalid(obj))
    ...
    if strcmpi(get(obj,'FlowControl'),'hardware') && strcmpi(serialPinStats.ClearToSend,'off')
        error('MATLAB:serial:fwrite:invalidState','FWRITE cannot be called. The FlowControl property is set to ''hardware'' and the Clear To Send (CTS) pin is high. This could indicate that the serial device may not be turned on, may not be connected, or does not use hardware handshaking.');
    end
end
...
try
    ...
    error('MATLAB:serial:fwrite:opfailed', aException.message);
end
