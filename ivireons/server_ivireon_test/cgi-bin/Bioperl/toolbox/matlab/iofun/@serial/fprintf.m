...
%   $Revision: 1.6.4.8 $  $Date: 2009/04/15 23:20:43 $
...
if ~isempty(errmsg)
    error('MATLAB:serial:fprintf:invalidFormat', errmsg);
end
...
if(isvalid(obj))
    ...
    if strcmpi(get(obj,'FlowControl'),'hardware') && strcmpi(serialPinStats.ClearToSend,'off')
        error('MATLAB:serial:fprintf:invalidState','FPRINTF cannot be called. The FlowControl property is set to ''hardware'' and the Clear To Send (CTS) pin is high. This could indicate that the serial device may not be turned on, may not be connected, or does not use hardware handshaking.');
    end
end
...
try
   ...
   error('MATLAB:serial:fprintf:opfailed', aException.message);
end
