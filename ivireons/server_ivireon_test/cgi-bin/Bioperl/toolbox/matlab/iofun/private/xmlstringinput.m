...
%   $Revision: 1.5.4.8 $  $Date: 2009/06/16 04:18:40 $
...
if isempty(xString)
    error('MATLAB:xmlstringinput:EmptyFilename', 'Filename is empty');
...
if nargin<2 || isFullSearch
    if ~exist(xString,'file')
        ...
        error('MATLAB:xml:FileNotFound','File %s not found',xString);
    else
        out = which(xString);
        if isempty(out)
            out = xString;
