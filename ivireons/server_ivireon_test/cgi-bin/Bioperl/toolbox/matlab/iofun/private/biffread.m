...
%   $Revision: 1.6.4.9 $  $Date: 2008/08/26 18:21:27 $
...
if fid < 0
    error('MATLAB:xlsread:biffread:FileNotFound', 'Unable to read XLS file %s. Cannot open file.', filename);
end
biffvector = fread(fid, inf, '*uint8');
fclose(fid);
if isempty(biffvector)
    error('MATLAB:xlsread:biffread:FileEmpty', 'Unable to read XLS file %s. File is empty.', filename);
end
...
elseif length(biffvector) > 2048 && biffvector(2049) == 9 
    ...
    if isempty(start)
        error('MATLAB:xlsread:biffread:FileFormat', 'Unable to read XLS file %s. File is not in recognized format.', filename);
    end
end
