...
%    $Revision: 1.17.2.20 $  $Date: 2009/12/05 01:58:58 $
...
if errorDuringRead
    ...
    % If the flag is set it indicates that we've handled a truncated file.
     warning('daq:daqread:filetruncated', ['This DAQ data file is truncated. ',...
...
if nargout ~= 1 && strncmpi(outputformat, 'tscollection', length(outputformat))
    fclose(fid);
    error('daq:daqread:onlydatareturned','Only 1 left hand argument can be specified when ''OutputFormat'' is set to ''tscollection''.')
end
...
if (strncmpi(timeformat, 'matrix', length(timeformat)) && strncmpi(outputformat, 'tscollection', length(outputformat)))
    fclose(fid);
    error('daq:daqread:nomatrixtimewithtsc','''TimeFormat'' cannot be set to ''matrix'' when ''OutputFormat'' is set to ''tscollection''.')
end
...
for i = 1:length(header)
   ...
   elseif (isfield(header, 'typestr'))
       ...
       error('daq:daqread:norecordtype','The record type is not available in the header.');   
   end
...
if rem(length(pv), 2) ~= 0
   error('daq:daqread:unexpected','Invalid Property-Value pairs supplied to DAQREAD.');
end
...
for i = 1:2:length(pv)
   if strncmpi(pv{i}, 'samples', length(pv{i}))
      ...
      if iscell(samp)
         if strcmp(samp{2}, 'end') && ~isempty(samplesacquired)
            ...
            error('daq:daqread:unexpected','Invalid ''Samples'' range specified.');
         end
      end
      % Error if the samples specified is negative or is not a range.
      index = find(samp <= 0,1);
      if ~isempty(index) 
         error('daq:daqread:unexpected','Invalid ''Samples'' range requested.');
      elseif ~(length(samp) == 2 && min(size(samp)) == 1 && samp(2) > samp(1))
         error('daq:daqread:unexpected','''Samples'' must be specified as a two-element range.');
      elseif any(round(samp) ~= samp)
         error('daq:daqread:unexpected','''Samples'' must contain integer values.');
      elseif ~isempty(timer) 
         error('daq:daqread:unexpected','The ''Time'' and ''Samples'' properties are mutually exclusive.');
     elseif ~isempty(trigger) 
         error('daq:daqread:unexpected','The ''Triggers'' and ''Samples'' properties are mutually exclusive.');
      end
   elseif strncmpi(pv{i}, 'time', length(pv{i}))
      ...
      if ~(length(timer) == 2 && min(size(timer)) == 1 && timer(2) > timer(1))
         error('daq:daqread:unexpected','''Time'' must be specified as a two-element range.');
      elseif timer < 0
         error('daq:daqread:unexpected','''Time'' must be positive.');
      elseif ~isempty(samp) 
         error('daq:daqread:unexpected','The ''Samples'' and ''Time'' properties are mutually exclusive.');
      elseif ~isempty(trigger)
         error('daq:daqread:unexpected','The ''Triggers'' and ''Time'' properties are mutually exclusive.');
      end
   elseif strncmpi(pv{i}, 'triggers', length(pv{i}))
      trigger = pv{i+1};
      % Error if the triggers specified is negative.
      if min(trigger) < 1  
         error('daq:daqread:unexpected','Invalid ''Triggers'' index: %d.',min(trigger));
      elseif ~(~isempty(trigger == 1) || length(trigger) == 2)
         error('daq:daqread:unexpected','''Trigger'' must be specified as a single trigger or a two-element range.');
      elseif length(trigger) == 2
         if ~(min(size(trigger)) == 1 && trigger(2) > trigger(1))
            error('daq:daqread:unexpected','''Trigger'' must be specified as a single trigger or a two-element range.');
         end
      elseif any(round(trigger) ~= trigger)
         error('daq:daqread:unexpected','''Trigger'' must contain integer values.');
      elseif ~isempty(samp) 
         error('daq:daqread:unexpected','The ''Samples'' and ''Triggers'' properties are mutually exclusive.');
      elseif ~isempty(timer)
         error('daq:daqread:unexpected','The ''Time'' and ''Triggers'' properties are mutually exclusive.');
      end
   elseif strncmpi(pv{i}, 'channels', length(pv{i}))
      ...
      if iscell(chan)
         ...
         if length(index) ~= length(chan)
            error('daq:daqread:unexpected','Invalid Channel Name specified.');
         else
            chan = index;
         end
      end
      if min(chan) < 1  
         error('daq:daqread:unexpected','Invalid ''Channels'' index: %d.',min(chan));
      elseif max(chan) > num_chans
         error('daq:daqread:unexpected',...
             '''Channels'' index: %d, exceeds the number of available channels: %d.',...
             max(chan),num_chans);
      elseif any(round(chan) ~= chan)
         error('daq:daqread:unexpected','''Channels'' must contain integer values.');
      end
   elseif strncmpi(pv{i}, 'dataformat', length(pv{i}))
      ...
      if ~(strncmpi(dataf, 'double', length(dataf)) || ...
         ...
         error('daq:daqread:unexpected',...
             'Invalid ''DataFormat'': %s.  ''DataFormat'' can be either ''double'' or ''native''.',...
             dataf);
      end
   elseif strncmpi(pv{i}, 'timeformat', length(pv{i}))
      ...
      if ~(strncmpi(timef, 'vector', length(timef)) || ...
         ...
         error('daq:daqread:unexpected',...
            'Invalid ''TimeFormat'': %s.  ''TimeFormat'' can be either ''vector'' or ''matrix''.',...
            timef);
      end
   elseif strncmpi(pv{i}, 'outputformat', length(pv{i}))
      ...
      if ~(strncmpi(outputf, 'matrix', length(outputf)) || ...
         ...
         error('daq:daqread:unexpected',...
             'Invalid ''OutputFormat'': %s.  ''OutputFormat'' can be either ''matrix'' or ''tscollection''.',...
             outputf);
      end
   else
      % Error if an invalid property is specified.
      error('daq:daqread:unexpected',...
          'Invalid property, ''%s'' specified to DAQREAD.',pv{i});
   end
end
