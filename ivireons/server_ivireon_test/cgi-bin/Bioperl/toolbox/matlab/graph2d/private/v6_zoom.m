...
%   $Revision: 1.1.6.6 $  $Date: 2009/04/15 23:17:57 $
...
case 2
    ...
    if ischar(varargin{1}) && strcmp(varargin{1},'newzoom')
        ...
        if ~any(ishghandle(fig))
           error('MATLAB:v6_zoom:InvalidArgument', 'First argument must be a figure handle.')
        end
...
if (nargout ~= 0) && ...
    ...
    error('MATLAB:v6_zoom:LimitedOutputReturn', 'ZOOM only returns an output if the command is getmode, getlimits, or getconnect');
...
otherwise
    error('MATLAB:v6_zoom:UnknownOption', 'Unknown option: %s.',zoomCommand);
end
