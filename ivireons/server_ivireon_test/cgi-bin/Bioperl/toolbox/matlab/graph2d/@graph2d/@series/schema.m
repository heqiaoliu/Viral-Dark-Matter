...
findpackage('specgraph'); % make sure the ChartDirtyEnum datatype is defined
...
function LdoYDataAction(hSrc, eventData) %#ok
...
if strcmp(h.refreshmode,'auto') && h.initialized
  listeners = getappdata(0,'Graph2dSeriesListeners');
  state = get(listeners,'enable');
  set(listeners,'enable','off');
  refresh(h);
  set(listeners,{'enable'},state);
end
