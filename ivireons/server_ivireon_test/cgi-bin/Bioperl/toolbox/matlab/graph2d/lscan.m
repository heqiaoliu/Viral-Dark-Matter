...
%   $Revision: 5.10.4.4 $  $Date: 2006/10/10 02:21:47 $
...
for i=1:size(Kids),
    if strcmp(get(Kids(i),'type'),'line'),
        Xdata=[Xdata,get(Kids(i),'Xdata')]; %#ok<AGROW>
        Ydata=[Ydata,get(Kids(i),'Ydata')]; %#ok<AGROW>
    elseif strcmp(get(Kids(i),'type'),'text'),
        ...
        Xdata=[Xdata,[tmp(1) tmp(1)+ext(3)]]; %#ok<AGROW>
        Ydata=[Ydata,[tmp(2) tmp(2)+ext(4)]]; %#ok<AGROW>
    end
end
...
sum(sum((Xdata>=tmp2(1)).*(Xdata<=tmp2(2)).*(Ydata>=tmp2(3)).*(Ydata<=tmp2(4))));
    tmp=tmp+ [ -dx/2 dx/2 -dy/2 dy/2
               ...
               -dx   0     0    dy] ;                  
           for k=1:9,
               if (tmp(k,:) <= 1 ).*(tmp(k,:)>=0), %#ok<BDSCA,BDLOG>
