...
%   $Revision: 1.28.4.6 $  $Date: 2009/06/22 14:31:45 $
%   j. H. Roh
...
case 1
   ...
   if any(ishghandle(arg1)) || isa(arg1(1),'scribehandle')
...
function localData = showhelp(selection, localData) %#ok<INUSL,DEFNU>
try
...
function localData = verifynumber(uic, localData) %#ok<DEFNU>
...
fieldName = get(uic,'TooltipString');
errordlg(sprintf('%s field requires a single numeric input',fieldName),'Error','modal');





function localData = key(fig,localData) %#ok<DEFNU>
...
function localData = toggle(uic,localData) %#ok<DEFNU>
...
try
   ...
   for iCol = 2:nCols+1,
      ...
      utick = uicontrol(editProps,...
         'Tag', [col 'TickStep'],...
         'TooltipString', [col ' Tick step size'],...
         'HorizontalAlignment','center',...
      ...
      rlin = uicontrol(radioProps,...
         ...
         'String', 'Linear',...
         'Tag', [col 'ScaleLinear'],...
         'TooltipString',[col 'Scale=''linear''']);
...
try
   ...
   for col = cols(1:data.nCols)
      ...
      if setflag,
         iProp = iProp+1;
         axProps{iProp} = [col 'Scale']; %#ok<AGROW>
         modeswitch = {'log' 'linear'};
         axVals{iProp} = modeswitch{mode+1}; %#ok<AGROW>
      ...
      if setflag
         iProp = iProp+1;
         axProps{iProp} = [col 'LimMode']; %#ok<AGROW>
         modeswitch = {'auto' 'manual'};
         axVals{iProp} = modeswitch{manual+1}; %#ok<AGROW>
         if manual
            %limits   
            ...
            if ~(isnan(valMax) || isnan(valMin)),
               iProp = iProp+1;
               axProps{iProp} = [col 'Lim']; %#ok<AGROW>
               limits = [valMin valMax];
               axVals{iProp} = limits; %#ok<AGROW>
            end
         else % auto
      ...
      if setflag,
         iProp = iProp+1;
         axProps{iProp} = [col 'TickMode']; %#ok<AGROW>
         modeswitch = {'auto' 'manual'};
         axVals{iProp} = modeswitch{manual+1}; %#ok<AGROW>
            ...
            if manual
               ...
               if ~isempty(tickstep), % Make sure something is there
                  if ~isempty(limits),
                     ...
                     iProp = iProp+1;
                     axProps{iProp} = [col 'Tick']; %#ok<AGROW>
                     ticks = limits(1):tickstep:limits(2);
                     % ranges checked on callbacks
                     axVals{iProp} = ticks; %#ok<AGROW>
                  else
                     TickInd = iProp;
                  end
               end
      ...
      if setflag,
         iProp = iProp+1;
         axProps{iProp} = [col 'Dir']; %#ok<AGROW>
      ...
      if setflag,
         iProp = iProp+1;
         axProps{iProp} = [col 'Grid']; %#ok<AGROW>
         modeswitch = {'off' 'on'};
         axVals{iProp} = modeswitch{mode+1};   %#ok<AGROW>
      end % if setflag
      
      if ~isempty(TickInd) || isnan(valMax) || isnan(valMin)
         % Going to have to loop through to set limits/ticks individually
         for ct = 1:length(ax)
            limits = get(ax(ct),[col 'Lim']);
            axPropsCustom{1} = [col 'Lim']; %#ok<AGROW>
            if ~isnan(valMax), limits(2) = valMax; end
            if ~isnan(valMin), limits(1) = valMin; end
            axValsCustom{1} = limits; %#ok<AGROW>
            
            if ~isempty(TickInd);
               axPropsCustom{2} = [col 'Tick']; %#ok<AGROW>
               ticks = limits(1):tickstep:limits(2);
               % ranges checked on callbacks
               axValsCustom{2} = ticks; %#ok<AGROW>
