...
%   $Revision: 1.1.6.2 $  $Date: 2006/10/10 02:21:46 $ 
...
if nargin>1 
    ...
    if ~iscell(propFields) 
        ...
        if (length(h)>1 && length(propFields)>1)
            values = cell(size(allValues,2),1);
...
elseif length(h)==1 
    ...
    [uniqTypes,uniqTypeIndexA,uniqTypeIndexB] = unique(get(h,'Type')); %#ok<NASGU>
...
if length(h)==1; 
    ...
    values = cell(size(allValues,1),1);
...
if length(h)>1 
    [uniqTypes,uniqTypeIndexA,uniqTypeIndexB]=unique(uniqTypes);  %#ok<NASGU>
    h=h(1); 
elseif isempty(h) 
    optionFields={};
    optionValues={};
    ...
    uniqTypes={uniqTypes}; 
    uniqTypeIndexA=1;  %#ok<NASGU>
    uniqTypeIndexB=1;  %#ok<NASGU>
end 
...
if nargin>1 
    ...
    optionValues = cell(length(optionFields),1);
...
if isVertical 
    hList = cell(length(h),1);
    nList = cell(length(h),1);
    for i=length(h):-1:1 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
function ObjectDeletedListener(hHandle,eventData)  %#ok<INUSD>
...
while (h>0 & ...
        ...
        ~strcmp(get(h,'HandleVisibility'),'on') )) %#ok<AND2,AND2>
    h=get(h,'parent'); 
end 
...
while ~isempty(h) & ...
        ~any(strcmp({'root','figure'},get(h,'type'))) %#ok<AND2>
    h=get(h,'parent');
end
...
%still exists.
if ~all(ishandle(objHandle)) | isempty(findall(objHandle,'flat','BeingDeleted','off')) %#ok<OR2>
    result = 'no';
else
