...
%   $Revision: 5.13.4.4 $  $Date: 2008/05/05 21:38:12 $
...
if nargin == 0
    ...
    if ischar(arg1)
        ...
        if nargin == 2
            error(id('FirstArgAxes'),'First argument must be an axes handle.')
        end
        ax = gca;
        opt_grid = lower(arg1);
    else
        ...
        if length(arg1) > 1
            error(id('ScalarHandle'),'Axes handle must be a scalar.');
        end
