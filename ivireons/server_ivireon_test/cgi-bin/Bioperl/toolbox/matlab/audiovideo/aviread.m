...
%   AVIREAD will be removed in a future release. Use MMREADER
%   instead.
%
...
%   $Revision: 1.1.6.14 $  $Date: 2009/07/27 20:16:16 $

removeWarningID = 'MATLAB:aviread:FunctionToBeRemoved';
removeWarningMsg = 'AVIREAD will be removed in a future release. Use MMREADER instead.'; 
warning( removeWarningID, removeWarningMsg );
...
if nargin == 2
    if isnumeric(varargin{1})
        ...
        error('MATLAB:aviread:nonNumericIndex','INDEX must be numeric.');
    end
end

if ~ischar(filename)
    error('MATLAB:aviread:invalidFilename','The filename must be a string.');
end
...
if (index(1) ~= -10000)
    if ( isfield(info.MainHeader,'HasIndex') == 0 )
        error('MATLAB:aviread:indexParameterNotSupported','%s does not support the ''Index'' parameter.',filename);
    elseif any(index > info.MainHeader.TotalFrames)
        error('MATLAB:aviread:indexOutOfRange','Index value exceeds, %d, the total number of movie frames in this AVI movie.',info.MainHeader.TotalFrames);
    elseif any(index <= 0)
        error('MATLAB:aviread:invalidZeroIndex','Index value must be greater than zero.');
    end
end
...
if ispc
    try
        ...
        if strcmpi(exception.identifier,'matlab:aviread:invalidcodec') == 1 && ...
            ...
            error('MATLAB:aviread:indeo5NotFound',...
                'Unable to locate decompressor Indeo5 to decompress video stream.\nSee <a href="http://www.mathworks.com/support/solutions/data/1-4G50QZ.html">Mathworks Technical Solution 1-4G50QZ</a> for more info.');
        else
            throw(exception);
        end
    end
    ...
    elseif info.VideoFrameHeader.BitDepth == 24
        ...
        error('MATLAB:aviread:invalidColorBitDepth','The AVI file must be 8-bit Indexed or grayscale images, 16-bit grayscale, or 24-bit TrueColor images');
    end
...
if isunix
    ...
    if isempty(strmatch(lower(info.VideoFrameHeader.CompressionType),...
        ...
        error('MATLAB:aviread:compressedFileInvalid','Only uncompressed AVI movies can be read on UNIX.');
    end
    ...
    if fid == -1
        error('MATLAB:aviread:unableToOpenFile','Unable to open %s.', filename);
    end
    ...
    if ( strcmp(rifftype,'AVI ') == 0 )
        errorWithFileClose('MATLAB:aviread:MissingAVIChunk','Not a valid AVI file. Missing ''AVI '' chunk.', fid);
    end
    ...
    for  i = 1:info.MainHeader.NumStreams
        ...
        if(strcmp(streamType,'vids'))
            ...
            if ( fseek(fid,listsize - 16,0) == -1 )
                errorWithFileClose('MATLAB:aviread:invalidChunkSize','Incorrect chunk size information in AVI file.', fid);
            end
        end
    end

    if (found == 0)
        errorWithFileClose('MATLAB:aviread:noVideoStreamFound','Unable to locate video stream.', fid);
    end
    ...
    elseif  info.VideoFrameHeader.BitDepth == 8
        ...
        if  fseek(fid,info.VideoFrameHeader.BitmapHeaderSize,0) == -1
            errorWithFileClose('MATLAB:aviread:invalidBitmapInfoHeaderSize','Incorrect BITMAPINFOHEADER size information in AVI file.');
        end
        map = readColormap(fid,info.VideoFrameHeader.NumColorsUsed);
    else
        errorWithFileClose('MATLAB:aviread:invalidColorBitDepth','Bitmap data must be 8-bit Index images or 24-bit TrueColor images', fid);
    end
    ...
    switch method
        ...
        case 'UseIndex'
            ...
            if ( fseek(fid,movisize-4,0) == -1 )
                errorWithFileClose('MATLAB:aviread:InvalidChunkSize','Incorrect chunk size information in WAV file.', fid);
            end
            % Find idx1 chunk
...
if ( count ~= 1 )
    ...
    msgID = 'MATLAB:aviread:IncorrectIDX1ChunkSize';
end
