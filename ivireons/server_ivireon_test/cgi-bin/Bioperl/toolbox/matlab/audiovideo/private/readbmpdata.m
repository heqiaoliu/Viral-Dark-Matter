...
%   $Revision: 1.1.6.4 $  $Date: 2007/07/26 19:30:59 $
...
switch info.CompressionType
 ...
 case 'bitfields'
  error('MATLAB:readbmpdata:bitfieldUnsupported','Bitfield compression not supported');
  
 case 'Huffman 1D'
  error('MATLAB:readbmpdata:huffmanUnsupported','Huffman 1D compression not supported');
  
 case '24-bit RLE'
  error('MATLAB:readbmpdata:rleUnsupported','24-bit RLE compression not supported');
...
if status==-1
  fclose(fid);
  error('MATLAB:readbmpdata:invalidDataOffset','Error reading BMP data.  Invalid data offset.');
end
...
if (count ~= paddedWidth*abs(height))
    warning('MATLAB:readbmpdata:truncatedImageData','Invalid BMP file: truncated image data');
