...
%   AVIINFO will be removed in a future release. Use MMREADER
%   instead.
%
...
%   $Revision: 1.1.6.9 $  $Date: 2009/07/27 20:16:15 $

warning('MATLAB:aviinfo:FunctionToBeRemoved', ...
    'AVIINFO will be removed in a future release. Use MMREADER instead.'); 
...
if (~ischar(filename))
  error('MATLAB:aviinfo:invalidInputType','Input must be a string.');
end
...
if fid == -1
  error('MATLAB:aviinfo:unableToOpenFile','Unable to open %s for reading.', filename);
else
  filename = fopen(fid);
end
...
if ( strcmpi(rifftype,'AVI ') == 0 )
  error('MATLAB:aviinfo:invalidAVIFile','Invalid AVI file.');
end
...
for i = 1:fileinfo.MainHeader.NumStreams
  ...
  if(strcmpi(streamtype,'vids') && (found==0))
    ...
    elseif ( strhchunk.cksize == 48 )
      ...
      error('MATLAB:aviinfo:unknownHeaderSize', 'Unknown stream header size. Unable to read AVI file.');
    end
    ...
    % However, there was a bug in AVIFILE which generates files such that
    % MainHeader.TotalFrames was greater than VideoStreamHeader's frames.
    % So, to maintain backward compatibility with AVIFILE generated AVIs
    % only update the main headers total frames if it is less than the
    % video stream headers frame count.
  ...
  elseif ( strcmpi(streamtype,'auds') && (audiofound == 0))
    ...
    elseif ( strhchunk.cksize == 56 )
      ...
      msgID = 'MATLAB:aviinfo:unknownAudioStreamHeader';
    end
    ...
    if isempty(msg)
      ...
      if fseek(fid,strhchunk.cksize-4,0) == -1
        errorWithFileClose('MATLAB:aviinfo:invalidFileChunkSize','Invalid AVI file. Incorrect chunk size information.',fid);
      end
    end
  else
    % Seek to end of strl list minus the amount we read.
    if ( fseek(fid,strlsize - 16,0) == -1 )  
        error('MATLAB:aviinfo:invalidFileChunkSize','Invalid AVI file. Incorrect chunk size information.');
    end
  end
end
  
if (found == 0)
  errorWithFileClose('MATLAB:aviinfo:noVideoStreamFound','No video stream found.',fid);
end
