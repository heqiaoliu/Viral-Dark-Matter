...
%    $Revision: 1.1.6.4 $  $Date: 2008/04/21 16:25:15 $
        ...
        if strcmp(Struct(lcv).type,'()')
            if isa(val{lcv},'audiorecorder') % if audiorecorders, work with internalObj
                ...
                elseif ~isa(val{lcv+1},'audiorecorder')
                    error('MATLAB:audiorecorder:nocatenation',audiorecordererror('MATLAB:audiorecorder:nocatenation'));
                elseif length(val{lcv+1}.internalObj) == 1
                    if (Struct(lcv).subs{:} ~= 1)
