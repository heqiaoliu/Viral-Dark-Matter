classdef FileFormatInfo < hgsetget
    ...
    %
    %   See also MMREADER.GETFILEFORMATS
    %
    %   Authors: NH
    %   Copyright 2009 The MathWorks, Inc.
    %   $Revision: 1.1.6.4 $  $Date: 2009/11/07 21:01:05 $
    ...
    methods(Access='public')
        ...
        function obj = FileFormatInfo(Extension, Description, ContainsVideo, ContainsAudio)
            if (~ContainsVideo && ~ContainsAudio)
                error('MATLAB:audiovideo:invalidArguments',...
                      'Cannot construct a FileFormatInfo object that sets ContainsVideo and ContainsAudio to false.');
            end
        ...
        function filterSpec = getFilterSpec( obj, varargin )
            ...
            if (strcmpi(mediaType, 'video') || strcmpi(mediaType, 'all'))
                videoSpec = getFilterSpecWithLabel(getVideoFormats(obj), 'All Video Files');
            end
            
            audioSpec = {};
            if (strcmpi(mediaType, 'audio') || strcmpi(mediaType, 'all'))
                audioSpec = getFilterSpecWithLabel(getAudioFormats(obj), 'All Audio Files');
            end
            
            allTypesSpec = {};
            if ~isempty(videoSpec) && ~isempty(audioSpec)
                allTypesSpec = getFilterSpecWithLabel(obj, 'All Video & Audio Files');
            end
            ...
            switch lower(mediaType)
                ...
                case 'all'
                    formats = obj;
                otherwise
                    error('MATLAB:audiovideo:invalidMediaType',...
                          'invalid mediaType specified.\nValid values are ''video'', ''audio'', or ''all''.'); 
            end
        ...
        function display(obj)
            printFormatsWithLabel(getVideoFormats(obj), xlate('Video File Formats'));
            printFormatsWithLabel(getAudioFormats(obj), ...
                                  xlate('Audio File Formats'));
        end
    end
