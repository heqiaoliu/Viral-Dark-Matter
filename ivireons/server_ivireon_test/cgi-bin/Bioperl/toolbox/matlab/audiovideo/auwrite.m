...
%   $Revision: 1.1.6.6 $  $Date: 2007/12/17 06:23:47 $
...
if nargin==1,
  ...
  warning('MATLAB:auwire:syntaxobsolete',['This syntax for auwrite is obsolete and should ' ...
           'be replaced by sound(y).']);
  sound(y);
  return;
elseif nargin>5,
  error('MATLAB:auwrite:invalidInput','Incorrect number of input arguments.');
elseif nargin==4,
  aufile=method;
  method=method_pref;
elseif nargin==3,
...
if ~ischar(aufile),
  error('MATLAB:auwrite:invalidFileType', 'Filename must be a string.');
end
...
if ndims(y)>2,
  error('MATLAB:auwrite:invalidDataSize', 'Data array must have 1- or 2-dimensions, only.');
end
...
if ~isempty(i),
  y(i)=sign(y(i));
  warning('MATLAB:auwrite:clipData', 'Data clipped during write to file.');
end

% Open the file for writing
fid=fopen(aufile,'wb','b'); % Big-endian
if (fid==-1),
  error('MATLAB:auwrite:openFileError', 'Cannot open sound file for output. File may be in use.');
end
...
if write_sndata(fid,snd,y)
  errMsg.message = 'Error while writing sound file.';
  errMsg.identifier = 'MATLAB:auwrite:writeData';
  localError(errMsg, fid);
end
...
if strncmp(method,'mu',length(method)),
  if nbits~=8,
    errMsg.message = ['Mu-law can only be used with 8 bit data.' ...
         ' Use method=''linear'' instead.'];
     errMsg.identifier = 'MATLAB:auwrite:muLawData';
     return;
  end
...
elseif strncmp(method,'linear',length(method)),
  ...
  elseif nbits==16,
    ...
    errMsg.message = 'Unrecognized data format selected.';
    errMsg.identifier = 'MATLAB:auwrite:invalidDataFormat';
    return;
  end
else
  errMsg.message = 'Unrecognized data format selected.';
