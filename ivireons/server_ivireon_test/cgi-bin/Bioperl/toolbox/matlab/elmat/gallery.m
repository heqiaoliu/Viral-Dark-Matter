...
%   $Revision: 5.20.4.5 $  $Date: 2009/04/21 03:23:51 $
...
if isnumeric(matname)
   if matname == 3 || matname == 5
      ...
      error('MATLAB:gallery:invalidN','Invalid N in GALLERY(N).')
   end
end

if isempty(varargin)
    error('MATLAB:gallery:parametersRequired', 'Not enough input arguments.');
else
...
switch matname
   ...
   otherwise
       error('MATLAB:gallery:invalidMatName','Invalid matrix name.')
...
if ischar(varargin{end})
   ...
   error('MATLAB:gallery:invalidClassName','Invalid class name.')
...
if strcmp(classname,'single') && ...
   ...
   error('MATLAB:gallery:SparseSingle', ['Sparse arrays of class'...
         ' ''single'' are not supported'])
end
