...
%   $Revision: 1.11.4.3 $  $Date: 2009/03/30 23:39:57 $
...
elseif n == 5
   ...
   if nargout > 1
       error('MATLAB:wilk:TooManyOutputsForFive', ...
             'GALLERY(''WILK'',5) supports only one output.');
   end
   A = hilb(6,classname);
   A = A(1:5, 2:6)*1.8144;
...
elseif n == 21
   % Wilkinson [3], p. 308.
   if nargout > 1
       error('MATLAB:wilk:TooManyOutputsForTwentyOne', ...
             'GALLERY(''WILK'',21) supports only one output.');
   end
