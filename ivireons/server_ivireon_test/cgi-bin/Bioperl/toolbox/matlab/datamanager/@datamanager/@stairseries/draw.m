...
for row=length(selectionHandles)+1:size(I,1)
    ...
    selectionHandles(row) = specgraph.stairseries('Parent',get(h,'Parent'),...
...
elseif ~isempty(I)
    ...
    for row=1:size(I,1)
       ...
       set(selectionHandles(row),'xdata',brushxdata,'ydata',brushydata,...
            'LineStyle','-','Visible',hvis,'Color',brushColor(row,:),...
            'MarkerFaceColor',brushColor(row,:),'MarkerEdgeColor',brushColor(row,:));
       refresh(handle(selectionHandles(row)));
    end
end