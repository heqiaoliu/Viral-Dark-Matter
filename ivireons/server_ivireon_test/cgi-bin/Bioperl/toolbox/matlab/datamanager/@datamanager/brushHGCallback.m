...
if isempty(colorChooserComponent) || ~ishandle(colorChooserComponent)
    if ~ismac
        com.mathworks.page.datamgr.brushing.FigureColorChooser.addTouitogglesplittool(...
            java(handle(ancestor(es,'figure'))));
    else % Works around a repaint issue on the MAC
