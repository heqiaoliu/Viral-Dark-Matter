...
%   $Revision: 1.16.4.2 $  $Date: 2006/06/20 20:12:25 $
...
elseif nargin == 1
   if (isnumeric(c) && ~isscalar(c)) || (ischar(c) && size(c,1) ~= 1)
     error('MATLAB:calendar:NonScalarDate','DATE must be scalar or a single string.');
   end
   c = datevec(c);
   c(3) = 1;
else
   if ~isscalar(c) || ~isscalar(m) 
     error('MATLAB:calendar:NonScalarArgs','YEAR and MONTH must be scalars.');
   end
   if m < 1 || m > 12, error('MATLAB:calendar:InvalidMonth', 'MONTH must contain values between 1 and 12.'); end
   c = [fix(c) fix(m) 1];
end
