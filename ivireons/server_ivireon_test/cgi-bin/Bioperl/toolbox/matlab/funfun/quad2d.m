...
%   $Revision: 1.1.6.3 $  $Date: 2009/01/23 21:37:07 $
%
...
SINGULAR = optionStruct.Singular;
...
if RTOL < EPS100
    if USER_SUPPLIED_RELTOL
        ...
        warning('MATLAB:quad2d:increasedRelTol', ...
            'RelTol was increased to 100*eps(''%s'') = %g.',outcls,EPS100);
    end
    RTOL = EPS100;
end
if isa(Q,'double')
...
if ~(isfinite(Q) && isfinite(ERRBND))
    ...
    warning('MATLAB:quad2d:nonFiniteResult', ...
...
elseif MAXNFEWARN
    if ERRBND > max(ATOL,RTOL*abs(Q))
        ...
        warning('MATLAB:quad2d:maxFunEvalsFail', ...
        ...
        warning('MATLAB:quad2d:maxFunEvalsPass',...
            ['Reached the maximum number of function evaluations (%d). ', ...
            'The result passes the global error test.'],MaxFunEvals);
    end
    if optionStruct.FailurePlot
    ...
    function [Qsub,esub] = tensor(thetaL,thetaR,phiB,phiT,Qsub,esub)
        ...
        if FIRSTFUNEVAL
            ...
            if ~isequal(size(Z),size(X)) || ~isequal(size(Z1),size(VTSTIDX))
                ...
                error('MATLAB:quad2d:SizeMismatch', ...
            ...
            if any(any(abs(Z1-Z0) > max(ATOL,RTOL*max(abs(Z1),abs(Z0)))))
                ...
                warning('MATLAB:quad2d:FunVectorization', ...
...
%--------------------------------------------------------------------------
    ...
    function p = validateAbsTol(x)
        if ~(isfloat(x) && isscalar(x) && isreal(x) && x >= 0)
            ...
            error('MATLAB:quad2d:invalidAbsTol','Invalid AbsTol');
        end
        p = true;
    end
...
%--------------------------------------------------------------------------
    ...
    function p = validateRelTol(x)
        if ~(isfloat(x) && isscalar(x) && isreal(x) && x >= 0)
            ...
            error('MATLAB:quad2d:invalidRelTol','Invalid RelTol');
        end
        p = true;
...
%--------------------------------------------------------------------------
    ...
    function p = validateSingular(x)
        if ~(islogical(x) && isscalar(x))
            ...
            error('MATLAB:quad2d:invalidSingular', ...
...
%--------------------------------------------------------------------------
    ...
    function p = validateFailurePlot(x)
        if ~(islogical(x) && isscalar(x))
            ...
            error('MATLAB:quad2d:invalidFailurePlot', ...
                'FailurePlot option must be true or false.');
        end
        p = true;
    end
...
%--------------------------------------------------------------------------
    ...
    function p = validateMaxFunEvals(x)
        if ~(isfloat(x) && isscalar(x) && isreal(x) && x > 0 && x == floor(x))
            ...
            error('MATLAB:quad2d:invalidMaxFunEvals', ...
                'MaxFunEvals must be a positive integer scalar.');
        end
        p = true;
    end
