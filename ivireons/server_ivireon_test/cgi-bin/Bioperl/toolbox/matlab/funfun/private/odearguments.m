...
%   $Revision: 1.12.4.13 $  $Date: 2010/05/13 17:39:05 $
...
elseif m ~= neq
    error('MATLAB:odearguments:SizeIC', ...
          ['%s returns a vector of length %d, but the length of initial conditions vector is %d. ',...
           'The vector returned by %s and the initial conditions vector must have the same ',...
           'number of elements.'],funstring(ode),m,neq,funstring(ode));
end
