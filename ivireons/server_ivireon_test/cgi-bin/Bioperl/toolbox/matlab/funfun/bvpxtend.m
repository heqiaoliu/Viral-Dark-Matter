...
%   $Revision: 1.1.6.4 $  $Date: 2008/10/08 17:14:55 $

  if nargin < 2 
    error('MATLAB:bvpxtend:NotEnoughInputs',...
          'Not enough input arguments.  See BVPXTEND.');
  end    
  ...
  if (a < b)     
    if (xnew < a)
      anew = xnew;
    elseif (xnew <= b)        
      error('MATLAB:bvpxtend:XnewInsideInterval',...
            'XNEW inside the previous interval of integration.');
    else  
      bnew = xnew;
    end
  ...
  elseif ischar(ynew)  % extrapolation method specified
    ...
    if isscalar(idx) % unique match found
      ...
      error('MATLAB:bvpxtend:UnexpectedExtrapolationMethod',...
            'Unexpected extrapolation method specified: %s.',extrap);
    end
  
  elseif ~isnumeric(ynew)
    error('MATLAB:bvpxtend:YnewIncorrectType',...            
          'YNEW must be numeric vector or a string.');  
  end  
    ...
    switch extrap
      ...
      case 'solution'
        if ~isfield(sol,'solver') 
          error('MATLAB:bvpxtend:NoSolverInSol',...
                ['Solver information not present in the solution structure. ',...
                 'Cannot use extrapolation flag: ''%s''.'],extrap);      
        else    
          switch sol.solver 
            case 'bvp4c'
            ...
            otherwise
              error('MATLAB:bvpxtend:UnrecognizedSolverInSol',...
                    'Cannot extend solution created by solver: %s.',sol.solver);
          end        
        end  
    end    
