...
%   $Revision: 1.1.6.4 $  $Date: 2008/06/20 08:00:23 $
...
if nargin < 5
  options = [];
  if nargin < 4
    error('MATLAB:ddesd:NotEnoughInputs',...
          'Not enough input arguments. See DDESD.');    
  end
end
...
if tfinal <= t0
  error('MATLAB:ddesd:TspandEndLTtspan1',...
...
if ~isa(ddefun,'function_handle')
  error('MATLAB:ddesd:DdefunNotFunctionHandle',...
        'The first argument, DDEFUN, must be a function handle.');  
end

if ~(isnumeric(delays) || isa(delays,'function_handle'))
  error('MATLAB:ddesd:DelaysInvalidType',...
        ['The second argument, DELAYS, must be either ',...
         'a function handle or a constant vector.']);  
end
...
elseif isstruct(history)
  if history.x(end) ~= t0
    error('MATLAB:ddesd:NotContinueFromHistoryEnd',...
...
elseif isa(history,'function_handle')
  ...
  error('MATLAB:ddesd:HistoryInvalidType',...
          ['HISTORY must be a constant vector, ',...
           'DDESD solution structure or a function handle.']);
end 
y0 = temp(:);
...
[m,n] = size(f0);
if n > 1
  error('MATLAB:ddesd:DDEOutputNotCol',...
        'DDEs must return column vectors.');
elseif m ~= neq
  error('MATLAB:ddesd:DDELengthMismatchHistory',...
...
if ~( strcmp(classT0,dataType) && strcmp(classY0,dataType) && ...
  ...
  warning('MATLAB:ddesd:InconsistentDataType',...
          ['Mixture of single and double data for ''t0'', ''y0'', ''y(d(t0,y0))'', ',...
           'and ''f(t0,y0,ylags)'' in call to DDESD.']);
end    
...
if ~isempty(jumps)
  error('MATLAB:ddesd:JumpsOptionNotAvailable',...
        ['JUMPS option is not available in DDESD. ',...
         'Solve the problem by restarting at each entry of JUMPS.']); 
end

rtol = ddeget(options,'RelTol',1e-3,'fast');
if ~isscalar(rtol) || (rtol <= 0)
  error('MATLAB:ddesd:OptRelTolNotPosScalar',...
        'Option ''RelTol'' must be a positive scalar.');
end
if rtol < 100 * eps(dataType)
  rtol = 100 * eps(dataType);
  warning('MATLAB:ddesd:RelTolIncrease',...
          'RelTol has been increased to %g.',rtol)
end

atol = ddeget(options,'AbsTol',1e-6,'fast');
if any(atol <= 0)
  error('MATLAB:ddesd:OptAbsTolNotPos',...
        'Option ''AbsTol'' must be positive.');
end
...
if haveeventfun
  if ~isa(events,'function_handle')
    error('MATLAB:ddesd:OptEventsNotFunctionHandle',...
          'Option ''Events'' must be a function handle.');        
  end        
...
if isempty(outputFcn)
  ...
  if ~isa(outputFcn,'function_handle')
    error('MATLAB:ddesd:OptOutputFcnNotFunctionHandle',...
          'Option ''OutputFcn'' must be a function handle.');        
  end    
  haveOutputFcn = true;
...
while ~done
  ...
  while true
    ...
    if err > rtol   
      nfailed = nfailed + 1; 
      if h <= hmin
        warning('MATLAB:ddesd:IntegrationTolNotMet', ...
