...
%   $Revision: 1.8.4.6 $  $Date: 2007/10/15 22:53:34 $
...
if t(end) ~= tfinal
  ...
  if ~hasEvents || (te(end) ~= t(end))  % did not stop on a terminal event
    warning('MATLAB:pdepe:TimeIntegrationFailed',...
            ['Time integration has failed. Solution is available at ', ...
             'requested time points up to t=%e.'],t(end));
  end
end  
