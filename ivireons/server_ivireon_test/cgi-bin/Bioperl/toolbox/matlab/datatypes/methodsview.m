...
%   $Revision: 1.13.4.13 $  $Date: 2009/12/31 18:51:19 $
...
if nargin > 1
    switch(lower(option))
        case 'noui'
            if nargout ~= 2
                error('MATLAB:methodsview:InvalidNumberOfOutputs',...
        ...
        otherwise           
            error('MATLAB:methodsview:InvalidInput',...
                'Argument option must be assigned a valid string.');
    end
end
 
% one input
if nargin == 1 && nargout > 0
    error('MATLAB:methodsview:TooManyOutputs',...
...
if ~nouiFlag && size(m,1) == 0 
  error('MATLAB:methodsview:UnknownClassOrMethod', ...
      'No %s %s can be located or no methods for %s', typeString, qcls, typeString);
...
%fields from METHOD
hdrs = { sprintf('Qualifiers'),... 
    sprintf('Return Type'),... 
    sprintf('Name'),...
    sprintf('Arguments'),...
    sprintf('Other'),...
    sprintf('Inherited From')}; 
...
import com.mathworks.mwswing.* com.mathworks.mwswing.table.* com.mathworks.util.*

tableM = UneditableTableModel(ds,ch); % Use an uneditable default table model
...
if (PlatformInfo.getVersion > PlatformInfo.VERSION_15)
...
f.add(scrollPane);
f.setDefaultCloseOperation(MJFrame.DISPOSE_ON_CLOSE); %clean-up
...
if strncmp(qcls,'lib.',4)
    %is a library
    title = sprintf('Functions in library %s', strrep(qcls,'lib.',''));
else
    %is a class
    title = sprintf('Methods for class %s', qcls);
end
