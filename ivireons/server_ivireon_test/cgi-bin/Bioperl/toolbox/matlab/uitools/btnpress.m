...
%  $Revision: 1.31.4.5 $  $Date: 2009/12/28 04:20:17 $
...
elseif (nargin == 3)
    ...
    error('MATLAB:btnpress:InvalidNumberInputs', 'Invalid number of input arguments.');
end
...
if ischar(buttonNumber)
    for k=1:length(ud.uicontrolButtons)
        ...
        if k==length(ud.uicontrolButtons)
            error('MATLAB:btnPress:InvalidButtonId',sprintf('sorry, can''t find button with ID == ''%s''.', buttonNumber))
        end
    end
else
    k = buttonNumber;
