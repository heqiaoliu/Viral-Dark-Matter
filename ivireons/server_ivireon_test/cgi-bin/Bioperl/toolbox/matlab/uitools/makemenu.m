...
%  $Revision: 1.18.4.4 $  $Date: 2008/06/24 17:15:12 $

error(nargchk(3,4,nargin))

num_objects = size(labels,1);
if (num_objects ~= size(calls,1))
  error('MATLAB:makemenu:LabelsCallsSameRows', 'LABELS and CALLS must have the same number of rows.');
end
if (nargin == 4)
  if (num_objects ~= size(tags,1))
    error('MATLAB:makemenu:LabelsTagsSameRows', 'LABELS and TAGS must have the same number of rows.');
  end
end
...
for k = 1:num_objects
  ...
  loc = find(labelStr ~= '>');
  if (isempty(loc))
    error('MATLAB:makemenu:InvalidLabel', ['Label strings must have at least one character ', ...
                  'that''s not a ">"']);
  end
  new_level = loc(1) - 1;
  ...
  if (labelStr(1) == '-')
    ...
    if (isempty(labelStr))
      error('MATLAB:makemenu:EmptyField', 'Empty label field.');
    end
