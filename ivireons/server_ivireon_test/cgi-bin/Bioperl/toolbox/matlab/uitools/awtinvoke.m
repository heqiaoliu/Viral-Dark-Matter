...
%   $Revision: 1.1.6.17 $ $Date: 2008/02/07 16:32:30 $
...
%
if (nargin < 1)
    error('MATLAB:awtinvoke:InvalidFirstArgument', 'A java object handle must be specified as the first argument');
end

if (nargin < 2)
    error('MATLAB:awtinvoke:InvalidSecondArgument', 'A method signature must be specified as the second argument');
end
...
elseif isjava(obj)
    ...
    error('MATLAB:awtinvoke:IllegalInput',...
        ['First argument must be a Java object, ' ...
...
elseif any(methodname == '(')
    ...
    for k=1:length(meths)
        if meths(k).getName.equals(jmethodname)
            if ~isempty(meth)
                error('MATLAB:awtinvoke:AmbiguousMethod',...
                    'Ambiguous method name ''%s''.', methodname);
            end
            meth = meths(k);
        end
    end
    if isempty(meth)
        error('MATLAB:awtinvoke:UnknownMethod',...
