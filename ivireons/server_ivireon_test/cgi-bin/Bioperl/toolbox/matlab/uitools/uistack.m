...
%   $Revision: 1.8.4.11 $ $Date: 2009/12/11 20:36:48 $

error(nargchk(0,1,nargout));
error(nargchk(1,3,nargin));

if isempty(Handles) && ~all(ishghandle(Handles)),
    error('MATLAB:uistack:PassedInvalidHandles', 'Invalid Handles passed to UISTACK');
end
if nargin==1,
    StackOpt='up';
    Step=1;
...
if length(UParent)>1,
    error('MATLAB:uistack:ParentMustBeSame', 'All handles passed to UISTACK must have the same Parent.');
end
...
switch StackOpt,
    ...
    otherwise
        error('MATLAB:uistack:InvalidStackOption', 'Invalid Stack option for UISTACK');
