...
% $Revision: 1.1.8.11 $  $Date: 2010/05/20 02:31:00 $
% Copyright 2006-2010 The MathWorks, Inc.

% Initialize return values of filename, pathname and filterindex.
filename = '';
pathname = '';
filterindex = 0;
...
if ((uigetputtype ~= 0) && (uigetputtype ~= 1))
    error('MATLAB:uigetputfile_helper:WrongType', ...
        'Wrong type entered for uigetputfile.');
end
...
if (uigetputtype == 0)
    ufd = UiDialog.UiFileOpenDialog();
    ufd.MultiSelection = dialog_multiselect;
else
    ufd = UiDialog.UiFileSaveDialog();
end
    ...
    function [dialog_filter dialog_title dialog_filename dialog_pathname dialog_multiselect] = parseArguments()
        ...
        if (numArgs > 1)
            if (isnumeric(varargin{numArgs}) && isnumeric(varargin{numArgs - 1}))
                warning('MATLAB:uigetputfile_helper:xyobsolete', ...
                    'The use of ''x'' and ''y'' as location parameters has been obsoleted and will be ignored.');
                if (numArgs > 2)
        ...
        if (numArgs > 3)
            error('MATLAB:uigetdir_helper:BadSyntax',...
        ...
        if (numArgs > 0)
            ...
            if (~(ischar(dialog_filter)) && ~(iscell(dialog_filter)))
                error('MATLAB:uigetdir_helper:BadFilename', ...
                    'Filterspec must be a string or cell array.')
            end
        ...
        while (index < newNumArgs)
            ...
            if ((oldPropsFound(1) == 0) && (oldPropsFound(2) == 0)) 
                ...
                if (oldPropsFound(1) == 1)
                    warning('MATLAB:uigetputfile_helper:locationobsolete', ...
                        '''Location'' has been obsoleted and will be ignored.');
                else
                    if (uigetputtype == 0)
                        multiSelectValue = newVarArgIn{index+1};
                        if (~(strcmpi('on', multiSelectValue)) && ~(strcmpi('off', multiSelectValue)))
                            error('MATLAB:uigetputfile_helper:badmultiselect', ...
                                'Valid values of ''MultiSelect'' are ''on'' and ''off''.');
                        else
                        ...
                        error('MATLAB:uigetputfile_helper:multiselectgetonly', ...
                            '''MultiSelect'' may only be used when opening files.');
                    end
                end
            ...
            if (size(filter, 2) == 2)  % Descriptors
                for i=1:size(filter,1)
                    if (isequal(filter(i,2), {''}))
                        error('MATLAB:uigetdir_helper:EmptyDescriptors',...
                            'File filter descriptors may not be empty.')
                    end
                end
            end
        end
        ...
        if (~isempty(filename) && ~isequal(filename, 0) && ~isempty(ufd.FileFilter))
            ...
            if (isempty(strfind(fileExtension, ';')))  % can only do this for non-compound extensions
                ...
                if (~iscell(filename))  % string
                    ...
                    [path name ext] = fileparts(char(filename(1)));
                    if (isempty(ext))
