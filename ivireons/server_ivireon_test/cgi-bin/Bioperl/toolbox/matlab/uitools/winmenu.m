...
%  $Revision: 1.36.4.16 $  $Date: 2010/02/25 08:12:52 $
...
if(nargin == 1)
    if ischar(in)
        ...
        elseif strcmp(in,'morewindowscb')
            ...
            error('MATLAB:winmenu:UnrecognizedStringArg', 'unrecognized string argument: ''%s''',in);
        end
        return;
    ...
    elseif any(ishghandle(in))
        ...
        elseif(strcmp(t,'figure'))
            ...
            error('MATLAB:winmenu:InvalidHandleArg', 'handle argument must be figure or uimenu handle');
        end
    else
        error('MATLAB:winmenu:InvalidInputArg', 'input argument must be a figure or uimenu handle, or ''callback''');
    end
else
    h = findwinmenu(gcf);
end
...
if ~isscalar(h) || ~ishghandle(h,'uimenu') 
    error('MATLAB:winmenu:WindowMenuNotFound', 'unable to find window menu');
end
...
set(menus(end),'label','&More Windows...','separator','on',...
...
if ~strcmp(get(fig,'type'),'figure')
    error('MATLAB:winmenu:RequiresFigureHandle', 'Assert: findwinmenu requires a figure handle');
end
...
switch l_type
    ...
    case 'pushbutton'
        l = get(l,'UserData');
    otherwise
        error('MATLAB:winmenu:UnexpectedObjType', 'unexpected object type in morewindowscb: %s',l_type);
end
...
function showCommandWindow(src, evd, gcbf) %#ok<INUSL,INUSL>
