...
if hasuimode(hObj,name)
    error('MATLAB:uimode:ExistingMode',...
        'A mode by this name is already registered with the mode manager.');
end

hObj = handle(hObj);

if ~(ishghandle(hObj,'figure') || (ishandle(hObj) && isa(hObj,'uitools.uimode')));
    error('MATLAB:uimode:InvalidInput','First argument must be a handle to a figure or mode.');
end
if ~ischar(name)
    error('MATLAB:uimode:InvalidInput','Second argument must be a string');
end
