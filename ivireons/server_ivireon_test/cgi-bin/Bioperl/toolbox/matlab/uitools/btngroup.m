...
%  $Revision: 1.46.4.3 $  $Date: 2008/05/27 19:50:57 $
...
for k = (1+propertyOffset):2:nargin
  ...
  if (isempty(idx))
    error('MATLAB:btngroup:UnknownProperty', 'Unknown property "%s"', propertyString);
  elseif (length(idx) > 1)
...
if (isempty(groupId))
  error('MATLAB:btngroup:NoGroupIDMAtrix', 'No group ID matrix specified.');
end
if (isempty(buttonId))
  error('MATLAB:btngroup:NoBtnIDMatrix', 'No button ID matrix specified.');
end
