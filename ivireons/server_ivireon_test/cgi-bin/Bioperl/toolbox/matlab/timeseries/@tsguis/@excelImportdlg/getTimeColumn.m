...
if ~isempty(h.Handles.ActiveX)
    ...
    h.checkTimeFormat(eAvtiveSheet.Name,'column',tmpColStr{get(h.Handles.COMBtimeIndex,'Value')});
    % check time format
    if h.IOData.formatcell.columnIsAbsTime>=0
        ...
        for i=1:size(blocks,1)
            ...
            startStr = [tmpColStr{get(h.Handles.COMBtimeIndex,'Value')} num2str(blocks(i,1))];
            endStr = [tmpColStr{get(h.Handles.COMBtimeIndex,'Value')} num2str(blocks(i,2))];
...
%                     return
            ...
            try
                if ischar(tmptime)
                    if ~isempty(h.IOData.formatcell.columnFormat{:})
                        tmptime=datestr(datenum(tmptime,h.IOData.formatcell.columnFormat{:}),h.IOData.formatcell.columnFormat{:});
                    else
                        tmptime=datestr(datenum(tmptime));
                    end
                else
                    if ~isempty(h.IOData.formatcell.columnFormat{:})
                        tmptime=datestr(tmptime,h.IOData.formatcell.columnFormat{:});
                    else
            ...
            if ~isempty(tmptime)
                if isinf(h.IOData.formatcell.columnIsAbsTime)
                    customFormat=h.IOData.formatcell.columnFormat{:};
                end
