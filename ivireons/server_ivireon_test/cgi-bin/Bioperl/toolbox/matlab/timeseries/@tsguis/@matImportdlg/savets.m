...
%  $Revision: 1.1.6.10 $ $Date: 2010/04/21 21:33:53 $
...
%         errordlg('This is not a valid MATLAB variable name.','Time Series Tools','modal');
%         flag=false;
%         return
%     end
...
if ~iscell(time)
    ...
    % get abs time in [year mon day hour min sec] format and sort them
        ...
        errordlg(xlate('File contains time strings with an invalid format. Specify a different variable as the time vector or define it manually.'),...
            'Time Series Tools','modal');
        flag=false;
        delete(h.Handles.bar);
        return
