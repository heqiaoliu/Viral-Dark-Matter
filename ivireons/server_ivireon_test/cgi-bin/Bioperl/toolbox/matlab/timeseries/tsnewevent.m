...
f = figure('Name',xlate('Define New Event'),'WindowStyle','modal','Units',...
  'Characters','Position',[103.8 49.231 54 12.231],'NumberTitle','off',...
  'HandleVisibility','off','IntegerHandle','off');
LBLname = uicontrol('Style','Text','Units','Characters','Parent',f,'String',...
    xlate('Name'),'Position',[1.8 9.077 10.6 1.154],'HorizontalAlignment',...
    'Left');
EDITname = uicontrol('Style','Edit','Units','Characters','Parent',f,...
    'Position',[14.6 8.769 27 1.615],'HorizontalAlignment','Left',...
    'BackgroundColor',[1 1 1]);
LBLtime = uicontrol('Style','Text','Units','Characters','Parent',f,'String',...
     xlate('Time/Date'),'Position',[1.8 6.308 10.6 1.154],'HorizontalAlignment',...
    'Left');
...
if ~isempty(reftime) && ischar(reftime) % Absolute events
    if isempty(get(EDITtime,'String'))
        errordlg(xlate('Time cannot be empty.'),...
    ...
    try
        ...
        errordlg(xlate('Invalid date string format'),'Time Series Tools','modal')
        return
    end
else
    ...
    if isempty(evtime) || length(evtime)>1 || any(isnan(evtime)) || ...
            ~all(isfinite(evtime))
        errordlg(xlate('Time must evaluate to a non-empty finite scalar.'),...
            'Time Series Tools','modal')
        return
    end    
end
...
if isempty(deblank(evname))
    errordlg(xlate('Event name cannot be empty.'),'Time Series Tools','modal')
    return
end
