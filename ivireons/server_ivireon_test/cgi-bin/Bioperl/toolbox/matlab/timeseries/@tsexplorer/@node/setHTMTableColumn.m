...
if nargin>=4
    pathColumn = varargin{2};
else
    pathColumn = tsColumn+1;
end
...
set(handle(tsTable,'callbackproperties'),...
    'MouseReleasedCallback',{@localMouseClicked tsTable h tsColumn pathColumn})
...
function localMouseClicked(es,eventData,tsTable,h,tsColumn,pathColumn)
...
if isequal(col,tsColumn-1)
    ...
    pathspec = tableData{row+1,pathColumn};
    if isempty(pathspec)
        return
    end
