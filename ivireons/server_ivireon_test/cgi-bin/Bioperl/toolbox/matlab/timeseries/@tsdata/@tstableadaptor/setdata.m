...
if tsrownum>=0
    ...
    if col==0 % Time vector modification
        ...
        if strcmp(recorder.Recording,'on')
            ...
            if reSortFlag
               ...
               T.addbuffer(sprintf('%s.Data = %s.Data(I,:);',thisTs.Name,thisTs.Name));
               if ~isempty(thisTs.Quality)
                   T.addbuffer(sprintf('%s.Quality = %s.Quality(I);',thisTs.Name,thisTs.Name));
               end
               T.addbuffer(sprintf('%s.Time = time;',thisTs.Name),h.Timeseries);
            else
    ...
    elseif col<=size(thisTs.Data,2) % Data column
        ...
        if strcmp(recorder.Recording,'on')
            T.addbuffer(sprintf('%s.Data(%d,%d) = %s;',thisTs.Name,tsrownum,col,newValue),thisTs); 
        end
