...
if ~isa(e,'tsdata.event'),
   ...
   return
elseif no && ni>2,
   error('event:set:invOutArg',...
       'Output argument allowed only in SET(E) or SET(E,Property).');
end
...
elseif ni==2,
   ...
   ename = inputname(1);
   if isempty(ename),
      error('event:set:invVarArg',...
          'First argument to SET must be a named variable.')
   elseif rem(ni-1,2)~=0,
      error('event:set:invPropValPairs',...
