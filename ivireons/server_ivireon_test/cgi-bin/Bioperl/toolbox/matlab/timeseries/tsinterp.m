...
%   $Revision: 1.1.6.11 $  $Date: 2010/04/05 22:22:15 $
...
if (nargin<=4 || ~noduptimes) && length(T)>=2
    ...
    if any(Iduplicate)
        ...
        % time produce output which is the positionaly last value of X at
...
if isinteger(X) ||  isfloat(X) 
    out = zeros([length(t),s(2:end)],class(X));
elseif islogical(X)
...
for k=1:length(t)
    if Iref(k)>0
       out(k,:) = X(I(Iref(k)),:);
        ...
        try
            out(k,:) = repmat(cast(NaN,class(out)),1,s(2:end));
        catch me
...
if t(1)<T(1) || t(end)>T(end)+tol
    warning('interpolation:interpolation:noextrap','Cannot extrapolate')
end
...
switch lower(type)
   ...
   case 'nan'
       out = nanhold(t,T,X);
   otherwise
       error('interpolation:interpolation:wrongtype','Unrecognized interpolation method')     
end
