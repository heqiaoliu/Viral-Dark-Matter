...
%   of the timemetadata obkect H to the value VALUE.  An equivalent syntax 
%   is 
...
if ~isa(h,'tsdata.timemetadata'),
   ...
   return
elseif no && ni>2,
   error('abstracttimemetadata:set:invSyntax',...
       'Output argument allowed only in SET(H) or SET(H,Property).');
end
...
elseif ni==2,
   ...
   ename = inputname(1);
   if isempty(ename),
      error('abstracttimemetadata:set:badVar',...
          xlate('First argument to SET must be a named variable. To set properties of metadata stored within timeseries objects use the dot assignment notation.'))
   elseif rem(ni-1,2)~=0,
      error('abstracttimemetadata:set:invPropValPairs',...
