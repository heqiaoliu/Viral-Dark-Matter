...
%   of the qualmetadata obkect H to the value VALUE.  An equivalent syntax 
%   is 
...
if ~isa(h,'tsdata.qualmetadata'),
   ...
   return
elseif no && ni>2,
   error('qualmetadata:set:invOutArg',...
...
elseif ni==2,
   ...
   ename = inputname(1);
   if isempty(ename),
      error('qualmetadata:set:invVarArg',...
   ...
   elseif rem(ni-1,2)~=0,
      error('qualmetadata:set:invPropValPairs',...
