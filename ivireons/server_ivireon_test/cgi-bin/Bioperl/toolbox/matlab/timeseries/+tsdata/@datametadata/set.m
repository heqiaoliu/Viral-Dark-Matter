...
%   of the datametadata obkect H to the value VALUE.  An equivalent syntax 
%   is 
...
if ~isa(h,'tsdata.datametadata'),
   ...
   return
elseif no && ni>2,
   error('datamatadata:set:invOutArgs',...
...
elseif ni==2,
   ...
   ename = inputname(1);
   if isempty(ename),
      error('datamatadata:set:invVarArg',...
   ...
   elseif rem(ni-1,2)~=0,
      error('datamatadata:set:invPropValPairs',...
