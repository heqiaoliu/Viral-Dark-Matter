...
if nargin>2
    ...
    for i=1:2:ni
        ...
        Property = varargin{i};
        if i+1>ni
            error('tscollection:addsample:pv1',...
        ...
        switch lower(Property)
            case 'time'
                time = Value;
                if isempty(time)
                    error('tscollection:addsample:notime', 'You must specify time for each sample.')
                end
            case 'overwriteflag'
                overwriteFlag = Value;
            otherwise
                ...
                if isTsObject
                    if iscell(Value)
                        if length(Value)~=2
                            error('tscollection:addsample:cell',...
                            'Cell array must contain two cells, one for data and one for quality.')
                        end
                    ...
                    error('tscollection:addsample:pv2',...
    ...
    if ~isa(s,'struct') || ~isscalar(s)
        error('tscollection:addsample:pv4',...
            'New samples must be specified using a structure or property-value pairs.')
    end
    ...
    elseif isfield(s,'time')
        ...
        error('tscollection:addsample:notime', 'You must specify time for each new sample.')
    end
    if isempty(time)
        error('tscollection:addsample:notime', 'You must specify time for each sample.')
    end
...
elseif isnumeric(timein)
    ...
    if isvector(timein)
        ...
        error('timeseries:addsample:badtime','Time must be a vector or a scalar.');
    end
else
    error('timeseries:addsample:badformat','Invalid time format');
end
