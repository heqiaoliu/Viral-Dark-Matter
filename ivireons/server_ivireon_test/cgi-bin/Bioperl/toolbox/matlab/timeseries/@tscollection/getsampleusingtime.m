...
elseif nargin==4 
    if ischar(varargin{1}) && strcmpi('allowduplicatetimes',varargin{1}) && ...
        ...
        error('tscollection:getsampleusingtime:invalidpv',...
...
if isnumeric(StartTime) && isnumeric(EndTime) && ...
        (~isscalar(StartTime) || ~isscalar(EndTime) || ~isreal(StartTime) || ...
            ~isreal(EndTime))
        error('tscollection:getsampleusingtime:noScalars',...
            'All numeric times must be real scalars')
end
...
if isempty(this.TimeInfo.StartDate)
    ...
    if isnumeric(StartTime) && isnumeric(EndTime)
        ...
        error('tscollection:getsampleusingtime:noNumericStart',...
            'The start time must be a numeric value.')
    end
else
    ...
    if (ischar(StartTime) && ischar(EndTime)) || (isnumeric(StartTime) && isnumeric(EndTime))
        ...
        error('tscollection:getsampleusingtime:invStartFormat',...
...
if ~allowduplicatetimes && this.hasduplicatetimes && length(index)>=2
    error('tscollection:getsampleusingtime:invalidduptimes',...
