...
if nargin==1
    ...
    for i=2:length(varargin)+1
        if isa(varargin{i-1},'tscollection')
            ...
            error('tscollection:horzcat:badtype',...
...
if ~tsIsSameTime(ts1timevec,ts2timevec)
    error('tscollection:utdualhorzcat:badtime',...
        'The time vectors of the tscollections you are trying to concatenate must be the same.')
end
...
for k=1:length(commonts)
   ...
   if ~isequal(ts1_.Data,ts2_.Data)
      error('tscollection:utdualhorzcat:baddata',...
          'The common tscollection member, %s, must have the same data to concatenate the tscollections.',commonts{k});
   end
end
