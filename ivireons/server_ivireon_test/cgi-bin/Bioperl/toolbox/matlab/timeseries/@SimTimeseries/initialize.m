...
%   $Revision: 1.1.8.6 $ $Date: 2009/10/24 19:19:00 $
...
if ~isempty(framesize) %This time series is frame based
    ...
    if ~isempty(time)
        error('Timeseries:initialize:noirregtime',...
            'Explicit time vectors cannot be used for frame based time series')
    end
    ...
    if this.Length>1
        ...
        if ndims(data)>2
            if s(end)==this.Length 
                ...
                error('Timeseries:initialize:wrongdatasize',...
                    'The last dimension of the data should be the same as the length of the time vector.')
            end
        end
...
% Restore time/data comatibility chacking
...
if ~all(isnumeric(starttimes)) || ~all(isnumeric(endtimes)) || length(starttimes)<1 || ...
    ...
    error('Timeseries:Initialize:invinterval','Invalid interval specification')
end % valid arg chk
