...
if numel(ts)~=1
    error('timeseries:detrend:noarray',...
        'The detrend method can only be used for a single timeseries object');
end
...
if ~isnumeric(dataContent) && ~islogical(dataContent)
    error('timeseries:detrend:nonnumeric',...
...
if (length(s)>2 && ts.IsTimeFirst) || ...
   ...
   length(s)>3
    ...
    warning(t_state);
    error('timeseries:detrend:noarray',...
        'Detrend cannot be applied to time series with data of dimension higher than two.')
end
