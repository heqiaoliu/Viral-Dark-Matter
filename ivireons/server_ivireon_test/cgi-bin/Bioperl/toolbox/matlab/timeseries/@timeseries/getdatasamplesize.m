...
if numel(this)~=1
    error('timeseries:getdatasamplesize:noarray',...
        'The getdatasamplesize method can only be used for a single timeseries object');
end
...
if this.IsTimeFirst
    ...
    if length(SampleSize)==2
        SampleSize(1) = 1;
    % >2d case, remove the 1st dimenstion (time)
    else
