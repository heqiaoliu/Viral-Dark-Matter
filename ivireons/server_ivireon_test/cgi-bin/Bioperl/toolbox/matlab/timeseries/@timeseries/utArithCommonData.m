...
if isa(ts2,'timeseries') || ~isscalar(ts2)
    switch operator
        case {'+' '-' '.*' './' '.\'}
            if ~isequal(s1,s2) && ~isequal(s2,[1 1])
                error('timeseries:arith:nonconfdim', ...
                    'Sample sizes of %s and %s are mismatched for matrix combination.',...
                    num2str(s1), num2str(s2))
            end
        case '*'
            if length(s1)>2 || length(s2)>2
                error('timeseries:mtimes:non3dimmax', ...
                    'Cannot multiply time series with data that has a dimension higher than three.')
            end
            if IsArrayFirst
                if s1(1)~=s2(2)
                        error('timeseries:mtimes:nonconfdim', ...
                            'Sample sizes of %s and %s are mismatched for matrix multiply.', num2str(s1), num2str(s2))
                end
            else
                if s1(2)~=s2(1)
                        error('timeseries:mtimes:nonconfdim', ...
        ...
        case '/'
            if length(s1)>2 || length(s2)>2
                error('timeseries:mrdivide:non3dimmax', ...
                    'Cannot multiply time series with data that has a dimension higher than three.')
            end
            if s1(2)~=s2(2)
                error('timeseries:mtimes:nonconfdim', ...
                    'Sample sizes of %s and %s are mismatched for matrix division.', num2str(s1), num2str(s2))
            end
        case '\'
            if length(s1)>2 || length(s2)>2
                error('timeseries:mldivide:non3dimmax', ...
                    'Cannot multiply time series with data that has a dimension higher than three.')
            end
            if s1(1)~=s2(1)
                error('timeseries:mtimes:nonconfdim', ...
                    'Sample sizes of %s and %s are mismatched for matrix left division.', num2str(s1), num2str(s2))
            end
    end
end
