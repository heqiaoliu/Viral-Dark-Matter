...
if ischar(event)
    ...
    if isempty(this.Events)
        error('timeseries:utGetEventTime:noEvents',...
...
elseif ~isa(event,'tsdata.event')
    error('timeseries:utGetEventTime:noTime',...
        'The second argument of utGetEventTime must be a valid event name.');
end
...
switch option
    ...
    case 'notAt'
        index = find(this.Time ~= eTime);
    otherwise
        error('timeseries:utGetEventTime:invOption',...
            'Invalid option attribute');
end