...
if isa(ts1,'timeseries')
    if numel(ts1)~=1
        error('timeseries:minus:noarray',...
         'The minus method can only be used for a single timeseries object');
    end
    ...
    try
        ...
        elseif ~ts1.IsTimeFirst
            ...
            if ts1_.DataInfo.isstorage
                try
                    tsout.DataInfo = minus(ts1_.DataInfo,ts2.DataInfo);
                catch
                    [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultminus(...
        ...
        elseif ~ts2.IsTimeFirst
            ...
            if ts1.DataInfo.isstorage
                try
                   tsout.DataInfo = minus(ts1.DataInfo,ts2_.DataInfo);
                catch
                    [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultminus(...
