...
if isa(ts1,'timeseries')
    if numel(ts1)~=1
        error('timeseries:rdivide:noarray',...
         'The rdivide method can only be used for a single timeseries object');
    end
    ...
    try
        ...
        elseif ~ts1.IsTimeFirst
            ...
            if ts1_.DataInfo.isstorage
                try
                   tsout.DataInfo = rdivide(ts1_.DataInfo,ts2.DataInfo);
                catch
                   [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1_.DataInfo,ts2.DataInfo,ts1_.Data,ts2.Data);
                end
            else           
                [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1_.DataInfo,ts2.DataInfo,ts1_.Data,ts2.Data);
            end
        ...
        elseif ~ts2.IsTimeFirst
            ...
            if ts1.DataInfo.isstorage
                try
                    tsout.DataInfo = rdivide(ts1.DataInfo,ts2_.DataInfo);
                catch
                    [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1.DataInfo,ts2_.DataInfo,ts1.Data,ts2_.Data);
                end
            else
                [tsout.DataInfo,tsout.Data] = tsdata.datametadata.defaultrdivide(ts1.DataInfo,ts2_.DataInfo,ts1.Data,ts2_.Data);
            end
