...
if numel(ts)~=1
    error('timeseries:idealfilter:noarray',...
        'The idealfilter method can only be used for a single timeseries object');
end
if ts.Length==0
    return
end
try
    ...
    if ~isnumeric(dataContent) && ~islogical(dataContent)
        error('timeseries:idealfilter:nonnumeric',...
    ...
    if (length(s)>2 && ts.IsTimeFirst) || ...
       ...
       length(s)>3
        error('timeseries:idealfilter:noarray',...
            'idealfilter cannot be applied to time series with data of dimension higher than two.')
    end
