...
if ~isa(ts2,'timeseries')
    error('timeseries:synchronize:type','Time series synchronization cannot be performed on a pair of %s object and timeseries object.',...
        class(ts1))    
end        
if numel(ts1)~=1 || numel(ts2)~=1
    error('timeseries:synchronize:noarray',...
     'The synchronize method can only be used for a single timeseries objects');
end        
...
if nargin>3
    ...
    for i=1:2:ni
        ...
        Property = varargin{i};
        if i+1>ni
            error('timeseries:synchronize:pvset',...
        ...
        switch lower(Property)
            ...
            case 'keeporiginaltimes'
                if ~islogical(Value) || ~isscalar(Value)
                    error('timeseries:synchronize:pvset',...
                        'KeepOriginalTimes must be a logical value.')
                else
...
if isempty(t1) || isempty(t2)
    error('timeseries:synchronize:noemptytime',...
...
% Calculate threshold based on the specified tolarance. The actual
% tolerance is the minimum of 10^-6*round10(minumim deltaT) and the
...
% equal
if ~isequal(ts1timevec_original,ts2timevec_original)
...
switch lower(method)
    ...
    otherwise
          error('timeseries:synchronize:interpsyntax', ...
              'Invalid synchronizemethod attribute')
end
...
elseif isa(method,'tsdata.interpolation')
    ...
    error('timeseries:synchronize:invinterp',...
...
if isempty(ts1.Quality) || isempty(ts2.Quality)
    error('timeseries:synchronize:noqual',...
        'Quality vector must be initialized.')
end
if isnumeric(Value) && isscalar(Value)
    if floor(Value)-Value<0
        warning('timeseries:synchronize:round',...
            'The specified quality code has been rounded to an integer.')
    end
    modcode = floor(Value);
else
    error('timeseries:synchronize:invcode',...
