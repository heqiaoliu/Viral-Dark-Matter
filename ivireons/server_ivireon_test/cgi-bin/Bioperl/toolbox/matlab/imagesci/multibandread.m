...
%   $Revision: 1.1.6.14 $  $Date: 2008/08/20 22:57:39 $
...
if fileSize < info.offset + info.dataSize
    error('MATLAB:multibandread:badFileSize', ...
...
if useMemMappedFile(info.inputClass)
    try
        ...
        if ~strcmp(myException.identifier, 'MATLAB:memmapfile:mapfile:cannotMap')
            rethrow(myException);
        end
...
if numel(varargin) > 0
    for i=1:length(varargin)
        ...
        if length(varargin{i})==2
            ...
            if ~isempty(match)
                ...
                error('MATLAB:multibandread:badSubset', ...
                    'Unrecognized subset string ''%s'' for METHOD.', ...
                    varargin{i}{2});
            end
            n = 3;
        end
        ...
        if ~isempty(match)
            ...
            error('MATLAB:multibandread:badSubset', ...
                'Unrecognized subset string ''%s'' for DIM.',varargin{i}{1});
        end
