...
%   $Revision: 1.1.6.7 $  $Date: 2009/09/03 05:19:20 $
...
if (~isstruct(format_values))
    ...
    error('MATLAB:imformats:attributesNotStructAdd', ...
...
if (~isfield(format_values, 'ext'))
    ...
    if (ischar(format_values.ext))
        ...
        if (extAlreadyExists(format_values.ext))
            ...
            error('MATLAB:imformats:duplicateExtension', ...
                  'The extension ''%s'' is already in the registry.', ...
                   format_values.ext)
...
elseif (~isstruct(new_values))
    error('MATLAB:imformats:attributesNotStructUpdate', ...
          'Format attributes must be stored in a structure.')
end
