...
%   $Revision: 1.1.6.11 $  $Date: 2010/05/13 17:39:18 $
...
if isempty(hinfo)
  error('MATLAB:hdfread:noDataSets', ...
...
for i=1:numPairs
  switch params{i}
   ...
   case 'numrecords'
    if iscell(values{i})   
      if length(values{i})>1
        error('MATLAB:hdfread:tooManyInputsToNumrecords', ...
              cellmsg,params{i}, 1);
      end
      numrecords = values{i}{:};
    else
   ...
   case 'firstrecord'
    if iscell(values{i})
      if length(values{i})>1
        error('MATLAB:hdfread:tooManyInputsToFirstrecord', ...
              cellmsg, params{i}, 1);
      end
   ...
   case 'level'
    if iscell(values{i})
      if length(values{i})>1
        error('MATLAB:hdfread:tooManyInputsToLevel', ...
              cellmsg, params{i}, 1);
      end
      level = values{i}{:};
    else
...
if nargin<1
    error('MATLAB:hdfread:invalidInput', msg)
end
...
if ischar(varargin{1}) %HDFREAD(FILENAME,DATASETNAME...)
  ...
  elseif isnumeric(varargin{2}) %Obsolete syntax
    ...
    error('MATLAB:hdfread:invalidDatasetName', ...
          '%s', msg); %Invalid input
  end
elseif isstruct(varargin{1}) %HDFREAD(HINFO,...)
  hinfo = varargin{1};
  if length(hinfo)>1
    error('MATLAB:hdfread:tooManyHINFO', ...
  ...
  error('MATLAB:hdfread:invalidFilename', '%s', msg);
end
return;
