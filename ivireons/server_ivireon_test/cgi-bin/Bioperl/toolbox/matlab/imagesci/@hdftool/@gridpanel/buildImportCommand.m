...
%   $Revision: 1.1.6.5 $  $Date: 2010/01/19 02:55:38 $
    ...
    baseCmd = [varName ' = hdfread(''' fileName ''', ''' gridName ''', ''Fields'', ''%s''%s);'];
    ...
    %=======================================================
    function outCmd = buildNoSubsettingCmd(baseCmd, h)
    ...
    function outCmd = buildTileCmd(baseCmd, h)
        ...
        tileCoords = str2num(tileCoords);
        if bImport && isempty(tileCoords)
            errordlg('Tile values must be numbers.','Invalid subset selection parameter');
            return
        end
