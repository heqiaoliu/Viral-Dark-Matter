...
%   $Revision: 1.1.6.7 $  $Date: 2009/07/06 20:38:08 $
    ...
    dimNames = cell(1,len);
    for n = 1:len
        dimNames{n} = sprintf('DIM:%s',istruct.Dims(n).Name);
    end
    ...
    maxStringLength = size(strvcat(dimNames),2);
    for m = 1:len
        text = uipanel('parent', hgrid);
    ...
    function setInfoStruct(istruct)
        ...
        dimNames = cell(1,len);
        for n = 1:newLen
            dimNames{n} = ['DIM:',istruct.Dims(n).Name];
        end
        % Increase the length by one if there is a vertical subset.
        if isfield(istruct,'vertical') && ~isempty(istruct.vertical)
            len = len+1;
