...
%   $Revision: 1.1.6.5 $  $Date: 2010/01/19 02:55:47 $
...
firstRec  = str2num(this.firstrecordApi.getSelectedString());
numRec    = str2num(this.numrecordsApi.getSelectedString());
...
if bImport
    if (isempty(firstRec) || firstRec < 1 || ...
        ...
        errorStr = 'First Record must be a number between 1 and %d';
    ...
    elseif (isempty(numRec) || numRec > availableRec || ...
        ...
        errorStr = 'Number of Records must be between 1 and %d';
        errorStr = sprintf(errorStr,availableRec);
        errordlg(errorStr,'Invalid subset selection parameter');
        return
