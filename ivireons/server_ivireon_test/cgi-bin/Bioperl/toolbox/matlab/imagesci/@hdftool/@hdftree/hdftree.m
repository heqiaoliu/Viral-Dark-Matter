...
%   $Revision: 1.1.6.9 $  $Date: 2009/07/06 20:38:14 $
...
% Add a file by populating the HDF tree with nodes.
    ...
    if (hasAnon)
        msg = sprintf (['%s contains Vdata datasets with zero-length ' ...
                        'names.  For display purposes, HDFTOOL is ' ...
                        'computing Vdata names from the reference ' ...
                        'numbers.'], filename );
        warndlg ( msg, 'Anonymous VData' );
    end
...
function [vdataNode, hasAnon] = createVdataNode(this, nodePath, vdataStruct)
    ...
    for n = 1:len
        ...
        if (numel(vdataStruct(n).Name) == 0)
            % Unnamed Vdata.
            hasAnon = true;
            anon_name = sprintf ( 'Anon Vdata Ref # %d', vdataStruct(n).Ref );
    ...
    for n = 1:len
        ...
        for k=1:numberOfFields
            ...
            dataFieldNode{n}(k) = hdftool.createTreeNode(gridStruct(n).Name,...
                ...
                this,...
                gridStruct(n).DataFields(k).Name,nodeIconSub,true);
        end
