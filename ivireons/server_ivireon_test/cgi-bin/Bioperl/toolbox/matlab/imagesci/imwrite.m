...
%   $Revision: 1.1.6.19 $  $Date: 2010/03/22 03:52:33 $
...
elseif (issparse(data))
    error('MATLAB:imwrite:sparseData', 'Cannot write sparse data.');
end
...
if (length(varargin) > firstString)
    ...
    if (rem(length(paramPairs), 2) ~= 0)
        msg = sprintf('Invalid input syntax or unrecognized format "%s"', ...
                      varargin{firstString + 1});
        return;
    end
...
otherwise
    ...
    error('MATLAB:imwrite:unsupportedDatatype', ...
          'Unsupported image datatype ''%s''.', class(data))
