...
%   $Revision: 1.1.6.6 $  $Date: 2008/06/16 16:50:17 $
    ...
    elseif (length(idx) > 1)
        ...
        case 'imagemodtime'
            try
                ...
                error('MATLAB:writepng:invalidImageModTime', ...
                      ['ImageModTime could not be converted to a date vector.  ' ...  
		      'See "help datevec" for allowable syntaxes.']);
            end
            
            if (numel(imagemodtime) > 6)
                error('MATLAB:writepng:tooMuchImageModTimeData', ...
                      'ImageModTime did not result in a six element date vector.')
            end
