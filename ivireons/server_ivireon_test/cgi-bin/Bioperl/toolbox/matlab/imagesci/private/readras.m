...
%  $Revision: 1.1.6.4 $  $Date: 2009/04/15 23:19:39 $
   ...
   if maplength
      ...
      if count < maplength
         ...
         error('MATLAB:readras:unexpectedEOF', ...
               '%s: file ended while reading color map.',filename);
      end
      ...
      switch info.MapType
         case RMT_NONE
            warning('MATLAB:readras:badMapLength', ...
                    '%s: maplength should be 0 when maptype is 0.',filename);
         case RMT_EQUAL_RGB
         ...
         otherwise
            error('MATLAB:readras:badMapType', ...
                  '%s: invalid RAS maptype.',filename);
      end
   ...
   if fsize > headersize + maplength + datalength
      warning('MATLAB:readras:tooMuchData', ...
              '%s: too much image data. Possibly corrupt image.',filename);
   end
   ...
   switch info.Type
      ...
      case RT_FORMAT_TIFF
         error('MATLAB:readras:tiffRasNotSupported', ...
               '%s: tiff RAS type is not supported.',filename);
      case RT_FORMAT_IFF
         error('MATLAB:readras:iffRasNotSupported', ...
               '%s: iff RAS type is not supported.',filename);
      case RT_EXPERIMENTAL
         error('MATLAB:readras:experimentalRasNotSupported', ...
               '%s: experimental RAS type is not supported.',filename);
      otherwise
         error('MATLAB:readras:invalidRasType', ...
               '%s: invalid RAS type.',filename);
   end
