...
%   $Revision: 1.1.8.7 $  $Date: 2010/04/05 22:22:11 $
...
case 0
    y = [y 'no strobe return detection function, '];
case 1
    y = [y 'reserved,' ];
case 2
    y = [y 'strobe return light not detected, ' ];
case 3
    y = [y 'strobe return light detected, ' ];
end
...
case 0
    y = [y 'unknown flash mode, '];
case 1
    y = [y 'compulsory flash firing, '];
case 2
    y = [y 'compulsory flash suppression, '];
case 3
    y = [y 'auto flash mode, '];
end
...
if bitshift ( bitand(x,32), -4 )
    y = [y 'no flash function, ' ];
else
    y = [y 'flash function present, '];
end
...
case 0
    y = [y 'no red-eye reduction mode or unknown.' ];
case 1
    y = [y 'red-eye reduction mode supported.' ];
end
...
if (any(x<0) || any(x>6))
    warning ( 'MATLAB:tifftagsprocess:invalidComponentsConfiguration', ...
            'ComponentsConfiguration tag value is invalid.  Tag value returned as-is.' );
	y = x;
	return;
end
...
switch ( x )
    ...
    otherwise
        warning ( 'MATLAB:tifftagsprocess:invalidExposureProgramValue', ...
            'ExposureProgram tag value is invalid.  Tag value returned as-is.' );
        y = x;
end
