...
%   $Revision: 1.1.6.6.4.1 $  $Date: 2010/07/06 14:40:34 $
...
if gridID==-1
    ...
    warning('MATLAB:hdfgridinfo:openInterface', ...
            'Unable to open Grid interface for ''%s'' grid in %s%s. File may be corrupt.', ...
            gridname,name,ext)
    return
...
[nfields, fieldListLong, ~, numberType] = hdfgd('inqfields',gridID);
hdfwarn(nfields)
if nfields>0
...
if status == -1
    [~,name,ext] = fileparts(filename);
	warning('MATLAB:hdfgridinfo:couldNotRetrieveProjection', ...
	        'Library error:  unable to retrieve projection information for %s grid in %s%s.', ...
			gridname, name, ext);
end
