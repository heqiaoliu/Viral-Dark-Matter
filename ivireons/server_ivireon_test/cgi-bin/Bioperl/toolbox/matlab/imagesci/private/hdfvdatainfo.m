...
%   $Revision: 1.1.6.8 $  $Date: 2009/07/06 20:38:17 $
...
elseif ischar(dataset)
  ...
  if ref == 0
    warning('MATLAB:hdfvdatainfo:find', msg)
    return;
  end
else
...
if vdID == -1
  warning('MATLAB:hdfvdatainfo:attach',  msg);
  return;
end
...
attrcount = 0;
[records, interlace, fieldListLong, size, vdata_name, status] = hdfvs('inquire',vdID);
if (status == -1)
    [~,name,ext] = fileparts(filename);
    warning ( 'MATLAB:hdfvdatainfo:inquireFailure', ...
              'Failed to acquire information about vdata dataset with reference number %d in %s%s.', ...
              ref, name, ext );
    name = '';
    records = '';
    fields = '';
  ...
  for i = 1:length(fieldnames)  
    ...
    for j = 1:fieldAttrCount
      [name, dataType, count, size, status] = hdfvs('attrinfo', vdID, i-1, j-1);
      hdfwarn(status)
...
if (vdataAttrCount > 0)
  ...
  for i = 1:(nattrs-attrcount)
    [name, dataType, count, size, status] = hdfvs('attrinfo', vdID, 'vdata', i-1);  
    hdfwarn(status) 
