...
%   $Revision: 1.1.6.10 $  $Date: 2008/05/14 22:01:55 $
...
case { 'hdf5.h5compound', 'hdf5.h5array', 'hdf5.h5enum', 'hdf5.h5string', ...
	...
	return
otherwise
    error('MATLAB:hdf5write:unknownDataClass', ...
          '''%s'' is not a valid datatype for HDF5WRITE.\n', class(data) );
end
...
for p = 1:numel(fields)
    ...
    for q = 1:numel(data)
		...
		if iscell(data(q).(fields{p}))
            ...
            if ~all(membersChar);
                error('matlab:hdf5write:cellInCompound', ...
                      'Cell arrays in structures must contain character data')
            end
		end
        ...
        if (~isequal(class(data(q).(fields{p})), class1))
            ...
            error('matlab:hdf5write:compoundInconsistentType', ...
        ...
        if ((~ischar(data(q).(fields{p}))) && ...
            ...
            error('matlab:hdf5write:compoundInconsistentShape', ...
                  'Compound members must have the same shape and type.')
...
for p = 1:numel(data)
    ...
    if (~isequal(class(data{p}), class1))
        error('matlab:hdf5write:arrayInconsistentType', ...
    ...
    if (~ischar(data{p})) && (~isequal(size(data{p}), size1))
        error('matlab:hdf5write:arrayInconsistentShape', ...
              'HDF5 array elements must have the same shape and type.')
    end
