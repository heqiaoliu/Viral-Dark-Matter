...
if ~isvector(xv) || ~isvector(yv)
    error('MATLAB:inpolygon:PolygonVecDef', 'Polygon must be defined by vectors (XV, YV).');
end
...
if ~any(xnan | ynan)
    ...
    if xnan ~= ynan
        error('MATLAB:inpolygon:InvalidLoopDef','NaN separated edge loop definitions are not consistent in x and y.');
    end
