...
%   $Revision: 1.16.4.7 $ $Date: 2008/12/01 07:18:42 $ 
...
if ndims(x) > 2
    error('MATLAB:voronoin:HigherDimArray',...
...
% of the points.
[B, I, J] = unique(x,'first','rows');
post_merge_size = length(I);
if (pre_merge_size > post_merge_size)
    sorted_I = sort(I)';
    x = x(sorted_I,:);
    %
    warning('MATLAB:voronoin:DuplicateDataPoints',['Duplicate data points have been detected.\n',...
                                                    'This may lead to inconsistencies in the point to Voronoi region relationship.\n',...
                                                    'To avoid this behavior, call UNIQUE on the data points prior to calling VORONOIN.']);
    %
    [m,n] = size(x);
end
