...
%   $Revision: 1.27.4.4 $  $Date: 2008/04/11 15:37:54 $
...
if ~any(size(p)<2),
    ...
    msg.identifier = 'MATLAB:zp2ss:pNotVector';
    return
end
% Columnize p
p = p(:);
...
if ~any(size(k)<2),
    ...
    msg.identifier = 'MATLAB:zp2ss:kNotVector';
    return
end
% Columnize k
k = k(:);
...
if size(z,1) > length(p),
    ...
    msg.identifier = 'MATLAB:zp2ss:improperSystem';
    return
end
...
if length(k) ~= size(z,2) && (~isempty(z))
    ...
    msg.identifier = 'MATLAB:zp2ss:zkLengthMismatch';
end
