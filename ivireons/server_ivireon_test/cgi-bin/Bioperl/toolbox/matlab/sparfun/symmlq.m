...
%   QMR, TFQMR, CHOLINC, FUNCTION_HANDLE.

%   Copyright 1984-2010 The MathWorks, Inc.
%   $Revision: 1.6.4.7 $ $Date: 2010/02/25 08:12:09 $
...
if strcmp(atype,'matrix')
    ...
    if ~isequal(size(b),[m,1])
        error('MATLAB:symmlq:RSHsizeMatchCoeffMatrix', ...
    ...
    n = m;
    if ~iscolumn(b)
        error('MATLAB:symmlq:RSHnotColumn', ...
...
if ((nargin >= 5) && ~isempty(M1))
    ...
    if strcmp(m1type,'matrix')
        if ~isequal(size(M1),[m,m])
            error('MATLAB:symmlq:WrongPrecondSize', ...
...
if ((nargin >= 7) && ~isempty(x0))
    if ~isequal(size(x0),[n,1])
        error('MATLAB:symmlq:WrongInitGuessSize', ...
            ...
            if moresteps >= maxmsteps
                if ~warned
                    warning('MATLAB:symmlq:tooSmallTolerance', ...
                        ...
                        ' eps*cond(A) and may not be achieved by SYMMLQ\n',...
                        '         Try to use a bigger tolerance'));
                end
                flag = 3;
                iter = ii;
