...
%   $Revision: 1.1.6.3 $ $Date: 2010/02/25 08:12:10 $

% Check for an acceptable number of input arguments
if nargin < 2
    error('MATLAB:tfqmr:NotEnoughInputs', 'Not enough input arguments.');
end
...
if strcmp(atype,'matrix')
    ...
    [m,n] = size(A);
    if (m ~= n)
        error('MATLAB:tfqmr:NonSquareMatrix','Matrix must be square.');
    end
    if ~isequal(size(b),[m,1])
        error('MATLAB:tfqmr:RSHsizeMatchCoeffMatrix', ...
    ...
    n = m;
    if ~iscolumn(b)
        error('MATLAB:tfqmr:RSHnotColumn', ...
...
warned = 0;
if tol < eps
    warning('MATLAB:tfqmr:tooSmallTolerance', ...
      ['Input tol is smaller than eps and may not be achieved by TFQMR\n', ...
       '         Try to use a bigger tolerance']);
    warned = 1;
    tol = eps;
elseif tol >= 1
    warning('MATLAB:tfqmr:tooBigTolerance', ...
...
if ((nargin >= 5) && ~isempty(M1))
    ...
    if strcmp(m1type,'matrix')
        if ~isequal(size(M1),[m,m])
            error('MATLAB:tfqmr:WrongPrecondSize', ...
...
if ((nargin >= 7) && ~isempty(x0))
    if ~isequal(size(x0),[n,1])
        error('MATLAB:tfqmr:WrongInitGuessSize', ...
...
if ((nargin > 7) && strcmp(atype,'matrix') && ...
    ...
    error('MATLAB:tfqmr:TooManyInputs', 'Too many input arguments.');
end
    ...
    if (normr <= tolb || stag >= maxstagsteps || moresteps)
        ...
        if (normr_act <= tolb)
            ...
            if moresteps >= maxmsteps
                if ~warned
                    warning('MATLAB:tfqmr:tooSmallTolerance', ...
                        strcat('Input tol might be smaller than',...
                        ' eps*cond(A) and may not be achieved by TFQMR\n',...
                        '         Try to use a bigger tolerance'));
                end
                flag = 3;
                iter = mm;
