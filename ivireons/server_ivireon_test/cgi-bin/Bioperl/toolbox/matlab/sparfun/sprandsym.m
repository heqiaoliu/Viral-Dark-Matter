...
%   $Revision: 5.11.4.9 $  $Date: 2010/02/25 08:12:07 $

if nargin == 1
    S = arg1;
    if ~ismatrix(S)
        error('MATLAB:sprandsym:ndInput','ND-sparse arrays are not supported.');
    end
    m = size(S,1);
    [ii,jj] = find(S);
...
elseif nargin == 2,
    ...
    if (lrc == 1)
        if( rcond <= 0 || rcond > 1 )
            error('MATLAB:sprandsym:invalidRC', 'rc must be > 0 and <= 1.')
        end
    end
    if(nargin == 3 )
        n = arg1;
        ...
        if(kind==3)
            ...
            n = arg1;  
            if ~isscalar(n)
                error('MATLAB:sprandsym:firstArgNotScalar', ...
