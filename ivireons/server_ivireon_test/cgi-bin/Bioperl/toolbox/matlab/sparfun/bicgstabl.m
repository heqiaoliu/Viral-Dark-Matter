...
if strcmp(atype,'matrix')
    ...
    [m,n] = size(A);
    if (m ~= n)
        error('MATLAB:bicgstabl:NonSquareMatrix','Matrix must be square.');
    end
    if ~isequal(size(b),[m,1])
        error('MATLAB:bicgstabl:RSHsizeMatchCoeffMatrix', ...
    ...
    n = m;
    if ~iscolumn(b)
        error('MATLAB:bicgstabl:RSHnotColumn',...
...
if ((nargin >= 5) && ~isempty(M1))
    ...
    if strcmp(m1type,'matrix')
        if ~isequal(size(M1),[m,m])
            error('MATLAB:bicgstabl:WrongPrecondSize', ...
...
if ((nargin >= 7) && ~isempty(x0))
    if ~isequal(size(x0),[n,1])
        error('MATLAB:bicgstabl:WrongInitGuessSize', ...
...
if ((nargin > 7) && strcmp(atype,'matrix') && ...
    ...
    error('MATLAB:bicgstabl:TooManyInputs', 'Too many input arguments.');
end
                ...
                if moresteps >= maxmsteps
                    if ~warned
                        warning('MATLAB:bicgstabl:tooSmallTolerance', ...
                            ...
                            ' eps*cond(A) and may not be achieved by BICGSTABL\n',...
                            '         Try to use a bigger tolerance'));
                    end
                    flag = 3;
                    iter = ((kk-1)*ell*2+jj)/(ell*2);
