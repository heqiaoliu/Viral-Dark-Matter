...
%   $Revision: 1.1.6.2 $  $Date: 2010/05/13 17:42:33 $

if R == 'N'
   G = nested(n);
else
   if ~(n >= 2)
      error('MATLAB:numgrid:InvalidNumberPoints','Invalid number of points.');
   end
   ...
   elseif R == 'B'
      ...
      error('MATLAB:numgrid:InvalidRegionType','Invalid region type.');
   end
   k = find(G);
