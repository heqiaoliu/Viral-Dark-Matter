...
%   $Revision: 1.19.4.7 $ $Date: 2010/02/25 08:11:59 $
...
if strcmp(atype,'matrix')
    ...
    if ~isequal(size(b),[m,1])
        error('MATLAB:qmr:RSHsizeMatchCoeffMatrix', ...
...
if ((nargin >= 5) && ~isempty(M1))
    ...
    if strcmp(m1type,'matrix')
        if ~isequal(size(M1),[m,m])
            error('MATLAB:qmr:WrongPrecondSize', ...
...
if ((nargin >= 7) && ~isempty(x0))
    if ~isequal(size(x0),[n,1])
        error('MATLAB:qmr:WrongInitGuessSize', ...
            ...
            if moresteps >= maxmsteps
                if ~warned
                    warning('MATLAB:qmr:tooSmallTolerance', ...
                        ...
                        ' and might not be achieved by QMR\n',...
                        '         Try to use a bigger tolerance'));
                end
                flag = 3;
                iter = ii;
