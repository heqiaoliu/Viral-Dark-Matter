...
%   $Revision: 1.6.4.12 $ $Date: 2010/05/13 17:42:32 $
...
if ~iscolumn(b)
    error('MATLAB:lsqr:RSHnotColumn', ...
...
if strcmp(atype,'matrix')
    ...
    if size(b,1) ~= m
        error('MATLAB:lsqr:RSHsizeMatchCoeffMatrix', ...
            'Right hand side must be a column vector of length %d.', m);
    end
else
    m = size(b,1);
end
    ...
    if strcmp(m1type,'matrix')
        ...
        if (size(M1,1) ~= size(M1,2)) || (size(M1,1) ~= n)
            error('MATLAB:lsqr:WrongPrecondSize', ...
                'Preconditioner must be a square matrix of size %d', n);
        end
    end
end
...
if ((nargin >= 7) && ~isempty(x0))
    ...
    if ~iscolumn(x0) || (size(x0,1) ~= n)
        error('MATLAB:lsqr:WrongInitGuessSize', ...
            'Initial guess must be a column vector of length %d.', n);
    end
    x = x0;
    xInit = true;
end
