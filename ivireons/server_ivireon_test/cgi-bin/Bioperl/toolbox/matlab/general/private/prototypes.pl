...
# $Revision: 1.1.6.33 $ $Date: 2010/05/13 17:39:13 $
...
sub cleanupType{
    ...
    $type =~ s/\b(?:_{0,2}const|signed|struct|enum|volatile)\b//g;
    ...
    if ($type =~ /\w+[^\w]+\w+/ ) {
        ...
        $type=$newtype . $type;        
        print "Translated compound type $_[0] into $type.\n" if exists $debug{'types'};
    }
