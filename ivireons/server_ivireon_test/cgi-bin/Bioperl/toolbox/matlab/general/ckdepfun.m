function [pmissing,pdata] = ckdepfun(funct,varargin)
...
%
disp('----------------------------------------------------------');
disp('For best results:');
disp('  - restart MATLAB right before calling CKDEPFUN');
disp('  - call CLEAR FUNCTIONS after restarting MATLAB and before calling CKDEPFUN');
disp('----------------------------------------------------------');
...
%              indices, which will return not a path, but the string
