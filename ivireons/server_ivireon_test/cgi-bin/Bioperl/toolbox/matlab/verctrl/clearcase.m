...
%   $Revision: 1.13.4.1 $ $Date: 2008/02/29 12:46:53 $
...
if (strcmp(action, 'checkin'))
  ...
  if (isempty(comments))
	error('MATLAB:sourceControl:noCommentsSupplied','Comments not specified');
  else 
	comments            = comments{1};
  end
...
elseif (strcmp(action, 'checkout'))
  ...
  if (~isempty(outputfile)) 
	if (length(fileNames) > 1)
	  error('MATLAB:sourceControl:tooManyFiles','Several files cannot be checked out to a single file');
	else
