...
%   $Revision: 1.12.4.4 $ $Date: 2009/09/28 20:28:34 $
...
case 'checkin'
	if (isempty(comments))                                                % Checking for mandatory arguments
		error('MATLAB:sourceControl:noCommentSpecified','Comments not specified');
	else
...
case 'checkout'
	if (~isempty(outputfile) & length(fileNames) > 1)
		error('MATLAB:sourceControl:tooManyFiles','Several files cannot be checkout to a one file');
	end
