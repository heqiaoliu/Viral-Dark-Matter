...
%   $Revision: 5.12.4.6 $  $Date: 2008/11/04 21:19:42 $
...
if ischar(args{end})
  ...
  elseif strcmp(args{end},'cdata')
    ...
    error('MATLAB:sufl:UnknownLightingMethod', 'Unknown lighting method "%s".',args{end});
  end
  nin = nin-1;
else
  useLight = 0;
...
if ischar(z) || ischar(x)
   error('MATLAB:surfl:InvalidNumberInputs', 'Must be 1, 3 or 4 numeric input arguments.')
end
...
if length(k)~=4,
  error('MATLAB:surfl:InvalidNumberComponents', 'Weighting vector k must have four components.');
end

[msg,x,y,z] = xyzchk(x,y,z); if ~isempty(msg), error(msg); end
if any(size(z)<[3 3])
    error('MATLAB:surfl:InvalidInput', 'X, Y, and Z must be at least 3-by-3.'); 
end
...
if (nin==1) || (nin==3), % Use default S
  ...
  if (length(s)~=2) && (length(s)~=3),
    error('MATLAB:surfl:MustSpecifyS', 'S must be specified using [AZ,EL] or [Sx,Sy,Sz].');
  end
end
