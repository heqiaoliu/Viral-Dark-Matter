...
%   $Revision: 1.14.4.3 $  $Date: 2008/05/23 15:35:34 $

error(nargchk(1,2,nargin,'struct'));
if ischar(arg1)
    % string input (check for valid option later)
    if nargin == 2
        error('MATLAB:lighting:NeedAxesHandle', 'First argument must be an axes handle.')
    end
    ax = gca;
    type = lower(arg1);
else
    ...
    if length(arg1) > 1
        error('MATLAB:lighting:HandleMustBeScalar', 'Axes handle must be a scalar');
    end
    % handle must be a handle and axes handle
    if ~any(ishghandle(arg1,'axes'))
        error('MATLAB:lighting:InputMustBeHandle', 'First argument must be an axes handle.')
    end
    ax = arg1;
    ...
    if nargin == 2
        ...
        error('MATLAB:lighting:InvalidLightingMethod', 'Lighting methods are none, flat, gouraud, and phong.');
    end
end
...
elseif (strcmp(type,'none') || strcmp(type, 'n') )
    ...
    error('MATLAB:lighting:LightingMethodInvalid', 'Lighting methods are none, flat, gouraud, and phong.');
end
