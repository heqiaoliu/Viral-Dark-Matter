...
%   $Revision: 1.10.4.5 $  $Date: 2008/05/23 15:35:24 $
...
if isempty(h)
  ...
  for k = 1:length(h)
    if ~any(ishghandle(h(k))) || h(k) == 0
      error('MATLAB:camlookat:InvalidHandleType', 'H must be a vector of axes children or an axes')
    end
    
    ax = ancestor(h(k), 'axes');

    if isempty(ax)
      error('MATLAB:camlookat:InvalidAxesHandle', 'H must be a vector of axes children or an axes')
    end
  end
end
