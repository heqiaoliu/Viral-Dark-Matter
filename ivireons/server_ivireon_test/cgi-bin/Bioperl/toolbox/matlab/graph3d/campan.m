...
%   $Revision: 1.8.4.2 $  $Date: 2008/05/23 15:35:26 $

if nargin>5 || nargin<2
  error('MATLAB:campan:InvalidNumberArguments', 'Wrong number of arguments')
elseif nargin<5
  
  if any(ishghandle(ax,'axes'))
    if nargin<3 
      error('MATLAB:campan:NotEnoughInputs', 'Wrong number of arguments')
    else
      direction = [0 0 1];
      if nargin==3
	coordsys = 'data';
