...
% $Revision: 1.7.4.3 $ $Date: 2010/05/20 02:25:45 $
...
if nargin > 0
    if isscalar(param1) ...
            && (ishghandle(param1,'figure') ...
                || isa(param1, 'matlab.graphics.axis.colorspace.ColorSpace'))
        hMapObject = param1;
        if nargin > 1
            param1 = param2;
            set_alphamap = 1;
    ...
    if ischar(param1)
        switch param1
            ...
            case 'default'
                map = param1;
            otherwise
                error('MATLAB:alphamap:UnknownSpecifier', 'Unknown alphamap specifier.');
        end
    else
        map = param1;
    end
