...
%   $Revision: 1.9.4.2 $  $Date: 2008/08/26 18:20:13 $

if nargin>3 || nargin<1
  error('MATLAB:lightangle:WrongNumberArguments', 'Wrong number of arguments')
elseif nargin==2
  el = az;
  az = h;
  h=light;
elseif ~any(ishghandle(h, 'light'))
  error('MATLAB:lightangle:MustBeHandle', 'H must be a handle to a single light object')
end

if nargin==3 && nargout>1
  error('MATLAB:lightangle:InvalidNumberOutput', 'Wrong number of output arguments')
end
...
if nargin==1  %getting
  ...
  if ~isnumeric([az el]) || length(az) > 1 || length(el) > 1
    error('MATLAB:lightangle:InputsMustBeScalar', 'AZ and EL must both be scalars')
  end
  deg2rad = pi/180;
  az = az*deg2rad;
