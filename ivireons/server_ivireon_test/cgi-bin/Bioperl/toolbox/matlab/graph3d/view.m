...
for i=1:length(varargin),
  if ~isnumeric(varargin{i}) && (i==1 && ~isAxesHandle(varargin{i}))
    error('MATLAB:view:InvalidInputs', 'Invalid input arguments');
  end
end
...
switch nargin,
 ...
 case 3
  if ~isAxesHandle(varargin{1}),
    error('MATLAB:view:InvalidFirstArgument', 'First argument must be a scalar axes handle');
  else
    hAxes = varargin{1};
    viewArgs = varargin(2:end);
  end
...
switch (nargin)
 ...
 case 2
  ...
  if (r == 1) && (c == 1)
    ...
    switch arg1
     ...
     otherwise
      error('MATLAB:view:InvalidScalarArgument', 'Single scalar argument must be 2 or 3');
    end
  elseif (r == 1) && (c == 2)
    %
    % view([az, el]
  ...
  elseif r*c == 3
    ...
    error('MATLAB:view:InvalidArgument', 'Argument must be scalar, or two-vector');
  end
 ...
 case 3
  if (length(arg1) == 1) && (length(arg2) == 1),
    ...
    error('MATLAB:view:InvalidArgumentType', 'Arguments must be scalars')
