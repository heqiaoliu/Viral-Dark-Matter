...
%   $Revision: 5.9.4.2 $  $Date: 2007/12/14 14:52:24 $
...
if ~isequal(size(ny),[m,n])
    error('MATLAB:specular:NySizeMustEqualNx', 'Ny must be the same size as Nx.'); 
end
if ~isequal(size(nz),[m,n])
    error('MATLAB:specular:NzSizeMustEqualNx', 'Nz must be the same size as Nx.'); 
end
...
elseif length(s)==3 
  ...
  error('MATLAB:specular:SpecifyInput', 'S must be specified using [AZ,EL] or [Sx,Sy,Sz].');
end
...
elseif length(v)==3 
  ...
  error('MATLAB:specular:MustSpecifyV', 'V must be specified using [AZ,EL] or [Vx,Vy,Vz].');
end
