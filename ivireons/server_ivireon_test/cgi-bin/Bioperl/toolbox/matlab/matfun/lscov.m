...
%   $Revision: 5.15.4.9 $  $Date: 2010/02/25 08:10:03 $
...
elseif isequal(alg,'orth') ...
    ...
    error('MATLAB:lscov:NoSparseOrthAlg', 'Inputs may not be sparse for the ''orth'' algorithm.');
end
...
elseif isequal(size(V),[nobs nobs])
    ...
    elseif errFlag > 0
        if issparse(V)
            error('MATLAB:lscov:RankDefSparseCovMat', 'V must be positive definite if sparse.');
    ...
    % V is not a valid covariance matrix.
        ...
        error('MATLAB:lscov:InvalidCovMatPosV', 'V must be positive definite or positive semidefinite.');
    end

else
    error('MATLAB:lscov:InvalidCovMat', '%s%d%s%d%s%d', ...
          'V must be a ',nobs,'-by-',nobs,' covariance matrix or a ',nobs,'-by-1 weight vector.');
end
...
case 'orth'
    ...
    if ~feasible
        if nrhs == 1
            ...
            error('MATLAB:lscov:InfeasibleRHScol', 'One or more columns of B are not consistent with A and V.');
        end
    end
