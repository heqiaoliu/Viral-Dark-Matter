...
%   $Revision: 5.17.4.11 $  $Date: 2010/02/25 08:10:01 $
...
if  ~isfloat(A) || ~ismatrix(A) || m ~= n
   error('MATLAB:funm:InputDim', 'First input must be a single or double square matrix.');
end
...
if isequal(A,triu(A))
   ...
   if isequal(fun,@fun_log) && ~all(diagT)
       warning('MATLAB:funm:zeroEig', ...
              ['At least one zero eigenvalue is detected in SCHUR(A).\n',...
...
if any(terms == -1)
   ...
   warning('MATLAB:funm:TaylorSeriesNotConverged', ...
          ['Taylor series failed to converge.\n',...
           '         Possible that function does not have an infinite radius of convergence.\n',...
           '         Try decreasing options.TolBlk or increasing options.TolTay or options.MaxTerms.'])
end

if isequal(fun,@fun_log) && any(terms == maxsqrt)
   exitflag = 1;
   warning('MATLAB:funm:TooManyMatrixSquareRoots', ...
          ['Maximum number of matrix square roots exceeded.\n',...
           '         Try decreasing options.TolBlk or increasing options.MaxSqrt.'])
end
