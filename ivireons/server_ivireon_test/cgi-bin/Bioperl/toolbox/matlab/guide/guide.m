...
ERRMSG = 'Call GUIDE with no args, an existing FIG-file name, or a list of figure handles.';
...
elseif ischar(filename)
    ...
    if ~strcmp(ext, '.fig')
        error(['MATLAB:', mfilename, ':InvalidFileExtension'], ERRMSG);
    end
    ...
    if ~exist(filename, 'file')
        error(['MATLAB:', mfilename, ':InvalidFileName'], ERRMSG);
    end
    ...
    if any(~ishandle(figs)) ||...
        ...
        error(['MATLAB:', mfilename, ':InvalidFigureHandle'], ERRMSG);
    end

    result = cell(length(figs));
    for i=1:length(figs)
        result{i} = LayoutEditor.openLayoutEditor(figs(i));
    end
