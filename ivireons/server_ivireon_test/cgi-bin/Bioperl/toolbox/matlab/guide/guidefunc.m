    ...
    if ~isempty(data)
        ...
        for i=1:length(hndls)
            ...
            elseif guidemfile('isManagedButtonGroupCallback',hndls{i}, whichCb, source)
                message = ['Button group %s manages the %s of this %s. \n\n'...
                           'To customize the behavior when the selected button in the button group changes, modify its SelectionChangeFcn callback.'];
                  helpdlg(sprintf(message, get(get(obj,'Parent'),'Tag'),whichCb, get(obj,'Tag')), 'GUIDE');
%                 if isequal(questdlg(sprintf(message, whichCb, get(obj,'Tag'),get(get(obj,'Parent'),'Tag')), 'GUIDE', 'Yes', 'No', 'No'), 'Yes')
%                     guidemfile('setAutoCallback',obj, whichCb);
%                 end
            else
...
if success && ~istemp
    ...
    if ~isempty(exportmfile)
        ...
        if filterindex == 1
            editorObj = editorservices.open(exportmfile);
            editorObj.reload;
...
for i = 1:limit
    ...
    if isExternalControl(obj)
        ...
        view =[];
        try
            if isa(handle(hndl{i}),'uicontrol')
...
% ****************************************************************************
function out = showErrorDialog(e,message)
out=[];

links = feature('hotlinks');
feature('hotlinks',0);
...
editorservices.openAndGoToLine(file, line);
