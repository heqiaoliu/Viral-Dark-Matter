...
%   $Revision: 1.1.6.26.4.1 $ $Date: 2010/06/17 14:12:25 $
...
if ~isempty(fcontents)
    ...
    % ask me again" dialog.
    editorObj = editorservices.find(filename);
...
if editorservices.isOpen(filename)
    editorObj = editorservices.find(filename);
    isDirty = editorObj.IsDirty;
else
    isDirty = false;
end
...
if needToSave || isSaveAs
    ...
    if bringEditorForward || isSaveAs
        editorservices.open(mname);
        if isSaveAs
            % close the previous filename in editor:
            if editorservices.isOpen(new_options.lastSavedFile)
                editorObj = editorservices.find(new_options.lastSavedFile);
                editorObj.close;
            end
        end
    end
...
if ~isAutoCallback(obj, whichCb)
    ...
    if ~isempty(mfunc) && ~isempty(subfunc)
        ...
        if ~isempty(where) && where(1)==1 
            ...
            if lineno
                editorservices.openAndGoToLine(mfunc, lineno);
            else
...
managed =   isa(handle(get(obj,'parent')), 'uitools.uibuttongroup') && ...
            ~isempty(value) && ...
            iscell(value) && ...
