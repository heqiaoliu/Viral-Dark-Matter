...
% $Revision: 1.1.22.1 $  $Date: 2010/07/23 15:39:59 $
...
if strcmp(fullpath,cellFilename) && ...
...
elseif strcmp(fullpath,cellFilename) && (cellIndex ~= 1)
    ...
    fid = fopen(fullpath);
    if (fid == -1)
        error('MATLAB:echodemo:FileNotFound','"%s" is not on the path',inputFilename);
    end
...
elseif cellIndex == length(cellStruct)
    leftHeader = sprintf('End of demo. Click <a href="matlab:echodemo(''%s'',1)">Replay</a> to see it again', ...
        fcnname);
elseif cellIndex > length(cellStruct)
    error('MATLAB:echodemo:TooHigh','There are only %d cells in file %s',length(cellStruct),fcnname)
end
...
for n = 1:length(thisCell.text)
    ...
    if ~isempty(image)
        ...
        [unused,unused,linkTokens] = regexp(line,linkPattern);
