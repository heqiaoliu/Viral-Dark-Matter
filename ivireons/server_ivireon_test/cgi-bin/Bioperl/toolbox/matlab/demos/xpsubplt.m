...
%   $Revision: 5.15.4.3 $  $Date: 2007/12/14 14:51:52 $

create_axis = 1;

% if we haven't recovered position yet, generate it from mnp info:
if (thisPlot < 1)
   error('MATLAB:xpsubplt:IllegalPlotNumber','Illegal plot number.')
elseif (thisPlot > ncols*nrows)
   error('MATLAB:xpsubplt:IllegalPlotNumber','Index exceeds number of subplots.')
else
