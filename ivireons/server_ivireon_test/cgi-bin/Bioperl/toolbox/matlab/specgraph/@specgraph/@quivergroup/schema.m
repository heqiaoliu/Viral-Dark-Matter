...
% Visible property - shadows the group visible property:
hProp = schema.prop(hClass, 'Visible', 'GObjectVisibleType');
hProp.FactoryValue = 'on';
hProp.SetFunction = @LdoVisibleAction;
...
function valueStored = LdoVisibleAction(hThis,valueProposed)
% Update the visibility of the quiver group.
valueStored = valueProposed;
if hThis.initialized
    ch = get(hThis,'Children');
    set(ch,'Visible',valueProposed);
    % If the "ShowArrowHead" property is set to "off", make sure this is
    % honored.
    if strcmpi(hThis.ShowArrowHead,'off')
