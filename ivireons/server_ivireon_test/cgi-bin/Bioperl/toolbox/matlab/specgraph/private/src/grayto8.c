...
void ValidateInputs(int nrhs, const mxArray *prhs[])
{
    if (nrhs < 1)
    {
        mexErrMsgIdAndTxt("MATLAB:grayto8:tooFewInputs",
                          "Too few input arguments.");
    }
    if (nrhs > 1)
    {
        mexErrMsgIdAndTxt("MATLAB:grayto8:tooManyInputs",
                          "Too many input arguments.");
    }
    if (!mxIsDouble(prhs[0]) && !mxIsUint16(prhs[0]) && !mxIsSingle(prhs[0]))
    {
        mexErrMsgIdAndTxt("MATLAB:grayto8:invalidType",
                          "Input must be double, single, or uint16.");
    }
    if (mxIsComplex(prhs[0]))
    {
        mexWarnMsgIdAndTxt("MATLAB:grayto8:ignoringImaginaryPartOfInput",
                           "Ignoring imaginary part of input.");
    }
}
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    {
        ConvertFromSingle((float *) mxGetData(prhs[0]), qr,
                          mxGetNumberOfElements(prhs[0]));
    }
