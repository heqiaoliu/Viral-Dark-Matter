...
static char rcsid[] = "$Revision: 1.8.4.4 $";
...
allocVertArray()
{
    ...
    if (verts==NULL) 
	  ...
	  if ((verts = (double *) malloc(vertLimit * 3 * sizeof(double))) == NULL) 
	    {
		mexErrMsgIdAndTxt("MATLAB:stream3c:InsufficientMemoryForVertices",
		 ...
		 )
{
    ...
    if (nrhs != 11) 
      mexErrMsgIdAndTxt("MATLAB:stream3c:WrongNumberOfInputs",
                        "stream3c requires 11 input arguments. verts = stream3c( x,y,z, u,v,w, sx,sy,sz, step, maxVert )");
    else if (nlhs > 2) 
      mexErrMsgIdAndTxt("MATLAB:stream3c:WrongNumberOfOutputs",
                        "stream3c requires 1 output argument. verts = stream3c( x,y,z, u,v,w, sx,sy,sz, step, maxVert  )");
    ...
    ySize = dims[0];
    if (xSize <= 1 || ySize <= 1) 
      mexErrMsgIdAndTxt("MATLAB:stream3c:WrongDimensionSizes",
                        "stream3c requires that all three dimensions be greater than 1");
