...
if (isappdata(ax,'PlotHoldStyle') && getappdata(ax,'PlotHoldStyle')) || ...
        ~firsttime
    if isprop(ax,'ColorIndex')
        ci(1) = get(ax,'ColorIndex');
    elseif isappdata(ax,'PlotColorIndex')
        ci(1) = getappdata(ax,'PlotColorIndex');
    end
    if isprop(ax,'LineStyleIndex')
        ci(2) = get(ax,'LineStyleIndex');
    elseif isappdata(ax,'PlotLineStyleIndex')
...
if isprop(ax,'ColorIndex')
    set(ax,'ColorIndex',ci(1));
else
    setappdata(ax,'PlotColorIndex',ci(1));
end
if isprop(ax,'LineStyleIndex')
    set(ax,'LineStyleIndex',ci(2));
else
