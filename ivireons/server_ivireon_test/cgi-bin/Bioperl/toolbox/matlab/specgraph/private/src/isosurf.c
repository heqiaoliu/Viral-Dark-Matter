...
static char rcsid[] = "$Revision: 1.10.4.2 $";
...
allocVertArray()
{
    ...
    if (verts==NULL) 
	  ...
	  if ((verts = (double *) malloc(vertLimit * 3 * sizeof(double))) == NULL) 
	    {
		mexErrMsgIdAndTxt("MATLAB:isosurf:InsufficientMemoryForVertices",
                          "Not enough memory to store vertices");
	    }
	  if (doColors)
	    if ((cols = (double *) malloc(vertLimit     * sizeof(double))) == NULL) 
	      {
		mexErrMsgIdAndTxt("MATLAB:isosurf:InsufficientMemoryForColors",
                          "Not enough memory to store colors");
	      }
      }
    else 
      {
...
allocFaceArray()
{
    ...
    if (faces==NULL) 
	  ...
	  if ((faces = (double *) malloc(faceLimit * 3 * sizeof(double))) == NULL) 
	    {
		mexErrMsgIdAndTxt("MATLAB:isosurf:InsufficientMemoryForFaces",
                          "Not enough memory to store faces");
	    }
      }
    else 
      {
		 ...
		 )
{
    ...
    if (nrhs != 5) 
      mexErrMsgIdAndTxt("MATLAB:isosurf:WrongNumberOfInputs",
                        "isosurf requires 4 input arguments. [v f c] = isosurf( data, isoValue, colors, noshare, verbose )");
    else if (nlhs > 3) 
      mexErrMsgIdAndTxt("MATLAB:isosurf:WrongNumberOfOutputs",
                        "isosurf requires 2 output arguments. [v f c] = isosurf( data, isoValue, colors, noshare, verbose )");

    /* Check the dimensions of data It must be 3 */
    ndims = mxGetNumberOfDimensions(DATA_IN);
    if (ndims != 3) 
      mexErrMsgIdAndTxt("MATLAB:isosurf:WrongNumberOfDimensions",
                        "isosurf requires 3 dimensional data");
    ...
    if (xSize <= 1 || ySize <= 1 || zSize <= 1) 
      mexErrMsgIdAndTxt("MATLAB:isosurf:WrongDimensionSizes",
                        "isosurf requires that all three dimensions be greater than 1");
