...
%   $Revision: 1.9.4.1 $  $Date: 2005/04/28 19:57:05 $
...
if step<=0
  error(id('NonPositiveStep'),'Step size must greater than 0.');
end

if maxvert<=0
  error(id('NonPositiveVertexCount'),'Maximum number of vertices must greater than 0.');
end

[msg x y z] = xyzuvwcheck(x,y,z,u,v,w);  error(msg) %#ok

if ~isequal(size(sx), size(sy), size(sz)) 
  error(id('StartSizeMismatch'),'STARTX,STARTY,STARTZ must all be the same size.');
end
