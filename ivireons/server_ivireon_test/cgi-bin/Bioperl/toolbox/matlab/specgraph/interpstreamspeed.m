...
%   $Revision: 1.9.4.1 $  $Date: 2005/04/28 19:56:33 $
...
if any(any(isnan(cat(1, verts{:}))))
  error(id('NaNVertices'),'Stream vertices cannot contain NaNs.')
end

if any(isnan(u(:))) ||  any(isnan(v(:))) ||  any(isnan(w(:))) 
  error(id('NaNUVW'),'U,V,W cannot contain NaNs')
end
...
function str=id(str)
str = ['MATLAB:interpstreamspeed:' str];
