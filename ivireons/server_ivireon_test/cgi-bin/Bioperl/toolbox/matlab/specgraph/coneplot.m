...
%   $Revision: 1.7.4.6 $  $Date: 2009/05/18 20:49:55 $
...
if isempty(s)
  s = 1;
elseif length(s)>1,
  error(id('ScalarSizeFactor'),'S must be a scalar.');
end

if ~isempty(color)
  if ~isequal(size(color), size(u))
    error(id('InvalidColorSize'),'COLOR must be the same size as U.');
  end
end
...
if nointerp==1
  ...
  if ~isequal(size(y(:)), sz) || ~isequal(size(z(:)), sz) || ...
     ...
     ~isequal(size(w(:)), sz)  
    error(id('InputSizesMustMatch'),'X,Y,Z,U,V,W must all be the same size.');
  end
  ...
  if ~isequal(size(cx), size(cy), size(cz)) 
    error(id('SizeMismatch'),'Cx,Cy,Cz must all be the same size.');
  end
