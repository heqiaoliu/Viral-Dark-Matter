function newh = hgline2lineseries(h,varargin)
    ...
    if isempty(h) || ~ishghandle(h,'line')
        error('MATLAB:hgline2lineseries:InvalidLineHndle',...
            'First argument must be a line handle.');
    end
    ...
    [dummy,ia,ib] = intersect(fields,{'BeingDeleted','Children','Type','UIContextMenu','Annotation'}); %#ok
    fields(ia) = [];
    vals = get(h,fields);
    pvpairs = [fields(:).' ; vals(:).'];
