function hout=streamtube(varargin)
    ...
    %   $Revision: 1.7.4.5 $  $Date: 2009/05/18 20:50:00 $
    ...
    if any(isnan(u(:))) ||  any(isnan(v(:))) ||  any(isnan(w(:)))
        error(id('NaNData'),'Vector data U,V,W must not contain any NaNs')
    end
...
function [x, y, z, u, v, w, sx, sy, sz, verts, div, width, scale, n] = parseargs(nin, vargin)
    ...
    if ~isempty(scale)
        if length(scale) == 2
            ...
            error(id('InvalidSizeInput'),'[SCALE N] must be a 2 element vector.');
        end
    end
end

function str = id(str)
    str = ['MATLAB:streamtube:' str];
end