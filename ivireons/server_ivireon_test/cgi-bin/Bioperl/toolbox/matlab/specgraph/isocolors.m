function fvcout = isocolors(varargin)
    ...
    %   $Revision: 1.5.4.3 $  $Date: 2009/12/11 20:36:00 $
...
function [msg,nx,ny,nz] = xyzrgbcheck(x,y,z,r,g,b)
    ...
    if nargin>4
        if ~isequal(size(g), sz) || ~isequal(size(b), sz)
            msg.identifier = id('RGBSizeMismatch');
            msg.message='R,G,B must all be the same size.';
            return
        end
    end
    
    if ndims(r)~=3
        msg.identifier = id('InvalidColorDimensions');
        msg.message='Color arrays must all be a 3D';
        return
    end
    if min(sz)<2
        msg.identifier = id('InvalidColorSizes');
        msg.message = 'Color arrays must be size 2x2x2 or greater.';
        return
    end
    
    nonempty = ~[isempty(x) isempty(y) isempty(z)];
    if any(nonempty) && ~all(nonempty)
        msg.identifier = id('InputsEmptyMismatch');
        msg.message = 'X,Y,Z must all be empty or all non-empty.';
        return;
    end
    ...
    if ~isempty(nx) && ~isequal(size(nx), sz)
        nx = nx(:);
        if length(nx)~=sz(2)
            msg.identifier = id('XSizeMismatch');
            msg.message='The size of X must match the size of R or the number of columns of R.';
            return
        else
    ...
    if ~isempty(ny) && ~isequal(size(ny), sz)
        ny = ny(:);
        if length(ny)~=sz(1)
            msg.identifier = id('YSizeMismatch');
            msg.message='The size of Y must match the size of R or the number of rows of R.';
            return
        else
    ...
    if ~isempty(nz) && ~isequal(size(nz), sz)
        nz = nz(:);
        if length(nz)~=sz(3)
            msg.identifier = id('ZSizeMismatch');
            msg.message='The size of Z must match the size of R or the number of pages of R.';
            return
        else
