...
%   $Revision: 5.10.4.7 $  $Date: 2005/12/15 20:55:06 $

if nargin > 2 || (nargin ==1 && ~ischar(a))
    error('MATLAB:colstyle:InvalidInput', 'Requires one string argument.');
end
...
if any(all(aa~=b)) 
  msg.message = 'Unrecognized characters in LineSpec string.'; 
  msg.identifier = id('UnrecognizedLinespecCharacter');
  return
end
...
if length(style)+length(color)+length(marker) ~= length(a)
  msg.message = 'Invalid LineSpec string.';
  msg.identifier = id('InvalidLinespec');
end
