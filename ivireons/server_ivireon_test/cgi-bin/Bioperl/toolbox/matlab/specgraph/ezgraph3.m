...
%   $Revision: 1.13.4.10 $  $Date: 2008/12/04 22:40:55 $
...
switch N
   ...
   case 1 % either bad inputs or the same as above
      error(id('ParametricPlot'),'To plot parametric curves, use EZPLOT3.');
   ...
   otherwise
      error(id('TooManyDimensions'),'Cannot plot in 4 dimensions')
end
...
domstyle = 'rect';
if isempty(varargin)
    error(id('MissingExpression'),'Mathematical expression required.');
end
...
case 3
   ...
   % ezgraph3(surfstyle,x,y,z)
      ...
      if (isempty(ymsg) && isempty(zmsg))
         ...
         error(id('IncorrectInputs'),'Incorrect inputs.');
      end
   end
case 4
...
if (isnan(xmin) || isnan(xmax) || isnan(ymin) || isnan(ymax) )
   error(id('NonRealInput'),'%s must be real-valued only.',flabel)
end
...
case 2
  % done
otherwise
  error(id('InvalidExpresion'),...
        'The expression %s must only have 2 symbolic variables',flabel);
end

function str=id(str)
str = ['MATLAB:ezgraph3:' str];
