...
%   $Revision: 1.6.4.2 $  $Date: 2006/10/10 02:25:21 $
...
if numel(r)~=3 
  error('MATLAB:reducevolume:InvalidReduction', 'Reduction must be [Rx Ry Rz] or R');
end

if ~isequal(r, floor(r)) || min(r)<1
  error('MATLAB:reducevolume:ReductionMustBeInt', 'Reduction values must be integers greater than or equal to 2');
end
...
if length(newsz)<3 || min(newsz)<2
  error('MATLAB:reducevolume:ReductionTooHigh', 'Reduction too high. Try a smaller reduction.');
end
if length(sz)>3
...
elseif nin==5       % reducevolume(x,y,z,v,r)
  ...
  error('MATLAB:reducevolume:WrongNumberInputs', 'Wrong number of input arguments.'); 
end
