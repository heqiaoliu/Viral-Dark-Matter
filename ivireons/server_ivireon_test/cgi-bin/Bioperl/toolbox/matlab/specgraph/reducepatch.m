...
%   $Revision: 1.8.4.2 $  $Date: 2006/10/10 02:25:20 $
...
if length(reduction)>1
  error(id('NonScalarReduction'),'Reduction must be a scalar.'); 
end
if reduction<0 || isnan(reduction)
  error(id('NonFinitePositiveReduction'),'Reduction must be finite and positive.'); 
end

if min(faces(:))<1 
  error(id('FaceValuesTooSmall'),'Faces array must contain values >=1 '); 
end
if max(faces(:))>size(verts,1)
  error(id('FaceValuesTooLarge'),'Faces array contains values which exceed vertices length'); 
end
...
if nin==1 || nin==2  % reducepatch(p),  reducepatch(fv), reducepatch(arg, r)
  ...
  elseif ishandle(firstarg) && strcmp(get(firstarg, 'type'), 'patch')
    ...
    error(id('InvalidFirstArgument'),...
          'The first argument must be a patch handle or a struct containing faces and vertices.');
  end
  if nin==2
    r = vargin{2};
  end
