...
%   $Revision: 1.8.4.1 $  $Date: 2005/04/28 19:57:11 $

[x y z data u v w limits] = parseargs(nargin,varargin);

if numel(limits)~=6 
  error(id('InvalidReductionSize'),'Reduction must be [xmin xmax ymin ymax zmin zmax]');
end

if limits(1) > limits(2)
  error(id('InvalidReductionXRange'),'In reduction vector, xmax must be greater than or equal to xmin');
end
if limits(3) > limits(4)
  error(id('InvalidReductionYRange'),'In reduction vector, ymax must be greater than or equal to ymin');
end
if limits(5) > limits(6)
  error(id('InvalidRedutionZRange'),'In reduction vector, zmax must be greater than or equal to zmin');
end
...
if isempty(u)
  ...
  [msg x y z] = xyzuvwcheck(x,y,z,u,v,w);  error(msg) %#ok
  sz = size(u);
end
