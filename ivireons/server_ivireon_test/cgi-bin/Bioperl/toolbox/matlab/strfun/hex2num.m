...
%   $Revision: 5.15.4.8 $  $Date: 2006/11/11 22:45:04 $
...
if ~ischar(s)
    if iscellstr(s)
        ...
        error('MATLAB:hex2num:InputMustBeString','Input to hex2num must be a string.')
    end
end
if isempty(s)
    x = [];
...
if any(d(:) > 15) || any(d(:) < 0) 
    error('MATLAB:hex2num:OutOfRange',...
        'Input string to hex2num should have just 0-9, a-f, or A-F.')
end
