...
for i=1:numel(l)
    localSetEnableState(l(i),newval);
end
...
for m = 1:length(hlist)
    ...
    if (~isobject(hlist(m)) && ishandle(hlist(m))) || (isobject(hlist(m)) && isvalid(hlist(m)))
        ...
        % Listen to deletion
        hDeleteListener(m) = eventListenerConstructor(hlist(m), ...
            'ObjectBeingDestroyed',...
            @(obj,evd)(localRemoveHandle(obj,evd,hlist(m),hLink)));
        ...
        if foundprop
            ...
            hListener(m) = propListenerConstructor(hlist(m),...
                hProp, ...
                propEvent,...
                @(obj,evd)(localUpdate(obj,evd,hLink,hlist)));
            localSetEnableState(hListener(m),get(hLink,'Enabled'));
            foundlistener = true;
        end
...
for i=1:numel(hListeners)
    localSetEnableState(hListeners(i),'off');
end
...
% Restore listeners
for i=1:numel(hListeners)
    localSetEnableState(hListeners(i),'on');
end
