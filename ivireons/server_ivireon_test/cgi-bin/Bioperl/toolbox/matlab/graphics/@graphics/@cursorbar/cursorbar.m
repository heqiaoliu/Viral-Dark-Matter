...
if numel(hTarget) == 1
    ...
    elseif isa(hTarget,'hg.axes')
        ...
        delete(hThis);
        error('MATLAB:graphics:cursorbar:invalidTarget','Target must be a single axes handle, or an array of line handles');
    end
else
    ...
    for n = 1:2:length(varargin)
        ...
        elseif strcmpi(propname,'Position')
            ...
            if numel(propval) > 3 || numel(propval) < 2
                error('MATLAB:graphics:cursorbar:invalidPosition','Position must have 2 or 3 elements')
            end            
...
switch newval
    ...
    otherwise
        error('MATLAB:graphics:cursorbar:invalidOrientation','Cursorbar''s Orientation can only be set to ''vertical'' or ''horizontal''')
end
...
if ~isa(newTarget,'hg.line') && ~isa(newTarget,'hg.axes')
    error('MATLAB:graphics:cursorbar:invalidTarget','Target must be a line or axes object.')
end
