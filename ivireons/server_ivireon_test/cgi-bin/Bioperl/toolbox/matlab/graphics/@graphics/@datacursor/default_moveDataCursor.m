...
if strcmp(dir,'up') | strcmp(dir,'right')
    ...
    if ~isempty(zdata)
        currLine = [currLine;zdata(currind)];
    end    
...
if ~isempty(zdata)
	...
	hDataCursor.TargetPoint = [xdata(currind),ydata(currind),zdata(currind)];
else
...
if newind > 0
   ...
   if len>2
	   ...
	   hDataCursor.TargetPoint = [pos(1),pos(2),pos(3)];
   elseif len>1
       hDataCursor.Position = [pos(1),pos(2)];
	   hDataCursor.TargetPoint = [pos(1),pos(2)];
   end
end
