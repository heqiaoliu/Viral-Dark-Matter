...
%   $Revision: 1.12.4.14 $  $Date: 2010/05/20 02:26:05 $ 


warning('MATLAB:print:DeprecatedMATLABCodeGenerationOption', ...
        'The generation of MATLAB code will be removed in a future release of MATLAB.');
...
if ~isempty(mat)
   ...
   if isfield(options, 'showMessageForMATFile') & options.showMessageForMATFile ==1 
       ...
       warning('MATLAB:printdmfile:KeepFilesTogether','%s', message); 
   end 
end
...
if ~ishandle(handle)
    error('MATLAB:printdmfile:invalidHandle','First argument must be a valid handle')
end


% Check if 'handle' refers to a figure
if ~strcmp(get(handle,'type'),'figure')
    error('MATLAB:printdmfile:invalidHandle','First argument must be a valid figure handle')
end
...
elseif isstruct(value) 
    ...
    if ~found
        error('MATLAB:printdmfile:unsupportedDataType','Data type is not supported for generating M-Code.'); 
    end
end
...
dim =size(value); 
if (length(dim)>2) 
    error('MATLAB:printdmfile:unableToGenerateMCodeForChar','Cannot generate M-Code for a char array with more than two dimensions.'); 
end 
...
if (length(dim)>2) 
    error('MATLAB:printdmfile:unableToGenerateMCodeForCell','Cannot generate M-Code for a cell array with more than two dimensions.'); 
end 
...
if fid < 1
    error('MATLAB:printdmfile:errorWritingFile','error writing file')
end
