...
%   $Revision: 1.11.4.2 $  $Date: 2005/11/27 22:40:47 $
...
if ~exist( theFileName, 'file' )
    error('MATLAB:print:NoFile', 'No file to send to printer.')
end
...
if ispc
    ...
    if ~isempty(findstr(cmd,'$portname$'))
		...
		error('MATLAB:print:NoDefaultPrinter', 'No default printer configured.');
	    end
	    
            error('MATLAB:print:InvalidPrinter', ['Invalid printer ''' pj.PrinterName ''' specified.' ] );
        end
        
        if strcmp(portName,'FILE:')
	    error('MATLAB:print:UnsupportedPort', ['Unsupported port''' portName ''' to printer ''' pj.PrinterName ''', use a file instead.' ] );
        end
...
if s && ~isempty(r)
    error('MATLAB:print:ProblemSendingFile', 'Problem sending file to output device, system returned error :\n%s', r )
end
