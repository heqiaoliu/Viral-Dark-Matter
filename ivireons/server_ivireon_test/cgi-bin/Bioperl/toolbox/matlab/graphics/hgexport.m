...
%         exporting.
%     'LockAxesTicks'  'on' or 'off'
%         specifies that all axes tick text (ticks and tick labels) should
%         be fixed while exporting. 
%         ==> Can only be 'on' if 'LockAxes' is also 'on'
%     'PSLevel'   1 or 2
...
if ~(isscalar(H) && ishghandle(H, 'figure'))
  error('MATLAB:hgexport:FirstInputMustBeFigHandle', 'First argument must be a handle to a figure.');
end
if ~ischar(filename)
  error('MATLAB:hgexport:SecondInputMustBeString', 'Second argument must be a string.');
end
isClipboard = strcmp(filename,'-clipboard');
if ~ispc && isClipboard, 
  error('MATLAB:hgexport:CopyToClipboardNotSupported', 'Copying to clipboard is not supported on this platform.');
end
...
if nargin < 3
  % if a Style structure isn't provided, retrieve one from: 
  %    Exportsetup appdata on the figure (export setup dialog), or
  %    a stylesheet named Default (in the ExportSetup preference dir), or
  %    the style "factory" function
...
for k = 1:2:length(paramPairs)
  ...
  if ~ischar(param)
    error('MATLAB:hgexport:ParameterMustBeString', 'Optional parameter names must be strings');
  end
  ...
  switch (param)
   ...
   case 'preview'
    ...
    if ~strcmp(opts.Preview,{'none','tiff'})
      error('MATLAB:hgexport:InvalidPreviewAction', 'Preview must be ''none'' or ''tiff''.');
    end
   case 'width'
    opts.Width = LocalToNum(value, auto.Width);
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~LocalIsPositiveScalar(opts.Width)
    error('MATLAB:hgexport:WidthMustBePositiveScalar', 'Width must be a numeric scalar > 0');
      end
    end
   case 'height'
    ...
    error('MATLAB:hgexport:HeightMustBePositiveScalar', 'Height must be a numeric scalar > 0');
      end
    end
   case 'units'
   ...
   case 'color'
    ...
    if ~strcmp(opts.Color,{'bw','gray','rgb','cmyk'})
      error('MATLAB:hgexport:InvalidColor', 'Color must be ''bw'', ''gray'',''rgb'' or ''cmyk''.');
    end
   case 'background'
   ...
   case 'fontmode'
    ...
    if ~strcmp(opts.FontMode,{'scaled','fixed','none'})
      error('MATLAB:hgexport:InvalidFontMode', 'FontMode must be ''scaled'', ''fixed'' or ''none''.');
    end
   case 'scaledfontsize'
    opts.ScaledFontSize = LocalToNum(value,auto.ScaledFontSize);
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~LocalIsPositiveScalar(opts.ScaledFontSize)
    error('MATLAB:hgexport:InvalidScaledFontSize', 'ScaledFontSize must be a numeric scalar > 0');
      end
    end
   case 'fixedfontsize'
    ...
    error('MATLAB:hgexport:InvalidFixedFontSize', 'FixedFontSize must be a numeric scalar > 0');
      end
    end
   case 'fontsizemin'
   ...
   case 'scaledlinewidth'
    ...
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~LocalIsPositiveScalar(opts.ScaledLineWidth)
        error('MATLAB:hgexport:InvalidScaleLineWidth', 'ScaledLineWidth must be a numeric scalar > 0');
      end
    end
   case 'fixedlinewidth'
    ...
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~LocalIsPositiveScalar(opts.FixedLineWidth)
        error('MATLAB:hgexport:InvalidFixedLineWidth', 'FixedLineWidth must be a numeric scalar > 0');
      end
    end
   case 'linewidthmin'
    ...
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~LocalIsPositiveScalar(opts.LineWidthMin)
        error('MATLAB:hgexport:InvalidLinWidthMinInput', 'LineWidthMin must be a numeric scalar > 0');
      end
    end
   case 'linestylemap'
   ...
   case 'renderer'
    ...
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~strcmp(opts.Renderer,{'painters','zbuffer','opengl'})
        error('MATLAB:hgexport:InvalidRendererInput', 'Renderer must be ''painters'', ''zbuffer'' or''''opengl''.');
      end
    end
   case 'resolution'
    ...
    if ~ischar(value) || ~strcmp(value,'auto')
      if ~(isnumeric(opts.Resolution) && (numel(opts.Resolution) == 1) && ...
         (opts.Resolution >= 0));
        error('MATLAB:hgexport:InvalidResolution', 'Resolution must be a numeric scalar >= 0');
      end
    end
   ...
   case 'bounds'
    ...
    if ~strcmp(opts.Bounds,{'tight','loose'})
      error('MATLAB:hgexport:InvalidBounds', 'Bounds must be ''tight'' or ''loose''.');
    end
   case 'lockaxes'
    opts.LockAxes = LocalCheckAuto(lower(value),auto.LockAxes);
    if ~strcmp(opts.LockAxes,{'on','off'})
      error('MATLAB:hgexport:InvalidLockAxesInput', 'LockAxes must be ''on'' or ''off''.');
    end
   case 'lockaxesticks'
    opts.LockAxesTicks = LocalCheckAuto(lower(value),auto.LockAxesTicks);
    if ~strcmp(opts.LockAxesTicks,{'on','off'})
      error('MATLAB:hgexport:InvalidLockAxesTicksInput', 'LockAxesTicks must be ''on'' or ''off''.');
    end
   case 'showui'
    opts.ShowUI = LocalCheckAuto(lower(value),auto.ShowUI);
    if ~strcmp(opts.ShowUI,{'on','off'})
      error('MATLAB:hgexport:InvalidShowUIInput', 'ShowUI must be ''on'' or ''off''.');
    end
   case 'separatetext'
    opts.SeparateText = LocalCheckAuto(lower(value),auto.SeparateText);
    if ~strcmp(opts.SeparateText,{'on','off'})
      error('MATLAB:hgexport:InvalidSeparateTextInput', 'Separatetext must be ''on'' or ''off''.');
    end
   case 'version'
    % ignore
   otherwise
    error('MATLAB:hgexport:UnknownOption', 'Unrecognized option %s.',param);
  end
end
...
% find all "data axes"
allAxes   = findobjFcn(H, 'type', 'axes');
...
allLines  = findobjFcn(allAxes, 'type', 'line');
allText   = findobjFcn(allAxes, 'type', 'text');
allImages = findobjFcn(allAxes, 'type', 'image');
allLights = findobjFcn(allAxes, 'type', 'light');
allPatch  = findobjFcn(allAxes, 'type', 'patch');
allSurf   = findobjFcn(allAxes, 'type', 'surface');
allRect   = findobjFcn(allAxes, 'type', 'rectangle');
...
% find all groups and add those that have custom properties to
% relevant lists
allGroups  = [findobjFcn(allAxes, 'type', 'hggroup');
              findobjFcn(allAxes, 'type', 'hgtransform')];
...
try
  ...
  if ~alreadyApplied
    % lock axes limits if requested
    if strcmp(opts.LockAxes,'on')
      old = LocalManualAxesMode(old, allAxes, 'LimMode');
      % also lock axes ticks and ticklabels if requested
      % (this is only valid if LockAxes is also 'on'
      if strcmp(opts.LockAxesTicks,'on')
    ...
    old = LocalPushOldData(old,H,'PaperUnits', ...
                           get(H,'PaperUnits'));
    old = LocalPushOldData(old,H,'PaperPosition', ...
                           get(H,'PaperPosition'));
    LocalSet(H, 'PaperPositionMode', 'auto');
    ...
    if ~strcmp(get(H,'WindowStyle'),'docked')
      ...
      LocalSet(H, 'Position', newPos);
    end
    LocalSet(H, 'Units', oldFigureUnits);

    old = LocalPushOldData(old,H,'Color', get(H,'Color'));
    old = LocalPushOldData(old,H,'InvertHardcopy', ...
                           get(H,'InvertHardcopy'));
    LocalSet(H,'InvertHardcopy','off');
    if ~isempty(opts.Background)
      if ischar(opts.Background) && (opts.Background(1) == '[')
        opts.Background = eval(opts.Background);
      end
      LocalSet(H,'Color',opts.Background);
    end
    
    old = LocalPushOldData(old,H,'PaperOrientation',get(H,'PaperOrientation'));
    LocalSet(H,'PaperOrientation','portrait');

    % process line-style map
    styleAxes = findobjFcn(H,'Type','axes','LineStyleOrder','-');
    styleLines = [];
    for k1 = 1:length(styleAxes)
      if ~usingMATLABClasses
         % add the 'auto' CodeGenLineStyleMode children to the set of objects 
         % that have a linestyle we need to make changes to.
         LineStyleModeProp = 'CodeGenLineStyleMode';
         ch = find(handle(styleAxes(k1)),'-isa','graph2d.lineseries');
      else
         % add the 'auto' LineStyleMode children to the set of objects 
         % that have a linestyle we need to make changes to.
         LineStyleModeProp = 'LineStyleMode';
         ch = findobjFcn(styleAxes(k1), '-isa', 'matlab.graphics.chart.primitive.Line');
      end
      for k2 = 1:length(ch)
          series = ch(k2);
          if isprop(series, LineStyleModeProp) && strcmp(get(series, LineStyleModeProp),'auto')
              styleLines = [styleLines series]; %#ok<AGROW>
          end
      end
    end
    if ~isempty(opts.LineStyleMap) && ...
	  ...
	  ~isempty(styleLines)
      if ~usingMATLABClasses
          oldlstylemode = LocalGetAsCell(styleLines, LineStyleModeProp);
          old = LocalPushOldData(old, styleLines, {LineStyleModeProp}, oldlstylemode);
      end
      ...
      if ischar(opts.LineStyleMap) && strcmpi(opts.LineStyleMap,'bw')
        ...
        try
          ...
          warning('MATLAB:hgexport:SkippingStylemap', [xlate('Skipping stylemap. ') ,ex.getReport('basic')]);
        end
      end
      LocalSet(styleLines,'LineStyle',newlstyle);
    end
  end
  ...
  switch (opts.Color)
          ...
          for k=1:n
            if isnumeric(lcolor{k})
              if (lcolor{k}(1) < 1-tiny)
                LocalSet(objs(k),loopProps{2*p},[0 0 0]);
              end
   ...
   otherwise
    error('MATLAB:hgexport:InvalidColorParam', 'Invalid Color parameter');
  end
  if ~alreadyApplied
    if ~isempty(opts.Renderer) && ~strcmp(opts.Renderer,'auto')
      ...
      old = LocalPushOldData(old,H,'Renderer', ...
                             get(H,'Renderer'));
      LocalSet(H,'Renderer',opts.Renderer);
    end
    if (~isempty(opts.ShowUI) && strcmp(opts.ShowUI,'off'))
      uicontrols = findobjFcn(H,'Type','uicontrol','Visible','on');
      old = LocalPushOldData(old,uicontrols,'Visible', 'on');
      LocalSet(uicontrols,'Visible','off');
    end
  end
  ...
  if ~alreadyApplied
    ...
    if ~isempty(opts.FontMode) && ~strcmp(opts.FontMode,'none')
      ...
      LocalSet(allFont,'FontUnits','points');
      switch (opts.FontMode)
       case 'fixed'
        LocalSet(allFont,'FontSize',opts.FixedFontSize);
       case 'scaled'
        ...
        LocalSet(allFont,'FontSize',newfonts);
       otherwise
        error('MATLAB:hgexport:InvalidFontModeParam', 'Invalid FontMode parameter');
      end
    ...
    if ~isempty(opts.FontName) && ~strcmp(opts.FontName,'auto')
      ...
      LocalSet(allFont,'FontName',opts.FontName);
    ...
    if ~isempty(opts.FontWeight) && ~strcmp(opts.FontWeight,'auto')
      ...
      LocalSet(allFont,'FontWeight',opts.FontWeight);
    ...
    if ~isempty(opts.FontAngle) && ~strcmp(opts.FontAngle,'auto')
      ...
      LocalSet(allFont,'FontAngle',opts.FontAngle);
  ...
  if ~alreadyApplied
    ...
    if ~isempty(opts.LineMode) && ~strcmp(opts.LineMode,'none')
      ...
      % exclude any that don't have a LineWidth property
      target = target(isprop(target, 'LineWidth'));  
      oldlines = LocalGetAsCell(target,'LineWidth');
      old = LocalPushOldData(old, target, {'LineWidth'}, oldlines);
      switch (opts.LineMode)
       case 'fixed'
        LocalSet(target,'LineWidth',opts.FixedLineWidth);
       case 'scaled'
        ...
        LocalSet(target,'LineWidth',newlines);
      end
    end
  end
  ...
  if ~isempty(allAxes)
    ...
    if strcmp(opts.Bounds,'tight') && ~alreadyApplied
      ...
      LocalSet(allDataAxes,'WarpToFill','on')
      ...
      LocalSet(ax,'Units','normalized');
      if length(ax) == 1
        LocalSet(ax,'OuterPosition',[0 0 1 1]);
        LocalSet(ax, 'LooseInset',[0 0 0 0]);
      elseif length(ax)  > 1,
	...
	if all(isfinite(slop))
	  ...
	  LocalSet(ax,'OuterPosition',outpos);
	  LocalSet(ax,'LooseInset',inset);
	end
      end
    end
  end
  ...
  if ~isequal(opts.ApplyStyle,1)
    if strcmp(opts.SeparateText,'on') && ~isClipboard
      ...
      [null{:}] = deal([]);
      LocalSet(allAxes,'XTickLabel',null);
      LocalSet(allAxes,'YTickLabel',null);
      LocalSet(allAxes,'ZTickLabel',null);
      print(H, filename, args{:});
      LocalSet(allText,'Visible',oldtvis);
      LocalSet(allAxes,'XTickLabel',oldax);
      LocalSet(allAxes,'YTickLabel',olday);
      LocalSet(allAxes,'ZTickLabel',oldaz);
      ...
      oldBox = LocalGetAsCell(allAxes, 'Box');
      LocalSet(allAxes, 'Box', 'off');
      ...
      [null{:}] = deal('off');
      LocalSet(allAxes,'XGrid',null);
      LocalSet(allAxes,'YGrid',null);
      LocalSet(allAxes,'ZGrid',null);
      LocalSet(allNonText,'Visible','off');
      LocalSet(allAxes,'Color','none');
      print(H, textFile, args{:});
      LocalSet(allNonText,'Visible',oldvis);
      LocalSet(allAxes,'Color',oldc);
      LocalSet(allAxes,'XGrid',oldaxg);
      LocalSet(allAxes,'YGrid',oldayg);
      LocalSet(allAxes,'ZGrid',oldazg);
      LocalSet(allAxes, 'Box', oldBox);
      if ~usingMATLABClasses
...
if isappdata(H,'BusyPrinting'), rmappdata(H,'BusyPrinting'); end
if isequal(opts.ApplyStyle,1)
  varargout{1} = old;
else
  for n=1:length(old.objs)
    ...
    vals = old.values{n};
    ok = ishghandle(objs);
    if iscell(vals) && isequal(size(vals),size(objs))
      vals = vals(ok); 
    end
    objs = objs(ok);
    LocalSet(objs, old.prop{n}, vals);
  end
end

if hadError
  error(deblank(ex.getReport('basic')));
end
...
% create an output structure containing the input objs, prop, and values 
% followed by the corresponding inData objs, prop, and values fields
...
% convert the value of the property prop to a cell array, if it isn't
% a cell array already. If the value is empty, simply return it unless the
% allowemptycell parameter is not empty/zero, in which case return the
% empty value in the cell array 
...
% multiply the values in inArray by the scale factor (scale), making sure
% the result is no smaller than minv; return result in a cell array
function newArray = LocalScale(inArray, scale, minv)
my_n = length(inArray);
newArray = cell(my_n,1);
for my_k=1:my_n
  newArray{my_k} = max(minv,scale*inArray{my_k}(1));
end
end

% convert a colorspec into a (numeric) shade of gray
% recognized character colorspecs ('r', 'blue', ...) are first converted 
...
% convert the colors in the input cell array (inArray) to shades of gray
...
% convert the colors of the objects in inArray to a shade of gray and then
% to a linestyle, mapping the same shades of gray to the same linestyle
...
for my_k=1:my_n
  gray = LocalMapToGray1(inArray{my_k});
...
% convert the contents of a CData array (MxNx3) to shades of gray 
...
% update the specified color property (prop) of the objects in
% inArray to some shade of gray, storing the previous setting in outData
...
if (~isempty(value))
  ...
  LocalSet(inArray,prop,value);
end
end

% returns true if value is a numeric, positive scalar; false otherwise
...
% helper function - if the first parameter is the string 'auto', return the
% 2nd parameter, otherwise return the 1st parameter (converted to a double
% if it is a string)
...
% function to set the X-, Y-, and Z-<base> properties of allAxes to 'manual'
% for axes that are not using log scale.
% (where <base> is the property name w/o the axis (X Y, or Z) prefix, 
% such as 'LimMode' for XLimMode, YLimMode, and ZLimMode). 
% Previous values for the properties are returned in c
...
nonlogyflag = logical([]);
nonlogzflag = logical([]);
for my_k = 1:length(xlog)
  nonlogxflag(my_k) =  logical(~strcmp(xlog{my_k},'log'));
  nonlogyflag(my_k) =  logical(~strcmp(ylog{my_k},'log'));
  nonlogzflag(my_k) =  logical(~strcmp(zlog{my_k},'log'));
end
...
LocalSet(allNonLogXAxes,xs,'manual');
LocalSet(allNonLogYAxes,ys,'manual');
LocalSet(allNonLogZAxes,zs,'manual');
c = old;
end

% helper function - if the first parameter is the string 'auto', return the
% 2nd parameter, otherwise return the 1st parameter
...
% helper function - return a structure representing the default settings for 
...
auto.Bounds = 'loose';
auto.LockAxes = 'on';
% default to 'off' for compatibility w/earlier behavior
auto.LockAxesTicks = 'off'; 
auto.ShowUI = 'on';
auto.SeparateText = 'off';
end

% convert a struct - s - to cell array {field1,val1,field2,val2,...}, 
% excluding the fields from s that match the value of the corresponding 
% field from auto (but including any fields from s that aren't present in
% auto). In other words, return a cell array holding the fieldnames and 
% values of s that are different from the corresponding field values 
% in auto, plus the fieldnames and values of any field in s that aren't 
% present in auto; fields in auto that aren't in s are not returned.
...
opts(1,:) = f;
opts(2,:) = v;
c = opts(:)'; % return as single row cell array 
end

% read the specified "stylesheet" (<stylename>.txt) from the ExportSetup
% subdirectory of the MATLAB preferences directory and return it in the
% structure h. Use the "factorystyle" (see LocalFactoryStyle) to provide 
% values for any fields that aren't present in <stylename>.txt 
function h = LocalReadStyle(stylename)
stylefile = fullfile(prefdir(0),'ExportSetup',[stylename '.txt']);
if ~exist(stylefile,'file')
  error('MATLAB:hgexport:StyleNotFound', 'Style ''%s'' does not exist.',stylename);
end
...
% write the content of the structure h to the "stylesheet" named
% <stylename>.txt in the ExportSetup subdirectory of the MATLAB 
% preferences directory. Each line in the file contains
%   * the field name 
%   * followed by a space
%   * followed by the property value
%   * followed by a newline ('\n') character
% Character/string data is written as strings, and
% everything else is written as a double. Empty string/character values 
% are not written to the file (so the entry contains just them fieldname 
% followed by a space). Empty non-string/non-character values
% are written as an empty array - '[]' (without quotes) 
...
function LocalSet(obj, propName, val)
  ...
  for idx = 1:length(obj) 
      if ~isempty(propName_I) && isprop(obj(idx), propName_I)
          propToSet = propName_I; 
      else
          propToSet = propName;
      end
      if iscell(val) 
         set(obj(idx), propToSet, val{idx});
      else
