...
if ~ishghandle(FigHandle,'figure')
  error('MATLAB:exportsetupdlg:invalidFigure','The figure handle passed was invalid');
end
...
function ui = createui(fig)
  ...
  esd.createWindow(tabstrs, xlate('Properties', '-s'));

  ui.esd = esd;
  % pull fields out of the java class
  ui.win = esd.win;
  ui.panelchoice = esd.panelchoice;
  ui.cardpane = esd.cardpane;
  ui.cardlayout = esd.cardlayout;
...
function ui = createStyleLoadSave(ui,fig)
  ...
  ui.esd.createStyleLoadSavePanel(ui.styles, styles, enableDelete, ...
                                  xlate('Export Styles'), ...
                                  xlate('Load', '-s'), xlate('Load settings from:'), ...
                                  xlate('Save as style named:'), xdefault, ...
                                  xlate('Save', '-s'), xlate('Delete a style:'), ...
                                  xlate('Delete', '-s'));
  
  ui.selectstylebutton = ui.esd.selectstylebutton;

  % Load
  ui.loadstylebutton = ui.esd.loadstylebutton;
  set(handle(ui.loadstylebutton, 'callbackproperties'), ...
      'actionPerformedCallback', {@doLoadStyle,fig});

  % Save
  ui.savetext = ui.esd.savetext;
  ui.savebutton = ui.esd.savebutton;
  set(handle(ui.savebutton, 'callbackproperties'), ...
      'actionPerformedCallback',{@doSave,fig});

  % Delete
  ui.deletestylelist = ui.esd.deletestylelist;
  ui.deletebutton = ui.esd.deletebutton;
...
function ui = createOKCancel(ui,fig)
  ui.esd.createOKCancelPanel(xlate('Apply to Figure'), xlate('Restore Figure'), ...
                                  xlate('Export...'), xlate('OK'), xlate('Cancel', '-s'), ...
                                  xlate('Help', '-s'));

  ui.applybutton = ui.esd.applybutton;
  ui.restorebutton = ui.esd.restorebutton;
  ui.exportbutton = ui.esd.exportbutton;
  ui.okbutton = ui.esd.okbutton;
  ui.closebutton = ui.esd.closebutton;
  ui.helpbutton = ui.esd.helpbutton;
...
function ui = createsizepanel(ui,fig)
  ...
  ui.esd.createSizePanel(ui.widthitems, xlate('  Width:'), ...
                         unitsstrs, xlate('  Units:'), ...
                         ui.heightitems, xlate('  Height:'), ...
                         xlate('Expand axes to fill figure'));
                     
  % Width
  ui.widthtext = ui.esd.widthtext;
  ...
  ui.boundsbutton = ui.esd.boundsbutton;
...
function ui = createrenderpanel(ui,fig)
  ...
  ui.esd.createRenderPanel(colorstrs, xlate('  Colorspace:'), xlate('Custom color:'), ...
                           renderstrs, xlate('Custom renderer:'), ...
                           ui.resitems, xlate('  Resolution (dpi):'), ...
                           xlate('Keep axis limits'), xlate('Show uicontrols'));

  ui.colorbutton = ui.esd.colorbutton;
  set(handle(ui.colorbutton,'CallbackProperties'), ...
      'actionPerformedCallback',{@colorChanged,fig});

  % "Colorspace" background color
  ui.backcolortext = ui.esd.backcolortext;
...
function ui = createfontpanel(ui,fig)
  ...
  ui.esd.createFontPanel(xlate('Custom size:'), ...
                         xlate('Scale font by '), ...
                         xlate('       with minimum of '), ...
                         xlate(' points'), ...
                         xlate('Use fixed font size '), ...
                         xlate('Custom name:'), ...
                         ui.fontlist, ...
                         xlate('Custom weight:'), ...
                         fontweightstrs, ...
...
function ui = createlinepanel(ui,fig)
  ...
  % Style button
  ui.stylebutton = ui.esd.stylebutton;
  ...
  if ui.active
    ...
    if strcmpi(stylename,'default')
      ...
      try
        ...
        errordlg(sprintf('Unable to load style %s%c ', stylename, '.'));
        return;
      end
    end
    ui.active = false;
...
function doApply(hSrc, eventData,fig)
  ...
  if ~isempty(ui.figure) && ishghandle(ui.figure)
    ...
    try
      ...
      hErrorDlg = errordlg(ex.getReport('basic'),'Export Setup');
...
function [out,strs] = getColorVals
  ...
  strs = {xlate('black and white'),xlate('grayscale', '-s'),xlate('RGB color'),xlate('CMYK color')};
...
% Creates a cellarray with xlate'd values.
