...
%  $Revision: 1.1.8.8 $  $Date: 2009/10/24 19:18:07 $
...
if length(Data.Fig)~=1, error('MATLAB:printdlg:invalidFigInput','FIG input must be a scalar'), end

% return if fig is not a figure
if ~isscalar(Data.Fig) && ~ishandle(Data.Fig) && ~strcmp(get(Data.Fig,'Type'),'figure'),
  warning('MATLAB:printdlg:InvalidFigureHandle', 'PRINTDLG requires a single figure handle');
  return
end  
