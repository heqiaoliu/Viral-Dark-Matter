...
% $Revision: 1.1.6.6 $ $Date: 2007/10/15 22:54:12 $
    ...
    while ai <= numArgs
        ...
        elseif strcmpi(name,'axisrotate')
          ...
          if ischar(name)
            error('MATLAB:makehgtform:BadTransformType', 'Unknown transform type: %s', name);
          elseif isnumeric(name)
            error('MATLAB:makehgtform:InvalidArg', 'Unrecognized argument: %s', num2str(name));
          else
