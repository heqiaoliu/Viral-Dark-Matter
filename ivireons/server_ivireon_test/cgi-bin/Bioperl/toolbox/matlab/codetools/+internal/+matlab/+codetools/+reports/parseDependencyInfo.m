...
if ~isempty(fileList) 
    ...
    for n = 1:length(fileList)
        ...
        if length(strc) == 1 && strc.type == internal.matlab.codetools.reports.matlabType.Unknown
            errorMessage = sprintf('Unknown file type or parsing error.');
            fileError = true;
        end
        ...
        for c = 1:length(strc)
            ...
            for i=1:length(outerCalls)
                ...
                try
                    [fullCallName,possibleAlternatives] = whichCaller(outerCalls{i});
                catch exception
                    % --- ERROR ---
                ...
                if isempty(fullCallName)
                    % ---- UNKNOWN ----
                    %assume a dotcall is a variable access whereas a regular call is unknown
            ...
            for i=1:length(innerCalls)
                ...
                if ~isempty(matches)
                    if sum(matches) == 1
                        ...
                        callInfo.type = strc(matches).type;
                        if isa(callInfo.type,'internal.matlab.codetools.reports.matlabType.MatlabFileType')
                            callInfo.type = char(callInfo.type);
                        end
                    else
                        ...
                        if sum(matches) >= 1
                            %if more than one match, use the first match. Since this is an inner call,
                            %it doesn't really effect the dependency matrix if it gets it wrong.
...
function [outputFullPathFunction,possibleAlternates] = whichCaller(inputTargetFunction)
...
possibleAlternates = [];
outputFullPathFunction = which(inputTargetFunction, '-all');
