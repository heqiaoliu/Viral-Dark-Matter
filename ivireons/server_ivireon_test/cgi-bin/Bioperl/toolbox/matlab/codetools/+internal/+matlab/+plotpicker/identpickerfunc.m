    ...
    switch lower(fname)
        case 'n4sid'
            ...
            elseif n==2
                ...
                if isnumeric(x) && isnumeric(y) && isreal(x) && isreal(y) && ...
                        size(x,1)==size(y,1) && ndims(x)==2 && ndims(y)==2
                    toshow = true;
       ...
       case 'oe'
            ...
            elseif n==2
                ...
                % rows
                if isnumeric(x) && isnumeric(y) && isreal(x) && isreal(y) && ...
                        size(x,1)==size(y,1) && isvector(y) && ndims(x)==2
                    toshow = true;
                % Frequency response matrix/vector with an increasing real
                % freq vector
                elseif isnumeric(x) && isnumeric(y) && ~isreal(y) && ...
                        isreal(x) && size(y,2)==1 &&  size(x,1)==size(y,1) && ...
                        ndims(y)==2 && issorted(x)
                    toshow = true;
                end
            end
         ...
         case 'arx'
            ...
            if n==1        
                ...
                if isa(x,'iddata')  && size(x,'Nu')>=1
                    toshow = true;
                % 2-column matrix
                elseif isnumeric(x) && isreal(x) && ndims(x)==2 && ...
        ...
        case 'identspa'  
            ...
            if n==1 
                ...
                if isa(x,'iddata') 
                    toshow = true;
                elseif isnumeric(x) && isreal(x) && size(x,1)>=2 && ndims(x)==2
                    toshow = true;
                end
            end
        case 'identcra'
        ...
        case 'identsim'   
            ...
            if n==2 
               ...
               if isa(idmodelObj,'idmodel') 
                   ...
                   elseif isnumeric(iddataObj) && isreal(iddataObj) && ndims(iddataObj)==2
...
elseif strcmp(action,'defaultdisplay') 
    ...
    switch lower(fname)
        case 'n4sid'
           ...
           dispStr =  sprintf('compare(%s,n4sid(%s));figure(gcf);',inArgs,inArgs);
        case {'arx','oe'}
           if n==1
               if isa(inputvals{1},'iddata') || isa(inputvals{1},'idfrd')
                   inArgs =  inputnames{1};
           ...
           if strcmpi(fname,'arx')
                dispStr = sprintf('compare(%s,arx(%s,%s));figure(gcf)',...
                    inArgs,inArgs,orderVecStr);
           else
                dispStr = sprintf('compare(%s,oe(%s,%s));figure(gcf)',...
                    inArgs,inArgs,orderVecStr);
           end   
        case 'arxtimeseries'       
           if n==1
           ...
           dispStr =  sprintf('compare(%s,arx(%s,%s),1);figure(gcf);',inArgs,inArgs,...
               orderVecStr);
        case 'artimeseries'   
           if n==1
               if isa(inputvals{1},'iddata') 
           ...
           dispStr =  sprintf('compare(%s,ar(%s,2),1);figure(gcf);',inArgs,inArgs);        
           
        case 'identspa' 
           dispStr =  sprintf('bode(spa(%s));figure(gcf);',inputnames{1});
        case 'identbodeband'
           dispStr =  sprintf('bode(%s,''sd'',1,''fill'');figure(gcf);',...
               localCreateArgStr(inputnames)); 
        case 'identstep'
               dispStr =  sprintf('step(%s);',localCreateArgStrInclIdfrd(inputnames,inputvals));
        case 'identstepband'
           dispStr =  sprintf('step(%s,''sd'',1,''fill'');figure(gcf);',...
                  localCreateArgStrInclIdfrd(inputnames,inputvals));            

        case 'identpzmapband'
           dispStr =  sprintf('pzmap(%s,''sd'',1);figure(gcf);',...
               localCreateArgStrInclIdfrd(inputnames,inputvals));
