function d = buildCoverageInfo( dirname, progressBar )
%BUILDCOVERAGEINFO parses a directory and the profiler info to build
%information about the coverge of the files in that directory
%
...
for n = 1:length(d)
    ...
    if ~isempty(progressBar)
        progressBar.setProgress(n/length(d));
    end
end
