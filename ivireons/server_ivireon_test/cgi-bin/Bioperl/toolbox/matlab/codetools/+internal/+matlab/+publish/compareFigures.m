...
% $Revision: 1.1.6.1 $  $Date: 2009/09/03 05:18:45 $
...
for iNew = find(unchanged)
    ...
    unchanged(iNew) = isequalwithequalnans( ...
...
disp('====End Differences====');
    ...
    function traverse(s1, s2)   
        ...
        elseif isequalwithequalnans(s1, s2)
            ...
            function dispValues(n, s)
                ...
                if isnumeric(s) && size(s, 1) == 1
                    ...
                    str(1) = '[';
                    str(end + 1) = ']';
                elseif ndims(s) == 2 && (ischar(s) || isnumeric(s))
                    str = mat2str(s);
                end
...
for iNew = find(~unchanged)
    id = newItems.id(iNew);
    if iscellstr(id)
        fprintf('Snapping new system: "%s".\n', id{1});
    else
        fprintf('Snapping new figure: "%f".\n', id);
    end
end
...
if ~unchanged(iNew)
    if ischar(id)
        fprintf('Snapping changed system: "%s".\n', id{1});
    else
        fprintf('Snapping changed figure: "%f".\n', id);
    end    
