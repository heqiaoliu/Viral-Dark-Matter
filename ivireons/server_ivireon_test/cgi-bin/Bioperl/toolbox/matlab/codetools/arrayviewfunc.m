...
%   $Revision: 1.1.6.31.2.1 $  $Date: 2010/07/14 23:29:15 $
...
switch whichcall
    ...
    case 'getCurrentContextPropsAndPerms',
        [varargout{1}, varargout{2}, varargout{3}] = getCurrentContextPropsAndPerms(varargin{1}, varargin{2});
...
dependentWithEmptyGetter = false(size(names));
...
for i = 1:length(names)
    ...
    dependentWithEmptyGetter(i) = dependent(i) && isempty(thisMetaProp.GetMethod);
...
propertyAttributes = PropertyAttributes(names, getAccess, setAccess, ...
    ...
    dependentWithEmptyGetter, description, ddescription);

%********************************************************************
function [names, writables, readables] = getCurrentContextPropsAndPerms(object, ...
...
available = ~strcmp(metaProperty.SetAccess, 'private')&& ~metaProperty.Dependent;

%********************************************************************
function available = setAccessAvailableToPublic(metaProperty)
available = strcmp(metaProperty.SetAccess, 'public') && ~metaProperty.Dependent;
