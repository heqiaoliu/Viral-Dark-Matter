...
if strcmp(hObj.XDataMode,'manual')
    ...
    arg = codegen.codeargument('Name',objName,...
        ...
        'IsParameter',true);
    if ~isVectorX
        set(arg,'Value',net_xdata,'Comment',sprintf('matrix of x data'));
    else
        set(arg,'Value',get(hObj,'Xdata'),'Comment',sprintf('vector of x data'));
    end
...
arg = codegen.codeargument('Name',objName,...
    'IsParameter',true);
if ~isVectorY
    set(arg,'Value',net_ydata,'Comment',sprintf('matrix of y data'));
else
    set(arg,'Value',get(hObj,'Ydata'),'Comment',sprintf('vector of y data'));
end
...
if is3D
    ...
    arg = codegen.codeargument('Name',objName,...
        ...
        'IsParameter',true);
    if ~isVectorZ
        set(arg,'Value',net_zdata,'Comment',sprintf('matrix of z data'));
    else
        set(arg,'Value',get(hObj,'Zdata'),'Comment',sprintf('vector of z data'));
    end
    addConstructorArgin(hCode,arg);
end
...
if isVectorOutput
    ...
    set(hFunc,'Comment',...
        sprintf('%% Create multiple lines using matrix input to %s',constructor_name));
