...
%   $Revision: 1.21.4.9 $  $Date: 2009/04/21 03:26:25 $
...
if isscalar(v) && isfloat(v) && isreal(v) && v==round(v) && v >= 0  
   ...
   if k <= 1
      ...
      c = round(prod(nums));
      if c > tolerance
         warning('MATLAB:nchoosek:LargeCoefficient', ...
                 'Result may not be exact. Coefficient is greater than %e and is only accurate to %d digits', ...
