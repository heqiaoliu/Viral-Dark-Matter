...
%   $Revision: 5.22.4.6 $  $Date: 2009/11/16 22:27:20 $
%
%   Reference:
...
if nargin < 2
    error('MATLAB:legendre:TooFewInputs', 'Not enough input arguments.')
end

if ~isscalar(n) || ~isreal(n) || ~isfinite(n) || n < 0 || n ~= round(n)
    error('MATLAB:legendre:InvalidN', 'N must be a positive scalar integer.');
end

if ~isreal(x) || ischar(x)|| max(abs(x(:))) > 1
    error('MATLAB:legendre:InvalidX', 'X must be real and in the range (-1,1).')
end
...
elseif strcmp(normalize,'norm')
    ...
    error('MATLAB:legendre:InvalidNormalize','Normalization option %s not recognized', normalize);
end
