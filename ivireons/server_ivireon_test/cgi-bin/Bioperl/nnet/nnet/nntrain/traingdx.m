...
% $Revision: 1.1.6.12.2.1 $ $Date: 2010/07/14 23:40:29 $
...
function info = get_info
  info = nnfcnTraining(mfilename,'Gradient Descent with Momentum & Adaptive LR',7.0,true,true,...
    [ ...
...
function [net,tr] = train_network(net,tr,data,fcns,param)
  ...
  for epoch=0:param.epochs
    ...
    nn_train_feedback('update',net,status,tr,data, ...
      [epoch,current_time,best.perf,gradient,val_fail lr]);
    ...
    WB2 = WB + dWB;
    net2 = setwb(net,WB2);
    [perf2,vperf2,tperf2,gWB2,gradient2] = nntraining.perfs_grad(net2,data,fcns);
    if (perf2/perf) > param.max_perf_inc
      lr = lr * param.lr_dec;
      dWB = lr * gWB;
    else
      if (perf2 < perf), lr = lr * param.lr_inc; end
      [net,WB,perf,vperf,tperf,gWB,gradient] = deal(net2,WB2,perf2,vperf2,tperf2,gWB2,gradient2);
    end
