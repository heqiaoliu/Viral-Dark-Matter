...
%   for i=1:numel(data.Ac), data.Ac{i}(isnan(data.Ac{i})) = 0; end
%   for i=1:numel(data.Y), data.Y{i}(isnan(data.Y{i})) = 0; end
%   for i=1:numel(data.Yp), for j=1:numel(data.Yp{i}), data.Yp{i}{j}(isnan(data.Yp{i}{j})) = 0; end, end
%   for i=1:numel(data.N), data.N{i}(isnan(data.N{i})) = 0; end
%   for i=1:numel(data.Zl), data.Zl{i}(isnan(data.Zl{i})) = 0; end
%   for i=1:numel(data.Zi), data.Zi{i}(isnan(data.Zi{i})) = 0; end
%   for i=1:numel(data.Pd), data.Pd{i}(isnan(data.Pd{i})) = 0; end
%   for i=1:numel(data.P), data.P{i}(isnan(data.P{i})) = 0; end
  ...
  [gB,gIW,gLW] = nnprop.grad_btt(net,data.Q,data.P,data.Pd,data.Zb,data.Zi,...
    data.Zl,data.N,data.Ac,gE,data.TS,fcns);
  gWB = formwb(net,gB,gIW,gLW);
end

function jWB = calc_jacobian(net,data,fcns)
  jWB = nnprop.jac_btt(net,data.P,data.Pd,data.Zb,data.Zi,data.Zl,...
