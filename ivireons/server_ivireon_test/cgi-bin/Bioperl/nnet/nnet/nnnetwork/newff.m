...
if ~(isa(p,'double') || isreal(p)  || islogical(t))
  nnerr.throw('Inputs are not a numerical or logical matrix or cell array.')
end
if ~(isa(t,'double') || isreal(t) || islogical(t))
  nnerr.throw('Targets are not a numerical or logical matrix or cell array.')
end
if isa(s,'cell')
  if (size(s,1) ~= 1)
