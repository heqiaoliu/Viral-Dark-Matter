function [out1,out2] = sae(varargin)
...
%
% SAE('dperf_dy',net,targets,outputs,errorWeights,perf,...parameters...)
% returns the derivative of performance with respect to outputs Y.
%
%   dy = <a href="matlab:doc sae">sae</a>('dperf_dy',net,targets,outputs,{1},'regularization',0.1)
%
% SAE(net,targets,outputs,errorWeights,perf,...parameters...) returns the
% derivative of performance with respect to the network weights and biases.
%
%   dwb = <a href="matlab:doc sae">sae</a>('dperf_dwb',net,targets,outputs,{1},'regularization',0.1)
%
...
%   net.<a href="matlab:doc nnproperty.net_performParam">performParam</a>.<a href="matlab:doc nnparam.squaredWeighting">squaredWeighting</a> = false;
%
% See also MSE, MAE, SAE.

% Copyright 1992-2010 The MathWorks, Inc.
% $Revision: 1.1.10.4.2.1 $
...
function name = function_name, name = 'Sum Absolute Error'; end
...
function param = parameters
  param = [...
    ...
    nnetParamInfo('squaredWeighting','Squared Weighting','nntype.bool_scalar',false,...
    'The kind of error normalization relative to target ranges.'),...
    ];
end
...
function [perfwb,n] = performance_wb(net,param)
  if isnumeric(net)
    ...
    [perfwb,n] = sumabs([net.IW net.LW net.b]);
  end
...
function d = dperf_dy(net,t,y,ew,perf,param)
  ...
  % errors less than 1e-15 are treated as zero to be
  % consistent with numerically calculated derivatives.
end
...
function d = dperf_dwb(wb,param)
  if param.regularization == 0
    ...
    if ~isnumeric(wb), wb = getwb(wb); end
    d = (-param.regularization) * sign(wb);
    d(abs(wb) < 1e-15) = 0;
    
    % weights less than 1e-15 are treated as zero to be
    % consistent with numerically calculated derivatives.
  end
end
