...
[perfb,E,Ac,N,Zb,Zi,Zl] = nnsim.perf(net_temp,X_temp,Pd,Tl,Ai,Q,TS);
g_flag = 0;
cnt1 = cnt1 + 1;
...
while retcode>3
  ...
  if (perfb <= perfa + alpha*lambda*dperfa) | ((perfb<perfa) & (perfa < -alpha*lambda*dperfa))        %CONDITION ALPHA IS SATISFIED
    ...
    if (g_flag == 0)
      gX_temp = -nnprop.grad_s(net_temp,X_temp,Pd,Zb,Zi,Zl,N,Ac,E,perfb,Q,TS);
      dperfb = gX_temp'*dX;
    end
...
if perfb <= perfa
   if (g_flag == 0)
      gX = -nnprop.grad_s(net_temp,X_temp,Pd,Zb,Zi,Zl,N,Ac,E,perfb,Q,TS);
   else
      gX = gX_temp;
   end
