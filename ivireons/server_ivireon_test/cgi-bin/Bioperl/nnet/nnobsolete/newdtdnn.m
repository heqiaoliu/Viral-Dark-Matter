...
if isa(d,'cell')
  if (size(d,1) > 1)
    nnerr.throw('Delay is not a cell row vector of delay vectors.')
  end
  for i=1:length(d)
    id = d{i};
  ...
  nnerr.throw('Delay is not a cell array.')
end
...
if (~isa(pr,'double')) | ~isreal(pr) | (size(pr,2) ~= 2)
  nnerr.throw('Input ranges is not a two-column matrix, or cell array with a single two-column matrix.')
end
if any(pr(:,1) > pr(:,2))
