...
% Check
if size(pr,2) ~= 2, nnerr.throw('NNET:Arguments','PR must have two columns.'), end
if size(pr,1) ~= size(w1,2), nnerr.throw('NNET:Arguments','PR and W1 sizes do not match.'), end
if size(w1,1) ~= size(w2,2), nnerr.throw('NNET:Arguments','W1 and W2 sizes do not match.'), end
