...
% $Revision: 1.1.8.2 $  $Date: 2010/04/24 18:11:39 $

show = nargin<3;
if isreal(a) && isreal(t)
  if any(size(a) ~= size(t)), nnerr.throw('A and T must have the same dimensions'), end
  if (show), clf reset, end
  [tsort,line,m,b,r] = postreg1(a,t);
  if(show), plotreg1(a,t,tsort,line,m,b,r,'','r'); end
elseif iscell(a) && iscell(t)
  if any(size(a) ~= size(t)), nnerr.throw('A and T must have the same dimensions'), end
  if size(a,1) ~= 1, nnerr.throw('A and T cell arrays must only have a single row.'),end
  n = size(a,2);
  if (n<1) || (n>3), nnerr.throw('A and T cell arrays must have 1, 2, or 3 elements.'),end
  for i=1:n
    if any(size(a{i})~=size(t{i})),nnerr.throw('A{i} and T{i} must have the same dimensions.'),end
  end
