...
<script src="viewsite/mapTable.js" type="text/javascript" ></script>
...
<section id="mainarea">
    <article id="description" >
        <p id="userGuid">For generating the site, start from selecting Bacteria external ID.</p>
    </article><!-- /#description -->
<div id="selectClones">
    ...
    $result=mysql_query("select b.bact_external_id,b.vc_id,b.bacteria_id from exp e left join bacteria b on b.bacteria_id=e.bacteria_id group by e.bacteria_id order by b.bact_external_id desc");
    while ($row =mysql_fetch_array($result)) 
        echo "<p class=\"beid\"  id=\"".$row['bact_external_id']."\" bacteria_id=\"".$row['bacteria_id']."\"title=\"".$row['vc_id']."\">".$row['bact_external_id']."</p>";
    
    $result=mysql_query("select p.base_name,p.plate_id,e.bacteria_id  from plate p left join exp e on p.plate_id=e.plate_id group by e.bacteria_id");
	while($row=mysql_fetch_array($result)){
		$plate=$row['base_name'];
		$plateString.="<td class=\"plate name ".$row['bacteria_id']."\" id=\"".$plate."\" plate_id=\"".$row['plate_id']."\" bacteria_id=\"".$row['bacteria_id']."\"style=\"display:none;\">".$plate."</td>";
	}
    $result=mysql_query("select e.replicate_num, e.bacteria_id, e.plate_id, e.exp_id, f.exp_date from exp e left join file f on f.file_id=e.file_id");
	while($row=mysql_fetch_array($result)){
		$rep=$row['replicate_num'];
		$replicateString.="<td class=\"replicate num ".$row['bacteria_id']." ".$row['plate_id']."\" id=\"".$rep."\" exp_date=\"".$row['exp_date']."\" exp_id=\"".$row['exp_id']."\" style=\"display:none;\">".$rep."</td>";
	}
    echo "</div>";
    ?>
